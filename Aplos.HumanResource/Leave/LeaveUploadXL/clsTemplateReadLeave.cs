...
{
        public class clsTemplateReadLeave
        {//
            public List<LeaveUploadTemplate> ReadData(string plantid, string path)
            {
                List<LeaveUploadTemplate> data = null;
                //string path = "";
                DataSet dsExcel = null;
                try
                {
                    data = new List<LeaveUploadTemplate>();
                    //SaveFile(out path);
                    ReadFile(path, out dsExcel);
                    Validation(dsExcel, plantid);
                    data = dsExcel.Tables[0].ToList<LeaveUploadTemplate>();
                    return data;
                }
                catch (Exception ex)
                {
