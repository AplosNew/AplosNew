...
public class clsSalaryProcessAplos
{
                ...
                for (int GrdEmp = 0; GrdEmp < dsGrid.Tables[0].Rows.Count; GrdEmp++)
                {
                    if (Convert.ToBoolean(dsGrid.Tables[0].Rows[GrdEmp]["IsSelectSlrProc"].ToString().Trim()) == true && Convert.ToBoolean(dsGrid.Tables[0].Rows[GrdEmp]["IsApproved"].ToString().Trim()) == false)
                    {
                        ...
                        if (SelectedEmpCnt == grdRowMaxCnt)
                        {
                            ...
                            spq.LoadLateINEarlyOUTLunchOUT(para.PlantId, para.FromDate, para.ToDate, out dsEOLILU); 
                            spq.LoadLWP(para.PlantId, para.FromDate, para.ToDate, out dsLWP); 
                            spq.LoadSpecificLeave(para.PlantId, para.FromDate, para.ToDate, out dsLeaveSpecific);
                            spq.LoadRouteEmpList(para.PlantId, out dsRouteEmp);
                            ...
                            objSlrProc.GetEarlyOut(sEmpSysIDColl, para.PlantId, para.FromDate, para.ToDate.Trim(), out dsEarlyOut);
