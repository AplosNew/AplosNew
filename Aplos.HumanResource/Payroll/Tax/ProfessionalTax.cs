using Library.HumanResource.Payroll.Tax.Model;
using System;
...
{
    public class xProfessionalTax
    {
        public void ProcessPT(string _plantid, string _companyId, int _monthno, int _yearno)
        {
            ...
            try
            {
                _getAllPTPolicy(_plantid,out dsTaxPolicy);
                for (int i = 0; i < dsTaxPolicy.Tables[0].Rows.Count; i++)
                {
                    ...
                    {
                        ProcessPT(_plantid, _companyId, _monthno, _yearno, _taxPolicyId);
                        break;
                    }
                    else
                    {
                        ProcessPT(_plantid, _companyId, _monthno, _yearno, _taxPolicyId, _GenderID);
                    }
                }
            }
            catch (Exception ex)
        ...
        void ProcessPT(string _plantid, string _companyId, int _monthno, int _yearno,string _taxPolicyId,string _Gender = "All")
        {
            #region Declaration
            //_monthno = 8;
            //_yearno = 2020;
            ...
            {                
                ...
                if (_Gender == "All")
                {
                    _getMonthlyTaxableIncome(_taxPolicyId,_plantid, _monthno, _yearno, out dsTI_emplist); 
                }
                else
                {
                    _getMonthlyTaxableIncome(_Gender, _taxPolicyId, _plantid, _monthno, _yearno, out dsTI_emplist); 
                }
                ...
                //get opening balance                
                _getOpeningAmount(_tax_year_id, out dsOB);
                ...
                _getDeductedAmount(_tax_year_id, _monthno, out dsDeductedAmount);
                        ...
                        if (dvChild.Count > 0)
                        {
                            ...
                            for (int h = 0; h < dsSalaryHead.Tables[0].Rows.Count; h++)
                            {
                                ...
                                if (dvTax.Count == 0)
                                {
                                    ...
                                    if (_HeadCategory == "Professional Tax")
                                    {
                                        ...
                                        dr["EntryAmount"] = pci.EntryAmount;
                                        dr["DefineCurrencyID"] = pci.DefineCurrencyID;

                                        dr["DefineAmount"] = pci.DefineAmount;
                                        dr["DisbusmentCurrencyID"] = pci.DisbusmentCurrencyID;
                                        dr["DisbusmentAmount"] = pci.DisbusmentAmount;
                                ...
                                else
                                {
                                    ...
                                    if (_HeadCategory == "Professional Tax")
                                    {
                                        ...
                                        dr["DefineAmount"] = 0;
                                        dr["DisbusmentCurrencyID"] = pci.DisbusmentCurrencyID;
                                        dr["DisbusmentAmount"] = item.EarnedAmount;
                                    ...
                                    {
                                        dr["DisbusmentAmount"] = pci.DisbusmentAmount - item.EarnedAmount;
                                    }
        ...
        ProfessionalTaxSlab _getYearlySlabAmount(MonthlyTaxableIncome empinfo, List<MonthlyTaxableIncome> YearlyIncomeAllEmp, List<ProfessionalTaxSlab> dicPTS)
        {
            ...
            var ob= dicPTS.Where(r => r.TaxPolicyMasterId == empinfo.TaxPolicyMstID && r.YearlyMinValue <= yti && r.YearlyMaxValue >= yti).FirstOrDefault();
            if(ob == null)
            {
        ...
        #region Func SQL
        void _getMonthlyTaxableIncome(string taxpolicyid, string _plantid, int _monthno, int _yearno, out System.Data.DataSet dsLocal)
        {
            string strSQL;
        ...
        void _getMonthlyTaxableIncome(string Gender, string taxpolicyid, string _plantid, int _monthno, int _yearno, out System.Data.DataSet dsLocal)
        {
            string strSQL;
            ConnectionManager.DAL.ConManager objCon;
            try
        ...
        void _getAllPTPolicy(string plantid,out System.Data.DataSet dsRef)
        {
            string strSQL = string.Empty;
            ConnectionManager.DAL.ConManager objCon;
            try
            {
                strSQL = @"SELECT m.*  FROM [dbo].[TaxPolicyMaster] m 
        ...
        void _getDeductedAmount(string TaxYearId, int _month, out System.Data.DataSet dsRef)
        {
            string strSQL = string.Empty;
            ConnectionManager.DAL.ConManager objCon;
            try
        ...
        void _getOpeningAmount(string TaxYearId, out System.Data.DataSet dsRef)
        {
            string strSQL = string.Empty;
            ConnectionManager.DAL.ConManager objCon;
            try
            {
                strSQL = @"SELECT 
                                  ...
                                  ,[OpeningTaxPaid]
                              FROM [ProfessionalTaxOpeningBalance] where TaxYearId='"+ TaxYearId + "'";
                objCon = new ConnectionManager.DAL.ConManager("1");
                objCon.OpenDataSetThroughAdapter(strSQL, out dsRef, false, "1");
            }
            catch (Exception ex)
