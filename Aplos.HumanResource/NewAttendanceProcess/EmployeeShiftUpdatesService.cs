...
{
    public class EmployeeShiftUpdatesService
    {

        ISqlRepository _sqlRepository;
        public EmployeeShiftUpdatesService()
        {
        ...
        public IEnumerable<object> getCurrentList()
        {
            try
            {
                var str = @"Select ROW_NUMBER() OVER(ORDER BY es.EffectiveDate desc) as Rows,ei.EmployeeCode,format(es.EffectiveDate,'dd-MMM-yyyy') as EffectiveDate,
                            sd.ShiftDefinationName as ShiftName , sd.ShiftDefinationDescription as ShiftDescription , ei.EmployeeName, p.UserName as Plant
                            from dbo.EmployeeProfileShift es
                            left join dbo.EmployeeInformation ei on ei.SystemId = es.EmpSystemId
                            left join dbo.ShiftDefination sd on sd.SystemID = es.ShiftId
                            left join org.Plant p on p.Id = ei.PlantId
                            order by CAST(es.EffectiveDate as Date) desc
                                ";
                return _sqlRepository.GetDataCollection(str);
            }
            catch(Exception e)
        ...
        public void SaveFileList(List<Dictionary<string,object>> data )
        {
            try
            {
                ...
                string TableName = "dbo.EmployeeProfileShift";
                DataSet dsMaster;
        ...
        public DataTable getEmployeeShiftFile( )
        {
            try
            {
                var str = @"Select ei.EmployeeCode , es.ShiftId as ShiftId,format(es.EffectiveDate,'dd-MMM-yyyy') as EffectiveDate
                            from dbo.employeeprofileshift es
                            left join dbo.EmployeeInformation ei on ei.SystemId = es.EmpSystemId
                            left join dbo.ShiftDefination sd on sd.SystemId = es.ShiftId
                            ";
        ...
        public DataTable getShiftFile()
        {
            try
            {
                var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
                var str = @"Select sd.SystemID ,p.UserName as Plant, sd.ShiftDefinationName , sd.ShiftDefinationDescription , sd.ShiftType 
                            from dbo.ShiftDefination sd 
                            left join org.Plant p on p.Id = sd.PlantId
                            where p.CompanyId = '"+identity.CompanyId+@"'
        ...
        public List<string> getRostersList()
        {
            try
            {
                var str1 = @"Select SystemId from dbo.ShiftDefination ";
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    {
                        roster.Add(dt.Rows[i]["SystemId"].ToString());
                    }
                }
