...
{
        public class clsTemplateReadBankInformation
    {//
            public List<BankInformationUploadTemplate> ReadData(string plantid, string path)
            {
                List<BankInformationUploadTemplate> data = null;
                //string path = "";
                DataSet dsExcel = null;
                try
                {
                    data = new List<BankInformationUploadTemplate>();
                    //SaveFile(out path);
                    ReadFile(path, out dsExcel);
                    Validation(dsExcel, plantid);
                    data = dsExcel.Tables[0].ToList<BankInformationUploadTemplate>();
                    return data;
                }
                catch (Exception ex)
                {
