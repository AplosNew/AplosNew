...
{
    public class clsManualAttendanceUploadUtility
    {
        public List<AttendanceManualData> ReadData(string plantid, string path)
        {
            List<AttendanceManualData> data = null;
            //string path = "";
            DataSet dsExcel = null;
            try
            {
                data = new List<AttendanceManualData>();
                //SaveFile(out path);
                ReadFile(path, out dsExcel);
                Validation(dsExcel, plantid);
                data = dsExcel.Tables[0].ToList<AttendanceManualData>();
                return data;
            }
            catch (Exception ex)
            {
