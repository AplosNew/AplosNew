...
left join EmployeeInformation EI on EI.SystemId = PM.EmpSystemId 
        ...
        #region GET SQPARAMETERMASTERSEQUENCE
        public double GetSQParaMasterSequence()
        {
            DataTable dt = _sqlRepository.GetDataTable("SELECT isnull(Max(Sequence),0) AS Sequence FROM [HKP].[SQParameterMaster]");
            if (dt.Rows.Count > 0)
                return clsStaticInfo.dbl(dt.Rows[0]["Sequence"].ToString()) + 1;

            return 1;
        }
        #endregion GET SQPARAMETERMASTERSEQUENCE
        ...
        public IEnumerable<object> GetSQParametertList(string column, string value)
        {
            try
            {
                string TableName = "[HKP].[SQParameterMaster]";
                string strkey = "1=1";
                if (string.IsNullOrEmpty(column) == false && string.IsNullOrEmpty(value) == false)
                    strkey = column + " like '%" + value + "%'";
                ...
                string sql = @"SELECT PM.*, (select p.Code from org.position p where p.Id=PM.PositionCodeId) PositionCode FROM HKP.SQParameterMaster PM
                                where " + strkey + "order by Sequence";
                return _sqlRepository.GetDataCollection(sql, null);
            }
            catch (Exception ex)
        ...
        public Dictionary<string, object> SQSave(Dictionary<string, object> data)
        {
            try
            {
                string TableNameHead = "[HKP].[SQParameterMaster]";
        ...
        public string SQParameterDelete(string id)
        {
            try
            {
