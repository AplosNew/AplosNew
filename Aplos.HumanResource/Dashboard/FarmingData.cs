...
namespace Library.HumanResource.Dashboard
{
    ...
    public class FarmingData
    {
        ...
        public IEnumerable<object> getInitialData()
        {
            try
            {
                var str = @"Select icm.[Group] as ICSGroup, Count(distinct icm.Id) as numberIcs,
                            sum(case when TotalFarmers.Active = 1 then 1 else 0 end) as Active ,sum(case when TotalFarmers.Active = 0 then 1 else 0 end) as Inactive,
                            sum(TotalFarmers.TotalArea) as TotalArea , isnull(sum(cp.PlannedArea),0) as PlannedArea  from 
                            (Select fm.Id as FarmerId,fm.Active ,  fmp.ICSMasterId , sum(fm.TotalArea) as TotalArea  from MST.FarmerMaster fm
                            left join (
                            Select distinct fmp.FarmerMasterId , fmp.ICSMasterId 
                            from MST.FarmerMasterPlot fmp
                            ) as fmp on fmp.FarmerMasterId = fm.id
                            group by fm.Id , fm.Active , fmp.ICSMasterId) as TotalFarmers
                            left join

                            (
                            Select cp.FarmerId , sum(isnull(cp.Plann,0)) as PlannedArea from
                            ( Select cpc.FarmerId , sum(isnull(cpc.PlanQuantity,0)) as Plann , cpc.CropId ,  cpc.CropTypeId , cm.CropCategoryId , cm.CropSubCategoryId from 
                            TRN.CropPlanningChild cpc 
                            join mst.CropMaster cm on cm.Id = cpc.CropId
                            group by cpc.FarmerId ,  cpc.CropId , cpc.FarmerPlotId , cpc.CropTypeId , cm.CropCategoryId , cm.CropSubCategoryId ) as cp
                            group by cp.FarmerId
                            ) as cp
                            on cp.FarmerId = TotalFarmers.FarmerId
                            left join MST.ICSMaster icm on icm.Id = TotalFarmers.ICSMasterId
                            group by icm.[Group]";
                return _sqlRepository.GetDataCollection(str);
            }
            catch (Exception ex)
                ...
                var str = @"
                           ...
                           --     DECLARE @cropCategoryId char(20) = " + cropCategoryId + @"
                            --    DECLARE @cropSubCategoryId char(20) = " + cropSubCategoryId + @"
                                ...
                                Select icm.[Group] as [Group], Count(distinct icm.Id) as numberIcs,
                            sum(case when TotalFarmers.Active = 1 then 1 else 0 end) as Active ,sum(case when TotalFarmers.Active = 0 then 1 else 0 end) as Inactive,
                            sum(TotalFarmers.TotalArea) as TotalArea , isnull(sum(cp.PlannedArea),0) as PlannedArea  from 
                            (Select fm.Id as FarmerId,fm.Active ,  fmp.ICSMasterId , sum(fm.TotalArea) as TotalArea  from MST.FarmerMaster fm
                            left join (
                            Select distinct fmp.FarmerMasterId , fmp.ICSMasterId 
                            from MST.FarmerMasterPlot fmp
                         --   where fmp.PlotStatus =IIF(@landId  IS NULL, fmp.PlotStatus,@landId )
                            ) as fmp on fmp.FarmerMasterId = fm.id
                            group by fm.Id , fm.Active , fmp.ICSMasterId) as TotalFarmers
                            left join
                            ...
                            TRN.CropPlanningChild cpc 
                            join mst.CropMaster cm on cm.Id = cpc.CropId
                          --  where cpc.CropId = IIF( @cropId IS NULL , cpc.CropId, @cropId) AND
        ...
        public IEnumerable<object> getFilterDrillData(string landId, string cropId, string cropTypeId, string cropCategoryId, string cropSubCategoryId, string icsGroup)
        {
            try
            {
                ...
                var str = @"Select icm.Id as [Group],  
sum(case when TotalFarmers.Active = 1 then 1 else 0 end) as Active ,sum(case when TotalFarmers.Active = 0 then 1 else 0 end) as Inactive,
sum(TotalFarmers.TotalArea) as TotalArea , isnull(sum(cp.PlannedArea),0) as PlannedArea  from 
(Select fm.Id as FarmerId,fm.Active ,  fmp.ICSMasterId , sum(fm.TotalArea) as TotalArea  from MST.FarmerMaster fm
left join (
...
group by icm.Id ";
       ...
       public IEnumerable<object> getActiveFarmers(string landId, string cropId, string cropTypeId, string cropCategoryId, string cropSubCategoryId, string column , string groups)
        ...
        public IEnumerable<object> IcsPie()
        {
            try
            {
                var str = @"Select ics.[Group] , Count(distinct ics.Id) as numberIcs
                            from MST.ICSMaster ics
                            group by ics.[Group]";
                return _sqlRepository.GetDataCollection(str);
            }
            catch(Exception ex)
        ...
        public IWorkbook GetActiveFarmersReport(string landId, string cropId, string cropTypeId, string cropCategoryId, string cropSubCategoryId, string column, string groups)
        {
            ...
            sheet.UsedRange.WrapText = true;
            sheet.UsedRange.CellStyle.Font.Size = 8;
            report.CompanyPlantHeader(ref sheet, endCol, "Active Farmers ", identity.CompanyId, identity.PlantName, null);
            //report.CompanyPlantHeader(ref sheet, endCol, "Farmer Master", "Odyssey", "Hyderbad", null);
            report.PageSetup(ref sheet, 5, ExcelPageOrientation.Landscape);
            return workbook;
        }
        ...
        public IWorkbook GetInactiveFarmersReport(string landId, string cropId, string cropTypeId, string cropCategoryId, string cropSubCategoryId, string column, string groups)
        {
            ...
            sheet.UsedRange.WrapText = true;
            sheet.UsedRange.CellStyle.Font.Size = 8;
            report.CompanyPlantHeader(ref sheet, endCol, "Inactive Farmers ", identity.CompanyId, identity.PlantName, null);
            //report.CompanyPlantHeader(ref sheet, endCol, "Farmer Master", "Odyssey", "Hyderbad", null);
            report.PageSetup(ref sheet, 5, ExcelPageOrientation.Landscape);
            return workbook;
        }
        ...
        public IWorkbook GetTotalAreaReport(string landId, string cropId, string cropTypeId, string cropCategoryId, string cropSubCategoryId, string column, string groups)
        {
            ...
            sheet.UsedRange.WrapText = true;
            sheet.UsedRange.CellStyle.Font.Size = 8;
            report.CompanyPlantHeader(ref sheet, endCol, "Total Area", identity.CompanyId, identity.PlantName, null);
            //report.CompanyPlantHeader(ref sheet, endCol, "Farmer Master", "Odyssey", "Hyderbad", null);
            report.PageSetup(ref sheet, 5, ExcelPageOrientation.Landscape);
            return workbook;
        }
        ...
        public IWorkbook GetPlannedAreaReport(string landId, string cropId, string cropTypeId, string cropCategoryId, string cropSubCategoryId, string column, string groups)
        {
            ...
            sheet.UsedRange.WrapText = true;
            sheet.UsedRange.CellStyle.Font.Size = 8;
            report.CompanyPlantHeader(ref sheet, endCol, "Planned Area", identity.CompanyId, identity.PlantName, null);
            //report.CompanyPlantHeader(ref sheet, endCol, "Farmer Master", "Odyssey", "Hyderbad", null);
            report.PageSetup(ref sheet, 5, ExcelPageOrientation.Landscape);
            return workbook;
        }
