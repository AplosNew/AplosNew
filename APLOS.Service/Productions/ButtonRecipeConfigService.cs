...
{
    public class ButtonRecipeConfigService : Service<ButtonRecipeConfig>, IButtonRecipeConfigService
	{
		#region Constructor
		...
		public ButtonRecipeConfigService(
			IRepositoryAsync<ButtonRecipeConfig> ButtonRecipeConfigRepository,
			IPKGeneratorService pkGeneratorService,
			IUnitOfWork unitOfWork
			, ISqlRepository sqlRepository
			) :
			base(ButtonRecipeConfigRepository, unitOfWork, pkGeneratorService)
		{
		...
		private void Check(ButtonRecipeConfig entity)
		{
		...
		public override void Insert(ButtonRecipeConfig entity)
		{
			try
			{
				Check(entity);
				entity.Id = GetAutoNumber(nameof(ButtonRecipeConfig), PKGeneratorEnum.Yearly, null, DateTime.Now);
				base.Insert(entity);
		...
		public override void Update(ButtonRecipeConfig entity)
		{
			try
			{
				Check(entity);
		...
		public GridModel Query(GridParameter parameters, string plantId)
		{
			try
			{
				parameters.CmdText = @"SELECT BRC.Id, BRC.CompanyGroupId, BRC.CompanyId, BRC.PlantId
								...
								, BRC.RecipeLevel
						FROM [SCS].[ButtonRecipeConfig] AS BRC
