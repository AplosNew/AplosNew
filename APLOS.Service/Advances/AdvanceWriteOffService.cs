...
namespace Library.Service.Advances
{
    public class AdvanceWriteOffService : Service<AdvanceWriteOff>, IAdvanceWriteOffService
    {
        ...
        public string InsertCustomerAdvanceWriteOff(VoucherViewModel voucherVM, IEnumerable<VoucherDetailViewModel> advanceDetailVMList, IEnumerable<VoucherDetailViewModel> voucherDetailExpensesList, IEnumerable<VoucherDetailCurrencyViewModel> voucherDetailCurrencyVMList)
        {
            var flag = false;
            try
            {
                ...
                else if (voucherVM.SettlementType == SettlementType.SetOff.ToString() && voucherVM.PaymentSource == PaymentSource.GL.ToString())
                {
               ...
               , IEnumerable<BankChargeViewModel> bankChargeDetailVMList,IEnumerable<VoucherViewModel> advanceVMList, VoucherDetailViewModel VoucherDetailVM)
        {
            ...
            try
            {
                if (string.IsNullOrEmpty(voucherVM.BankMasterId) && voucherVM.PaymentSource == PaymentSource.Bank.ToString())
                    ...
                    throw new CustomException("Cash Id not found!");
                else if (string.IsNullOrEmpty(voucherVM.AdvanceId) && VoucherDetailVM.EmployeeAdvanceDetailId == null && voucherVM.PaymentSource == PaymentSource.Employee.ToString())
                    throw new CustomException("Advance Id not found!");
                AccountCommonExtensionService _accountsCommonService = new AccountCommonExtensionService();
                _accountsCommonService.GetParallelCurrency(voucherVM.CompanyId, out string companyCurrencyId, out string companyCurrencyCode);
                _accountsCommonService.CheckingFiscalYearPeriod(voucherVM);
                ...
                if (voucherVM.PaymentSource == PaymentSource.Employee.ToString())
                {
                    if (null != advanceVMList && advanceVMList.Count() > 0)
                    {
                        foreach (var advanceVM in advanceVMList)
                        {
                            ...
                            var advanceWriteOffDetail = new AdvanceWriteOffDetail
                            {
                                CompanyId = advanceVM.CompanyId,
                                PlantId = advanceVM.PlantId,
                                AdvanceId = voucherVM.AdvanceId,
                                AdvanceDetailId = voucherVM.AdvanceDetailId,
                                ...
                                CurrencyId = voucherVM.CurrencyId,
                                PartyType = advanceVM.PartyType,
                                PartyId = advanceVM.PartyId,
                                PartyPlantId = advanceVM.PartyPlantId,
