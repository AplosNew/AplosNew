...
";
        ...
        public void DeleteVendorAdvance(string companyId, string plantId, string voucherId)
        {
            ...
            try
            {
                ...
                vendorAdWr.Append(vendorAdWrsql);
                vendorAdWrsql = @"update trn.VoucherDetail set UpdatedBy='" + identity.Name + @"', InvoiceTaxDetailId=NULL  where voucherId in (select Id from trn.voucher where CompanyId='" + companyId + "' AND PlantId='" + plantId + "' AND Id = '" + voucherId + "')";
                vendorAdWr.Append(vendorAdWrsql);
