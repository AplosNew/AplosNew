...
namespace Library.Service.Products
{
    ...
    ///
    /// </summary>
    public partial class ItemSubCategoryService : Service<ItemSubCategory>, IItemSubCategoryService
    {
        #region Constructor
        ...
        /// <param name="machineRepository">    The repChartOfAccountsLevel3. </param>
        /// <param name="unitOfWork">   The unit of work. </param>
        ///-------------------------------------------------------------------------------------------------
        public ItemSubCategoryService(
            IRepositoryAsync<ItemSubCategory> machineRepository,
        ...
        public override IQueryFluent<ItemSubCategory> Query()
        {
        ...
        public IEnumerable<object> GetItemSubCategoryList()
        {
            try
            {
        ...
        #endregion GetItemSubCategoryList
        ...
        public override void Insert(ItemSubCategory entity)
        {
            try
            {
                entity.Id = GetAutoId();
        ...
        {
            return _pkGeneratorService.GetAutoNumber(nameof(ItemSubCategory), PKGeneratorEnum.Auto, null, DateTime.Now);
        }

        public override void Update(ItemSubCategory entity)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                throw new CustomException(ex.Message, ex,
                Logger.ThrowError(GetType().Name, MethodBase.GetCurrentMethod().Name,
                entity.AddedBy, ErrorType.ServiceError, null, ex.Message, ex.GetType().Name, false,
                ModuleEnum.Product.ToString()));
            }
        }
    ...
    public partial class ItemSubCategoryService : IItemSubCategoryService
    {
        ...
        public virtual GridModel GetSearchData(GridParameter parameters)
        {
            try
            {
                parameters.CmdText = $"SELECT Id, Sequence, Code, ShortName, StandardName, UserName, Description, Remarks, Active as IsActive, Archive as IsArchive,ADDEDBY, ADDEDDATE, ADDEDFROMIP, UPDATEBY, UPDATEDDATE, UPDATEDFROMIP " +
                          $"FROM {DbSchema.HKP}.[{DbTable.ItemSubCategory}] WHERE Archive=0";
        ...
        public virtual Dictionary<string, object> GetModelData(object id)
        {
            try
            {
                var sql = $"SELECT Id, Sequence, Code, ShortName, StandardName, UserName, Description, Remarks, Active as IsActive, Archive as IsArchive,ADDEDBY As AddedBy, CONVERT(VARCHAR(10),ADDEDDATE,6) As AddedDate, ADDEDFROMIP As AddedFromIP, UPDATEBY, UPDATEDDATE, UPDATEDFROMIP " +
                    $"FROM  { DbSchema.HKP}.[{DbTable.ItemSubCategory}] WHERE Id = @0";
