...
{
    public partial class ItemCategoryService : Service<ItemCategory>, IItemCategoryService
    {
        #region Constructor
        ...
        public ItemCategoryService(
            IRepositoryAsync<ItemCategory> machineRepository,
        ...
        public override IQueryFluent<ItemCategory> Query()
        {
        ...
        public IEnumerable<object> GetItemCategoryList()
        {
            try
            {
        ...
        #endregion GetItemCategoryList

        #region InsertUpdate

        public override void Insert(ItemCategory entity)
        {
            try
            {
                entity.Id = GetAutoId();
        ...
        {
            return _pkGeneratorService.GetAutoNumber(nameof(ItemCategory), PKGeneratorEnum.Auto, null, DateTime.Now);
        }
        ...
        public virtual GridModel GetSearchData(GridParameter parameters)
        {
            try
            {
                parameters.CmdText = $"SELECT Id, Sequence, Code, ShortName, StandardName, UserName, Description, Remarks, Active as IsActive, Archive as IsArchive,ADDEDBY, ADDEDDATE, ADDEDFROMIP, UPDATEBY, UPDATEDDATE, UPDATEDFROMIP " +
                          $"FROM {DbSchema.HKP}.[{DbTable.ItemCategory}] WHERE Archive=0";
        ...
        public virtual Dictionary<string, object> GetModelData(object id)
        {
            try
            {
                var sql = $"SELECT Id, Sequence, Code, ShortName, StandardName, UserName, Description, Remarks, Active as IsActive, Archive as IsArchive,ADDEDBY As AddedBy, CONVERT(VARCHAR(10),ADDEDDATE,6) As AddedDate, ADDEDFROMIP As AddedFromIP, UPDATEBY, UPDATEDDATE, UPDATEDFROMIP " +
                    $"FROM  { DbSchema.HKP}.[{DbTable.ItemCategory}] WHERE Id = @0";
