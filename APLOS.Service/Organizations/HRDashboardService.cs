...
{
    public class ManpowerBudgetDashboardService : Service<EmployeeInformation>, IManpowerBudgetDashboardService
    {
        ...
        public ManpowerBudgetDashboardService(
            ...
            ) : base(employeeInformationRepository, unitOfWork)
        {
        ...
        public IEnumerable<OrgStructureListViewModel> OrgStructureList(string CompanyGroupId, string CompanyId)
        {
            try
            {
                ...
                return _employeeInformationRepository.SqlQuery<OrgStructureListViewModel>(sql);
            }
            catch (Exception ex)
            {
        ...
        public IEnumerable<object> GroupWiseCompanyList(string date, string status, string EmplyeeTypeOrCategoryId)
        {
            ...
            try
            {
                ...
                var sql = @"SELECT  GroupName,CompanyId,UserName,Case when  ISNULL(IsDirect,0) = 0 then 'Indirect' else 'Direct' end AS  IsDirect, ISNULL(SUM(TotalNumber),0) ProposedManpowerBudget, ISNULL(SUM(TotalManpower),0) TotalManpower , " + shortExcess + @"
								,ISNULL(SUM(TotalSalary),0) OnRoleSalaryC,ISNULL((SUM(MaxSal)+SUM(MinSal))/2,0) ProposedSalaryC
                                 FROM
                                 (
                                     SELECT m.IsDirect,m.Id,b.TotalNumber,m.GroupName,m.CompanyId,m.CName as UserName,EmpInfo.TotalSalary
                                            ...
                                            (SELECT po.IsDirect, MB.Code,MB.Id,Cg.Id as CgId,Cg.UserName as GroupName, c.Id as CompanyId, c.UserName as CName FROM [MST].[ManpowerBudget]  MB
                                             ...
                                             LEFT OUTER JOIN [ORG].[Position] AS PO ON Po.Id = MB.PositionId
								              ...
								              LEFT JOIN [HKP].EmployeeCategory EmpC ON EmpC.Id = DesM.EmployeeCategoryId
                                               ...
                                               WHERE Cg.Id = '" + identity.CompanyGroupId + @"' " + dStatus + @" " + EmployeeCategory + @" AND MB.Active = 1
                                            )  M
                                           -----------------------2. EmployeeInformation from [dbo].[EmployeeInformation]--------------------------------
                                             ...
                                             (SELECT PO.IsDirect,COUNT(SystemID) TotalManpower,BudgetCode,GroupID,c.Id cid,SUM(TotalSalary) TotalSalary
                                              ...
                                              LEFT outer JOIN [ORG].[Position] AS PO ON Po.Id = MB.PositionId
                                               ...
                                               WHERE EmployeeStatus = 'Active'
                                                AND GroupID = '" + identity.CompanyGroupId + @"'  " + dStatus + @" " + EmployeeCategory + @"
								            ...
								            (
									            ...
									            SELECT MAX(EffectiveDate) EffectiveDate,ManpowerBudgetId,CompanyId from [MST].[ManpowerBudgetAllowance]
									             ...
									             WHERE
									             ED.EffectiveDate IS NOT NULL AND m.CompanyGroupId =  '" + identity.CompanyGroupId + @"'
                                            ...
                                            (SELECT TOP 1 WITH TIES TotalNumber,ManpowerBudgetId,EffectiveDate
                                              ...
                                              LEFT outer JOIN [ORG].[Position] AS PO ON Po.Id = MB.PositionId
								               ...
								               LEFT JOIN [ORG].[Unit] ON Unit.Id = E.UnitId
								               LEFT JOIN [ORG].[Department] ON Department.Id = Po.DepartmentId
                                             WHERE CG.Id = '" + identity.CompanyGroupId + @"' " + dStatus + @" AND TotalNumber > 0 
                                             ) B

                                             ON M.id = b.ManpowerBudgetId AND B.Id = M.CgId AND B.cid = M.CompanyId AND B.IsDirect = M.IsDirect
                                 ) EDE GROUP BY GroupName,CompanyId,UserName,IsDirect ORDER BY UserName";
        ...
        public IEnumerable<object> DetailDrillDownTable(IEnumerable<ChartColumnList> ChartColumnList, int seq, string date, string status, string EmplyeeTypeOrCategoryId)
        {
            var EmployeeCategory = string.Empty;
            var shortExcess = string.Empty;
            if (EmplyeeTypeOrCategoryId == null || EmplyeeTypeOrCategoryId == "")
            ...
            try
            {
                                 ...
                                 ,m.CompanyId
                                  ...
                                  (SELECT
                                   ...
                                   LEFT OUTER JOIN [ORG].[Company] AS c on c.Id = MB.CompanyId AND c.CompanyGroupId = cg.Id
                                    ...
                                    " + join + @"
                                    WHERE Cg.Id = '" + identity.CompanyGroupId + @"' " + dStatus + @"  " + wc + @" " + EmployeeCategory + @" AND MB.Active = 1
                                    )  m
                                -----------------------2e--------------------------------
                                   LEFT OUTER JOIN
                                  ...
                                  LEFT outer JOIN [ORG].[Position] AS PO ON Po.Id = MB.PositionId
                                   ...
                                   WHERE EmployeeStatus = 'Active'
                                    ...
                                    AND  em.GroupID  = '" + identity.CompanyGroupId + @"' " + wcem + @" " + dStatus + @" " + EmployeeCategory + @"
                                   GROUP BY BudgetCode,em.GroupID,C.Id  " + cListEmpG + @"
                               ...
                               --------------------------------ManpowerBudgetWisealary-----------------------------------
								...
								(
                                    ...
                                    LEFT outer JOIN [ORG].[Position] AS PO ON Po.Id = mB.PositionId
									 ...
									 where
									 ED.EffectiveDate IS NOT NULL AND cg.Id ='" + identity.CompanyGroupId + @"' " + wc + @" " + dStatus + @"
                                ...
                                (SELECT TOP 1 WITH TIES TotalNumber,ManpowerBudgetId,EffectiveDate
                                 ...
                                 WHERE CG.Id = '" + identity.CompanyGroupId + @"' " + wc + @" " + dStatus + @" " + EmployeeCategory + @" AND TotalNumber > 0
                                 ) B
                ...
                .Select(x =>
                {
                    DataRow row = dt.NewRow();
                    row["IsDirect"] = "General";
                    row["ColumnName"] = x.Key.ColumnName; row["CompanyId"] = x.Key.CompanyId; row["UserName"] = x.Key.UserName; //row["CompanyGroupId"] = x.Key.CompanyGroupId;
                    row["ProposedManpowerBudget"] = x.Sum(r => (decimal)OTSBD.clsStaticInfo.dbl(r["ProposedManpowerBudget"]));
                    row["TotalManpower"] = x.Sum(r => (decimal)OTSBD.clsStaticInfo.dbl(r["TotalManpower"])); row["Short"] = x.Sum(r => (decimal)OTSBD.clsStaticInfo.dbl(r["Short"]));
                    row["Excess"] = x.Sum(r => (decimal)OTSBD.clsStaticInfo.dbl(r["Excess"])); row["OnRoleSalaryC"] = x.Sum(r => (decimal)OTSBD.clsStaticInfo.dbl(r["OnRoleSalaryC"]));
                    row["ProposedSalaryC"] = x.Sum(r => (decimal)OTSBD.clsStaticInfo.dbl(r["ProposedSalaryC"]));
        ...
        public IEnumerable<object> ModalBudgetSummary(IEnumerable<ChartColumnList> ChartColumnList, int seq, string date, string status, string EmplyeeTypeOrCategoryId, GridParameter parameters)
        {
            var EmployeeCategory = string.Empty;
            if (EmplyeeTypeOrCategoryId == null || EmplyeeTypeOrCategoryId == "")
            {
                                 ...
                                 " + cListext + @",Des.UserName Designation,EmpC.UserName EmployeeCategory,DesG.UserName DesGName
								    ...
								    LEFT JOIN [HKP].EmployeeCategory EmpC ON EmpC.Id = DesM.EmployeeCategoryId
                                     ...
                                     " + Join + @"
                                       ...
                                       WHERE EmployeeStatus = 'Active'
                                        AND em.GroupID = '" + identity.CompanyGroupId + @"'  " + DStatus + @"
									...
									ORDER BY ROW_NUMBER() OVER(PARTITION BY ManpowerBudgetId ORDER BY EffectiveDate DESC)
                                      ...
                                      LEFT outer JOIN [ORG].[Position] AS PO ON Po.Id = MB.PositionId
                                      " + Join + @"
                                      where CG.Id = '" + identity.CompanyGroupId + @"'  " + DStatus + @" " + wc + @"
                                     ) B
                    ...
                    sqlText = @"select m.Id MbId,Code BudgetCode
                                ...
                                (select MB.Code, MB.Id,Cg.Id as CgId,Cg.UserName as GroupName, c.Id as CompanyId, c.UserName as CName
                                 " + cListext + @",Des.UserName Designation,EmpC.UserName EmployeeCategory, DesG.UserName DesGName
                                      ...
                                      LEFT JOIN [HKP].EmployeeCategory EmpC ON EmpC.Id = DesM.EmployeeCategoryId
                                     " + Join + @"
                                     where Cg.Id = '" + identity.CompanyGroupId + @"' " + wc + @" " + EmployeeCategory + @" AND MB.Active = 1
                                    )  m
                                    -----------------------2e--------------------------------
                                    left outer join
                                     (SELECT count(em.SystemID) TotalManpower,BudgetCode,em.GroupID
								    ...
								    LEFT JOIN [HKP].EmployeeCategory EmpC ON EmpC.Id = DesM.EmployeeCategoryId
                                     " + Join + @"
                                       ...
                                       WHERE EmployeeStatus = 'Active' " + DStatus + @"
                                        AND em.GroupID = '" + identity.CompanyGroupId + @"' " + EmployeeCategory + @"
                                        group by BudgetCode,em.GroupID
                                       ) e on m.Id = e.BudgetCode and e.GroupID = m.CgId
                                     -------------------------3b--------------------------------------------------------
                                      left outer join
									...
									ORDER BY ROW_NUMBER() OVER(PARTITION BY ManpowerBudgetId ORDER BY EffectiveDate DESC)
                                      ...
                                      LEFT outer JOIN [ORG].[Position] AS PO ON Po.Id = MB.PositionId
                                      " + Join + @"
                                      where CG.Id = '" + identity.CompanyGroupId + @"'  " + wc + @" " + DStatus + @" " + EmployeeCategory + @"
                                     ) B
                                     on m.id = b.ManpowerBudgetId and b.Id = m.CgId and B.cid = m.CompanyId
                                		 where TotalNumber>0
                                     group by m.Code,GroupName,CompanyId,m.Id,b.ManpowerBudgetId,TotalManpower,TotalNumber,CName " + cListextM + @"
        ...
        public IEnumerable<Object> ModalBudgetDetail(IEnumerable<ChartColumnList> ChartColumnList, int seq, string date, string status, string EmplyeeTypeOrCategoryId, GridParameter parameters)
        {
            var EmployeeCategory = string.Empty;
            if (EmplyeeTypeOrCategoryId == null || EmplyeeTypeOrCategoryId == "")
            {
            ...
            try
            {
                ...
                sqlText = @"SELECT m.Id MbId,Code BudgetCode
								...
								LEFT JOIN [HKP].Designation GDes ON GDes.Id = PO.DesignationId
                                 ...
                                 " + join + @"
                                 WHERE CG.Id = '" + identity.CompanyGroupId + @"' " + DStatus + @" " + wc + @" " + EmployeeCategory + @"
                                 ) B
                                 ON m.id = b.ManpowerBudgetId AND b.Id = m.CgId AND B.cid = m.CompanyId
             	                		 WHERE TotalNumber>0
                                 GROUP BY m.Code,GroupName,CompanyId,m.Id,b.ManpowerBudgetId,TotalManpower,TotalNumber,CName " + cListextM + @"
