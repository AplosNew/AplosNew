...
namespace Library.Service.Invoices
{
    public class InvoiceService : Service<Invoice>, IInvoiceService
    {
        ...
        public string InsertDirectorRemunaretion(VoucherViewModel voucherVM, IEnumerable<VoucherDetailViewModel> voucherDetailVMList
            , IEnumerable<InvoiceTaxViewModel> taxDetailVMList, IEnumerable<InvoiceTaxViewModel> tdsVMList, IEnumerable<InvoiceDetailCharges> invoiceDetailChargesList, IEnumerable<VoucherViewModel> existingLoanList, IEnumerable<MachineMasterAssetSeviceDistribution> machineMasterAssetSeviceDistributionList)
        {
            var flag = false;
            try
            {
                ...
                if (voucherVM.IsExcludingTax == false)
                {
                    ...
                    var partyType = PartyType.Director.ToString();
                    var companyParty = _accountsCommonService.GetCompanyParty(invoice.CompanyId, invoice.PlantId, invoice.PartyId, partyType);
                    ...
                    var invoiceDetail = new InvoiceDetail
                    {
                        ...
                        // Amount = voucherVM.Amount,
                        // Amount = voucherVM.IsExcludingTax ? voucherVM.Amount  : voucherVM.Amount + totalcreditableDrAmount,
                        Amount = voucherDetailVMList.Sum(r => r.TotalAmount),
                        NetAmount = voucherDetailVMList.Sum(r => r.TotalAmount) - totalwithholdCrAmount,
                        TaxAmount = totalwithholdCrAmount
                    };
                    if (null != taxDetailVMList && taxDetailVMList.Count() > 0)
                    {
                        //invoiceDetail.Amount = voucherVM.Amount + totalcreditableDrAmountAddTax - totalwithholdCrAmountAddTax + totalExpensesDrAmountAddTax;
                        invoiceDetail.Amount = voucherDetailVMList.Sum(r => r.TotalAmount) - totalwithholdCrAmountAddTax + totalExpensesDrAmountAddTax;
                        invoiceDetail.NetAmount -= taxDetailVMList.Sum(r => r.TaxAmount);
        ...
        public void PostVoucher(Voucher voucher, VoucherViewModel voucherVM, string invoiceId, string type, IEnumerable<VoucherDetailViewModel> voucherDetailList)
        {
            var flag = false;
            try
            {
                ...
                inDirectsql = @"update [TRN].[Voucher] set PostingDate='" + voucherVM.PostingDate + @"',DocDate='" + voucherVM.DocDate + @"',FiscalYearId='" + voucherVM.FiscalYearId + @"',FiscalYearPeriodId='" + voucherVM.FiscalYearPeriodId + @"' ,TaxYearId='" + voucherVM.TaxYearId + @"' ,TaxYearPeriodId='" + voucherVM.TaxYearPeriodId + @"' ,DocRefNo='" + voucherVM.DocRefNo + @"',Narration='" + voucherVM.Narration + @"',EntityId='" + voucherVM.EntityId + @"' where Id='" + invoice.VoucherId + @"'
                            update [TRN].[VoucherDetail]  set FiscalYearId='" + voucherVM.FiscalYearId + @"',FiscalYearPeriodId='" + voucherVM.FiscalYearPeriodId + @"',DocDate='" + voucherVM.DocDate + @"' ,DocRefNo='" + voucherVM.DocRefNo + @"',Narration='" + voucherVM.Narration + @"' ,EntityId='" + voucher.EntityId + @"' where VoucherId='" + invoice.VoucherId + @"' 
                            update [TRN].[Invoice]  set PostingDate='" + voucherVM.PostingDate + @"',DocDate='" + voucherVM.DocDate + @"',FiscalYearId='" + voucherVM.FiscalYearId + @"',FiscalYearPeriodId='" + voucherVM.FiscalYearPeriodId + @"' ,TaxYearId='" + voucherVM.TaxYearId + @"' ,TaxYearPeriodId='" + voucherVM.TaxYearPeriodId + @"' ,DocRefNo='" + voucherVM.DocRefNo + @"',Narration='" + voucherVM.Narration + @"',EntityId='" + voucher.EntityId + @"' where VoucherId='" + invoice.VoucherId + @"' ";
