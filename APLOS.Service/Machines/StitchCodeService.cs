...
{
    public class StitchCodeService : Service<StitchCode>, IStitchCodeService
    {
        #region Constructor
        ...
        public StitchCodeService(
            IRepositoryAsync<StitchCode> stitchCodeRepository,
            IPKGeneratorService pkGeneratorService,
            IUnitOfWork unitOfWork
            , ISqlRepository sqlRepository
            ) : base(stitchCodeRepository, unitOfWork, pkGeneratorService)
        {
        ...
        private void Check(StitchCode entity)
        {
        ...
        public GridModel Query(GridParameter parameters, string companyGroupId)
        {
            try
            {
                parameters.CmdText = @"SELECT * FROM [HKP].[StitchCode] WHERE CompanyGroupId='" + companyGroupId + "'";
        ...
        public override void Insert(StitchCode entity)
        {
            try
            {
                Check(entity);
                entity.Id = GetAutoNumber(nameof(StitchCode), PKGeneratorEnum.Auto, entity.CompanyGroupId, DateTime.Now);
                entity.Active = true;
                base.Insert(entity);
            }
            catch (Exception ex)
        ...
        public override void Update(StitchCode entity)
        {
            try
            {
                Check(entity);
