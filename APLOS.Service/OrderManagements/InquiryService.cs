...
namespace Library.Service.OrderManagements
{
    public class InquiryService : Service<Inquiry>, IInquiryService
    {
        ...
        public GridModel QueryForResponsible(GridParameter parameters,string entityId,string buyerMasterId,string buyerActivityId)
        {
            try
            {
                parameters.CmdText = @"SELECT EI.SystemId, C.UserName AS CompanyName, EI.PositionId AS PositionCode, EI.BudgetCode, EI.EmployeeCode, EI.FirstName, EI.MiddleName, EI.LastName,   EI.EmployeeName, REPLACE(CONVERT(CHAR(11), EI.DOB, 106),' ','-') AS DateOfBirth,'' AS Flag FROM dbo.EmployeeInformation AS EI 
                                    ...
                                    UNION SELECT EI.SystemId, C.UserName AS CompanyName, EI.PositionId AS PositionCode, EI.BudgetCode, EI.EmployeeCode, EI.FirstName, EI.MiddleName, EI.LastName,   EI.EmployeeName, REPLACE(CONVERT(CHAR(11), EI.DOB, 106),' ','-') AS DateOfBirth,'' AS Flag FROM dbo.EmployeeInformation AS EI 
