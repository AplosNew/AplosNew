...
namespace OTSBD
{
    ...
    public class clsStaticInfo
    {
        ...
        {
            return Convert.ToDouble(bplib.clsWebLib.GetNumData(value));
        }
        public static double dbl(object value)
        {
            if (value == null)
                value = "";
            return Convert.ToDouble(bplib.clsWebLib.GetNumData(value.ToString()));
        }
        ...
        {
            if (bplib.clsWebLib.IsNumeric(pNumber) == false)
            {
        ...
        public static bool IsIntegratedWithSAP(string GroupID)
        {
            ...
            try
            {
                ...
                {
                    if (bplib.clsWebLib.GetBoolData(dsLocal.Tables[0].Rows[0]["IsIntegratedWithSAP"].ToString().Trim()) == true)
                    {
                        FLAG = true;
                    }
                }
  ...
  FROM [SCS].[OpeningBalanceCutOffDate] where PlantId='" + PlantId + "' and ModuleName='HR'";
            ...
            try
            {
                strSql = @" SELECT plant.plantid systemid
                            ...
                            WHERE  userentitymodule.usersystemid = '" + UserSystemID + @"'
                                   AND applicablemodules.systemid = '" + bplib.clsWebLib.MODULEID + @"'
        ...
        public IEnumerable<object> GetBankInfo()
        {
            try
            {
                string strSQL = string.Empty;
        ...
        public static int dateDiff(string firstDate, string lastDate)
        {
            ...
            try
            {
                ...
                if (bplib.clsWebLib.IsDateOK(firstDate) == false)
                {
                    Exception ex = new Exception("Invalid [First Date]");
                    throw (ex);
                }
                if (bplib.clsWebLib.IsDateOK(lastDate) == false)
                {
        ...
        public static void numericValidation(ref TextBox txtBox, bool isMandatory, bool isInteger, bool negativeAllowed, string fieldName)
        {
            try
            {
                if (isMandatory == true)
                {
                    ...
                    if (Convert.ToDouble(bplib.clsWebLib.GetNumData(txtBox.Text.Trim())) == 0)
                    {
                        Exception ex = new Exception("please insert [" + fieldName + "]");
                        txtBox.Focus();
                        throw (ex);
                    ...
                    if (txtBox.Text.Trim() != "")
                    {
                        if (bplib.clsWebLib.IsNumeric(txtBox.Text.Trim()) == false)
                        {
        ...
        public void TaxPeriodValidation(DropDownList ddlTY, TextBox txtTaxEffDate, TextBox txtSalaryEffDate)
        {
            clsTax ob = null;
            DataSet dsValidPeriod = null;
            try
            {
                ob = new clsTax();
                ob.GetTaxYearPeriodValidity(ddlTY.SelectedValue.Trim(), txtTaxEffDate.Text.Trim(), out dsValidPeriod);
                if (dsValidPeriod.Tables[0].Rows.Count == 0)
                {
                    throw new Exception("Selected 'Tax Effective Date' [" + txtTaxEffDate.Text + "] must be within the selected 'Tax Year' [" + ddlTY.Items[ddlTY.SelectedIndex].Text + "]");
                }

                ob.GetTaxYearPeriodValidity(ddlTY.SelectedValue.Trim(), txtSalaryEffDate.Text.Trim(), out dsValidPeriod);
                if (dsValidPeriod.Tables[0].Rows.Count == 0)
                {
                    throw new Exception("Selected 'Salary Effective Date' [" + txtSalaryEffDate.Text + "] must be within the selected 'Tax Year' [" + ddlTY.Items[ddlTY.SelectedIndex].Text + "]");
                }
            }
            catch (Exception ex)
            {
