...
{
   public class clsTemplateReadProfile
    {//
        public List<EmployeeProfileUploadTemplate> ReadData(string plantid, string path)
        {
            List<EmployeeProfileUploadTemplate> data = null;
            //string path = "";
            DataSet dsExcel = null;
            try
            {
                data = new List<EmployeeProfileUploadTemplate>();
                //SaveFile(out path);
                ReadFile(path, out dsExcel);
                Validation(dsExcel, plantid);
                data = dsExcel.Tables[0].ToList<EmployeeProfileUploadTemplate>();
                return data;
            }
            catch (Exception ex)
            {
