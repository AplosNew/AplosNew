...
{
   public class clsTemplateReadEmployeeWeekOff
    {//
        public List<EmployeeWeekOffUploadTemplateVM> ReadData(string plantid, string path)
        {
            List<EmployeeWeekOffUploadTemplateVM> data = null;
            //string path = "";
            DataSet dsExcel = null;
            try
            {
                data = new List<EmployeeWeekOffUploadTemplateVM>();
                //SaveFile(out path);
                ReadFile(path, out dsExcel);
                Validation(dsExcel, plantid);
                data = dsExcel.Tables[0].ToList<EmployeeWeekOffUploadTemplateVM>().Where(x=>x.EmpSystemID !=null).ToList();
                return data;
            }
            catch (Exception ex)
            {
