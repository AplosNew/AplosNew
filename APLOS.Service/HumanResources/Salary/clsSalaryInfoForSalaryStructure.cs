...
namespace OTSBD
{
    public class clsSalaryInfoForSalaryStructure
    {
        public clsSalaryInfoForSalaryStructure()
        {
                            ...
                            FROM SalaryRuleGeneral SG 
                                    ...
                                    LEFT JOIN (
												...
												WHERE SDM.EmpInfoSystemID = '" + sEmpSystemID + @"'
														...
														AND SG.SalaryHeadID = SLID.SalaryHeadID	
                            WHERE --(SG.IsOpen = 1 OR SG.IsFixed = 1)  
                                  SG.IsOpen = 1 AND isnull(SG.IsSlabBased,0 )= 0  and SG.IsFormula != 1 AND SG.IsOpen = 1 AND SG.SalaryRuleMasterSystemID = '" + sSlrRuleMstSystemID + @"'     
