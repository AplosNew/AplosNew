...
{
    public class PayrollReportsService : Service<PlantWiseSalaryRegisterSortingParameters>, IPayrollReportsService
    {
        #region Constructor
                ...
                if (dtEmpInfo.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i <= dtEmpInfo.Rows.Count - 1; i++)
                    {
                        ...
                        if ((string.Compare(x.ToUpper(), dtEmpInfo.Rows[i]["EmpSystemId"].ToString().Trim().ToUpper())) != 0)
                        {
                            ...
                            sheet1.Range[xlsRow, xlsCol + 5].Text = ru.GetLabelname(labelList, LabelNameInLocalLanguage.Designation.ToString(), "Designation");
                            sheet1.Range[xlsRow, xlsCol + 5, xlsRow, xlsCol + 6].Merge();
                            sheet1.Range[xlsRow, xlsCol + 7].Text = ":   " + dtEmpInfo.Rows[i]["legalDesignation"].ToString();
                            ...
                            xlsRow += 1;
                            sheet1.Range[xlsRow, xlsCol].Text = ru.GetLabelname(labelList, LabelNameInLocalLanguage.PFUANNo.ToString(), "PF UAN No.");//"PF UAN No";
                            sheet1.Range[xlsRow, xlsCol, xlsRow, xlsCol + 1].Merge();
                            ...
                            sheet1.Range[xlsRow, xlsCol + 2].Text = ru.GetLabelname(labelList, LabelNameInLocalLanguage.ActualValue.ToString(), "Act Value");// "Act Value";
                        ...
                        sheet1.Range[xlsRow + 1, xlsColTot].Text = "Total Deduction";
...
INNER JOIN
        ...
        public Dictionary<string, List<DataRow>> GetEmployeeSalaryInfoDetail(string companyGroupId, string companyId, string plantId, string fromDate, string toDate, string salaryProcessSystemId, string payRollGroup, Dictionary<string, string> parameters, out DataTable distinctSalaryHead)
        {
            ...
            try
            {
                strSQL = @"SELECT EmpSlr.*,PSH.Sequence,ISNULL(crc.IsDecimalInDisb,0) IsDecimalInDisb,ISNULL(CRC.IntegerInDisb,0) IntegerInDisb,ISNULL(CRC.DecimalNo,0) DecimalNo FROM(SELECT SPC.SystemID AS SlrProcChdSysID, SPC.SlrProcMstSystemID, SPM.SalaryProcID, SPM.FromDate, SPM.ToDate,
        ...
        public Dictionary<string, List<DataRow>> GetEmployeeSalaryInfoDetailPaySlip(string companyGroupId, string companyId, string plantId, string fromDate, string toDate, string languageId, Dictionary<string, string> parameters, bool isActive, bool isSeperated, bool isMaternity, out DataTable distinctSalaryHead)
        {
            ...
            try
            {
                ...
                strSQL = @"SELECT EmpSlr.*,ISNULL(PSH.Sequence,99) Sequence,ISNULL(crc.IsDecimalInDisb,0) IsDecimalInDisb,ISNULL(CRC.IntegerInDisb,0) IntegerInDisb,ISNULL(CRC.DecimalNo,0) DecimalNo FROM(SELECT SPC.SystemID AS SlrProcChdSysID, SPC.SlrProcMstSystemID, SPM.SalaryProcID, SPM.FromDate, SPM.ToDate,
                                                    SPC.EmpInfoSystemID , SPC.PlantID, SPM.UserGroupSystemID, SPM.MonthNo, SPM.YearNo, SPC.PayAbleShSystemID,
                                                    SPC.SalaryHeadID, SPC.EntryCurrencyID, SPC.EntryAmount, SPC.DefineCurrencyID, SPC.DefineAmount,
                                                    SPC.DisbusmentCurrencyID, SPC.DisbusmentAmount, SPC.AcltExcDisbSlrHDID, SPC.AcltExcDisbSlrHDAmt,
                                                    CRE.Name AS PlantWiseExchangeCR, EXR.ToCurrencyBuying ExchangeRate, SPM.AmtDefinitionCurrencyID,
