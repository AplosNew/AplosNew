...
{
    public class MaterialBudgetService : Service<MaterialBudget>, IMaterialBudgetService

    {
        #region Constructor

        private readonly ISqlRepository _sqlRepository;
        private readonly IRepositoryAsync<MaterialBudget> _inventoryReceiveRepository;
        private readonly IRepositoryAsync<MaterialRequsitionDetails> _materialRequsitionDetailsRepository; 
        private readonly IUnitOfWork _unitOfWork;

        public MaterialBudgetService( 
            IRepositoryAsync<MaterialBudget> inventoryReceiveRepository
        ...
        {
            return GetAutoNumber(nameof(PurchaseOrder), PKGeneratorEnum.Yearly, null, DateTime.Now);
        }

        public override void Insert(MaterialBudget entity)
        {
            try
            {
        ...
        public override void Update(MaterialBudget entity)
        {
            try
            {
      ...
      //                  WHERE IR.PlantId='" + plantId + @"' AND ISNULL(IR.[Status],'')<>'Posting'  AND IR.EmployeeId IS NULL AND IR.IsApproved=1  AND IR.IsClosed=0 AND pgl.CtnId is not null Order by IR.PODate DESC, IR.ID DESC";
        ...
        public double makeMaterialDetailsTable(WordDocument document, DataTable dsOrderMaster, string purchaseOrderId)
        {
            ...
            {
                if (C == colRate || C == colArticle || C == colChar1 || C == colChar2 || C == colChar3 || C == colQty || C == colUOM || C == colOriginCountry || dicTaxes.ContainsValue(C))
                    continue;
...
ORDER BY IR.ID DESC";
        ...
        public IEnumerable<object> GetAllReqdata() 
        {
            try
            {
                var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
                var sql = @"SELECT  M.Id
		                    ...
		                    ,M.GLGeneralInfoId
		                    , MGM.UserName AS MaterialGroupMasterName
		                    ,M.MaterialMasterId, MM.UserName
		                    ,M.ArticleId, ART.StandardName
		                    ,M.FirstCharacteristicsId, FC.UserName AS FirstCharacteristics
		                    ,M.FirstCharacteristicsValueId, FCV.UserName AS FirstCharacteristicsValue
		                    ,M.SecondCharacteristicsId, SC.UserName AS SecondCharacteristics
		                    ,M.SecondCharacteristicsValueId, SCV.UserName AS SecondCharacteristicsValue
		                    ,M.ThirdCharacteristicsId, TC.UserName AS ThirdCharacteristics
		                    ,M.ThirdCharacteristicsValueId, TCV.UserName AS ThirdCharacteristicsValue
		                    ,M.BudgetPeriod
		                    ,M.BudgetQty
		                    ,M.ProcurementFrequencyCategory
		                    ,M.ProcurementsPlanDay
		                    ,M.Remarks
		                    ,M.AddedBy
		                    ,M.AddedDate
		                    ,M.AddedFromIP
		                    ...
		                    ,M.UpdatedDate
		                    ,M.UpdatedFromIP
                      FROM TRN.MaterialBudget M
                      Left Join ORG.Entity E On E.Id=M.EntityId
                      LEFT JOIN DBO.EmployeeInformation EI On Ei.SystemId=M.EmployeeId
                      LEFT JOIN HKP.Activity A ON A.Id=M.ActivityId
                      JOIN MST.MaterialMaster AS MM ON M.MaterialMasterId=MM.Id
                      LEFT JOIN MST.MaterialGroupMaster AS MGM ON MM.MaterialGroupMasterId=MGM.Id
	                    LEFT JOIN MST.MaterialMasterArticle AS ART ON M.ArticleId=ART.Id
	                    LEFT JOIN HKP.Characteristics AS FC ON M.FirstCharacteristicsId=FC.Id
	                    LEFT JOIN HKP.Characteristics AS SC ON M.SecondCharacteristicsId=SC.Id
	                    LEFT JOIN HKP.Characteristics AS TC ON M.ThirdCharacteristicsId=TC.Id
	                    LEFT JOIN HKP.CharacteristicsValue AS FCV ON M.FirstCharacteristicsValueId=FCV.Id
	                    LEFT JOIN HKP.CharacteristicsValue AS SCV ON M.SecondCharacteristicsValueId=SCV.Id
	                    LEFT JOIN HKP.CharacteristicsValue AS TCV ON M.ThirdCharacteristicsValueId=TCV.Id 
                        Where M.CompanyGroupId='"+identity.CompanyGroupId+"' Order By M.Id Desc";
                return _sqlRepository.GetDataCollection(sql);
            ...
            try
            {
                ...
                var sql = @"SELECT  M.Id
                      ...
                      LEFT JOIN MST.MaterialGroupMaster AS MGM ON MM.MaterialGroupMasterId=MGM.Id
	                    ...
	                    LEFT JOIN HKP.CharacteristicsValue AS SCV ON M.SecondCharacteristicsValueId=SCV.Id
	                    LEFT JOIN HKP.CharacteristicsValue AS TCV ON M.ThirdCharacteristicsValueId=TCV.Id 
                        Where M.Id='" + Id + "' AND M.CompanyGroupId='" + identity.CompanyGroupId + @"'";
        ...
        public void DeleteReq(string id) 
        {
            try
            {
                //var data = _inventoryReceiveRepository.Find(id);
                //if (data.IsNull()) throw new CustomException(ServiceResources.RecordNoLonger);
                _inventoryReceiveRepository.Delete(id);
                _unitOfWork.SaveChanges();
            }
            catch (Exception ex)
            {
