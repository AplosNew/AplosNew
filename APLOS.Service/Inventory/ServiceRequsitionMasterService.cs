...
//	                            ,MRM.Remarks
//	                            ,MRM.CheckedBy
//								,CheckedByName=CASE When MRM.CheckedByStatus='Checked' 
//								 Then (EEICHK.EmployeeName) --EEICHK.EmployeeCode +'-'+ 
//								 ELSE '' End 
//	                            ,MRM.CheckedByStatus
//	                            ,MRM.AuthorizedBy
//								,AuthorizedByName=CASE WHEN MRM.AuthorizedByStatus='Approval'
//								then   EEIAuth.EmployeeName --EEIAuth.EmployeeCode +'-'+
//								ELSE '' end
...
//	                            Left Join EmployeeInformation EEICHK on EEICHK.SystemId=MRM.CheckedBy
//	                            Left Join EmployeeInformation EEIAuth on EEIAuth.SystemId=MRM.AuthorizedBy
...
//            range = wTable.Rows[ROW].Cells[COL].AddParagraph().AppendText("MaterialDetail");
//            range.ApplyCharacterFormat(FontBold);
//            int colMaterialDetail = COL; COL++;


//            range = wTable.Rows[ROW].Cells[COL].AddParagraph().AppendText("RequiredDate");
//            range.ApplyCharacterFormat(FontBold);
...
//            range = wTable.Rows[ROW].Cells[COL].AddParagraph().AppendText("OwnStock");
//            range.ApplyCharacterFormat(FontBold);
//            int colOnStock = COL; COL++;

//            range = wTable.Rows[ROW].Cells[COL].AddParagraph().AppendText("OtherStock");
//            range.ApplyCharacterFormat(FontBold);
        ...
        public IEnumerable<object> GetCheckedByAndApprovedBYServiceRequisitionCreation(string CheckedBy, string ApprovedBy)
        {
            ...
            try
            {
                ...
                else if (CheckedBy == "false" && ApprovedBy == "true")
                {
                    sql = @"select E.SystemId As Value, E.EmployeeName As Text from dbo.AuthorizationConfig A 
                          Inner JOin dbo.EmployeeInformation E On E.systemId=A.EmployeeId 
                          where  A.ActionStatus='ServiceRequisitionApproveBy'";//A.PlantId='" + identity.PlantId + "' AND
                }
