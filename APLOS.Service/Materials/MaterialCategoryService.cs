...
{
    public class MaterialCategoryService : Service<MaterialCategory>, IMaterialCategoryService
    {
        #region Constructor
        ...
        public MaterialCategoryService(
            IRepositoryAsync<MaterialCategory> materialCategoryRepository
            , IPKGeneratorService pkGeneratorService
            , IUnitOfWork unitOfWork
            , ISqlRepository sqlRepository
            ) : base(materialCategoryRepository, unitOfWork, pkGeneratorService)
        {
        ...
        public override void Insert(MaterialCategory entity)
        {
            try
            {
        ...
        {
            return GetAutoNumber(nameof(MaterialCategory), PKGeneratorEnum.Auto, null, DateTime.Now);
        }

        private void CheckUnique(MaterialCategory entity)
        {
        ...
        public override void Update(MaterialCategory entity)
        {
            try
            {
                CheckUnique(entity);
        ...
        public GridModel Query(GridParameter parameters, string companyGroupId)
        {
            try
            {
                parameters.CmdText = @"SELECT * FROM HKP.MaterialCategory WHERE CompanyGroupId='" + companyGroupId + "'";
        ...
        {
            string _sql = @"SELECT Id, UserName FROM HKP.MaterialCategory WHERE Id IN (SELECT DISTINCT MaterialCategoryId FROM MST.MaterialMaster WHERE CompanyGroupId='" + companyGroupId + "') ORDER BY UserName";
