...
namespace Library.Service.Finances
{
    public class AutoLoanService : IAutoLoanService
    {
        ...
        public string ParkAutoLoan(VoucherViewModel voucherVM, IEnumerable<VoucherViewModel> existingLoanList, IEnumerable<FinancingScheduleViewModel> financingScheduleVMList)
        {
            var flag = false;
            try
            {
                 ...
                 if (financing.TransactionType == TransactionType.LoanTaken.ToString())
                {
                    ...
                    var invoice = _invoiceRepository.Find(voucherVM.InvoiceId);
                    var invoiceDetail = _invoiceDetailRepository.Find(voucherVM.InvoiceDetailId);


                    if (null == invoiceDetail)
                        throw new CustomException("Invoice not found!");

                    invoiceDetail.WrittenOffAmount += voucherVM.Amount;
                    ...
                    var currentInvoiceWriteOffDetailId = 1;
