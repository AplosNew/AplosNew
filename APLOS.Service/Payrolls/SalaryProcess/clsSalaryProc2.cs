...
namespace OTSBD
{
    public class xxclsSalaryProc2
    {
        public xxclsSalaryProc2()
        {
        ...
        public void xxxGetAttdnDataForMonthlyProc(string sEmpSystemID, string sfrmDate, string sToDate, out DataSet dsRef)
        {
            string strSQL;
            ConnectionManager.DAL.ConManager objCon;
        ...
        public void GetMonthWiseExtraSalaryAmt(string sPlantID, string sEmpInfo, int intMonthNo, int intYearNo, out DataSet dsRef)
        {
            ConnectionManager.DAL.ConManager objCon;
            string strSql = "";
            ...
            try
            {
                strSql = @"SELECT * FROM (SELECT
                                            ...
                                            AcltExcDisbSlrHDID = CASE WHEN CRC.AccumulateExchangeRate = 1 
                                                ...
                                                convert(decimal,1) AmtDefinitionRate,-- p.Amount AmtDefinitionRate,
                                             CRC.RoundOption, ISNULL(CRC.IntegerInDisb, 0) IntegerInDisb, 
                                            ISNULL(CRC.IsDecimalInDisb, 0) IsDecimalInDisb, ISNULL(CRC.DecimalNo, 0) DecimalNo 
        ...
        public void GetBonusAmount(string sPlantID, string sEmpInfo, int intMonthNo, int intYearNo, out DataSet dsRef)
        {
            string strSQL;
            ConnectionManager.DAL.ConManager objCon;
            try
        ...
        public void xGetSelectedEmployee(string sEmpInfo, out System.Data.DataSet dsRef)
        {
            string strSQL;
            ConnectionManager.DAL.ConManager objCon;
            try
            {
                ...
                strSQL = @"SELECT DISTINCT E.SystemID EmpSystemID, E.EmployeeCode, E.EmployeeName, E.PlantID, P.UserName PlantName, REPLACE(CONVERT(VARCHAR(11), E.DOJ, 106),' ','-') DOJ, 
                           ...
                           FROM EmployeeInformation E
									...
									LEFT OUTER JOIN EmployeeBankInfo EBI ON E.SystemID = EBI.EmpSystemID AND EBI.IsApproved = 1
									left join (
									select EmpSystemID,FromDate from LeaveTransaction where FromDate between 
                                        '01-oct-2019' and '31-oct-2019'-- and PlantId='20188' 
        ...
        public void GetSelectedEmployee(string sEmpInfo,string sFromDate,string sToDate, out System.Data.DataSet dsRef)
        {
            ...
            try
            {
                ...
                strSQL = @"SELECT DISTINCT E.SystemID EmpSystemID, E.EmployeeCode, E.EmployeeName, E.PlantID, P.UserName PlantName, REPLACE(CONVERT(VARCHAR(11), E.DOJ, 106),' ','-') DOJ, 
								  ...
								  --BankAccountStatus = CASE WHEN EBI.BankSystemID IS NOT NULL THEN 'Bank Payment' ELSE 'Cash Payment' END
                                 ,BankAccountStatus=isnull(e.PaymentMode,'Bank')
								 ,x.EmpSystemID mlvempid,format(x.FromDate,'dd-MMM-yyyy') FromDate,format(y.ToDate,'dd-MMM-yyyy') ToDate
        ...
        public void GetEmployeeWiseAttdnBonusDayType(string sEmpInfo, out DataSet dsRef)
        {
            ...
            try
            {
                strSql = @"SELECT ABPDT.AttdnBonusPmtPolicyDetailsID, ABPDT.DayType,  
	                              ABPDT.DayTypeOperator, ABPDT.DayTypeOperatorValue
                            FROM [dbo].[AttdnBonusPmtPolicyDetails] ABPD
        ...
        public void xxGetEmployeeWiseOTPolicy(string FromDate,string ToDate, string sEmpInfo, out DataSet dsRef)
        {
            ConnectionManager.DAL.ConManager objCon;
            string strSql = "";
            try
            {
                strSql = @"SELECT DM.EmpSystemID, OTPD.OverTimePmtPolicyID OverTimePmtPolicyMasterID, OTPD.ID, DM.SalaryRuleMasterId, OTPD.OverTimeDayType, 
                                  OTPD.IsFixed, OTPD.FixedValue, OTPD.IsFormula, OTPD.FormulaDes, OTPD.FormulaDesID, SROTM.SalaryHeadID,  
	                              SH.SalaryHead, SH.HeadType, SH.HeadCategory,  	
	                              CRC.AmtEntryCurrency EntryCurrencyID, ECR.Name AS EntryCurrency, CRC.AmtDefinitionCurrency DefineCurrencyID, 
	                              DECR.Name AS DefinitionCurrency, ISNULL(CRC.AccumulateExchangeRate, 0) AccumulateExchangeRate, 
	                              ...
	                              CRC.AmtDisbusmentCurrency AS DisbusmentCurrencyID, DICR.Name AS DisbusmentCurrency, CRC.RoundOption, 
                                  ISNULL(CRC.IntegerInDisb, 0) IntegerInDisb, ISNULL(CRC.IsDecimalInDisb, 0) IsDecimalInDisb, ISNULL(CRC.DecimalNo, 0) DecimalNo
                            FROM [dbo].[OverTimePmtPolicyDetails] OTPD
				                            INNER JOIN (
							                            SELECT E.SystemId EmpSystemID, D.SalaryRuleMasterId, D.IsOTEntitled, D.OverTimePmtPolicyMasterID, D.PFPolicyMasterID
								                            FROM [dbo].[EmployeeInformation] E
												                            INNER JOIN  (SELECT DC.LeavePolicyMasterId,DC.PlantId,DM.DesignationId,DC.AttdnBonusPmtPolicyMasterId,
																			DC.SalaryRuleMasterId,DC.IsOTEntitled,DC.OverTimePmtPolicyMasterID,DC.PFPolicyMasterID FROM MST.DesignationMaster DM
                                                        LEFT JOIN SCS.DesignationMasterConfiguration DC ON DM.Id=DC.DesignationMasterId) D ON D.DesignationId = E.GivenDesignationId AND D.PlantId=E.PlantId
							                            WHERE D.AttdnBonusPmtPolicyMasterId IS NOT NULL AND E.SystemId IN (" + sEmpInfo + @") 
                                                        union

                                                         SELECT E.SystemId EmpSystemID, D.SalaryRuleMasterId, D.IsOTEntitled,
                                                         (select ID  from OverTimePmtPolicyMaster where isDefault=1)  OverTimePmtPolicyMasterID, D.PFPolicyMasterID
	                                                       FROM  (
                                                                    select * from EmployeeOTEntitle WHERE  (ISNULL(OTStartDate, GETDATE()) <='" + ToDate + @"' AND ISNULL(OTEndDate, GETDATE())>='" + FromDate + @"'   AND ISNULL(IsOTEntitle, 0) = 1)
		                                                        ) ot 
	                                                        left join [dbo].[EmployeeInformation] E on e.SystemId=ot.EmpSystemID
					                                                        left JOIN  (SELECT DC.LeavePolicyMasterId,DC.PlantId,DM.DesignationId,DC.AttdnBonusPmtPolicyMasterId,
					                                                        DC.SalaryRuleMasterId,DC.IsOTEntitled,DC.OverTimePmtPolicyMasterID,DC.PFPolicyMasterID 
					                                                        FROM MST.DesignationMaster DM
                                                        LEFT JOIN SCS.DesignationMasterConfiguration DC ON DM.Id=DC.DesignationMasterId) D ON D.DesignationId = E.GivenDesignationId AND D.PlantId=E.PlantId

                                                        WHERE  ot.EmpSystemID IN (
                                                       (" + sEmpInfo + @") 
                                                        )

							                           ) DM ON OTPD.OverTimePmtPolicyID = DM.OverTimePmtPolicyMasterID
				                            LEFT JOIN [dbo].[SalaryRuleMaster] SRM ON DM.SalaryRuleMasterId = SRM.SystemID
				                            LEFT JOIN [dbo].[SalaryRuleOT] SROTM ON SRM.SystemID = SROTM.SalaryRuleMasterSystemID AND OTPD.OverTimeDayType = SROTM.OverTimeDayType
				                            LEFT JOIN [dbo].[SalaryHead] SH ON SROTM.SalaryHeadID = SH.SalaryHeadID
				                            LEFT JOIN [dbo].[CurrencyRuleChild] CRC ON SRM.CurrencyRuleSystemID = CRC.MstSystemID AND SROTM.SalaryHeadID = CRC.SalaryHeadID
			                                LEFT JOIN scs.Currency ECR ON CRC.AmtEntryCurrency = ECR.Id
			                                LEFT JOIN scs.Currency DECR ON CRC.AmtDefinitionCurrency = DECR.Id
			                                LEFT JOIN scs.Currency DICR ON CRC.AmtDisbusmentCurrency = DICR.Id
                                            where SROTM.SalaryHeadID is not null
                                            and DM.EmpSystemID not in 
											(
											select EmpSystemID from EmployeeOTEntitle WHERE  (ISNULL(OTStartDate, GETDATE()) <='" + ToDate + @"' AND ISNULL(OTEndDate, GETDATE())>='" + FromDate + @"'   AND ISNULL(IsOTEntitle, 0) = 0)
											)
                            ORDER BY DM.EmpSystemID";
        ...
        public void xGetEmployeeWiseOTPolicy(string FromDate,string sEmpInfo, out DataSet dsRef)
        {
            ConnectionManager.DAL.ConManager objCon;
            string strSql = "";
            try
        ...
        public void GetPFStructureData(string sEmpInfo, string sDate, out DataSet dsRef)
        {
            ...
            try
            {
                strSql = @" select 
								...
								INNER JOIN SalaryHead SH ON d.SalaryHeadID = SH.SalaryHeadID AND ISNULL(SH.HeadCategory, '') != 'Tax' AND ISNULL(SH.HeadCategory, '') 
								in ('PF Employer Contribution','PF Employee Contribution','PF Voluntary','Pension')
							    INNER JOIN SalaryRuleMaster SRM ON mm.SalaryRuleMasterSystemID = SRM.SystemID
        ...
        public void GetEarlyOut(string sEmpInfo, string plantid, string sFromDate, string sToDate, out DataSet dsRef)
        {
            ConnectionManager.DAL.ConManager objCon;
            string strSql = "";
            try
            {
                strSql = @"select count(id) c ,EmpSystemId
                                    ...
                                    where plantid='" + plantid + @"' and EmpSystemId in (" + sEmpInfo + @")
