...
namespace OTSBD
{
    public class xclsBonusMonthlyRetain
    {
        public string sFormulaValue = "";

        public xclsBonusMonthlyRetain()
        {
                            ...
                            while (SelectedEmpCnt < dsUnTagEmp.Tables[0].Rows.Count)
                            {
                                ...
                                if (EmpCntForLoop == grdRowMaxCnt)
                                {
                                    ...
                                    if (dsEnumBonus.Tables[0].Rows.Count > 0)
                                        dicEnum = dsEnumBonus.Tables[0].ToList<EmployeeEligibleForSalaryHeadEnum>();
                                    ...
                                    for (int iUnTgEmCnt = 0; iUnTgEmCnt < dsUnTagEmp.Tables[0].Rows.Count; iUnTgEmCnt++)
                                    {
                                        ...
                                        GetBoolEligibility_SP(ref bIsEligibleApp, dsBnsMonthNo, dtStartDate, para);
                                            ...
                                            if (dicSalInfo_Sub.Count > 0)
                                            {
                                                ...
                                                ///is eligi                                           
                                                var dicSubEnumPF = dicEnum.FindAll(x => x.EmpSystemId == dsUnTagEmp.Tables[0].Rows[iUnTgEmCnt]["SystemID"].ToString().Trim() && x.SalaryStructureId == _sstructureid && x.SalaryHeadEnum == "Bonus" && x.IsEligible == true);
        ...
        //    {
        //        strSQL = @"select * from (
        //                                SELECT Id,SalaryHeadEnum,EmpSystemId,SalaryStructureId,IsEligible,CompanyGroupId,PlantId FROM EmployeeEligibleForSalaryHeadEnum
        //                                ) x
        //                      where (" + EmpSystemId + ") ";
