...
clsESICProcess()
        ...
        public void XGetUnTagEmployeeListWithESICPolicyMaster(ESICParaList para, out System.Data.DataSet dsRef)
        {
            string strSQL;
            ConnectionManager.DAL.ConManager objCon;
            try
        ...
        public void GetUnTagEmployeeListWithESICPolicyMaster(ESICParaList para, out System.Data.DataSet dsRef)
        {
            string strSQL;
            ConnectionManager.DAL.ConManager objCon;
            try
        ...
        public void ReLoadFormulaWithValueNew(string sEmpSystemID, ESICParaList para, string strFormulaID, out string sFormulaValue, bool bEarning, List<SPvalueHeadWise> dtValue, List<SPSalaryHead> dicSlrHd)
        {
            DataSet dsLocal = null;
            //DataView dvLocal = null;
        ...
        private void ReLoadFormulaWithValue(string sEmpSystemID, ESICParaList para, string sFormulaID, bool bEarning, ref DataTable dtValue, ref DataTable dtSlrHd)
        {
            DataSet dsLocal = null;
            DataView dvLocal = null;
            DataView dvSlrHd = null;
        ...
        public void xLoadEmpSlrDefForSlrProcess(ESICParaList para, string sEmpInfo, out DataSet dsRef)
        {
            ConnectionManager.DAL.ConManager objCon;
            string strSql = "";
        ...
        public void LoadEmpSlrDefForSlrProcess(ESICParaList para, string sEmpInfo, out DataSet dsRef)
        {
            ConnectionManager.DAL.ConManager objCon;
            string strSql = "";
        ...
        public void GetEnumEligibility(string EmpSystemId, out System.Data.DataSet dsRef)
        {
            ...
            {
                strSQL = @"select * from (
                                         SELECT Id,SalaryHeadEnum,EmpSystemId,SalaryStructureId,IsEligible,CompanyGroupId,PlantId,AddedBy, FORMAT(AddedDate, 'dd-MMM-yyyy') AddedDate, AddedFromIp, UpdatedBy, FORMAT(UpdatedDate,'dd-MMM-yyyy') UpdatedDate, UpdatedFromIp FROM EmployeeEligibleForSalaryHeadEnum
                                        ) x
                              where (" + EmpSystemId + ") ";
        ...
        public void CalculateEarnESIC(ESICParaList para,out List<EmpSalaryHeadAmount> _List_ESICHeadValue)
        {
            ...
            try
            {
                ...
                _List_ESICHeadValue = new List<EmpSalaryHeadAmount>();
                        ...
                        if (dsUnTagEmp.Tables[0].Rows.Count > 0)
                        {
                            ...
                            while (SelectedEmpCnt < dsUnTagEmp.Tables[0].Rows.Count)
                            {
                                ...
                                if (EmpCntForLoop == grdRowMaxCnt)
                                {
                                    ...
                                    List<EmployeeEligibleForSalaryHeadEnum> dicEnum = new List<EmployeeEligibleForSalaryHeadEnum>();
                                    GetEnumEligibility(sEmpSystemID, out dsEnumPF);//and SalaryHeadEnum='PF' and IsEligible=1
                                    if (dsEnumPF.Tables[0].Rows.Count > 0)
                                        dicEnum = dsEnumPF.Tables[0].ToList<EmployeeEligibleForSalaryHeadEnum>();
                                    ...
                                    for (int iUnTgEmCnt = 0; iUnTgEmCnt < dsUnTagEmp.Tables[0].Rows.Count; iUnTgEmCnt++)
                                    {
                                        ...
                                        if (dicSalInfo_Sub.Count > 0)
                                        {
                                            ...
                                            var dicSubEnumPF = dicEnum.FindAll(x => x.EmpSystemId == dsUnTagEmp.Tables[0].Rows[iUnTgEmCnt]["SystemID"].ToString().Trim() && x.SalaryStructureId == _sstructureid && x.SalaryHeadEnum == "ESIC" && x.IsEligible == true);
                                            ...
                                            if (dtValue.Count > 0)
                                            {
                                                for (int iESICDtl = 0; iESICDtl < dsESICPolicyDtl.Tables[0].Rows.Count; iESICDtl++)
                                                {
                                                    ...
                                                    else if (bIsFormulaEmp == true)
                                                    {
                                                        ...
                                                        ReLoadFormulaWithValueNew(sEmpSysID, para, sFormulaDesIDEmp, out sFormulaValue, bEarning, dtValue, dicSalaryHead);
                                                        sFormulaResult = Evaluate(sFormulaValue.Trim()).ToString();
                                                    ...
                                                    dvCurRl.RowFilter = "SalaryHeadID = '" + sESICContSalaryHeadIDEmp + "' AND CurrencyRuleSystemID = '" + sCurrencyRuleSystemID + "'";
                                                    if (dvCurRl.Count > 0)
                                                    {
                                                    ...
                                                    string sOutValue = "0";
                                                    obSS.FractionCalculation(sRoundOption, bIntegerInDisb, bIsDecimalInDisb, iDecimalNo, decEmpCtbtnAmount.ToString(), out sOutValue);
                                                    decEmpCtbtnAmount = Convert.ToDecimal(sOutValue);

                                                    GetHeadWiseAmount(decEmpCtbtnAmount, sEmpSysID, sESICContSalaryHeadIDEmp, ref _List_ESICHeadValue);
                                                    ...
                                                    else if (bIsFormulaEmpr == true)
                                                    {
                                                        ...
                                                        //ReLoadFormulaWithValueNew(sEmpSysID, para, sFormulaDesIDEmpr, bEarning, ref dtValue, ref dtSalHd);
                                                        ReLoadFormulaWithValueNew(sEmpSysID, para, sFormulaDesIDEmpr, out sFormulaValue, bEarning, dtValue, dicSalaryHead);
                                                        sFormulaResult = Evaluate(sFormulaValue.Trim()).ToString();
                                                    ...
                                                    dvCurRl.Table = dtCurRl;
                                                    dvCurRl.RowFilter = "SalaryHeadID = '" + sESICContSalaryHeadIDEmpr + "' AND CurrencyRuleSystemID = '" + sCurrencyRuleSystemID + "'";
                                                    if (dvCurRl.Count > 0)
                                                    {
                                                        sRoundOption = dvCurRl[0].Row["RoundOption"].ToString().Trim();
                                                        bIntegerInDisb = Convert.ToBoolean(dvCurRl[0].Row["IntegerInDisb"].ToString().Trim());
                                                    ...
                                                    //string sOutValue = "0";
                                                    obSS.FractionCalculation(sRoundOption, bIntegerInDisb, bIsDecimalInDisb, iDecimalNo, decEmprCtbtnAmount.ToString(), out sOutValue);
                                                    decEmprCtbtnAmount = Convert.ToDecimal(sOutValue);

                                                    GetHeadWiseAmount(decEmprCtbtnAmount, sEmpSysID, sESICContSalaryHeadIDEmpr, ref _List_ESICHeadValue);
        ...
        public void GeneratorESICEligibleEmployee(ESICParaList para)
        {
            #region Variable Dataset
            DataTable dtValue = null;
            DataSet dsESICEligibleEmp = null;
