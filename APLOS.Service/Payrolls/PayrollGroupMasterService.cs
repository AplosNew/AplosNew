...
{
    public class PayrollGroupMasterService : Service<PayrollGroupMaster>, IPayrollGroupMasterService
    {
        #region Constructor

        private readonly IUnitOfWork _unitOfWork;
        private readonly ISqlRepository _sqlRepository;
        private readonly IRepositoryAsync<PayrollGroupMaster> _PayrollGroupMasterRepository;
        private readonly IRepositoryAsync<EmployeeLeaveSummary> _EmployeeLeaveSummaryRepository;

        public PayrollGroupMasterService(
            IRepositoryAsync<PayrollGroupMaster> PayrollGroupMasterRepository,
            IRepositoryAsync<EmployeeLeaveSummary> EmployeeLeaveSummaryRepository,
            IPKGeneratorService pkGeneratorService,
            IUnitOfWork unitOfWork,
            ISqlRepository sqlRepository) : base(PayrollGroupMasterRepository, unitOfWork, pkGeneratorService)
        {
            _PayrollGroupMasterRepository = PayrollGroupMasterRepository;
        ...
        public void InsertOrUpdateGraph(IEnumerable<PayrollGroupMaster> entities)
        {
            var flag = false;
            try
            {
                if (entities == null)
                    throw new CustomException("Please Select Payroll Group And Employee");
                _unitOfWork.BeginTransaction();
                flag = true;
                //var pk = GetMaxNumber(nameof(PayrollGroupMaster), PKGeneratorEnum.Yearly, null, DateTime.Now);
        ...
        public void InSertOrUpdate(PayrollGroupMaster entity)
        {
            ...
            {
                data.PayrollGroupId = entity.PayrollGroupId;
                Update(data);
            }
        }
        ...
        {
            return base.GetMaxNumber(nameof(PayrollGroupMaster), PKGeneratorEnum.Auto, null, DateTime.Now);
        }
...
LEFT JOIN MST.ManpowerBudget mb ON mb.Id = e.BudgetCode
                            LEFT JOIN ORG.Position PR ON MB.PositionId=PR.Id
										...
										LEFT JOIN HKP.EmployeeCategory EC ON EC.Id=dmt.EmployeeCategoryId
                                        LEFT JOIN [MST].[PayrollGroupMaster] PG ON PG.EmployeeId=E.SystemId AND PG.PayrollGroupId='" + payrollGroupId + @"' 
                                        WHERE  E.GroupID='" + companyGroupId + @"' AND E.PlantId = '" + plantId + @"' AND PG.PayrollGroupId='" + payrollGroupId + @"' AND E.EmployeeStatus='Active'";
