'use strict';
RoleMappingPositionController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function RoleMappingPositionController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    ...
    $scope.roleMappingSearch = function () {
        $http({
            method: 'GET',
            url: 'Securities/rolemapping/getlistbyposition?positionStructureId=' + $scope.roleMapping.PositionStructureId
    ...
    $scope.onCompanyChange = function () {
        $http({
            method: 'GET',
            url: 'Organizations/Position/getcbolist?companyId=' + $scope.roleMapping.CompanyId
        }).then(function successCallback(response) {
    ...
    $scope.getPosition = function (id) {
        ...
        $http({
            method: 'GET',
            url: 'Organizations/Position/get?companyId=' + $scope.roleMapping.CompanyId + '&&id=' + id
        }).then(function successCallback(response) {
    ...
    $scope.popUp = function () {
        if (baseService.isUndefinedOrNull($scope.roleMapping.CompanyId) || baseService.isUndefinedOrNull($scope.roleMapping.PositionStructureId))
            return ShowResult('Please select all drop down', 'failure');

        $scope.popUpUrl = 'Securities/rolemapping/getrolelistbyposition?roleId=' + isRoleIdExistInRoleMapping($scope.roleMappingList);
        ...
        angular.forEach($scope.popUpDataList, function (a) {
            if (a.Flag) {
                $scope.roleMappingList.push({
                    Id: null,
                    PositionStructureId: $scope.roleMapping.PositionStructureId,
                    RoleId: a.Id,
                    RoleName: a.RoleName,
                    Remarks: a.Remarks,
                    Archive: false
    ...
    $scope.Save = function () {
        ...
        if ($scope.roleMapping.$valid) {
            $http({
                method: 'POST',
                url: 'Securities/rolemapping/createpositionstructure',
