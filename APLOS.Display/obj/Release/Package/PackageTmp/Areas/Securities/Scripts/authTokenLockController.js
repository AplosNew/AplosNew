'use strict';
function AuthTokenLockController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = "AuthToken UnLock";
    $scope.index = -1;
    $scope.Action = 'UnLock';
    $scope.users = [];
    $scope.path = 'Securities/user/';
    $scope.getListUrl = $scope.path + 'authtokenlockdatewithoutsyadmin';
    ...
    $scope.authTokenUnLock = function () {
        if ($rootScope.id != null) {
            $http({
                method: 'POST',
                url: 'Securities/user/authtokenunlock?id=' + $rootScope.id,
                dataType: 'JSON'
...
AuthTokenLockController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];