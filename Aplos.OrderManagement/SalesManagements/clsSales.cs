...
Order By A.sequence";
        ...
        {
            var cmdText = @"SELECT * FROM(SELECT Flag=CAST(CASE WHEN SM.InputCreditId IS NULL THEN 0 ELSE 1 END AS bit),SM.Id,MO.Id MasterOrderId,SO.Id SONo,po.PONumber, FORMAT(SO.DeliveryDate,'dd-MMM-yyyy') DeliveryDate,DT.UserName DestinationName,MM.UserName MaterialMasterName,ART.StandardName AS MaterialMasterArticleName
...
,ServiceCharge=0,ServiceTax=0,SM.InputCreditId,'InventorySales' SourceType
            ...
            WHERE IM.PlantId='" + plantId + @"' AND SM.AddedDate between '" + fromDate + @"' AND '" + toDate + @"' AND ISNULL(SM.InputCreditId,'" + inputCreditId + @"')='" + inputCreditId + @"')A Order By A.Id";
