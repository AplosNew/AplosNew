...
{
    public class OrderControlReport
    {
        ...
        public void GetData(out DataSet dsSavedData)
        {
            ConManager = new ConnectionManager.clsConnectionManager();
            ConManager.getDataSet(@"select oct.ControlTypeDesc, oc.SalesOrderId,oc.CriticalityLevel,oc.Status, so.Description,
                                so.DeliveryDate, cp.PONumber, sm.UserName as ShipmentMode, d.UserName as Destination, 
                                oc.ProductionOrderId, ocr.Remarks
                                from OrderControlTypes oct
                                left join OrderControl oc on oc.ControlTypeId = oct.Id
                                left join OrderControlRemarks ocr on ocr.OrderControlId = oc.Id
                                left join TRN.SalesOrder so on so.Id = oc.SalesOrderId
                                left join TRN.CustomerPO cp on cp.Id = so.CustomerPOId
                                left join MST.ShipMode sm on sm.Id = so.ShipmentModeId
                                left join MST.Destination d on d.Id = so.DestinationId", out dsSavedData);
        ...
        public IWorkbook ControlChartReportXls()
        {
            ...
            try
            {
                ...
                DataTable dtDistinctControl = dtOrderMaster.DefaultView.ToTable(true, "ControlTypeId", "ControlTypeDesc");
                ...
                for (int k = 0; k < dtDistinctControl.Rows.Count; k++)
                {
                    ...
                    if (dtDistinctControl.Rows[k]["ControlTypeDesc"].ToString() == OrderControlTypes.ShipmentControl.ToString())
                        DataCaption = "Delivery Date";
                    if (dtDistinctControl.Rows[k]["ControlTypeDesc"].ToString() == OrderControlTypes.MainRMInhouse.ToString() || dtDistinctControl.Rows[k]["ControlTypeDesc"].ToString() == OrderControlTypes.MainRMShipment.ToString())
                        DataCaption = "Main RM Inhouse Date";
                    if (dtDistinctControl.Rows[k]["ControlTypeDesc"].ToString() == OrderControlTypes.OtherRMInhouse.ToString() || dtDistinctControl.Rows[k]["ControlTypeDesc"].ToString() == OrderControlTypes.OtherRMShipment.ToString())
                        DataCaption = "Other RM Inhouse Date";

                    if (dtDistinctControl.Rows[k]["ControlTypeDesc"].ToString() == OrderControlTypes.BaseProcessInput.ToString())
                    ...
                    COL++;
                    sheet[ROW, COL].Text = "Buyer Country";
                    sheet[ROW, COL].ColumnWidth = 8;
        ...
        private void UnplannedPRList(IWorksheet sheet)
        {
            try
            {
                ...
                COL++;
                sheet[ROW, COL].Text = "Buyer Order#";
                sheet[ROW, COL].ColumnWidth = 10;
        ...
        private void UnplannedPRs(out DataTable dt)
        {
            string sql = @"SELECT 
                            ...
                            SO.Id AS SalesOrderId,so.OrderStatusId AS SalesOrderStatusId,ps.UserName AS ProductionStatus,
                             pod.ProductionOrderId,EN.UserName AS EntityName,PLN.UserName AS Plant,
...
and OCT2.ControlType NOT IN ('" + OrderControlTypes.MainRMShipment.ToString() + "','" + OrderControlTypes.OtherRMShipment.ToString() + @"')
