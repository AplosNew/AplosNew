...
namespace Library.Service.Extension.HumanResource.Payroll.IncrementReport
{
...
left join scs.LegalSalaryGrade grd on grd.id=gr.LegalSalaryGradeId";
                ...
                otc.LoadSalaryStructure(identity.PlantId, FromDate, ToDate, out DataSet dsNewSStructure);

                Dictionary<string, DataRow> dicNewGross = Cluster(dsNewSStructure, "Gross");
                Dictionary<string, DataRow> dicNewBasic= Cluster(dsNewSStructure, "Basic");


                otc.LoadOldSalaryStructure(identity.PlantId, FromDate, ToDate, out DataSet dsOldSStructure);

                Dictionary<string, DataRow> dicOldGross = Cluster(dsOldSStructure, "Gross");
                Dictionary<string, DataRow> dicOldBasic = Cluster(dsOldSStructure, "Basic");
