...
namespace Library.Accounting.Accounts
{
    public class AccountsSalaryPayableService
    {
        ...
        public List<Dictionary<string, object>> GetSalaryLockDataList(string yearNo, string monthNo, string employeeId, bool isActive, bool isSeperated, bool isMaternity, string plantId, string entityId)
        {
            ...
            SELECT sh.SalaryHead,sh.[Sequence],sl.YearNo,sl.MonthNo,sh.HeadType
                        ...
                        WHERE sl.MonthNo='" + monthNo + "' and sl.YearNo='" + yearNo + @"' AND MPB.EntityId='" + entityId + @"' AND sl.Islocked=1 AND sl.PayableVoucherId IS NULL--and sl.EmpSystemId='" + employeeId + @"' 
                        AND ISNULL(sh.HeadCategory,'') not in ('CTC','Gross','Total Gross','Net Payable') and spc.DisbusmentAmount!=0 AND SPL.PlantId='" + plantId + @"'  and DMC.plantId='" + plantId + @"'
                        ...
                        left join dbo.SalaryHead sh on sh.SalaryHeadID=spc.SalaryHeadID
                        WHERE sl.MonthNo='" + monthNo + "' and sl.YearNo='" + yearNo + @"' AND MPB.EntityId='" + entityId + @"' AND sl.Islocked=1 AND sl.PayableVoucherId IS NULL --and sl.EmpSystemId='" + employeeId + @"' 
                        AND ISNULL(sh.SalaryHead,'')  in ('Net Pay') and spc.DisbusmentAmount!=0 " + wcEmpStatus + @" AND SPL.PlantId='" + plantId + @"'  and DMC.plantId='" + plantId + @"'
                        ...
                        and spc.DisbusmentAmount!=0 and  PO.DirectManpowerCost=1 AND sh.HeadType='D' " + wcEmpStatus + @" AND SPL.PlantId='" + plantId + @"'  and DMC.plantId='" + plantId + @"'
                        AND  ei.SystemId not in (select EmpSystemId from [dbo].[ExceptionEmployee])
                        GROUP BY sh.SalaryHead,sl.YearNo,sl.MonthNo,sh.HeadType,sh.[Sequence]
                        ) x
        ...
        public List<Dictionary<string, object>> GetSalaryLockCTCDataList(string yearNo, string monthNo, string employeeId, bool isActive, bool isSeperated, bool isMaternity, string plantId, string entityId)
        {
        ...
        public List<Dictionary<string, object>> GetSalaryLockDataGLList(string yearNo, string monthNo, string employeeId, bool isActive, bool isSeperated, bool isMaternity, string plantId, string entityId)
        {
            string wcEmpStatus = " AND spm.SalaryProcFlag=''";
            ...
            string sql = @"SELECT 
                       X.SalaryHeadId,X.AccountsGroupId ,X.AccountsGroup,'Direct' SalaryType,X.SalaryHead ,X.SalaryHeadCategory,
                        ...
                        FROM
                        (
                         select sh.SalaryHeadID SalaryHeadId, SGL.AccountsGroupId,AG.UserName AccountsGroup,sh.SalaryHead,sh.HeadCategory SalaryHeadCategory,sl.YearNo,sl.MonthNo,sh.HeadType
                        ...
                        LEFT JOIN [dbo].[AccountsGroup] AG ON AG.Id=SGL.AccountsGroupId
                        ...
                        WHERE sl.MonthNo='" + monthNo + "' and sl.YearNo='" + yearNo + @"' AND MPB.EntityId='" + entityId + @"' AND sl.Islocked=1 AND sl.PayableVoucherId IS NULL " + wcEmpStatus + @" AND SPL.PlantId='" + plantId + @"'  and DMC.plantId='" + plantId + @"'--and sl.EmpSystemId='" + employeeId + @"' 
                        ...
                        GROUP BY sh.SalaryHeadID,sh.SalaryHead, SGL.AccountsGroupId,AG.UserName,sh.HeadCategory,sl.YearNo,sl.MonthNo,sh.HeadType,sh.[Sequence],SGL.DrDirectGLId,SGL.DrDirectBudgetMasterId,SGL.DrDirectActivityId
			...
			SELECT sh.SalaryHeadID SalaryHeadId, SGL.AccountsGroupId,AG.UserName AccountsGroup,sh.SalaryHead,sh.HeadCategory SalaryHeadCategory,sl.YearNo,sl.MonthNo,sh.HeadType
                        ...
                        GROUP BY sh.SalaryHeadID,sh.SalaryHead, SGL.AccountsGroupId,AG.UserName,sh.HeadCategory,sl.YearNo,sl.MonthNo,sh.HeadType,sh.[Sequence]
						,SGL.CrDirectGLId,SGL.CrDirectBudgetMasterId,SGL.CrDirectActivityId
                        ,CDGL.AccountCode,CDGL.UserName ,CDB.UserName ,CDA.UserName
						)X
						GROUP BY 
						X.SalaryHeadId,X.SalaryHead, X.AccountsGroupId,X.AccountsGroup,X.SalaryHeadCategory,
        ...
        public List<Dictionary<string, object>> GetSalaryLockInDirectTakeAwayDataList(string yearNo, string monthNo, string employeeId, bool isActive, bool isSeperated, bool isMaternity, string plantId, string entityId)
        {
            string wcEmpStatus = " AND spm.SalaryProcFlag=''";
            ...
            string sql = @"
                    select x.* from (
						...
						left join ORG.Position PO on PO.Id=MPB.PositionId
                        left join dbo.SalaryHead sh on sh.SalaryHeadID=spc.SalaryHeadID
                        WHERE sl.MonthNo='" + monthNo + "' and sl.YearNo='" + yearNo + @"' AND MPB.EntityId='" + entityId + @"'  AND sl.Islocked=1 AND sl.PayableVoucherId IS NULL " + wcEmpStatus + @"--and sl.EmpSystemId='" + employeeId + @"' 
                        AND  ei.SystemId not in (select EmpSystemId from [dbo].[ExceptionEmployee] where month(effectivedate)<=sl.MonthNo and year(effectivedate)=sl.YearNo)
                        AND ISNULL(sh.HeadCategory,'') not in ('CTC','Gross','Total Gross','Net Payable') and spc.DisbusmentAmount!=0 
                        AND  PO.DirectManpowerCost=0 AND sh.HeadType='E' AND sh.IsGrossComponent=1 AND sh.PartOfNetPay=1 AND SPL.PlantId='" + plantId + @"'  and DMC.plantId='" + plantId + @"'
                        GROUP BY sh.SalaryHead,sl.YearNo,sl.MonthNo,sh.HeadType,sh.[Sequence]
                        UNION
                        SELECT sh.SalaryHead,sh.[Sequence],sl.YearNo,sl.MonthNo,sh.HeadType
                        , SUM(spc.DisbusmentAmount) DrAmount
                        ...
                        left join dbo.SalaryHead sh on sh.SalaryHeadID=spc.SalaryHeadID
                        WHERE sl.MonthNo='" + monthNo + "' and sl.YearNo='" + yearNo + @"' AND MPB.EntityId='" + entityId + @"' AND sl.Islocked=1 AND sl.PayableVoucherId IS NULL " + wcEmpStatus + @"--and sl.EmpSystemId='" + employeeId + @"' 
                        AND ISNULL(sh.SalaryHead,'')  in ('Net Pay') and spc.DisbusmentAmount!=0 AND SPL.PlantId='" + plantId + @"'  and DMC.plantId='" + plantId + @"'
                        AND  ei.SystemId not in (select EmpSystemId from [dbo].[ExceptionEmployee] where month(effectivedate)<=sl.MonthNo and year(effectivedate)=sl.YearNo)
        ...
        public List<Dictionary<string, object>> GetSalaryLockInDirectCTCDataList(string yearNo, string monthNo, string employeeId, bool isActive, bool isSeperated, bool isMaternity, string plantId, string entityId)
        {
            string wcEmpStatus = " AND spm.SalaryProcFlag=''";
            ...
            string sql = @"
                        ...
                        AND  ei.SystemId not in (select EmpSystemId from [dbo].[ExceptionEmployee] where month(effectivedate)<=sl.MonthNo and year(effectivedate)=sl.YearNo)
                        AND  PO.DirectManpowerCost=0 AND sh.HeadType='E' AND sh.IsGrossComponent=0 AND sh.PartOfNetPay=0 " + wcEmpStatus + @" AND SPL.PlantId='" + plantId + @"'  and DMC.plantId='" + plantId + @"'
                        GROUP BY sh.SalaryHead,sl.YearNo,sl.MonthNo,sh.HeadType,sh.[Sequence]
						ORDER BY sh.[Sequence],sh.SalaryHead";
            return _sqlRepository.GetDataCollection(sql);
        }
        ...
        public List<Dictionary<string, object>> GetSalaryLockInDirectDataGLList(string yearNo, string monthNo, string employeeId, bool isActive, bool isSeperated, bool isMaternity, string plantId, string entityId)
        {
            string wcEmpStatus = " AND spm.SalaryProcFlag=''";
            ...
            string sql = @"SELECT 
                            X.SalaryHeadId,X.AccountsGroupId,X.AccountsGroup ,'InDirect' SalaryType, X.SalaryHead ,X.SalaryHeadCategory,
                            ...
                            FROM
                            (
                            select sh.SalaryHeadID SalaryHeadId,SGL.AccountsGroupId,AG.UserName AccountsGroup,sh.SalaryHead,sh.HeadCategory SalaryHeadCategory,sh.[Sequence],sl.YearNo,sl.MonthNo,sh.HeadType
                        , DrAmount=case when SUM(spc.DisbusmentAmount) < 0 then SUM(spc.DisbusmentAmount)*-1 else SUM(spc.DisbusmentAmount) end
                        , 0 CrAmount
                        ,SUM(spc.DisbusmentAmount) DisbusmentAmount
                       ...
                       AND  ei.SystemId not in (select EmpSystemId from [dbo].[ExceptionEmployee] where month(effectivedate)<=sl.MonthNo and year(effectivedate)=sl.YearNo)
                        group by sh.SalaryHeadID,SGL.AccountsGroupId,AG.UserName,sh.SalaryHead,sh.HeadCategory,sl.YearNo,sl.MonthNo,sh.HeadType,sh.[Sequence],SGL.DrInDirectGLId,SGL.DrInDirectBudgetMasterId,SGL.DrInDirectActivityId
                        ...
                        AND  ei.SystemId not in (select EmpSystemId from [dbo].[ExceptionEmployee] where month(effectivedate)<=sl.MonthNo and year(effectivedate)=sl.YearNo)
                        group by sh.SalaryHeadID,SGL.AccountsGroupId,AG.UserName,sh.SalaryHead,sh.HeadCategory,sl.YearNo,sl.MonthNo,sh.HeadType,sh.[Sequence]
						...
						GROUP BY 
						X.SalaryHeadId,X.AccountsGroupId,X.AccountsGroup,X.SalaryHead,X.SalaryHeadCategory,
                        X.InDirectGLName,X.InDirectBudgetName,X.InDirectActivityName,X.GLGeneralInfoId,X.BudgetMasterId,X.ActivityId 
						ORDER BY 7";
            return _sqlRepository.GetDataCollection(sql);
        }

        public List<Dictionary<string, object>> GetDirectSalaryLockSalarySheetData(string yearNo, string monthNo, bool isActive, bool isSeperated, bool isMaternity, string plantId, string entityId)
        {
            string wcEmpStatus = " AND spm.SalaryProcFlag=''";
            ...
            SELECT x.* FROM (
                        ...
                        ,spc.DisbusmentAmount*-1 DisbusmentAmount,HeadHeadFilter='NetPay',EA.AdvanceId,EA.AdvanceDetailId,ESA.EmployeeSalaryAdvanceId,WA.EmployeeAdvanceDetailId,shgl.CrDirectActivityId ActivityId,0 IsOrderSpecific
                        from  dbo.SalaryProcMaster spm 
						left join dbo.SalaryProcChild spc on spc.SlrProcMstSystemID=spm.SystemID 
						LEFT JOIN SalaryLock AS sl ON sl.EmpSystemId=spc.EmpInfoSystemID AND sl.YearNo=spm.YearNo AND sl.MonthNo=spm.MonthNo
                        LEFT JOIN dbo.SalaryProcessLogDetail SPL ON SPL.SalaryProcessId=SPM.SystemID AND spl.EmpSystemId=spc.EmpInfoSystemID
						...
						LEFT JOIN (SELECT EAD.EmployeeId,ARS.MonthNo,ARS.YearNo,ARS.InstallmentAmount,ARS.PrincipalAmount,ARS.ProfitAmount,ARS.EmployeeAdvanceDetailId
											FROM  DBO.AdvanceReqSchedule ARS
											JOIN TRN.EmployeeAdvanceDeduction EAD ON EAD.AdvanceReqScheduleId=ARS.Id
											JOIN [TRN].[EmployeeAdvanceDetail] WAD ON WAD.Id=ARS.EmployeeAdvanceDetailId
											WHERE ARS.MonthNo='" + monthNo + "' AND ARS.YearNo='" + yearNo + @"' ) WA ON WA.EmployeeId=SL.EmpSystemId
                        WHERE sl.MonthNo='" + monthNo + "' and sl.YearNo='" + yearNo + @"' AND MPB.EntityId='" + entityId + @"' AND sl.Islocked=1 AND sl.PayableVoucherId IS NULL 
                        AND ISNULL(sh.HeadCategory,'')  in ('Advance') and spc.DisbusmentAmount!=0 " + wcEmpStatus + @" AND SPL.PlantId='" + plantId + @"' and DMC.plantId='" + plantId + @"'
                        AND  ei.SystemId not in (select EmpSystemId from [dbo].[ExceptionEmployee] where month(effectivedate)<=sl.MonthNo and year(effectivedate)=sl.YearNo)
        ...
        public List<Dictionary<string, object>> GetInDirectSalaryLockSalarySheetData(string yearNo, string monthNo, bool isActive, bool isSeperated, bool isMaternity, string plantId, string entityId)
        {
            string wcEmpStatus = " AND spm.SalaryProcFlag=''";
            ...
            SELECT x.* FROM (
						...
						,CASE WHEN ISNULL(ESA.InstallmentAmount,0)>0 THEN ISNULL(ESA.InstallmentAmount,0) 
						WHEN ISNULL(EA.InstallmentAmount,0)>0 THEN ISNULL(EA.InstallmentAmount,0) ELSE  ISNULL(WA.InstallmentAmount,0) END InstallmentAmount
                        ,spc.DisbusmentAmount*-1 DisbusmentAmount,HeadHeadFilter='NetPay',EA.AdvanceId,EA.AdvanceDetailId,ESA.EmployeeSalaryAdvanceId,WA.EmployeeAdvanceDetailId,shgl.CrInDirectActivityId ActivityId,0 IsOrderSpecific
                        from  dbo.SalaryProcMaster spm 
						left join dbo.SalaryProcChild spc on spc.SlrProcMstSystemID=spm.SystemID 
						LEFT JOIN SalaryLock AS sl ON sl.EmpSystemId=spc.EmpInfoSystemID AND sl.YearNo=spm.YearNo AND sl.MonthNo=spm.MonthNo
                        LEFT JOIN dbo.SalaryProcessLogDetail SPL ON SPL.SalaryProcessId=SPM.SystemID AND spl.EmpSystemId=spc.EmpInfoSystemID
        ...
        public GridModel GetGoodWorkPaymentAdviseDisbursementVoucherList(GridParameter parameters)
        {
            try
            {
                ...
                parameters.CmdText = @"SELECT V.Id PayableVoucherId, V.VoucherDate, V.PostingDate, V.DocRefNo, V.VoucherTypeId, V.CurrencyId, V.DocDate, V.EntityId, C.Code AS CurrencyCode
                                    , VD.DrAmount, V.VoucherNo,ISNULL(BM.AccountTitle,CM.UserName) PaymentBank
                                    ,V.IsPark,Status= case when V.IsPark=0 then 'Posted' else 'Parked' end, V.Narration
									,GWPA.PaymentAdviseId,GWPA.VoucherTypeName
                                    FROM TRN.[Voucher] AS V
                                    LEFT JOIN SCS.Currency AS C ON C.Id = V.CurrencyId
                                    LEFT JOIN (SELECT SUM(VD.DrAmount) AS DrAmount, VD.VoucherId,VD.BankMasterId FROM [TRN].[VoucherDetail] AS VD WHERE VD.DrAmount <> 0 
									GROUP BY VD.VoucherId,VD.BankMasterId
                                    ...
                                    ,CASE WHEN GWP.PaymentSource='Attendance' THEN 'ExtraOTDisbursement' ELSE 'GoodWorkDisbursement' END VoucherTypeName
									from dbo.GoodWorkPaymentAdviseDetail GWPD
									LEFT JOIN [dbo].[GoodWorkPaymentAdvise] AS GWP ON GWP.Id=GWPD.PaymentAdviseId
        ...
        private Dictionary<string, object> GetJournalHeader(string companyGroupId, string companyId, string plantId, string voucherId, SourceType sourceType)
        {
            var cmdText = @"SELECT VT.UserName AS VoucherTypeName, V.VoucherNo, REPLACE(CONVERT(VARCHAR(11), V.VoucherDate, 106), ' ', '-') AS VoucherDate, REPLACE(CONVERT(VARCHAR(11), V.PostingDate, 106), ' ', '-') AS PostingDate
                            ...
                            , V.CurrencyId, C.Code AS CurrencyCode,ET.UserName Entity
                            FROM  [TRN].[Voucher] AS V 
        ...
        private DataTable GetOtherGLJournalData(string companyGroupId, string companyId, string plantId, string voucherId)
        {
            var cmdText = @"SELECT V.Id, GL.Id AS AccountCodeId, GL.AccountCode, VDC.VoucherDetailId, FY.FiscalYearName, FYP.PeriodName, FYP.PeriodNo, V.IsPark, REPLACE(CONVERT(VARCHAR(11), V.PostingDate, 106), ' ', '-') AS PostingDate
                            , [Park/Post]=CASE WHEN V.IsPark=1 THEN 'Parked' ELSE 'Posted' END, REPLACE(CONVERT(VARCHAR(11), V.DocDate, 106), ' ', '-') AS DocDate, V.DocRefNo, REPLACE(CONVERT(VARCHAR(11), V.VoucherDate, 106), ' ', '-') AS VoucherDate
                            , V.VoucherNo, V.CurrencyId, CU1.Code AS TrnCurrency, V.AddedBy, V.PostedBy, VDC.ParallelCurrencyId, CU.Code AS CurrencyCode, VDC.FromCurrencyId, VDC.ToCurrencyId, VDC.ToCurrencyRate
                            , VD.DrAmount+VD.CrAmount AS Value,VD.DrAmount,VD.CrAmount, VDC.DrAmount AS CompanyCurrencyDrAmount, VDC.CrAmount AS CompanyCurrencyCrAmount, [DRCR]=CASE WHEN VDC.DrAmount>0 THEN '1' ELSE '2' END, VD.GLGeneralInfoId
,GL=Case when VD.DrAmount>0 and shg.DrDirectOtherGL<>'' then shg.DrDirectOtherGL 
									 when VD.DrAmount>0 and shg.DrInDirectOtherGL<>'' then shg.DrInDirectOtherGL 
									 when VD.CrAmount>0 and shg.CrDirectOtherGL<>'' then shg.CrDirectOtherGL 
									 when VD.CrAmount>0 and shg.CrInDirectOtherGL<>'' then shg.CrInDirectOtherGL 
									 WHEN VD.SalaryHeadId IS NULL THEN ACT.UserName
									 else '' end
							,GLGeneralInfoCode=Case when VD.DrAmount>0 and shg.DrDirectOtherGLCode<>'' then shg.DrDirectOtherGLCode 
									 when VD.DrAmount>0 and shg.DrInDirectOtherGLCode<>'' then shg.DrInDirectOtherGLCode 
									 when VD.CrAmount>0 and shg.CrDirectOtherGLCode<>'' then shg.CrDirectOtherGLCode 
									 when VD.CrAmount>0 and shg.CrInDirectOtherGLCode<>'' then shg.CrInDirectOtherGLCode 
									 when ISNULL(VD.SalaryHeadId,NULL) IS NULL  then GLA.AccountCode  end
                            , REPLACE(CONVERT(VARCHAR(11), VD.DocDate, 106), ' ', '-') AS InvoiceDate, VD.DocRefNo AS InvoiceNo, UPPER(VD.Narration) AS DetailNarration, ENT.UserName AS Entity
                            , VD.Id AS BudgetMasterId, BUD.UserName AS BudgetName, ACT.UserName AS Activity, UPPER(V.Narration) AS Narration, P.UserName AS PartyName, PP.UserName AS PartyLocation,VD.PartyType, VD.FAType,VD.FixedAssetMasterId
							,[ParticularName]=CASE
								WHEN BM.AccountTitle<>'' THEN BM.AccountTitle
                            ...
                            INNER JOIN [TRN].[VoucherDetail] AS VD ON VD.Id =VDC.VoucherDetailId
                            INNER JOIN [TRN].[Voucher] AS V ON V.Id=VD.VoucherId
                            LEFT JOIN mst.SalaryHeadGL shg on shg.SalaryHeadId=vd.SalaryHeadId and shg.AccountsGroupId=vd.AccountsGroupId
                            LEFT JOIN [HKP].[GLGeneralInfo] AS GL ON GL.Id=VD.GLGeneralInfoId
                            LEFT JOIN [HKP].[GLGeneralInfo] AS GLA ON GLA.Id=VD.GLGeneralInfoId
                            LEFT JOIN [SCS].[Currency] AS CU ON CU.Id=VDC.ParallelCurrencyId
                            LEFT JOIN [SCS].[Currency] AS CU1 ON CU1.Id=V.CurrencyId
                            LEFT JOIN [SCS].[FiscalYear] AS FY ON FY.Id=V.FiscalYearId
                            LEFT JOIN [SCS].[FiscalYearPeriod] AS FYP ON FYP.Id=V.FiscalYearPeriodId
        ...
        public IWorkbook GetSalaryPayableVoucherOtherGLReport(out string reportFileName, string companyGroupId, string companyId, string plantId, string plantName, string voucherId)
        {
            ...
            var dsLocal = GetOtherGLJournalData(companyGroupId, companyId, plantId, voucherId);
            ...
            reportUtility.SetText(ref sheet, row, 5, header["Entity"].ToString(), ExcelHAlign.HAlignLeft);
            ...
            if (dsLocal.Rows.Count > 0)
            {
                ...
                {
                    var glName = dsLocal.Rows[i]["GL"].ToString();


                    reportUtility.SetText(ref sheet, row, colGl, dsLocal.Rows[i]["GLGeneralInfoCode"] + " - " + glName);
        ...
        public IWorkbook GetGoodWorkExtraOTDisbursementVoucherReport(out string reportFileName, string companyGroupId, string companyId, string plantId, string plantName, string voucherId, string voucherTypeName)
        {
            AccountsCommonService accountsCommonService = new AccountsCommonService(_sqlRepository);
            ...
            if (dsLocal.Rows.Count > 0)
            {
                ...
                reportUtility.CompanyPlantHeader(ref sheet, colLast, voucherTypeName, companyId, plantName, null);
                reportUtility.PageSetup(ref sheet, colLast, ExcelPageOrientation.Portrait);
            }
            else
            {
                sheet.UsedRange.WrapText = true;
                sheet.UsedRange.CellStyle.Font.Size = 8;
                reportUtility.CompanyPlantHeader(ref sheet, 5, voucherTypeName, companyId, plantName, null);
                reportUtility.PageSetup(ref sheet, 5, ExcelPageOrientation.Portrait);
            }
            return workbook;
        }
