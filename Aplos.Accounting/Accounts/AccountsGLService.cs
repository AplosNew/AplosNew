...
namespace Library.Accounting.Accounts
{
    public class AccountsGLService
    {
        ...
        public GridModel GetExpensesGLBudgetActivityCOAWise(GridParameter parameters, string coaId)
        {
            try
            {
                parameters.CmdText = @"SELECT DISTINCT C.Id AS COAId, AG.UserName AS AccountGroupName, C.UserName AS COAName, GLGI.UserName AS GLGeneralInfoName
                                        , GLGI.AccountCode AS GLGeneralInfoCode, GLGI.Id AS GLGeneralInfoId, BU.BudgetId, BU.BudgetName, BU.RefNo,
                                        A.ActivityId, A.ActivityName, BU.BudgetMasterId FROM HKP.GLGeneralInfo AS GLGI
                                        JOIN HKP.COA AS C ON C.Id=GLGI.COAId
                                        ...
                                        LEFT OUTER JOIN HKP.AccountType AS ACT ON ACT.Id = AG.AccountTypeId
                                        LEFT OUTER JOIN (
