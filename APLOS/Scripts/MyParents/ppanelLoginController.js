...
'use strict';
ppanelLoginController.$inject = ['$scope', '$rootScope', '$routeParams', '$http', '$filter', '$window', '$cookies','cboService'];
function ppanelLoginController($scope, $rootScope, $routeParams, $http, $filter, $window, $cookies, cboService) {
            ...
            $http({
                ...
                if (response.data.result.Error === true || response.data.result.Status === 'Fail') {
                    ...
                    if (response.data.result.Status === 'Success') {
                        $cookies.put('MyParentspanel', 'ppanel');
                        $cookies.put("MyParentsemployeeId", response.data.result.EmployeeId);
                        $cookies.put("MyParentsgroupId", response.data.result.CompanyGroupId);
                        $cookies.put("MyParentscompanyId", response.data.result.CompanyId);
                        $cookies.put("MyParentsplantId", response.data.result.PlantId);
