...
function ShowResult(msg, status, position, dataUrl) {
    var html = '';
    if (status == "success") { html += '<div style="z-index: 10000000000000000000000000000000000000000 !important;" class="alert alert-success fade alert-site alert-site-ok" data-dismiss="alert">'; }
    else html += '<div class="alert alert-danger fade alert-site" data-dismiss="alert" style="margin-bottom: -5px;z-index: 10000000000000000000000000000000000000000 !important;">';
    html += '' + msg + '</div>';
    $("div.alert").remove();
    if (undefined !== position) {
        var id = $("#" + position);
        $("#" + position + " .modal-header").after(html);
        $('.alert-site').css({ 'width': '100%' });
    }
    else {
        $("#main").after(html);
...
function CloseModalShowResult(modalId) {
    $('#' + modalId + ' .alert-site').fadeOut();
}
...
function btnDisabled() {
    ...
    for (var t = 0; t < buttons.length; t++) {
        if (!buttons[t].getAttribute('ng-disabled') && buttons[t].getAttribute('id') !== 'btnConfirm')
            buttons[t].setAttribute('disabled', 'disabled');
    }
}
function btnEnabled() {
    var buttons = document.getElementsByTagName('button');
    for (var t = 0; t < buttons.length; t++) {
        if (!buttons[t].getAttribute('ng-disabled') && buttons[t].getAttribute('id') !== 'btnConfirm')
            buttons[t].removeAttribute('disabled');
    }
}