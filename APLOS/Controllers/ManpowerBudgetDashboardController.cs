...
namespace Aplos.Controllers
{
    public class ManpowerBudgetDashboardController : BaseController
    {
        ...
        {
            return Json(_hrDashboardService.GroupWiseCompanyList(date, status, EmplyeeTypeOrCategoryId), JsonRequestBehavior.AllowGet);
        }

        [HttpPost, Authorize]
        public ActionResult GetDrillDownListJSON()
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            return Json(_hrDashboardService.DrillDownList(identity.CompanyGroupId), JsonRequestBehavior.AllowGet);
        }
        ...
        {
            return Json(_hrDashboardService.DetailDrillDownTable(ChartColumnList, seq, date, status, EmplyeeTypeOrCategoryId), JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult ModalEmployeeSummary(IEnumerable<ChartColumnList> ChartColumnList, int seq, string status, string EmplyeeTypeOrCategoryId)
        {
            ...
            return jsondata;
            //var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
           // return Json(_hrDashboardService.ModalGroupWiseEmlpoyeeList(identity.CompanyGroupId, ChartColumnList, seq, status, EmplyeeTypeOrCategoryId, parameters), JsonRequestBehavior.AllowGet);
        }
