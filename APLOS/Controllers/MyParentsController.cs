...
{
	public class MyParentsController : BaseController
	{
		...
		[HttpGet, Authorize]
		public ActionResult PLayout()
		{
#if DEBUG
			ViewBag.BasePath = "/ppanel";
#else
            var appName = IISManager.GetApplicationName("APP_NAME");
            if (string.IsNullOrEmpty(appName))
                ViewBag.BasePath = "/tpanel";
            else
                ViewBag.BasePath = "/" + appName + "/ppanel";
#endif
			try
			{
		...
		public ActionResult Login(string timezoneoffset, string employeeId, string password, string remember)
		{
			...
			var _result = _empAuthService.ParentsLogin(employeeId, password);
			...
			Dictionary<string, object> empProfile = new Dictionary<string, object>();
			empProfile = _sqlRepository.GetDataCollection("select * from ParentsInformation where id='" + employeeId + "'")[0];
