...
namespace Aplos.Controllers
{
	public class MyTeacherController : BaseController
	{
		...
		[HttpGet, Authorize]
		public ActionResult TLayout()
		{
#if DEBUG
			ViewBag.BasePath = "/tpanel";
#else
            var appName = IISManager.GetApplicationName("APP_NAME");
            if (string.IsNullOrEmpty(appName))
                ViewBag.BasePath = "/tpanel";
            else
                ViewBag.BasePath = "/" + appName + "/tpanel";
#endif
			try
			{
				...
				empProfile = _sqlRepository.GetDataCollection("select * from employeeinformation where systemid='" + ViewBag.EmployeeId + "'")[0];
