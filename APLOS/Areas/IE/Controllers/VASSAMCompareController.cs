...
{
    public class VASSAMCompareController : BaseController
    {
        #region Constructor
        ...
        public VASSAMCompareController(
        ...
        #region Forhad Code
        [HttpPost, Authorize]
        public JsonResult GetAllData(string FromDate, string ToDate)
        {
            string sql = "";
            sql = @"     SELECT 
                 convert(bit,0) AS Checked,   M.VasDescription,M.OperationVariationSystemId, M.Id,V.Code AS OperationCode,V.StandardName,
                   ...
                   ,UPPER(APPR.ApprovedBy)ApprovedBy,convert(datetime, APPR.ApprovedDate)ApprovedDate,
                    ...
                    ORDER BY M.AddedDate,v.Code,M.Version";
        ...
        private IWorkbook VASReportSheet(string Id)
        {
            ...
            //sheet1[3, 5, 3, 10].Text = "SPT Compare Analysis Data";
            //sheet1[3, 5, 3, 10].CellStyle.VerticalAlignment = ExcelVAlign.VAlignCenter;
            //sheet1[3, 5, 3, 10].CellStyle.HorizontalAlignment = ExcelHAlign.HAlignCenter;
            //sheet1[3, 5, 3, 10].CellStyle = HFontstyle;
            ...
            //sheet1[ROW, 1].Text = "SPT Compare Analysis Data";
            //sheet1[ROW, 1].CellStyle = HFontstyle;
            //sheet1[ROW, 1].RowHeight = 18;
            ...
            int ColVASSAM = COL;
            COL++;
            sheet1[ROW, COL].Text = "Opr. Vs VAS SPT";
            sheet1[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
            sheet1[ROW, COL].ColumnWidth = 7;
            int ColOperationVsVASSAM = COL;
            COL++;
            ...
            COL++;
            sheet1[ROW, COL].Text = "Opr. Vs Std. SPT";
            sheet1[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
            sheet1[ROW, COL].ColumnWidth = 7;
            int ColOperationVsStandardSAM = COL;
            COL++;
            ...
            string sql = @"    SELECT 
                    M.VasDescription,M.OperationVariationSystemId, M.Id,V.Code AS OperationCode,V.StandardName,V.UserName AS OperationVariation,
                    M.SPI,M.RPM,M.MachineAllowances,M.PersonalAllowances,CASE WHEN M.IsApproved = 1 THEN 'Approved' ELSE '' END Status
                    ,UPPER(APPR.ApprovedBy)ApprovedBy,convert(datetime, APPR.ApprovedDate)ApprovedDate,M.Remarks,
                    UPPER(M.AddedBy)AddedBy,convert(datetime, M.AddedDate)AddedDate,M.OperatorId,
                    mma.ShortName AS Machine,mmax.ShortName AS MachineActual,M.VASQuantity,sc.UserName AS StitchCode,  
                     v.TotalSAM AS OperationSAM,M.VASSAM,M.StandardSAM,M.Version,M.AdditionalAllowances,
                    ps.UserName AS ProductionSystem,M.FactorValue AS ProductionSystemAllowance,M.OriginalVideoName,  
                    CASE WHEN M.IsApproved = 1 THEN 'Approved' Else CAST('' As VARCHAR(50)) END ApproveStatus,
                    O.UserName as Operation
					FROM [MST].[VASMaster] M
                    INNER JOIN [MST].[OperationVariation] V ON V.Id = M.OperationVariationSystemId
                    LEFT JOIN mst.MaterialMasterArticle AS mma ON mma.Id=v.ArticleId
				    ...
				    AND APPR.Id=(SELECT TOP 1 Id FROM mst.VASMaster AS XV WHERE XV.OperationVariationSystemId = m.OperationVariationSystemId AND XV.IsApproved=1)
                        ...
                        WHERE M.Id IN (" + inQuery + ")  AND isnull(M.Archive,0)=0";
            ...
            foreach (string val in reportId)
            {
                ...
                dt = dtLocal.DefaultView.ToTable();
                ...
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    sheet1[ROW, ColOperationVsVASSAM].Formula = clsStaticInfo.GetxlsCol(ColOperationSAM) + ROW.ToString() + "-" + clsStaticInfo.GetxlsCol(ColVASSAM) + ROW.ToString();
                    sheet1[ROW, ColOperationVsStandardSAM].Formula = clsStaticInfo.GetxlsCol(ColOperationSAM) + ROW.ToString() + "-" + clsStaticInfo.GetxlsCol(ColStandardSAM) + ROW.ToString();
            ...
            reportUtility.CompanyGroupHeader(ref sheet1, 1, "SPT Analysis Data", identity.CompanyGroupId);
