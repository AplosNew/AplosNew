...
{
    public class BartackCodeController : Controller
    {
        #region Constructor
        ...
        {
            string sql = "";
            sql = @"SELECT Id,Code,ShortName,StandardName,UserName,Description,TMU,MCHand,CycleTime,Frequency,Activity,Element
                    FROM HKP.ElementCode WHERE CodeType='Bartack' ORDER BY Code";
        ...
        public JsonResult GetSelectedData(int Id)
        {
            string sql = "";
            sql = @"SELECT Id,Code,ShortName,StandardName,UserName,Description,TMU,MCHand,CycleTime,Frequency,Activity,Element
                    FROM HKP.ElementCode WHERE CodeType='Bartack' AND Id='" + Id.ToString() + "'";
        ...
        public ActionResult SaveData(Dictionary<string, object> elementType)
        {
            ...
            try
            {
                ...
                string _Message = "";
                int GSDID = 0;
                con.getDataSet("SELECT * FROM [HKP].[ElementCode] WHERE CodeType='Bartack' AND Id='" + elementType["Id"].ToString() + "'", out DataSet dsElementType);
                if (dsElementType.Tables[0].Rows.Count == 0)
                {
                    ...
                    dr["CycleTime"] = elementType["CycleTime"].ToString();
                    dr["Frequency"] = elementType["Frequency"].ToString();
        ...
        public ActionResult DeleteSelectedData(int Id)
        {
            try
            {
                ...
                con.executeQuery("DELETE FROM [HKP].[ElementCode] WHERE CodeType='Bartack' AND Id='" + Id.ToString() + "'");
