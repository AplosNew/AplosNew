...
Where  MA.Active=1";
        ...
        #region MachineGroup
        [HttpPost]
        public ActionResult GetMGList(string column, string value)
        {
            string strkey = "1=1";
            if (string.IsNullOrEmpty(column) == false && string.IsNullOrEmpty(value) == false)
                strkey = column + " like '%" + value + "%'";
            ...
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            string sql = @"select top 100 * from (SELECT * FROM HKP.MachineGroup) AS TEMP WHERE " + strkey + " order by sequence";
        ...
        [HttpGet, Authorize]
        public JsonResult GetMGAutoSequence()
        {
            return Json(GetMGSequence(), JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public JsonResult CreateMG(Dictionary<string, object> data)
        {
            try
            {
                DataSet dsMaster;
                ConnectionManager.DAL.ConManager con = new ConnectionManager.DAL.ConManager("1");
                con.OpenDataSetThroughAdapter("select * from HKP.MachineGroup where Code='" + data["Code"] + "' AND  Id<>'" + data["Id"] + "'", out dsMaster, false, "1");
                if (dsMaster.Tables[0].Rows.Count > 0)
                    throw new Exception("Same Code already exists!!!");

                con.OpenDataSetThroughAdapter("select * from HKP.MachineGroup where UserName='" + data["UserName"] + "' AND  Id<>'" + data["Id"] + "'", out dsMaster, false, "1");
                if (dsMaster.Tables[0].Rows.Count > 0)
                    throw new Exception("Same User Name already exists!!!");


                con.OpenDataSetThroughAdapter("select * from HKP.MachineGroup where Id='" + data["Id"] + "'", out dsMaster, false, "1");
                ...
                {
                    bplib.clsGenID genid = new bplib.clsGenID();
                    genid.GenID("MachineGroup", out _Id);
                ...
                return Json(new { Error = false, Data = data, Sequence = GetMGSequence(), Message = AplosMessage.Insert });
        ...
        public ActionResult DeleteMG(string id)
        {
            ...
            try
            {
                ...
                ConnectionManager.clsConnection con = new ConnectionManager.clsConnection();
                con.BeginTransaction();
                con.executeQuery("delete from HKP.MachineGroup where id='" + id + "'");
                con.CommitTransaction();

                return Json(new { Error = false, Sequence = GetMGSequence(), Message = AplosMessage.Deleted }, JsonRequestBehavior.AllowGet);
        ...
        private double GetMGSequence()
        {
            DataTable dt = _sqlRepository.GetDataTable("SELECT  isnull(Max(Sequence),0) AS Sequence FROM HKP.MachineGroup");
            if (dt.Rows.Count > 0)
