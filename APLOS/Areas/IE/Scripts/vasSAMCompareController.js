'use strict';
VASSAMCompareController.$inject = ["cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$routeParams", "$location", "$http", "$filter", "$sce"];
function VASSAMCompareController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, $sce) {
    $rootScope.title = "VAS SAM Compare";
    $scope.Action = 'Save';
    $scope.path = 'IE/VASSAMCompare/';
    ...
    function CheckBoxSelectAllOperation(e) {
        ...
        if (angular.isUndefinedOrNull(filtered) || filtered.length == 0) {
            for (var i = 0; i < $scope.samCompareDataList.length; i++) {
                $scope.samCompareDataList[i].Checked = ChkOrUnchk;
            }
        }
        else {
    ...
    //            }
    //            $scope.samCompareDataList = response.data;
    //        });
    //    } catch (e) {
    ...
    $scope.onClickVASReport = function () {
        ...
        if (Griddata.length == 0)
            Griddata = $scope.samCompareDataList;
