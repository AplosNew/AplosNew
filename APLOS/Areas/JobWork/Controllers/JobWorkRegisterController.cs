...
namespace Aplos.Areas.JobWork.Controllers
{
    public class JobWorkRegisterController : BaseController
    {
        ...
        [Authorize, HttpGet]
        public JsonResult LoadAllPOForSelection(string JWPOPartyId, string POType)
        {
            try
            {

                return Json(JWR.LoadAllPOForSelection(JWPOPartyId, POType), JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        private IWorkbook GetTRANSFORMATIONData(string FromDate, string ToDate, string PartyVendorId, string ContractId)
        {
            ...
            report.SetHeaderText(ref sheet, ROW, COL, "SKU1", 15, ExcelHAlign.HAlignLeft);
            int ColFirstCharacteristicsValue = COL;
            COL++;

            report.SetHeaderText(ref sheet, ROW, COL, "SKU2", 15, ExcelHAlign.HAlignLeft);
            int ColSecondCharacteristicsValue = COL;
            COL++;

            report.SetHeaderText(ref sheet, ROW, COL, "SKU3", 15, ExcelHAlign.HAlignLeft);
        ...
        // JW Value Added Register

        [HttpPost]
        public ActionResult GetValueAddedRegisterReport(string FromDate, string ToDate, string PartyVendorId, string ContractId)
        {
            try
            {
                var workbook = GetValueAddedData(FromDate, ToDate, PartyVendorId, ContractId);
        ...
        private IWorkbook GetValueAddedData(string FromDate, string ToDate, string PartyVendorId, string ContractId)
        {
            ...
            DataTable data = JWR.GetValAddedRegisterReportData(FromDate, ToDate, PartyVendorId, ContractId);
       //     DataTable dataChild = JWR.GetTransRegisterByProductReportData(FromDate, ToDate, PartyVendorId, ContractId);
            ...
            report.SetHeaderText(ref sheet, ROW, COL, "No Of Order Wise Item", 15, ExcelHAlign.HAlignLeft);
            int ColTotalNoOfInputItem = COL;
            COL++;
            ...
            report.SetHeaderText(ref sheet, ROW, COL, "JW Order Wise Plannned Quantity", 15, ExcelHAlign.HAlignLeft);
            int ColJWInputPlannnedQuantity = COL;
            COL++;

            report.SetHeaderText(ref sheet, ROW, COL, "JW Order Wise Issue Quantity", 12, ExcelHAlign.HAlignLeft);
            int ColJWJWInputIssueReturnQuantity = COL;
            COL++;

            report.SetHeaderText(ref sheet, ROW, COL, "JW Order Wise Balance Quantity", 15, ExcelHAlign.HAlignLeft);
            int ColJWInputBalQty = COL;
            COL++;
       ...
       //         sheet[ROW, ColTotalGrossConsumption].Text = data.Rows[i]["TotalGrossConsumption"].ToString();
            ...
            report.CompanyPlantHeader(ref sheet, endCol, "Job Work Value Added Register", identity.CompanyId, identity.PlantName, null);
            report.PageSetup(ref sheet, 5, ExcelPageOrientation.Landscape);
            return workbook;
        }
