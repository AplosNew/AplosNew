...
{
	public class JWReceiptController : BaseController
	{
		JWReceiptService R = new JWReceiptService();
		...
		public JWReceiptController(SqlRepository Repository)
		{
			_sqlRepository = Repository;
			R = new JWReceiptService();
		}
		#endregion
		#region Pages
...
LEFT JOIN (SELECT A.JWTransformationPOId, SUM(A.Quantity) AS TransactionQty, SUM(A.Quantity * A.RatePerUnit) AS TransactionAmount, SUM(A.BaseAmount) AS BaseAmount FROM dbo.JWTransformationPODetail AS A
                        ...
                        left join (Select SUM(IRD.TransactionQty) as ReceivedQty, SUM(IRD.TotalMaterialBooksCurrencyAmount) as TotalAmt,IRD.JWTransformationPOId
						from TRN.InventoryReceive IR left join TRN.InventoryReceiveDetail IRD on IR.Id=IRD.InventoryReceiveId
						group by IRD.JWTransformationPOId) TT on TT.JWTransformationPOId=IR.Id
