'use strict';
JWIssueReturnController.$inject = ['$window', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function JWIssueReturnController($window, cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
	...
	$scope.path = 'JobWork/JWIssueReturn/';
	...
	$scope.SelectedValAddedMaterialStorage = function () {
		if ($scope.ModelNew.OrderSpecific == "Yes") {
			...
			$http({
				method: 'GET',
				url: 'JobWork/JWIssueReturn/gejobworklocation?TId=' + $scope.ModelNew.Id,
			}).then(function successCallback(response) {
				$scope.ValAddedJobWorkLocList = response.data;
				if ($scope.ValAddedJobWorkLocList.length > 0) {
					$scope.Issue.MaterialStorageId = $scope.ValAddedJobWorkLocList[0].Value;
	...
	$scope.SelectedValAddedEntity = function () {
		$http({
			method: 'GET',
			url: 'JobWork/JWIssueReturn/getentitylist/',
		}).then(function successCallback(response) {
...
//		$scope.GRN = 6;
	...
	$scope.DownloadReport = function (data) {
		try {
			...
			if (TabType == "Value Added") {
				...
				window.open('JobWork/JWIssueReturn/GetValueAddedPrintReport?reportFormat=' + reportFormat + '&PrintTabId=' + $scope.PrintTabId + '&IssueId=' + $scope.IssueId, '_blank');
				$scope.getData();
			}
	...
	$scope.SelectedTConMaterialStorage = function () {
		if ($scope.Transformation.OrderSpecific == "Yes") {
			$http({
				method: 'GET',
				url: 'JobWork/JWIssueReturn/getalljobworklocation/',
			}).then(function successCallback(response) {
			...
			$http({
				method: 'GET',
				url: 'JobWork/JWIssueReturn/gejobworklocation?TId=' + $scope.Transformation.Id,
			}).then(function successCallback(response) {
				$scope.JobWorkLocList = response.data;
				if ($scope.JobWorkLocList.length > 0) {
					$scope.IssueTransformation.MaterialStorageId = $scope.JobWorkLocList[0].Value;
	...
	$scope.GetSelectedMaterialStorage = function () {
		if ($scope.ModelNew.TabType == "Transformation") {
			$http({
				method: 'GET',
				url: 'JobWork/JWIssueReturn/getStoragloc?JLId=' + $scope.IssueTransformation.MaterialStorageId,
			}).then(function successCallback(response) {
			...
			if ($scope.ModelNew.OrderSpecific == "Yes") {
				$http({
					method: 'GET',
					url: 'JobWork/JWIssueReturn/getStoragloc?JLId=' + $scope.Issue.MaterialStorageId,
				}).then(function successCallback(response) {
					$scope.SelectedMaterialStorage = response.data;
					if ($scope.SelectedMaterialStorage.length > 0) {
	...
	$scope.GetLotNoRate = function (RowData) {
		...
		$http({
			method: 'GET',
			url: 'JobWork/JWIssueReturn/GetLotNoRate?LotNumber=' + $scope.LotNum,
		}).then(function successCallback(response) {
	...
	$scope.CostCenterLoad = function () {
		...
		$http({
			method: "GET",
			url: 'JobWork/JWIssueReturn/GetCostCenterLoadNewFun?EntityId=' + $scope.ModelNew.EntityId
		}).then(function successCallback(response) {
  ...
  //      }
	...
	function ValAddedMaterialStorageForEdit(IssueId, MaterialStorageId) {
		...
		if ($scope.ModelNew.TabType == "Transformation") {
			$http({
				method: 'GET',
				url: 'JobWork/JWIssueReturn/ValAddedMaterialStorageForEdit?IssueId=' + IssueId + '&MaterialStorageIdInventory=' + MaterialStorageId,
			}).then(function successCallback(response) {
				$scope.JobWorkLocList = response.data;
				if ($scope.JobWorkLocList.length > 0) {
					$scope.IssueTransformation.MaterialStorageId = $scope.JobWorkLocList[0].Value;
