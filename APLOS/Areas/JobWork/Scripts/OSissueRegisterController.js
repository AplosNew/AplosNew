...
function OSissueRegisterController(fileReader, commonMessage, $scope, $rootScope, baseService, $http, $filter, cboService, $window, $controller) {
	...
	$scope.index = -1;
	$scope.products = [];
	$scope.path = 'JobWork/OSissueRegister/';
	...
	$scope.getaldataMaterialLedger = function () {
		...
		$http({
			...
			url: 'JobWork/OSissueRegister/GetMaterialLedger',
			data: {
	...
	$scope.GetIssueRegister = function () {
		...
		$http({
			method: 'POST',
			//url: $scope.getSearchListUrl,
			url: 'JobWork/OSissueRegister/GetOSIssueRegister',
			data: {
				fromDate: $scope.report.FromDate,
				toDate: $scope.report.ToDate,
				Type: $scope.productNew.Type
	...
	$scope.GetIssueRegisterListByGRN = function () {
		...
		$http({
			method: 'POST',
			//url: $scope.getSearchListUrl,
			url: 'JobWork/OSissueRegister/GetOSIssueRegisterBYGRN',
			data: {
				fromDate: $scope.report.FromDate,
				toDate: $scope.report.ToDate,
				Type: $scope.productNew.Type
	...
	$scope.IssueRegisterReportPdf = function (id, reportFormat) {
		...
		$window.open('JobWork/OSissueRegister/Report?reportFormat=' + reportFormat + '&fromDate=' + $scope.report.FromDate + '&toDate=' + $scope.report.ToDate + '&Type=' + $scope.productNew.Type, '_blank');
		//};
	...
	$scope.IssueRegisterReportExcel = function (reportFormat) {
		...
		try {
			var Excel;
			var file_src = 'JobWork/OSissueRegister/Report?reportFormat=' + reportFormat + '&fromDate=' + $scope.report.FromDate + '&toDate=' + $scope.report.ToDate + '&Type=' + $scope.productNew.Type;
	...
	$scope.GRNIssueRegisterReportPdf = function (id, reportFormat) {
		...
		var reportFormat = "Pdf";
		//if (baseService.isUndefinedOrNull(id)) return ShowResult('No Id found', 'failure');
		$window.open('JobWork/OSissueRegister/CreateOSIssueRegisterGRNIssueReport?reportFormat=' + reportFormat + '&fromDate=' + $scope.report.FromDate + '&toDate=' + $scope.report.ToDate + '&Type=' + $scope.productNew.Type, '_blank');
	};
	...
	$scope.GRNIssueRegisterReportExcel = function (reportFormat) {
		...
		try {
			var Excel;
			var file_src = 'JobWork/OSissueRegister/CreateOSIssueRegisterGRNIssueReport?reportFormat=' + reportFormat + '&fromDate=' + $scope.report.FromDate + '&toDate=' + $scope.report.ToDate + '&Type=' + $scope.productNew.Type;
			$rootScope.report(file_src);
