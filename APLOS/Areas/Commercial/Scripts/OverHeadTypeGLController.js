"use strict";
OverHeadTypeGLController.$inject = ["cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$http"];
function OverHeadTypeGLController(cboService, commonMessage, $scope, $rootScope, baseService, $http) {
    $rootScope.title = "OverHead Type GL Sales";
    $scope.hadding = "OverHead Type GL Sales";
    $scope.Action = "Save";
    ...
    $scope.showAll = function () {
        ...
        if (!baseService.isUndefinedOrNull($scope.ChargesTypeGL.COAId)) {
                $http({
                    method: "GET",
                    url: 'Commercial/OverHeadTypeGL/GetList?coaId=' + $scope.ChargesTypeGL.COAId,
                    dataType: "JSON"
    ...
    $scope.Save = function () {
        if ($scope.investmentTypeGivenGLWithCombineList.length > 0) {
            for (var i = 0; i < $scope.investmentTypeGivenGLWithCombineList.length; i++) {
                $scope.investmentTypeGivenGLWithCombineList[i].GLType = "Sale";
            }
        }
