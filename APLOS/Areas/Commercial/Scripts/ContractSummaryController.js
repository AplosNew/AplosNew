'use strict';
ContractSummaryController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', '$window', 'cboService', 'bankService', '$controller'];
function ContractSummaryController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, $window, cboService, bankService, $controller) {
    ...
    $scope.GetContractSummaryReport = function (obj) {
        var gridObj = $("#Grid2").data("ejGrid");
        var data = obj.data;
        $scope.modelNew.ContractNo = data.Id;

        try {
            var file_src = $scope.path + "GetContractSummaryReport?ContractId=" + $scope.modelNew.ContractNo;
