'use strict';
paySlipsContractorController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$window'];
function paySlipsContractorController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $window) {
    ...
    $scope.GetPaySlip = function () {
        try {
            ...
            $http({
                method: 'POST',
                url: 'payrolls/PaySlips/GetEmployeePaySlipContractor',
                data: {
                    'month': $scope.month,
                    'year': $scope.year,                   
