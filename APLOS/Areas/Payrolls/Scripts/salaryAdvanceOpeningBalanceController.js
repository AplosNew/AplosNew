'use strict';
salaryAdvanceOpeningBalanceController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function salaryAdvanceOpeningBalanceController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = "Salary Advance Opening Balance";
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.masterDataList = [];
    $scope.path = 'payrolls/salaryadvanceopeningbalance/';
    $scope.getListUrl = $scope.path + 'getlist';
    $scope.saveUrl = $scope.path + 'createopeningbalance';
    ...
    $scope.AddSalaryAdvance = function () {
        try {
            ...
            $scope.loanAdvanceNew.PaidAmount = 0;
            ...
            $scope.loanAdvanceNew.IsOpeningBalance = true;
    ...
    function setPartyName(ob) {
        try {
            if ($scope.fieldName === 'EmployeeInformation') {
                ...
                $scope.loanAdvanceNew.CurrencyRuleSystemID = ob.CurrencyRuleSystemID;

                $scope.imageSrc = virtualPath.EmployeePic + '/' + $scope.loanAdvance.EmpPicPath;

                if (baseService.isUndefinedOrNull($scope.loanAdvanceNew.CurrencyRuleSystemID)) {
    ...
    $scope.GetLoanMasterByEmployee = function (employeeId) {
        $http.get('payrolls/salaryadvanceopeningbalance/getopeningbalancebyemployee?employeeId=' + employeeId)
            .then(function (response) {
                ...
                if ($scope.masterDataList.length > 0) {
                    ...
                    //$scope.loanAdvanceNew.IsOpeningBalance = $scope.masterDataList[0].IsOpeningBalance;
    ...
    $scope.GetLoanChildByMaster = function (masterId) {
        $http.get('payrolls/salaryadvanceopeningbalance/getopeningbalancechildbymaster?masterId=' + masterId)
            .then(function (response) {
    ...
    $scope.Define = function () {
        ...
        PaidAmt = parseInt($scope.loanAdvanceNew.PaidAmount);
        BalanceAdvAmt = $scope.loanAdvanceNew.AdvanceAmount - PaidAmt;
        //BalanceAdvAmt = $scope.loanAdvanceNew.AdvanceAmount - $scope.loanAdvanceNew.PaidAmount;
    ...
    $scope.Save = function () {
        try {
            ...
            if ($scope.loanAdvanceNewForm2.$valid) {
                ...
                if (baseService.isUndefinedOrNull($scope.loanAdvanceMaster.EmpInfoSystemID)) {
                    $scope.loanAdvanceMaster.EmpInfoSystemID = $scope.loanAdvanceNew.EmployeeId;
                }
