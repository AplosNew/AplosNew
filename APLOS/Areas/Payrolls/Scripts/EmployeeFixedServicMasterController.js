'use strict';
EmployeeFixedServicMasterController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'cboService'];
function EmployeeFixedServicMasterController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService) {
    $rootScope.title = 'Employee Fixed Servic Master';
    $scope.Action = 'Save';
    $scope.path = 'Payrolls/EmployeeFixedServicMaster/';
    ...
    $scope.getListData = function () {
        $http.get($scope.path +'getSalaryHeadWiseAmountSettinglist')
            .then(
    ...
    $scope.Delete = function () {
        if (!baseService.isUndefinedOrNull($scope.salaryHeadWiseAmountSettingModel.Id)) {
            $http.get($scope.path +'Delete?Id=' + $scope.salaryHeadWiseAmountSettingModel.Id)
                .then(function successCallback(response) {
                    if (response.data.Error === true) {
                        ShowResult(response.data.Message, 'failure');
                    }
    ...
    function ValidationMaster() {
        try {
            ...
            CheckField("Servic Component", $scope.salaryHeadWiseAmountSettingModel.ServicComponent);         
