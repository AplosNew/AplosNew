'use strict';
PromotionIncrementApprovalController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function PromotionIncrementApprovalController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = 'Salary Structure Approval';
    $scope.index = -1;

    $scope.EmpSalaryInfoDefine = [];
    $scope.path = 'Payrolls/PromotionIncrementApproval/';
    ...
    $scope.LoadEmployeeForPromotion = function (url) {
        try {
            $http.get(url)
                .then(function successCallback(response) {
                    if (response.data.Error === true) {
                        ...
                        $scope.PromotionEmployees = null;
                        $scope.PromotionEmployees = response.data;
    ...
    $scope.LoadEmployeeForPromotion($scope.getPromotionEmpListUrl);
    $scope.commandPromotionApproval = [{
        type: "details",
        buttonOptions: {
            text: "Approve",
            width: "100",
            ...
            click: onClickPromotionApproval
        }
    }];
    function onClickPromotionApproval(arg) {
        $scope.ApprovalTitle = "Employee Promotion Approval";
        var eDialog = $("#dialogPromotionApproval").data("ejDialog");
        eDialog.open();

        var gridObj = $("#GridPromotion").data("ejGrid");
        ...
        try {
            ...
            //    $scope.budgetCodeChangeNew.IsExceptionalDesigApplicable = 1;
                ...
                $http({
                    ...
                    data: { 'EmpSystemId': $scope.budgetCodeChangeOld.SystemId },
                    dataType: 'JSON'
                }).then(function successCallback(response) {
                    if (response.data.Error === true) {
