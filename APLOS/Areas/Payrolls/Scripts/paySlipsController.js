'use strict';
paySlipsController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$window'];
function paySlipsController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $window) {
    ...
    $scope.GetPaySlip = function () {
        try {
            ...
            $http({
                method: 'POST',
                url: 'payrolls/PaySlips/GetEmployeePaySlip',
                data: {
                    'month': $scope.month,
                    'year': $scope.year,
