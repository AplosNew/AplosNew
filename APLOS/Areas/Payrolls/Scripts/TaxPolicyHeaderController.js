...
function TaxPolicyHeaderController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService) {
    ...
    $scope.DeleteIncomePolicySlab = function (obj) {
        $scope.DeleteIncomeSlab = $scope.Header.Id;
    ...
    $scope.DeletePolicySlab = function () {
        try {
            $scope.$broadcast('show-errors-check-validity');
            $http({
                method: 'POST',
    ...
    $scope.SaveIncomeSlab = function () {
        try {
            $http({
                ...
                data: { 'IncomeSlab': $scope.DataList, PolicyId: $scope.Header.Id },
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
