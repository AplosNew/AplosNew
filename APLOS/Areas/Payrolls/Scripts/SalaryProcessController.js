'use strict';
SalaryProcessController.$inject = ['addressService', 'fileReader', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', '$window'];
function SalaryProcessController(addressService, fileReader, cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, $window) {
    $rootScope.title = ' Salary Process ';
    $scope.Action = 'Save';
    $scope.path = 'payrolls/SalaryProcess/';//
    $scope.cbxMLVR = true;
    $scope.cbxZero = true;
    ...
    $scope.GetEmployee_list = function () {
        try {
            ...
            $http({
                method: "POST",
                dataType: 'JSON',
                url: 'payrolls/SalaryProcess/GetEmpList',
                data: parameters
