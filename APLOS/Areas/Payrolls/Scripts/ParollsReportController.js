'use strict';
ParollsReportController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', '$window', 'cboService'];
function ParollsReportController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, $window, cboService) {
    $rootScope.title = "Salary Process";
    $scope.Action = 'Delete';
    $scope.salaryProcesss = [];
    $scope.path = 'Payrolls/SalaryProces/';
    ...
    $scope.salaryProcessNew = Object.assign({}, $scope.salaryProcess);
    ...
    $scope.getSavedPayRollGroupData = function () {
        if (!baseService.isUndefinedOrNull($scope.salaryProcessNew.PayrollGroupId)) {
            $http.get("Payrolls/EmployeeAttendanceGroup/PayRollGroupQuery?payrollGroupId=" + $scope.salaryProcessNew.PayrollGroupId)
                .then(
                    function successCallback(response) {
                        $scope.salaryProcesss = response.data.Rows;
                    },
    ...
    $scope.popUp = function () {
        try {
            ...
            $scope.popUpUrl = 'employees/approvalconfiguration/GetEmployeeWithSalaryProcessData?MonthId=' + $scope.salaryProcessModel.MonthId + '&YearId=' + $scope.salaryProcessModel.YearId;
    ...
    $scope.popUp2 = function (name) {
        try {
            ...
            $scope.employeeParameters.searchBy = '';
            $scope.popUpTitle = 'Employee';
            $scope.popUpUrl = 'employees/approvalconfiguration/GetEmployeeDataWithIds?departmentIds=' + $scope.departmentIdstr + '&divisionIds=' + $scope.divisionIdstr + '&sectionIds=' + $scope.sectionIdstr + '&employeeCateogoryIds=' + $scope.employeeCategoryIdstr + '&givenDesignationIds=' + $scope.givenDesignationIdstr + '&employeeCode=' + $scope.salaryProcessNew.EmployeeCode + '&employeeName=' + $scope.salaryProcessNew.EmployeeName;
            $scope.employeeParameters.sort = 'EmployeeCode';
            $scope.employeeParameters.searchBy = 'EmployeeCode';
        ...
        angular.forEach($scope.employeeList, function (item) {
            if (item.Flag) {
                if (!checkExisting(item.SystemId)) {
                    var ob = {};
                    ob.Id = null;
                    ob.PayrollGroupId = $scope.salaryProcessNew.PayrollGroupId;
                    ob.CompanyGroupId = $scope.salaryProcessNew.CompanyGroupId;
                    ob.PlantId = $scope.salaryProcessNew.PlantId;
    ...
    function checkExisting(id) {
        for (var i = 0; i < $scope.salaryProcesss.length; i++) {
            var ob = $scope.salaryProcesss[i];
            if (ob.EmployeeId === id) {
                return true;               
            }
        }
    ...
    $scope.Save = function () {
        ...
        if ($scope.salaryProcessNewForm.$valid) {
            $http({
                method: 'POST',
                url: $scope.saveUrl,
                data: $scope.salaryProcesss,
                dataType: 'JSON'
    ...
    //Deleting Rows from RetentionAllowanceList
    $scope.SalaryProcessDeleteMessage = function () {
        ...
        try {
            ...
            $scope.message_confirmation = 'Are you sure want to parmenently delete Salary Process Data ?';

            angular.element(document.querySelector('#confirm_PopUp-Delete')).modal('show');
    ...
    $scope.removeRow = function () {
        if (baseService.isUndefinedOrNull($scope.tempEmpOb.Id) === true) {
            $scope.salaryProcesss.splice($scope.empIndex, 1);
            $scope.empIndex = -1;
    ...
    $scope.SalaryProcessDelete = function () {

        try {

            var empId = Array.prototype.map.call($scope.salaryProcesss, function (item) { return "'" + item.EmployeeId + "'"; }).join(",");

            $scope.Delete(empId, $scope.salaryProcessModel.MonthId, $scope.salaryProcessModel.YearId);
        } catch (e) {
            ShowResult(e, 'Error');
        }
    };
    ...
    //});
    $scope.Delete = function (id, month, year) {
        try {
            $http({
                method: 'POST',
                url: 'Payrolls/SalaryProcess/SalaryProcessDelete',
                dataType: 'JSON',
    ...
    $scope.removeFromDb = function (id, index) {
        try {
            $http({
                method: 'POST',
                url: 'Payrolls/EmployeeAttendanceGroup/Delete',
                dataType: 'JSON',
                data: { 'id': id }
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
    ...
    function ClearFields(seq) {
        $scope.salaryProcess = {};
        $scope.salaryProcessNew = {};
        $scope.salaryProcessHeadList = [];
        $scope.popUpList = [];
        $scope.valueData = [];
    }
}
