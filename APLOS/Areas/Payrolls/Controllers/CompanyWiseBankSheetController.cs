    ...
    public class CompanyWiseBankSheetController : BaseController
    {
        ...
        public string GetReport(string PaymentModeList, string BankList, string PlantList, string EntityList, string DepartmentList, string DesignationList, string SectionList, string SubSectionList, string Month, string Year, bool isActive, bool isSeperated, bool isMaternity, string MonthName)
        {
            ...
            try
            {
                ...
                StringCollection PayMentListx = new StringCollection();
                StringCollection BankListx = new StringCollection();
                StringCollection ForCashBankListx = new StringCollection();
                StringCollection ForBankListx = new StringCollection();
                ...
                foreach (var PList in PaymentListSeperated)
                {
                    if (PList.ToString().ToUpper().Trim() != "CASH" )
                    {
                        ...
                        foreach (var item in Bank)
                        {
                            if (item != "''" || item != "")
                            {
                                String[] Banks = new String[] { item };
                                if (PaymentNewMoodList.Contains(item))
                                {
                                    PaymentNewMoodList.AddRange(Banks);
                                }
                                else
                                {
                                    PaymentNewMoodList.AddRange(Banks);
                    ...
                    {
                        String[] Bank = new String[] { "Cash" };
                        PaymentNewMoodList.AddRange(Bank);
                        BankListx.AddRange(Bank);
                        ForCashBankListx.AddRange(Bank);
                    }
                }
                excelEngine = new ExcelEngine();
                application = excelEngine.Excel;
                workbook = application.Workbooks.Create(PaymentNewMoodList.Count + 1);

                #region DataSet


                #region Sumerry Report 

                sheet1 = workbook.Worksheets[0];
                sheet1.IsGridLinesVisible = true;
                sheet1.Name = "SummerySheet";
                ...
                xlsCol += 1;
                iReportName = xlsCol;
                sheet1.Range[xlsRow, iReportName].Text = "Payment Type";
                ...
                xlsCol += 1;
                iCount = xlsCol;
                sheet1.Range[xlsRow, iCount].Text = "Total";
                sheet1.Range[xlsRow, iCount].ColumnWidth = 15;
                ...
                sheet1.Range[xlsRow, isl, xlsRow, iCount].CellStyle.Interior.Color = System.Drawing.Color.LightYellow;
                sheet1.Range[xlsRow, isl, xlsRow, iCount].BorderAround(ExcelLineStyle.Hair);
                sheet1.Range[xlsRow, isl, xlsRow, iCount].BorderInside(ExcelLineStyle.Hair);
                sheet1.Range[xlsRow, isl, xlsRow, iCount].CellStyle.Font.Bold = true;
                ...
                for (int dsi = 0; dsi < PayMentListx.Count; dsi++)
                {
                    ...
                    {
                        BankListx = ForCashBankListx;
                    }
                    else
                    {
                        BankListx = ForBankListx;
                    }
                    for (int x = 0; x < BankListx.Count; x++)
                    {
                        GetTotalData(PayMentListx[dsi], BankListx[x], PlantList, EntityList, DepartmentList, DesignationList, SectionList, SubSectionList, Month, Year, isActive, isSeperated, isMaternity, out dslocal);
                        ...
                        if (dvLocal.Count > 0)
                        {
                            ...
                            if (PayMentListx[dsi].ToUpper().Trim() == "CASH")
                            {
                                sheet1.Range[xlsRow, iReportName].Text = "Cash";
                                for (int i = 0; i < dvLocal.Count; i++)
                                {
                            ...
                            {
                                if (string.IsNullOrEmpty(dvLocal[0]["Bank"].ToString()))
                                {
                                    sheet1.Range[xlsRow, iReportName].Text = PayMentListx[dsi].ToString();
                                }
                                else
                                {
                ...
                for (int dsi = 0; dsi < PayMentListx.Count; dsi++)
                {
                    ...
                    for (int x = 0; x < BankListx.Count; x++)
                    {
                        ...
                        GetReportData(PayMentListx[dsi], BankListx[x], PlantList, EntityList, DepartmentList, DesignationList, SectionList, SubSectionList, Month, Year, isActive, isSeperated, isMaternity, out dslocal);
                        ...
                        if (dvAttn.Count > 0)
                        {
                            ...
                            excelEngine = new ExcelEngine();
                            application = excelEngine.Excel;
                            if (PayMentListx[dsi].ToUpper().Trim() == "CHECK")
                            {
                            }
                            if (PayMentListx[dsi].ToUpper().Trim() == "CASH")
                            {
                                if (PayMentListx.Count > 1)
                                {
                                    sheet1 = workbook.Worksheets[count];
                                    xx = PayMentListx[dsi].Replace("\"", "").Trim();
                                }
                                else
                                {
                            ...
                            else
                            {
                                ...
                                {
                                    xx = PayMentListx[dsi].Trim() + "_" + BankListx[x].Replace("\"", "").Trim();
                                }
                            }
                            if (xx == "")
                            {
                            ...
                            for (int i = 0; i < dvAttn.Count; i++)
                            {
                                ...
                                Total = Convert.ToDecimal(dvAttn[i]["NetSalary"].ToString()) + Total;
                            ...
                            if (clsStaticInfo.dbl(Total) > 0)
                            {
                                ...
                                xlsCol++;
                                sheet1.Range[xlsRow, xlsCol].Number = Convert.ToDouble(Total);
                                sheet1.Range[xlsRow, xlsCol].NumberFormat = clsStaticInfo.NumberFormat(2);
                                sheet1.Range[xlsRow, xlsCol].CellStyle.Font.Bold = true;
                            }
                            ...
                            xlsRow += 1;
                            sheet1.Range[xlsRow, 3].Text = "Company Wise Payment Sheet";
                            sheet1.Range[xlsRow, 3, xlsRow, endXlsCol].Merge();
                            sheet1.Range[xlsRow, 3].CellStyle.Font.Bold = true;
                            ...
                            xlsRow += 1;
                            sheet1.Range[xlsRow, 3].Text = xx + ", Month:- " + MonthName + " Year:-" + Year;
                            sheet1.Range[xlsRow, 3, xlsRow, endXlsCol].Merge();
                            sheet1.Range[xlsRow, 3].CellStyle.Font.Bold = true;
                ...
                if (Check == "Ok")
                {
                    ...
                    //workbook = application.Workbooks.Create(lastSheet);
                    sheet1 = workbook.Worksheets[lastSheet];
                    sheet1.IsGridLinesVisible = true;
                    ...
                    sheet1.Range[xlsRow, xlsCol].VerticalAlignment = ExcelVAlign.VAlignCenter;
                    xlsCol += 1;
                    sheet1.Range[xlsRow, xlsCol].Text = "Payment Mode";
                    sheet1.Range[xlsRow, xlsCol].ColumnWidth = 25;
                    sheet1.Range[xlsRow, xlsCol].HorizontalAlignment = ExcelHAlign.HAlignCenter;
                    sheet1.Range[xlsRow, xlsCol].VerticalAlignment = ExcelVAlign.VAlignCenter;
                    xlsCol += 1;
                    ...
                    #endregion ------------------Column Header------------------
                    int strCount = 0;
                    for (int g = 0; g < PayMentListx.Count; g++)
                    {
                        GetReportDatas(PayMentListx[g], BankList, PlantList, EntityList, DepartmentList, DesignationList, SectionList, SubSectionList, Month, Year, isActive, isSeperated, isMaternity, out dslocal);
                        ...
                        for (int i = 0; i < dvAttn.Count; i++)
                        {
                            ...
                            sheet1.Range[xlsRow, xlsCol].VerticalAlignment = ExcelVAlign.VAlignCenter;
                            xlsCol += 1;
                            sheet1.Range[xlsRow, xlsCol].Text = dvAttn[i]["PaymentMode"].ToString().ToUpper();
                            sheet1.Range[xlsRow, xlsCol].RowHeight = 13;
                            sheet1.Range[xlsRow, xlsCol].HorizontalAlignment = ExcelHAlign.HAlignLeft;
                            sheet1.Range[xlsRow, xlsCol].VerticalAlignment = ExcelVAlign.VAlignCenter;
                            xlsCol += 1;
                            ...
                            sheet1.Range[xlsRow, xlsCol].VerticalAlignment = ExcelVAlign.VAlignCenter;
                            xlsCol += 1;
                            AllTotal = Convert.ToDouble(dvAttn[i]["NetSalary"].ToString()) + AllTotal;
                            #region Line Setup
                            sheet1.Range[xlsRow - 1, 1, xlsRow - 1, endXlsCol].BorderInside(ExcelLineStyle.Hair);
                            sheet1.Range[xlsRow - 1, 1, xlsRow - 1, endXlsCol].BorderAround(ExcelLineStyle.Hair);
                            sheet1.Range[xlsRow - 1, 1, xlsRow - 1, endXlsCol].RowHeight = 30;
                    ...
                    xlsCol--;
                    xlsCol--;
                    if (clsStaticInfo.dbl(AllTotal) > 0)
                    {
                        sheet1.Range[xlsRow, xlsCol].Text = "Total ";
                        sheet1.Range[xlsRow, xlsCol].ColumnWidth = 19;
                        sheet1.Range[xlsRow, xlsCol].HorizontalAlignment = ExcelHAlign.HAlignCenter;
                        ...
                        sheet1.Range[xlsRow, xlsCol].CellStyle.Font.Bold = true;
                        xlsCol++;
                        sheet1.Range[xlsRow, xlsCol].Number = Convert.ToDouble(AllTotal);
                        sheet1.Range[xlsRow, xlsCol].NumberFormat = clsStaticInfo.NumberFormat(2);
                        sheet1.Range[xlsRow, xlsCol].CellStyle.Font.Bold = true;
                    }
                    ...
                    xlsRow += 1;
                    sheet1.Range[xlsRow, 3].Text = "TotalPaymentSheet";
                    sheet1.Range[xlsRow, 3, xlsRow, endXlsCol].Merge();
                    sheet1.Range[xlsRow, 3].CellStyle.Font.Bold = true;
                    sheet1.Range[xlsRow, 3].CellStyle.Font.Size = 11;
                    sheet1.Range[xlsRow, 3, xlsRow, endXlsCol].RowHeight = 20;
                    ...
                    xlsRow += 1;
                    sheet1.Range[xlsRow, 3].Text = "Company Wise Total Payment Report, Month:- " + MonthName + " Year:-" + Year;
                    sheet1.Range[xlsRow, 3, xlsRow, endXlsCol].Merge();
                    sheet1.Range[xlsRow, 3].CellStyle.Font.Bold = true;
                    sheet1.Range[xlsRow, 3].CellStyle.Font.Size = 9;
                    sheet1.Range[xlsRow, 3, xlsRow, endXlsCol].RowHeight = 20;
                    ...
                    sheet1.Name = "CompanyWiseTotalPaymentSheet";
                    sheet1.PageSetup.TopMargin = 0.5;
                    sheet1.PageSetup.BottomMargin = 0.7;
                    sheet1.PageSetup.PrintTitleRows = "$1:$5";
                    sheet1.PageSetup.RightFooter = "&\"Times New Roman\"&06" + "Page " + "&p" + " of " + "&N";
        ...
        public void GetReportData(string PaymentModeList, string BankList, string PlantList, string EntityList, string DepartmentList, string DesignationList, string SectionList, string SubSectionList, string Month, string Year, bool isActive, bool isSeperated, bool isMaternity, out DataSet dsRef)
        {
            ...
            try
            {
                string xx = "'" + PaymentModeList.Replace('"', ' ').Trim() + "'";
                string strSqlStruc = "";
                DataSet dsRefs;
                DataTable dtslProcId = null;
                strSqlStruc = @"SELECT SystemID FROM SalaryProcMaster SPM
                ...
                empStatus += ")";
                if (PaymentModeList.Trim().ToUpper() == "CASH")
                {
                    Check = @" --and bank.Id in ('" + BankList.Trim() + @"')";
                }
                else
                {
                    Check = @"and isnull(bank.Id,'') in ('" + BankList.Trim() + @"')";
                }
        ...
        public void GetTotalData(string PaymentModeList, string BankList, string PlantList, string EntityList, string DepartmentList, string DesignationList, string SectionList, string SubSectionList, string Month, string Year, bool isActive, bool isSeperated, bool isMaternity, out DataSet dsRef)
        {
            ConnectionManager.DAL.ConManager objCon;
            string strSql = string.Empty;
            string secSQL = string.Empty;
            ...
            try
            {
                ...
                strSql = @" select SUM(case when spcc.PaymentMode='Bank' then (ISNULL(spcc.SalaryPercentage,0)*ISNULL(spc.DisbusmentAmount,0))/100 else ISNULL(SPC.DisbusmentAmount,0) end)Total,Bank.UserName Bank
						From SalaryProcChild SPC
                        INNER JOIN SalaryProcMaster SPM ON SPM.SystemID = SPC.SlrProcMstSystemID 
						left join SalaryProcessLogDetail spcc on spcc.SalaryProcessId = SPC.SlrProcMstSystemID and spcc.EmpSystemId=SPC.EmpInfoSystemID
						INNER JOIN SalaryHead SH ON SH.SalaryHeadID = SPC.SalaryHeadID 
        ...
        public void GetReportDatas(string PaymentModeList, string BankList, string PlantList, string EntityList, string DepartmentList, string DesignationList, string SectionList, string SubSectionList, string Month, string Year, bool isActive, bool isSeperated, bool isMaternity, out DataSet dsRef)
        {
            ConnectionManager.DAL.ConManager objCon;
            string strSql = string.Empty;
            string secSQL = string.Empty;
            ...
            try
            {
                ...
                foreach (var item in BankL)
                {
                    if (Bankes == "")
                    {
                        Bankes = "'" + item.Trim().Replace('"', ' ').Trim() + "'";
                    }
                    else
                    {
                        Bankes += ",'" + item.Trim() + "'";
                ...
                if (PaymentModeList.Trim().ToUpper() == "CASH")
                {
                    Check = @" --and bank.Id in (" + Bankes + @")";
                }
                else
                {
                    Check = @"and isnull(bank.Id,'') in (" + Bankes + @")";
                }

                strSql = @" select p.UserName PlantName,EI.EmployeeCode,EI.EmployeeName,spcc.PaymentMode
						,NetSalary = case when spcc.PaymentMode='Bank' then (ISNULL(spcc.SalaryPercentage,0)*ISNULL(spc.DisbusmentAmount,0))/100 else ISNULL(SPC.DisbusmentAmount,0) end
						,spcc.BankAccNo BankAccountNo,spcc.IFSCCode,spcc.MICRCode,spcc.SalaryPercentage,Bank.UserName BankName, ec.UserName as EmployeeType
						From SalaryProcChild SPC
                        INNER JOIN SalaryProcMaster SPM ON SPM.SystemID = SPC.SlrProcMstSystemID 
