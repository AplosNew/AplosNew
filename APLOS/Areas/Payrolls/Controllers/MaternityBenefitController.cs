...
namespace Aplos.Areas.Payrolls.Controllers
{
    ...
    public class MaternityBenefitController : BaseController
    {
        #region Constructor
        /// <summary>   The separationTypeService service. </summary>
       
        private readonly ISqlRepository _sqlRepository;
        public MaternityBenefitController( ISqlRepository sqlRepository)
        {          
        ...
        [HttpPost]
        public ActionResult Save(MaternityBenefitMaster master, IEnumerable<MaternityBenefitDetail> detail,decimal TotalWorkingDays,decimal TotalPayable)
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            ...
            obj.SaveMasterAndDetail(master, detail, TotalWorkingDays, TotalPayable);
            return Json(new { Message = AplosMessage.Success }, JsonRequestBehavior.AllowGet);
        }

        [HttpGet, Authorize]
        public ActionResult ShowInfo(string empid, string LeaveStartDate)
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            clsMaternityBenefit obj = new clsMaternityBenefit(_sqlRepository);
            var data = obj.ShowSalaryInfo(empid, LeaveStartDate);
...
LEFT JOIN MST.ManpowerBudget mb ON mb.Id = EI.BudgetCode
                        ...
                        left join (
                        select * FROM LeaveTransaction where LTSystemID in (select id from LeaveType where LeaveType='Maternity')
                        and (
                        ...
                        ) t on t.EmpSystemID=ei.SystemId
                        left join mst.MaternityLeavePolicy mp on mp.id=t.MaternityLeavePolicyId
                        where ei.SystemId in
                        (
                        select EmpSystemID FROM LeaveTransaction t
                        inner join mst.MaternityLeavePolicy mp on mp.id=t.MaternityLeavePolicyId
                        where t.LTSystemID in (select id from LeaveType where LeaveType='Maternity') and mp.IsNoBenefit=0
                        and(
                        (FromDate between '" + FromDate + @"' and '" + ToDate + @"') or (ToDate between '" + FromDate + @"' and '" + ToDate + @"')
                        or ('" + FromDate + @"' between FromDate and ToDate) or ('" + ToDate + @"' between FromDate and ToDate)
                        )
                        )
                        and ei.systemid not in
                        (
                        select empsystemid FROM MaternityBenefitMaster where (IsPaidAfter=1 or IsPaidBefore=1) and LeaveTransactionId in
                        (
                        select systemid from LeaveTransaction where
                        (
                        (FromDate between '" + FromDate + @"' and '" + ToDate + @"') or (ToDate between '" + FromDate + @"' and '" + ToDate + @"')
                        or ('" + FromDate + @"' between FromDate and ToDate) or ('" + ToDate + @"' between FromDate and ToDate)
                        )and PlantID='" + identity.PlantId + @"'
        ...
        [HttpGet,Authorize]
        public ActionResult LoadNoBenefitEmployeelist()
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            string sql = string.Empty;
            try
...
LEFT JOIN MST.ManpowerBudget mb ON mb.Id = EI.BudgetCode
                            LEFT JOIN ORG.Position PR ON MB.PositionId=PR.Id
		                        ...
		                        select EmpSystemID FROM LeaveTransaction t
                                    inner join mst.MaternityLeavePolicy mp on mp.id=t.MaternityLeavePolicyId	
                                    where t.LTSystemID in (select id from LeaveType where LeaveType='Maternity') and mp.IsNoBenefit=1
		                        ) 
        ...
        [HttpGet,Authorize]
        public ActionResult LoadPaidEmployeelist()
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            string sql = string.Empty;
            try
...
LEFT JOIN MST.ManpowerBudget mb ON mb.Id = EI.BudgetCode
                            LEFT JOIN ORG.Position PR ON MB.PositionId=PR.Id
                                ...
                                and ei.systemid  in
                                    (
                                   select empsystemid FROM MaternityBenefitMaster where IsPaidAfter=1 and IsPaidBefore=1
                                    )
                                    ";
