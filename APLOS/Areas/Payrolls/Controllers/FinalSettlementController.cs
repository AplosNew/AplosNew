...
AND ISNULL(sl.PayableVoucherId,'')<>'' and sl.islocked=1 AND ISNULL(sl.PastBonusDisbursed,0) = 0 AND ISNULL(sl.IsBonusDisbursed,0) = 1 AND SPC.EmpInfoSystemID=" + EmpSystemId + @"";
...
AND ISNULL(sl.IsBonusDisbursed,0) = 1 AND SPC.EmpInfoSystemID in (" + empIds + @"))";
