"use strict";
nonAssetRegisterController.$inject = ["addressService", "commonMessage", "$scope", "$rootScope", "baseService", "cboService", "$http", "$filter", "$controller", "$window"];
function nonAssetRegisterController(addressService, commonMessage, $scope, $rootScope, baseService, cboService, $http, $filter, $controller, $window) {
    ...
    $scope.nonAssetregisterList = [];

    $scope.getAssetData = function (faType) {
        $http({
            method: 'GET'
            , url: 'FixedAssets/FixedAssetRegister/GetNonAssetItem?faType=' + faType
            , dataType: 'JSON'
        }).then(function (response) {
            $scope.nonAssetregisterList = response.data;
        }), function (response) {
            ShowResult(response.data.Message, 'failure');
        };
    };
    $scope.changeRegisterType = function () {
        if ($scope.register.registerType == 'Individual')
            $scope.isDisable = true;
        else
            $scope.isDisable = false;

    }
    $scope.getAssetData('AssetNonCapitalized');
    ...
    $scope.getRegisterData = function (registerid) {
        $http({
            method: "GET",
            url: $scope.path + "getregisterlist?registerid=" + registerid
        }).then(function successCallback(response) {
            //console.log(result);
    ...
    function individualRegister() {
        var rowcount = 0;
        for (var i = 0; i < $scope.nonAssetregisterList.length; i++) {
            if ($scope.register.registerType == 'Individual' && rowcount == 1) {
                ShowResult("More than one item is not allow!", "failure");
                break;
            }
            if ($scope.nonAssetregisterList[i].Active == true && rowcount == 0 && $scope.register.registerType == 'Individual') {
                rowcount++;
                $scope.assetRegisterCharactreristicsList = [];
                $scope.register.InvoiceDate = $scope.nonAssetregisterList[i].InvoiceDate;
                $scope.register.InvoiceNo = $scope.nonAssetregisterList[i].InvoiceNo;
                $scope.register.InvoiceDate = $scope.nonAssetregisterList[i].InvoiceDate;
                $scope.register.CurrencyId = $scope.nonAssetregisterList[i].CurrencyId;
                $scope.register.CountryOfOriginId = $scope.nonAssetregisterList[i].CountryId;
                $scope.register.VoucherNo = $scope.nonAssetregisterList[i].IssueVoucherNo;
                $scope.register.VoucherDate = $scope.nonAssetregisterList[i].IssuePostingDate;

                $scope.register.BaseUOMName = $scope.nonAssetregisterList[i].TransactionUoM;
                $scope.register.CapitalizationDate = $scope.nonAssetregisterList[i].CapitalizePostingDate;
                $scope.register.Vendor = $scope.nonAssetregisterList[i].VendorName;
                $scope.register.VendorId = $scope.nonAssetregisterList[i].PartyId;
                $scope.register.MaterialMasterId = $scope.nonAssetregisterList[i].MaterialMasterId;
                $scope.register.MaterialMasterArticleId = $scope.nonAssetregisterList[i].ArticleId;
                $scope.register.FirstCharacteristicsValueId = $scope.nonAssetregisterList[i].FirstCharacteristicsValueId;
                $scope.register.VoucherDetailId = $scope.nonAssetregisterList[i].VoucherDetailNo;
                $scope.register.InventoryIssueHistoryId = $scope.nonAssetregisterList[i].InventoryIssueHistoryId;

                $scope.register.MaterialMasterName = $scope.nonAssetregisterList[i].MaterialMasterName;
                $scope.register.ArticleStandardName = $scope.nonAssetregisterList[i].ArticleStandardName;

                $scope.register.ADBaseAmount = 0;
                if ($scope.nonAssetregisterList[i].Qty > 1)
                    $scope.register.Price = $scope.nonAssetregisterList[i].Amount / $scope.nonAssetregisterList[i].Qty;
                else
                    $scope.register.Price = $scope.nonAssetregisterList[i].Amount
                $scope.register.FABaseAmount = $scope.register.Price;
                $scope.NumberOfQuantity = $scope.nonAssetregisterList[i].Qty;
                $scope.register.TotalPrice = $scope.nonAssetregisterList[i].Amount

                $scope.materialArticleInfo = {
                    ArticleCode: null,
                    ArticleStandardName: null
                };
                $scope.materialArticleInfo.ArticleStandardName = $scope.nonAssetregisterList[i].ArticleStandardName;
                $scope.register.AssetBudgetMasterId = $scope.nonAssetregisterList[i].BudgetMasterId;
                $scope.register.AssetGLId = $scope.nonAssetregisterList[i].AssetGLId;
                $scope.register.BaseUOMName = $scope.nonAssetregisterList[i].TransactionUoM;
                $scope.register.FixedAssetMasterId = $scope.nonAssetregisterList[i].FixedAssetMasterId;
                $scope.register.AssetMasterName = $scope.nonAssetregisterList[i].AssetMasterName;
                $scope.register.AssetGLName = $scope.nonAssetregisterList[i].AssetGLName;
                $scope.register.AssetBudgetName = $scope.nonAssetregisterList[i].AssetBudgetName;
                $scope.register.AssetActivityName = $scope.nonAssetregisterList[i].ActivityName;
                $scope.register.AssetActivityId = $scope.nonAssetregisterList[i].ActivityId;
                $scope.fixedAssetRegisterDetailList.push($scope.nonAssetregisterList[i]);
                $scope.getWithFAM();
                if (!$rootScope.isCollapsed) {
                    $rootScope.toggle();
                }
    ...
    function accumulatedRegister() {
        $scope.NumberOfQuantity = 0;
        for (var i = 0; i < $scope.nonAssetregisterList.length; i++) {
            if ($scope.nonAssetregisterList[i].Active == true && $scope.register.registerType == 'Accumulated') {

                $scope.register.ADBaseAmount = 0;

                $scope.NumberOfQuantity = 1;
                $scope.register.TotalPrice += parseFloat($scope.nonAssetregisterList[i].Amount);
                $scope.register.Price = $scope.register.TotalPrice;
                $scope.register.FABaseAmount = $scope.register.Price;
                $scope.materialArticleInfo = {
                    ArticleCode: null,
                    ArticleStandardName: null
                };
                $scope.getOpeningBalanceDataWithFAM.FABaseAmountTotal = $scope.register.TotalPrice;
                $scope.getOpeningBalanceDataWithFAM.ADBaseAmountTotal = 0;
                $scope.getDataWithFAM.TotalRow = 1;
                $scope.getOpeningBalanceDataWithFAM.TotalRow = 1;
                $scope.fixedAssetRegisterDetailList.push($scope.nonAssetregisterList[i]);
                if (!$rootScope.isCollapsed) {
                    $rootScope.toggle();
                }
    ...
    function subAssetRegister() {
        $scope.subAsseType.Amount = 0;
        for (var i = 0; i < $scope.nonAssetregisterList.length; i++) {
            if ($scope.nonAssetregisterList[i].Active == true && $scope.register.registerType == 'SubAsset') {
                //var sAAmount = $scope.nonAssetregisterList[i].Amount.toFixed(4);
                //var amount = $scope.subAsseType.Amount.toFixed(4);
                $scope.subAsseType.Amount += $scope.nonAssetregisterList[i].Amount;
                $scope.fixedAssetRegisterDetailList.push($scope.nonAssetregisterList[i]);
                if (!$rootScope.isCollapsed) {
                    $rootScope.toggle();
                }
    ...
    $scope.Apply = function () {
        ...
        if ($scope.register.registerType == 'Individual')
            individualRegister();
        else if ($scope.register.registerType == 'Accumulated')
            accumulatedRegister();
        else if ($scope.register.registerType == 'SubAsset')
            subAssetRegister();
    };
    ...
    function ValidationSubAssetRegister() {
        try {
            for (var i = 0; i < $scope.selectedAssetRegisterList.length; i++) {
                if ($scope.selectedAssetRegisterList[i].CapitalizationDate == null) {
                    throw "Please input Capitalization Date.";
                    break;
                }
            }
    ...
    $scope.SaveInduvidualIntegratedRegister = function () {
        try {
            ...
            if ($scope.form0.$valid) {
                ...
                $http({
                    method: "POST",
                    url: $scope.path + "CreateRegisterIndividualNonAsset",
                    dataType: "JSON",
                    data: {
    ...
    $scope.SaveSubAssetRegister = function () {
        try {
            $scope.$broadcast("show-errors-check-validity");
            if ($scope.form1.$valid) {
                ValidationSubAssetRegister();
                $scope.saveBtnDisable = true;
                $http({
                    method: "POST",
                    url: $scope.path + "CreateRegisterSubAsset",
                    dataType: "JSON",
                    data: {
                        "registerid": null
                        , "subFixedAssetRegister": $scope.selectedAssetRegisterList
                        , "fixedAssetRegisterDetail": $scope.fixedAssetRegisterDetailList
                    }
                }).then(function successCallback(response) {
                    if (response.data.Error === true) {
    ...
    $scope.SaveRegister = function () {
        if ($scope.register.registerType == 'SubAsset')
            $scope.SaveSubAssetRegister();
        else
            $scope.SaveInduvidualIntegratedRegister();
    };
    ...
    $scope.addCharge = function () {
        if (manualValidation("td_FinancingType", baseService.isUndefinedOrNull($scope.subAsseType.SubAssetTypeId), "SubAsset Type is required.")) {
            ...
            $scope.subAsseType.SubAssetTypeName = $.grep($scope.subAssetTypeList, function (item) {
                return item.Value === $scope.subAsseType.SubAssetTypeId;
            })[0].Text;
            $scope.subAsseType.Amount = parseFloat($scope.subAsseType.Amount).toFixed(4);
    ...
    $scope.getFixedAssetData = function () {
        var url = "Materials/MaterialMaster/GetMaterialMasterWithFixedAssetMaster";
    ...
    $scope.checkIsRegisterApplyInMaterialMaster = function (data) {
        ...
        $scope.register.MaterialMasterName = data.UserName;
        $scope.register.BaseUOMName = data.BaseUOMName;
    ...
    $scope.getAssetRegisterItemForSubAsset = function () {
        if (baseService.isUndefinedOrNull($scope.subAsseType.CapitalizationDate)) {
            ShowResult("Please input Capitalize Date!", "failure");
        }
        else {
            $http({
                method: 'GET'
                , url: 'FixedAssets/FixedAssetRegister/GetAssetRegisterItemForSubAsset'
                , dataType: 'JSON'
            }).then(function (response) {
                $scope.assetRegisterItemForSubAssetList = response.data;
                console.log($scope.assetRegisterItemForSubAssetList);
            }), function (response) {
                ShowResult(response.data.Message, 'failure');
            };
            angular.element(document.querySelector("#assetRegistermodal")).modal("show");

        }
        
    };
    $scope.closeAssetRegisterPopUp = function () {
        angular.element(document.querySelector("#assetRegistermodal")).modal("hide");
    }
    $scope.selectedAssetRegisterList = [];
    $scope.checkassetRegisterApply = function () {
        $scope.subAsseType.Amount
        $scope.TotalPrice = $filter('sumByKey')($filter('filter')($scope.assetRegisterItemForSubAssetList, { Active: true }), 'Price');

        for (var i = 0; i < $scope.assetRegisterItemForSubAssetList.length; i++) {
            if ($scope.assetRegisterItemForSubAssetList[i].Active == true && $scope.register.registerType == 'SubAsset') {

                $scope.assetRegisterItemForSubAssetList[i].Amount = ($scope.subAsseType.Amount / $scope.TotalPrice) * $scope.assetRegisterItemForSubAssetList[i].Price;
                $scope.assetRegisterItemForSubAssetList[i].SubAssetTypeId = $scope.subAsseType.SubAssetTypeId
                $scope.assetRegisterItemForSubAssetList[i].CapitalizationDate = $scope.subAsseType.CapitalizationDate
                $scope.selectedAssetRegisterList.push($scope.assetRegisterItemForSubAssetList[i]);
            }
        }
        $scope.closeAssetRegisterPopUp();
    }

    $scope.calUnitPrice = function (qty) {
        $scope.register.Price = parseFloat($scope.register.TotalPrice / qty).toFixed(4);
    }

    $scope.tabtoggle = 1;
    $scope.setTabtoggle = function (newTab) {
        $scope.tabtoggle = newTab;
    };
    
    $scope.isSettoggle = function (tabNum) {
        return $scope.tabtoggle === tabNum;
    };
    $scope.expenseregisterList = [];

    $scope.getExpenseAssetData = function (faType) {
        $http({
            method: 'GET'
            , url: 'FixedAssets/FixedAssetRegister/GetExpensesRegisterItem?faType=' + faType
            , dataType: 'JSON'
        }).then(function (response) {
            $scope.expenseregisterList = response.data;
        }), function (response) {
            ShowResult(response.data.Message, 'failure');
        };
    };
    $scope.getExpenseAssetData('ExpensesCapitalized');
    $scope.ApplyExpense = function () {
        $scope.fixedAssetRegisterDetailList = [];
        $scope.register.AssetMasterName = null;
        $scope.register.AssetGLName = null;
        $scope.register.AssetBudgetName = null;
        ...
        $scope.register.AssetActivityId = null;
        $scope.register.VendorId = null;
         if ($scope.register.registerType == 'Accumulated')
            expensesAccumulatedRegister();
        else if ($scope.register.registerType == 'SubAsset')
            expensesSubAssetRegister();
    };
    function expensesAccumulatedRegister() {
        $scope.NumberOfQuantity = 0;
        for (var i = 0; i < $scope.expenseregisterList.length; i++) {
            if ($scope.expenseregisterList[i].Active == true && $scope.register.registerType == 'Accumulated') {

                $scope.register.ADBaseAmount = 0;

                $scope.NumberOfQuantity = 1;
                $scope.register.TotalPrice += parseFloat($scope.expenseregisterList[i].Amount);
                $scope.register.Price = $scope.register.TotalPrice;
                $scope.register.FABaseAmount = $scope.register.Price;
                $scope.materialArticleInfo = {
                    ArticleCode: null,
                ...
                $scope.getDataWithFAM.TotalRow = 1;
                $scope.getOpeningBalanceDataWithFAM.TotalRow = 1;
                $scope.fixedAssetRegisterDetailList.push($scope.expenseregisterList[i]);
                if (!$rootScope.isCollapsed) {
                    $rootScope.toggle();
                }
            }
    ...
    function expensesSubAssetRegister() {
        $scope.subAsseType.Amount = 0;
        for (var i = 0; i < $scope.expenseregisterList.length; i++) {
            if ($scope.expenseregisterList[i].Active == true && $scope.register.registerType == 'SubAsset') {
                //var sAAmount = $scope.nonAssetregisterList[i].Amount.toFixed(4);
                //var amount = $scope.subAsseType.Amount.toFixed(4);
                $scope.subAsseType.Amount += $scope.expenseregisterList[i].Amount;
                $scope.fixedAssetRegisterDetailList.push($scope.expenseregisterList[i]);
                if (!$rootScope.isCollapsed) {
                    $rootScope.toggle();
                }
