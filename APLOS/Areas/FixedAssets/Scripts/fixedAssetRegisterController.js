"use strict";
fixedAssetRegisterController.$inject = ["addressService", "commonMessage", "$scope", "$rootScope", "baseService", "cboService", "$http", "$filter", "$controller", "$window"];
function fixedAssetRegisterController(addressService, commonMessage, $scope, $rootScope, baseService, cboService, $http, $filter, $controller, $window) {
    ...
    $scope.getListUrl = $scope.path + "GetOBFixedAssetRegisterList";
    $scope.GridShow = true;
    $scope.DivShow = false;
    ...
    baseService.init('fixedassets/fixedassetregister/GetOBFixedAssetRegisterList', null, null, "DESC", "FixedAssetMasterName", "FixedAssetMasterName");
    ...
    $scope.getAssetData = function () {
        ...
        var url = "Materials/MaterialMaster/GetFixedAssetMasterBudgetTagForRegister?budgetMasterId=" + $scope.register.AssetBudgetMasterId + '&activityId=' + $scope.register.AssetActivityId;
    ...
    $scope.priceCalculate = function () {
        $scope.register.Price = parseFloat($scope.register.TotalPrice / $scope.NumberOfQuantity).toFixed(2);
    };
    function ValidationRegister() {
        try {
            ...
            CheckField($scope.register.TotalPrice, "TotalPrice");
            if ($scope.register.IsFinancial === false) {
                $scope.register.FABaseAmount = 0;
                $scope.register.ADBaseAmount = 0;
            }
            if ($scope.register.IsOriginApplicable) {
                if ($scope.register.CountryOfOriginId == null) {
                    throw "Please select Origin. !";
                }
            }
            if ($scope.register.IsFinancial) {
                ...
                $scope.register.TotalPrice = parseFloat($scope.register.TotalPrice);

                $scope.getOpeningBalanceDataWithFAM.FABaseAmountTotal = parseFloat($scope.getOpeningBalanceDataWithFAM.FABaseAmountTotal);
                if (baseService.isUndefinedOrNull($scope.register.FABaseAmount)) {
                ...
                if ($scope.register.FABaseAmount !== 0) {
                    ...
                    //}
                    if ($scope.register.CurrencyId === $scope.register.FABaseCurrencyId) {
                        if ($scope.subAssetList.length) {
                            $scope.TotalSubAssetAmount = parseFloat($filter("sumByKey")($filter("filter")($scope.subAssetList), "Amount"));
                            //var Amount = $scope.register.Price + parseFloat($scope.TotalSubAssetAmount);
                            var Amount = ($scope.register.TotalPrice + $scope.TotalSubAssetAmount).toFixed(2);
                            if (Amount != $scope.register.FABaseAmount)
                                throw $scope.parallelCurrencyList[0].Code + " purchase price must be equal to asset historical value. ";
                        }
                        //else if ($scope.register.Price < $scope.register.FABaseAmount) {
                        //    throw $scope.parallelCurrencyList[0].Code + " purchase price must be equal to asset historical value. ";
                        //}
                        else if ($scope.register.TotalPrice < $scope.register.FABaseAmount) {
                            throw $scope.parallelCurrencyList[0].Code + " purchase price must be equal to asset historical value. ";
                        }
                    }

                    //if ($scope.register.CurrencyId === $scope.register.FABaseCurrencyId && $scope.register.Price > $scope.register.FABaseAmount) {
                    if ($scope.register.CurrencyId === $scope.register.FABaseCurrencyId && $scope.register.TotalPrice > $scope.register.FABaseAmount) {
                        throw $scope.parallelCurrencyList[0].Code + " purchase price must be equal to asset historical value.";
                    }
                    //TODO:
                    ...
                    //if (($scope.getOpeningBalanceDataWithFAM.ADBaseAmountTotal - $scope.getOpeningBalanceDataWithFAM.ADBaseRegsiterAmount) < $scope.register.ADBaseAmount) {
                    //    throw $scope.parallelCurrencyList[0].Code + " Acc Dep amount  can\'t be greater then Total Acc Dep amount : " + ($scope.getOpeningBalanceDataWithFAM.ADBaseAmountTotal - $scope.getOpeningBalanceDataWithFAM.ADBaseRegsiterAmount);
                    //}

                }
            }

            if (new Date($scope.register.CapitalizationDate) >= new Date($scope.cutOffDate)) {
                throw " Capitalization Date must be <  Cut Off Date <b>(" + $filter("dateFiltering")($scope.cutOffDate) + ")</b>";
            }
    ...
    $scope.getWithFAM = function () {
        $http({
            method: "GET",
            url: $scope.path + "GetRegisterInfoWithFAMId?assetMasterId=" + $scope.register.MaterialMasterId + "&budgetMasterId=" + $scope.register.AssetBudgetMasterId + "&assetGLId=" + $scope.register.AssetGLId + "&companyId=" + $scope.register.CompanyId
        }).then(function successCallback(response) {
            $scope.getDataWithFAM = {
                ADBaseAmountTotal: 0,
                ADGroupAmountTotal: 0,
    ...
    $scope.getOpeningWithFAM = function () {
        $http({
            method: "GET",
            url: $scope.path + "GetOpeningBalanceInfoWithBudgetMasterId?assetBudgetId=" + $scope.register.AssetBudgetMasterId
    ...
    //    $http({
    //        method: "GET",
    //        url: "FixedAssets/FixedAssetRegister/CheckMasterIsRegisterApplyByAssetId?assetMasterId=" + id
    //    }).then(function successCallback(response) {
    //        $scope.IsRegisterApplyList = response.data;
    //    });
    //};
    ...
    $scope.FAAccDepList = [];
    $scope.getTotalAccDepRegister = function () {
        $http({
            method: "get",
            dataType: 'JSON',
            url: 'FixedAssets/FixedAssetRegister/getTotalAccDepRegister?budgetMasterId=' + $scope.register.AssetBudgetMasterId,
           // data: { entity: $scope.register.AssetBudgetMasterId}

        }).then(function successCallback(response) {
            $scope.FAAccDepList = response.data;
            angular.element(document.querySelector('#FAAccDepPopup')).modal('show');
        });
    };
    $scope.closeFAAccDepPopup = function () {
        angular.element(document.querySelector('#FAAccDepPopup')).modal('hide');
    }

    $scope.FAAccDepOBList = [];
    $scope.getTotalAccDepOB = function () {
        $http({
            method: "get",
            dataType: 'JSON',
            url: 'FixedAssets/FixedAssetRegister/getTotalAccDepOB?budgetMasterId=' + $scope.register.AssetBudgetMasterId,
            // data: { entity: $scope.register.AssetBudgetMasterId}

        }).then(function successCallback(response) {
            $scope.FAAccDepOBList = response.data;
            angular.element(document.querySelector('#FAAccDepOBPopup')).modal('show');
        });
    };
    $scope.closeFAAccDepOBPopup = function () {
        angular.element(document.querySelector('#FAAccDepOBPopup')).modal('hide');
    }
    $scope.OBFixedAssetList = [];
    $scope.getOBFixedAssetList = function () {
        $http({
            method: "get",
            dataType: 'JSON',
            url: 'FixedAssets/FixedAssetRegister/GetOBFixedAssetList'

        }).then(function successCallback(response) {
            $scope.OBFixedAssetList = response.data;
        });
    };
    $scope.getOBFixedAssetList();
    $scope.ShowDiv = function (args) {
        var gridObj = $("#Grid").data("ejGrid");        var data = gridObj.getSelectedRecords()[0];
        $scope.register.AssetMasterName = data.FixedAsset;
        $scope.register.AssetGLName = data.GLName;
        $scope.register.AssetBudgetName = data.BudgetName;
        $scope.register.AssetActivityName = data.ActivityName;
        $scope.register.AssetGLId = data.AssetGLId;
        $scope.register.AssetBudgetMasterId = data.BudgetMasterId;
        $scope.register.AssetActivityId = data.ActivityId;
    ...
    $scope.getSubAssetList = function (id) {
        $http({
            method: "GET",
            url: "FixedAssets/FixedAssetRegister/GetAUCJVSubAssetList?fixedAssetRegisterId=" + id
        }).then(function successCallback(response) {
            $scope.subAssetList = response.data;
        });
    };

    $scope.removeDetaillRow = function (Id, index) {
        if (Id === null) {
            //$(this).remove();
            $scope.subAssetList.splice(index, 1);
            return false;
        }
        else {
