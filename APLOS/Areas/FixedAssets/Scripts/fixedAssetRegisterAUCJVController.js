"use strict";
fixedAssetRegisterAUCJVController.$inject = ["addressService", "commonMessage", "$scope", "$rootScope", "baseService", "cboService", "$http", "$filter", "$controller", "$window"];
function fixedAssetRegisterAUCJVController(addressService, commonMessage, $scope, $rootScope, baseService, cboService, $http, $filter, $controller, $window) {
    ...
    $scope.getListUrl = $scope.path + "GetRegisterAUCList";
    ...
    baseService.init('fixedassets/fixedassetregister/GetAUCJVFixedAssetRegisterList', null, null, "DESC", "FixedAssetMasterName", "FixedAssetMasterName");
    ...
    $scope.getAssetData = function () {
        var url = "Materials/MaterialMaster/GetMaterialMasterAUCFixedAssetMaster";
    ...
    $scope.getRegisterData = function (registerid) {
        $http({
            method: "GET",
            url: $scope.path + "GetRegisterAUCList?registerid=" + registerid
        }).then(function successCallback(response) {
            //console.log(result);
    ...
    $scope.checkIsRegisterApplyInMaterialMaster = function (data) {
        ...
        $http({
            method: "GET",
            url: "FixedAssets/FixedAssetRegister/CheckFixedMasterIsRegisterApplyByAUCJV?assetMasterId=" + data.FixedAssetMasterId
        }).then(function successCallback(response) {
                ...
                if ($scope.register.FABaseAmount !== 0 || $scope.register.FAGroupAmount !== 0 || $scope.register.FAHardAmount !== 0) {
                    if ($scope.register.CurrencyId === $scope.register.FABaseCurrencyId) {
                        if ($scope.subAssetList.length) {
                            $scope.TotalSubAssetAmount = $filter("sumByKey")($filter("filter")($scope.subAssetList), "Amount");
    ...
    $scope.getWithFAM = function () {
        $http({
            method: "GET",
            url: $scope.path + "GetAUCJVRegisterInfoWithFAMId?fixedAssetMasterId=" + $scope.register.FixedAssetMasterId + "&budgetMasterId=" + $scope.register.AssetBudgetMasterId + "&assetGLId=" + $scope.register.AssetGLId + "&activityId=" + $scope.register.AssetActivityId + "&companyId=" + $scope.register.CompanyId
        }).then(function successCallback(response) {
            $scope.getDataWithFAM = {
                ADBaseAmountTotal: 0,
                ADGroupAmountTotal: 0,
    ...
    $scope.getOpeningWithFAM = function () {
        $http({
            method: "GET",
            url: $scope.path + "GetAUCJVFixedAssetItem?fixedAssetMasterId=" + $scope.register.FixedAssetMasterId + "&assetGLId=" + $scope.register.AssetGLId + "&assetBudgetId=" + $scope.register.AssetBudgetMasterId
    ...
    $scope.SaveRegister = function () {
        try {
            ...
            if ($scope.form0.$valid) {
                ...
                $http({
                    method: "POST",
                    url: $scope.path + "CreateRegisterAUCJV",
                    dataType: "JSON",
                    data: {
    ...
    $scope.checkIsRegisterApplyInUpdate = function (id) {
        ...
        $http({
            method: "GET",
            url: "FixedAssets/FixedAssetRegister/CheckFixedMasterIsRegisterApplyByAUCJV?assetMasterId=" + id
        }).then(function successCallback(response) {
            $scope.IsRegisterApplyList = response.data;
        });
    };
    ...
    $scope.detailDelete = function () {
        try {
            $http({
                method: 'POST',
                url: 'Accounts/Voucher/DeleteVoucherDetail?Id=' + $scope.subAssetId + '&voucherId=' + $scope.fixedAssetRegisterId,
            }).then(function successCallback(response) {
                if (response.data.Error === true)
                    ShowResult(response.data.Message, 'failure');
                else {
                    ShowResult(response.data.Message, 'success');
                    $scope.subAssetId = null;
                    $scope.fixedAssetRegisterId = null;
                    $scope.subAssetList.splice($scope.subAssetIndex, 1);
                }
            }), function errorCallBack(response) {
                ShowResult(response.data.Message, 'failure');
            };
