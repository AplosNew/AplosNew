"use strict";
fixedAssetRegisterJVController.$inject = ["addressService", "commonMessage", "$scope", "$rootScope", "baseService", "cboService", "$http", "$filter", "$controller", "$window"];
function fixedAssetRegisterJVController(addressService, commonMessage, $scope, $rootScope, baseService, cboService, $http, $filter, $controller, $window) {
    ...
    $scope.GetCurrencyParallel = function () {
        $http({
            ...
            $scope.register.FABaseCurrencyId = $scope.CurrencyParallel[0].CurrencyId;
        });
    };
    ...
    $scope.getRegisterData = function (registerid) {
        $http({
            method: "GET",
            url: $scope.path + "getregisterlistjv?registerid=" + registerid
        }).then(function successCallback(response) {
            //console.log(result);
    ...
    $scope.getMaterialMasterCode = function (data) {
        try {
            ...
            $scope.register.FABudgetMasterId = data.BudgetMasterId;
            ...
            $scope.register.FAActivityId = data.ActivityId;
            $scope.getWithFAM();
        } catch (e) {
            throw e;
        }
    ...
    $scope.priceCalculate = function () {
        if ($scope.NumberOfQuantity == 0 || baseService.isUndefinedOrNull($scope.NumberOfQuantity)) {
            $scope.NumberOfQuantity = 1;
        }
        $scope.register.UnitPrice = Math.round(($scope.register.TotalPrice / $scope.NumberOfQuantity) * 100 + Number.EPSILON) / 100;
    };
    ...
    function ValidationRegister() {
        try {
            ...
            CheckField($scope.register.UnitPrice, "Unit Price");
            ...
            if ($scope.register.IsFinancial) {
                ...
                if ($scope.subAssetList.length > 0 && $scope.register.FABaseCurrencyId === $scope.register.CurrencyId) {
                    $scope.register.FABaseAmount = $scope.register.TotalPrice + Math.round($filter("sumByKey")($filter("filter")($scope.subAssetList), "Amount") * 100 + Number.EPSILON) / 100;;

                }
            }
            else {
                $scope.register.FABaseAmount = $scope.register.TotalPrice;
            }
    ...
    $scope.getOpeningWithFAM = function () {
        $http({
            method: "GET",
            url: $scope.path + "GetJVFixedAssetItem?fixedAssetMasterId=" + $scope.register.FixedAssetMasterId + "&assetGLId=" + $scope.register.AssetGLId + "&assetBudgetId=" + $scope.register.AssetBudgetMasterId
    ...
    $scope.SaveRegister = function () {
        try {
            ...
            if ($scope.form0.$valid) {
                ...
                $http({
                    method: "POST",
                    url: $scope.path + "CreateRegisterJV",
                    dataType: "JSON",
                    data: {
    ...
    function ClearRegister() {
        ...
        $scope.assetRegisterCharactreristicsList = [];
        $scope.subAssetList = [];
        $scope.subAsseType.CurrencyId = $scope.register.FABaseCurrencyId;
        $scope.loadDDL();
    }
    ...
    $scope.addCharge = function () {
        if (manualValidation("td_FinancingType", baseService.isUndefinedOrNull($scope.subAsseType.SubAssetTypeId), "SubAsset Type is required.")) {
            ...
            $scope.subAsseType.SubAssetCurrencyCode = $.grep($scope.tranCurrencyList, function (item) {
                return item.CurrencyId === $scope.subAsseType.CurrencyId;
            })[0].Text; 

            
            $scope.subAsseType.BaseCurrencyId = $scope.register.FABaseCurrencyId;
            $scope.subAssetList.push($scope.subAsseType);
            $scope.subAsseType = {};
        }
    };
    $scope.calBaseSubAmount = function () {
        if ($scope.subAsseType.CurrencyId == $scope.register.FABaseCurrencyId) {
            $scope.subAsseType.BaseAmount = $scope.subAsseType.Amount;
        }
    }
    ...
    $scope.removeSubAssetDetaillRow = function (Id, index) {
        if (Id === null) {
            //$(this).remove();
