'use strict';
menuSyncController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', 'cboService', '$sce'];
function menuSyncController(commonMessage, $scope, $rootScope, baseService, $http, $filter, cboService, $sce) {
    ...
    $scope.MenuDetailList = function () {
        $http({
            method: 'GET',
            url: 'Menus/MenuSync/GetMenuDetailList?moduleId=' + $scope.companyGroupMenuMaster.ModuleId + '&MenuFrameId=' + $scope.companyGroupMenuMaster.MenuFrameId
        }).then(function successCallback(response) {
    ...
    $scope.saveCompanyGroupMenuMaster = function () {
        ...
        $http({
            method: 'POST',
            url: 'Menus/MenuSync/SaveCompanyGroupMenuMaster',
    ...
    $scope.saveCompanyGroupMenuMasterSingle = function (x) {
        //$scope.MenuListTemp = [];
        //var row = $filter('filter')($scope.MenuList, { 'isToBeSelect': true });
        //if (!baseService.isUndefinedOrNull(row) && row.length > 0) {
        //    $scope.MenuListTemp = row;
        ...
        //    $scope.MenuListTemp = ej.DataManager(row).executeLocal(ej.Query().select(["MenuMasterId", "ModuleId"]));
        //}
        $scope.MenuListTemp.push(x);
        $scope.MenuListTemp.isToBeSelect = true;
        $http({
            method: 'POST',
            url: 'Menus/MenuSync/UncheckMenu',
            data: { 'MenuMasterId': x.MenuMasterId},
            dataType: 'JSON'
        }).then(function successCallback(response) {
            if (response.data.Error === true) {
                ShowResult(response.data.Message, 'failure');
