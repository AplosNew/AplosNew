'use strict';
civilStatusController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function civilStatusController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = 'Civil Status';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.jobDescriptionCategories = [];
    $scope.path = 'employees/civilstatus/';
    ...
    $scope.getData = function (pageno) {
        baseService.pagination(pageno)
            .then(function (result) {
                $scope.civilStatuss = result.Rows;
            }, function () {
    ...
    $scope.civilStatusNew = Object.assign({}, $scope.civilStatus);

    $scope.GetSequence = function () {
        cboService.getSequence($scope.getSeqUrl, function (data) {
            $scope.civilStatusNew.Sequence = data;
        })
    };
    $scope.GetSequence();
    ...
    $scope.Get = function (index) {
        $scope.index = index;
        $scope.civilStatus = $scope.civilStatuss[$scope.index];
        $scope.civilStatusNew = Object.assign({}, $scope.civilStatus);
        clearLanguage();
    ...
    $scope.Save = function () {
        ...
        if ($scope.civilStatusForm.$valid) {
            angular.copy($scope.civilStatusNew, $scope.civilStatus);
            if ($scope.Action === 'Save') {
                $http({
                    method: 'POST',
                    url: $scope.saveUrl,
                    data: {
                        'model': $scope.civilStatus
                    ...
                    if (response.data.Error === true) {
                        ...
                        $scope.civilStatuss.push(response.data.CivilStatus);
                        $scope.civilStatuss = $filter('orderBy')($scope.civilStatuss, 'Sequence');
                ...
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        ShowResult(response.data.Message, 'success');
                        if ($scope.index > -1) {
                            $scope.civilStatuss[$scope.index] = $scope.civilStatus;
                            $scope.civilStatuss = $filter('orderBy')($scope.civilStatuss, 'Sequence');
                        }
                        ClearFields(response.data.Sequence);
                    }
    ...
    $scope.Delete = function () {
        if (!baseService.isUndefinedOrNull($scope.civilStatusNew.Id)) {
            $http({
                method: 'POST',
                url: $scope.deleteUrl + $scope.civilStatusNew.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
                    ...
                    ShowResult(response.data.Message, 'success');
                    $scope.civilStatuss.splice($scope.index, 1);
    ...
    $scope.languageData = function () {
        $scope.languageDataUrl = 'Setups/LocalLanguage/GetCivilStatusLanguageList?civilStatusId=' + $scope.civilStatusNew.Id;
    ...
    function ClearFields(seq) {
        $scope.Action = 'Save';
        $scope.civilStatus = {};
        $scope.civilStatusNew = { Sequence: seq, Active: true };
        clearLanguage();
        $scope.languageDataList = [];
    }
