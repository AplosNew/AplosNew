'use strict';
QualificationLevelController.$inject = ['cboService', 'commonMessage', 'baseService', '$scope', '$rootScope', '$routeParams', '$location', '$http', '$filter'];
function QualificationLevelController(cboService, commonMessage, baseService, $scope, $rootScope, $routeParams, $location, $http, $filter) {
    $rootScope.title = "Qualification Level";
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.qualificationLevels = [];
    $scope.path = 'employees/qualificationlevel/';
    ...
    $scope.getData = function (pageno) {
        baseService.pagination(pageno)
            .then(function (result) {
                $scope.qualificationLevels = result.Rows;
            }, function () {
    ...
    $scope.qualificationLevelNew = Object.assign({}, $scope.qualificationLevel);
    ...
    $scope.GetSequence = function () {
        $http.get($scope.getSeqUrl)
            .then(function (response) {
                $scope.qualificationLevelNew.Sequence = response.data;
            });
    }
    $scope.GetSequence();
    ...
    $scope.Get = function (index) {
        $scope.index = index;
        $scope.qualificationLevel = $scope.qualificationLevels[$scope.index];
        $scope.qualificationLevelNew = Object.assign({}, $scope.qualificationLevel);
        clearLanguage();
    ...
    $scope.Save = function () {
        ...
        if ($scope.qualificationLevelForm.$valid) {
            angular.copy($scope.qualificationLevelNew, $scope.qualificationLevel);
            if ($scope.Action === 'Save') {
                $http({
                    method: 'POST',
                    url: $scope.saveUrl,
                    data: {
                        'qualificationLevel': $scope.qualificationLevel
                    ...
                    if (response.data.Error === true) {
                        ...
                        $scope.qualificationLevels.push(response.data.QualificationLevel);
                        $scope.qualificationLevels = $filter('orderBy')($scope.qualificationLevels, 'Sequence');
                ...
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        ShowResult(response.data.Message, 'success');
                        if ($scope.index > -1) {
                            $scope.qualificationLevels[$scope.index] = $scope.qualificationLevel;
                            $scope.qualificationLevels = $filter('orderBy')($scope.qualificationLevels, 'Sequence');
                        }
                        ClearFields(response.data.Sequence);
                    }
    ...
    $scope.Delete = function () {
        if (!baseService.isUndefinedOrNull($scope.qualificationLevelNew.Id)) {
            $http({
                method: 'POST',
                url: $scope.deleteUrl + $scope.qualificationLevelNew.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
                    ...
                    ShowResult(response.data.Message, 'success');
                    $scope.qualificationLevels.splice($scope.index, 1);
    ...
    $scope.languageData = function () {
        $scope.languageDataUrl = 'Setups/LocalLanguage/GetQualificationLabelInfoLanguageList?qualificationLabelInfoId=' + $scope.qualificationLevelNew.Id;
    ...
    function ClearFields(seq) {
        $scope.Action = 'Save';
        $scope.qualificationLevel = {};
        $scope.qualificationLevelNew = { Sequence: seq, Active: true };
        clearLanguage();
        $scope.languageDataList = [];
    }
