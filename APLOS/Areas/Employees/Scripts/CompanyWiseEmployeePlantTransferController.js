'use strict';
CompanyWiseEmployeePlantTransferController.$inject = ['fileReader', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', '$compile', '$window'];

function CompanyWiseEmployeePlantTransferController(fileReader, cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, $compile, $window) {
    $rootScope.title = 'Company Wise Employee Plant Transfer';
    $scope.path = 'Employees/CompanyWiseEmployeePlantTransfer/';
    ...
    $scope.IsOtherPlant = false;
    $scope.PlantId = null;
    $scope.CompanyId = null;
    $scope.plantListForOtherPlant = [];
    $scope.companyOnChangeForOtherPlant = function () {
        $scope.plantListForOtherPlant = [];
        cboService.getCboPlantByCompany($scope.CompanyId, function (result) {
            $scope.plantListForOtherPlant = result;
        });
    }
    $scope.PlantChangeForOtherPlant = function () {
        try {
            ...
            $http.get($scope.PlantWiseEmployeeDetailsUrl + '?EmployeeId=' + $scope.EmployeeModel.SystemId + '&PlantId=' + $scope.PlantId)
    ...
    $scope.IsOtherPlantOnChange = function () {
        if ($scope.IsOtherPlant==false) {
            $scope.CustomPara.JobLocationId = null;
            $scope.CustomPara.ShiftId = null;
        }
    ...
    $scope.SelectEmployee = function () {
        try {
            ...
            var eDialog = $("#dialogEmployeeInfo").data("ejDialog");
            eDialog.close();
            $scope.CustomPara.CompanyId = $scope.EmployeeModel.CompanyId;
    ...
    $scope.SaveData = function () {
        try {
            ...
            if (baseService.isUndefinedOrNull($scope.CustomPara.CompanyId)) {
