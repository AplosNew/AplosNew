'use strict';
EmployeeCategoryController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function EmployeeCategoryController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = 'Employee Category';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.employeeCategories = [];
    $scope.path = 'employees/employeecategory/';
    ...
    $scope.Delete = function () {
        if (!baseService.isUndefinedOrNull($scope.employeeCategoryNew.Id)) {
            $http({
                method: 'POST',
                url: $scope.deleteUrl + $scope.employeeCategoryNew.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
