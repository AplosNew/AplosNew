'use strict';
postRecruitmentDocumentByDepartmentController.$inject = ['fileReader', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function postRecruitmentDocumentByDepartmentController(fileReader, cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = 'Document by Department';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.path = 'employees/postrecruitmentdocumentbydepartment/';
    ...
    $scope.LoadData = function () {
        try {
            $scope.GLUrl3 = 'employees/postrecruitmentdocumentbydepartment/getlist',
                    ...
                    baseService.paginationBase($scope.GLUrl3, pageno, $scope.SelectionParameters)
                        .then(function (data) {
                            if (data.Error) return $scope.message = data.Message;
                                ...
                                $scope.postRecruitmentEmployees = data.Data.Rows;
    ...
    $scope.showDocumentPopUp = function (obj, index) {
        $scope.index = index;
        $http.get('employees/postrecruitmentdocumentbydepartment/getdocumentdata?companyGroupId=' + obj.GroupID + '&budgetId=' + obj.BudgetCode + '&plantId=' + obj.PlantId + '&pId=' + obj.SystemId)
            .then(function (response) {
                $scope.documentdataList = response.data;
                for (var i = 0; i < $scope.documentdataList.length; i++) {
                    $scope.documentdataList[i].EmpSystemId = obj.SystemId;
                }
            });
    ...
    $scope.showEntityPopUp = function () {
        $http.get('employees/postrecruitmentdocumentbydepartment/getentitybyemployee')
            .then(function (response) {
    ...
    $scope.SaveDocument = function () {
        try {
            ...
            if ($scope.Action === "Save") {
                $http({
                    method: 'POST',
                    url: 'employees/postrecruitmentdocumentbydepartment/createdocument',
    ...
    $scope.removeDoc = function () {
        ...
        $http({
            method: 'POST',
            url: 'employees/postrecruitmentdocumentbydepartment/deletedocument?Id=' + $scope.idd,
            dataType: 'JSON'
        }).then(function successCallback(response) {
            if (response.data.Error === true) {
