'use strict';
employeeExpenseBookingReportController.$inject = ['$scope', '$rootScope', '$http', '$filter', '$controller', 'baseService', "$window"];
function employeeExpenseBookingReportController($scope, $rootScope, $http, $filter, $controller, baseService, $window) {
    $rootScope.title = 'Employee Expenses';
    ...
    $scope.report = function () {
        if (baseService.isUndefinedOrNull($scope.ledgerReport.EmployeeId)) {
            ...
            var url = 'Employees/EmployeeReport/GetEmployeeExpenseBookingReport?reportFormat=' + $scope.ledgerReport.ReportFormat + '&fromDate=' + $scope.ledgerReport.FromDate + '&toDate=' + $scope.ledgerReport.ToDate + '&employeeId=' + $scope.ledgerReport.EmployeeId;
