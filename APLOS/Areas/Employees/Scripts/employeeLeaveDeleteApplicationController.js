'use strict';
employeeLeaveDeleteApplicationController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'cboService'];
function employeeLeaveDeleteApplicationController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService) {
    ...
    $scope.getLeaveYear = function () {
        $http.get('Employees/LeaveApplication/LoadYearlyCalendar')
    ...
    $scope.getData = function (pageno) {
        baseService.paginationBase('Employees/leaveApplication/GetEmpLeaveListForDelete?EmpsystemId=' + $scope.leaveApplicationNew.EmpSystemID + '&yearNo=' + $scope.YearNo, pageno, $scope.leaveParameters)
            .then(function (data) {
