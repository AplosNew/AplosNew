'use strict';
ExceptionForHolidayController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function ExceptionForHolidayController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = "Exception for Holiday";
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.hSNTaxPercentages = [];
    $scope.path = 'Employees/ExceptionForHoliday/';
    $scope.LoadExceptionForHolidayurl = $scope.path + 'LoadExceptionForHoliday';
    ...
    $scope.ExceptionForHolidayEmployee = [];

    $scope.loadExceptionForHoliday = function () {

        $http({
            method: 'POST',
            url: $scope.LoadExceptionForHolidayurl,
            data: { 'id': $scope.ExceptionForHoliday.Id },
            dataType: 'JSON'
            ...
            if (response.data.Error === true) {
                ...
                $scope.ExceptionForHoliday = response.data.master[0];
    ...
    $scope.treatment = function () {
        if ($scope.ExceptionForHoliday.ToDateTreatmentType == "W")
            $scope.ExceptionForHoliday.HolidayCategoryId = "";
    }
    $scope.ShowSave = function () {
        $scope.employeeSave = true;
        if (angular.isUndefinedOrNull($scope.ExceptionForHoliday.ForEntirePlant) == false) {
            if ($scope.ExceptionForHoliday.ForEntirePlant == true) {
    ...
    $scope.FutureDateAccepted = function () {
        $scope.minDate = $scope.ExceptionForHoliday.FromDate;
        if (Date.parse($scope.ExceptionForHoliday.ToDate) < Date.parse($scope.ExceptionForHoliday.FromDate))
            $scope.minDate = $scope.ExceptionForHoliday.ToDate;
        ...
        try {
            $http({
                ...
                data: {
                    ...
                    'offdate': $scope.ExceptionForHoliday.FromDate,
                    'ToDate': $scope.ExceptionForHoliday.ToDate,
    ...
    $scope.validations = function () {
        try {
            ...
            if (angular.isUndefinedOrNull($scope.ExceptionForHoliday.FromDate) == true)
                throw "Please select From Date";
            if (angular.isUndefinedOrNull($scope.ExceptionForHoliday.ToDate) == true)
    ...
    $scope.saveplantdata = function () {
        try {
            ...
            $http({
                ...
                url: $scope.saveurl,
                data: {
                    'masterdata': $scope.ExceptionForHoliday, 'employeedata': $scope.selectedemployees
                },
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error == true) {
    ...
    $scope.deleteEmployee = function (args) {
        try {
            ...
            $http({
                ...
                data: {
                    'masterdata': $scope.ExceptionForHoliday, 'employeedata': args.data.Id
                },
                dataType: 'JSON'
            }).then(function successCallback(response) {
    ...
    ////edit screen
    $scope.editdata = [];
    $scope.editListExceptionForHoliday = function () {
    ...
    $scope.editdoubleclick = function (args) {
        $scope.ExceptionForHoliday.Id = args.data.Id;
        $scope.loadExceptionForHoliday();
    ...
    $scope.Delete = function () {
        try {
            if (angular.isUndefinedOrNull($scope.ExceptionForHoliday.Id))
                throw 'Select compensation off policy first';
            $http({
                method: 'POST',
                url: $scope.deleteurl,
                data: {
                    'id': $scope.ExceptionForHoliday.Id
                },
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error == true) {
    ...
    $scope.Clear = function () {
        ...
        $scope.ExceptionForHolidayEmployee = [];
        $scope.selectedemployees = [];
        $scope.editListExceptionForHoliday();
        $scope.searchEmployee();
        $scope.employeeSave = true;
        $scope.Back();
    }
