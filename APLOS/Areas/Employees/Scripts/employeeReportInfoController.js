'use strict';
employeeReportInfoController.$inject = ['fileReader', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'cboService', '$window', 'toaster'];
function employeeReportInfoController(fileReader, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService, $window, toaster) {
    ...
    $scope.EmpReportPrint = function () {
        try {
            location.href = 'employees/EmployeeInformation/EmployeeInfoReport?employeeId=' + $scope.emp.Id;
            $scope.Clear();
        } catch (e) {
            ShowResult(e, 'Error');
        }
