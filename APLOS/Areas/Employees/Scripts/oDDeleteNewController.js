'use strict';
oDDeleteNewController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'cboService','$window'];
function oDDeleteNewController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService, $window) {
    $rootScope.title = 'OD Delete';
    $scope.index = -1;
    $scope.OdList = [];
    $scope.path = 'Employees/ODDeleteNew/';
...
// #endregion
    ...
    $scope.getData = function (PlantId, EmpSystemId) {
        $http({
            method: 'GET',
            url: $scope.path + '/Query?PlantId=' + PlantId + '&EmpSystemId=' + EmpSystemId
    ...
    $scope.Delete = function () {
        $http({
            method: 'POST',
            url: $scope.path+ '/Delete?id=' + $scope.employeeInformation.Id
        }).then(function successCallback(response) {
