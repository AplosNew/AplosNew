...
{
    public class ExceptionForHolidayController : BaseController
    {
        #region Constructor
        private readonly IUnitOfWork _unitOfWork;
        private readonly ISqlRepository _sqlRepository;
        public ExceptionForHolidayController(IUnitOfWork U, ISqlRepository R)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult Save(ExceptionForHoliday masterdata, List<ExceptionForHolidayEmpList> employeedata)
        {
            try
            {
        ...
        public JsonResult Delete(string id)
        {
            try
            {
                ...
                objCon.ExecuteNonQueryWrapper("delete FROM [MST].[ExceptionForHolidayEmpList] where ExceptionForHolidayId='" + id + "'", true, "1");
                objCon.ExecuteNonQueryWrapper("delete FROM [MST].[ExceptionForHoliday] where id='" + id + "'", true, "1");
        ...
        [HttpPost]
        public JsonResult deleteemployee(ExceptionForHoliday masterdata, string employeedata)
        {
            try
            {
                DataSet dsMaster;
                string sql = " SELECT * FROM [MST].[ExceptionForHolidayEmpList] WHERE ExceptionForHolidayId='" + masterdata.Id + "' AND EmpSystemId='" + employeedata + "'";
        ...
        #endregion Actions
        private void saveData(ExceptionForHoliday masterdata, List<ExceptionForHolidayEmpList> employeedata)
        {
            string EmpIdLoop = "''";
            if (employeedata != null)
            {

                foreach (ExceptionForHolidayEmpList item in employeedata)
                {
                    if (EmpIdLoop == "''")
                    {
                        EmpIdLoop = "'" + item.Id + "'"; ;
            ...
            {
                if (employeedata == null)
                    employeedata = new List<ExceptionForHolidayEmpList>();
                ...
                string sql = @"SELECT distinct c.EmpSystemId,EI.EmployeeCode FROM [MST].[ExceptionForHoliday] M
                            INNER JOIN [MST].[ExceptionForHolidayEmpList] C ON m.Id=c.ExceptionForHolidayId
                ...
                {
                    IEnumerable<ExceptionForHolidayEmpList> duplicate = employeedata.Where(ee => ee.Id == dsTemp.Tables[0].Rows[i]["EmpSystemId"].ToString());
                    if (duplicate != null)
                        foreach (ExceptionForHolidayEmpList item in duplicate)
                ...
                sql = "SELECT * FROM [MST].[ExceptionForHoliday] where FromDate='" + masterdata.FromDate + "' and plantid='" + identity.PlantId + "' and id<>'" + masterdata.Id + "'";
                objCon = new ConnectionManager.DAL.ConManager("1");
                objCon.OpenDataSetThroughAdapter(sql, out dsMaster, false, "1");
                ...
                sql = "SELECT * FROM [MST].[ExceptionForHoliday] where id='" + masterdata.Id + "'";
                objCon = new ConnectionManager.DAL.ConManager("1");
                objCon.OpenDataSetThroughAdapter(sql, out dsMaster, false, "1");

                sql = "SELECT * FROM [MST].[ExceptionForHolidayEmpList] where ExceptionForHolidayId='" + masterdata.Id + "'";
                objCon = new ConnectionManager.DAL.ConManager("1");
                ...
                else
                {
                    for (int i = 0; i < dsEmployee.Tables[0].Rows.Count; i++)
                    {
                        ...
                        IEnumerable<ExceptionForHolidayEmpList> filter = employeedata.Where(ee => ee.Id == dsEmployee.Tables[0].Rows[i]["EmpSystemId"].ToString());
                    ...
                    for (int i = 0; i < employeedata.Count; i++)
                    {
                        ...
                        if (dsEmployee.Tables[0].DefaultView.Count == 0)
                        {
                            while (FromDate <= ToDate)
                            {
                                ...
                                dr["ExceptionForHolidayId"] = MasterID;
        ...
        [HttpPost,Authorize]
        public JsonResult LoadExceptionForHoliday(string ID)
        {


            try
            {
                string sql = @"SELECT Id,Description, FORMAT( FromDate,'dd-MMM-yyyy') AS FromDate,FORMAT( ToDate,'dd-MMM-yyyy') AS ToDate FROM [MST].[ExceptionForHoliday] where ID='" + ID + "'";

                var masterData = _sqlRepository.GetModelCollection<ExceptionForHoliday>(sql);
                ...
                sql = @"SELECT distinct Emp.SystemID AS Id,
                                ...
                                case when isnull(O.Id,'')<>'' THEN 1 ELSE 0 END as Active,EMP.EmployeeName,EMP.EmployeeCode,EMP.EmpPicPath,
                                    EMP.BudgetCode,E.UserName EntityName,D.UserName Designation,
                                        ...
                                        INNER JOIN [MST].[ExceptionForHolidayEmpList] O ON EMP.SystemID=o.EmpSystemID 
                                        LEFT OUTER JOIN mst.ExceptionForHoliday AS co ON co.Id=o.ExceptionForHolidayId
                                        ...
                                        WHERE emp.PlantID='" + identity.PlantId + @"' AND O.ExceptionForHolidayId='" + ID + "'";
            ...
            string sql = @"select * from (SELECT distinct Emp.SystemID AS Id,format(EMP.DOJ,'dd-MMM-yyyy') AS DOJ," + joinedAfter + @"
                                case when isnull(O.Id,'')<>'' THEN 1 ELSE 0 END as Active,EMP.EmployeeName,EMP.EmployeeCode,EMP.EmpPicPath,
                                    EMP.BudgetCode,E.UserName EntityName,D.UserName Designation,
                                        ...
                                        ,PL.UserName Plant
                                        FROM EmployeeInformation EMP
                                        LEFT OUTER JOIN [MST].[ExceptionForHolidayEmpList] O ON EMP.SystemID=o.EmpSystemID and ExceptionForHolidayID=(
                                            select top 1 Id from [MST].[ExceptionForHoliday] O where PlantID='" + identity.PlantId + @"' and FromDate='" + offdate + @"')
                                        LEFT JOIN MST.ManpowerBudget PMB ON EMP.BudgetCode=PMB.Id
                                        LEFT JOIN ORG.Position PR ON PMB.PositionId=PR.Id
                                        LEFT JOIN ORG.Entity E ON PMB.EntityId=E.Id
                                        LEFT JOIN ORG.Section S ON S.Id=PR.SectionId
        ...
        public JsonResult GetView()
        {
            ...
            string sql = @"SELECT o.Description,
                            ...
                            FORMAT( o.ToDate,'dd-MMM-yyyy') AS ToDate
                             FROM [MST].[ExceptionForHoliday] O
    ...
    public class ExceptionForHoliday : BaseModel
    {
    ...
    public class ExceptionForHolidayEmpList : BaseModel
    {
        public string Id { get; set; } = "";
        public string ExceptionForHolidayId { get; set; } = "";
