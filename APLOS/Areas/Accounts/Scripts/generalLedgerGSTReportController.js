...
function generalLedgerGSTReportController($scope, $rootScope, $filter, accountService, $window, baseService) {
    ...
    $scope.getReport = function () {
        if (baseService.isUndefinedOrNull($scope.report.GLGeneralInfoId)) {
            ...
            var url = "";
            url = "Accounts/Voucher/GetGeneralLedgerGSTReport?reportFormat=" + $scope.report.ReportFormat + "&fromDate=" + $scope.report.FromDate + "&toDate=" + $scope.report.ToDate + "&glId=" + $scope.report.GLGeneralInfoId + '&active=' + $scope.report.Active + '&IsGroupBy=' + $scope.report.IsGroupBy;
