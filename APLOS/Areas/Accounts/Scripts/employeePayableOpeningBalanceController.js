'use strict';
employeePayableOpeningBalanceController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', '$controller'];
function employeePayableOpeningBalanceController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, $controller) {
    $rootScope.title = 'Employee Payable Opening Balance';
    $scope.url = 'accounts/OpeningBalance';
    $scope.listUrl = $scope.url + '/GetEmployeePayableList';
    $scope.saveUrl = $scope.url + '/InsertEmployeePayable';
    $scope.updateUrl = $scope.url + '/UpdateEmployeePayable';
    ...
    $scope.getTransactionTypeGL = function (id) {
        if (!baseService.isUndefinedOrNull(id)) {
            ...
            if (manualValidation('div_TransactionType', baseService.isUndefinedOrNull($scope.advanceCA.PayableGLId), 'Transaction Type GL not found!')) {
                ...
                && manualValidation('div_TransactionType', baseService.isUndefinedOrNull($scope.advanceCA.PayableBudgetMasterId), 'Transaction Type Budget not found!')) {
                $scope.advanceCA = null;
                $scope.openingBalanceDetailList = [];
            }
        }
