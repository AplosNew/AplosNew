"use strict";
customerInvoiceWriteOffController.$inject = ["bankService", "cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$routeParams", "$http", "$filter", "$controller"];
function customerInvoiceWriteOffController(bankService, cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $http, $filter, $controller) {
    $rootScope.title = "Customer Invoice Set-off";
    $scope.hideSource = true;
    $scope.url = "Accounts/Advance";
    $scope.listUrl = $scope.url + "/GetCustomerInvoiceWriteOffList";
    $scope.parkUrl = $scope.url + "/ParkCustomerInvoiceWriteOff";
    $scope.updateUrl = $scope.url + "/UpdateCustomerInvoiceWriteOff";
    $scope.postUrl = $scope.url + "/PostCustomerInvoiceWriteOff";
    $scope.reportUrl = $scope.url + "/ReportCustomerInvoiceWriteOff?voucherId=";
