customerInvoiceSettlementController.$inject = ["bankService", "cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$http", "$filter", "$controller"];
function customerInvoiceSettlementController(bankService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller) {
    $rootScope.title = "Customer Invoice Settlement";
    $scope.hideSource = true;
    $scope.url = "Accounts/Invoice";
    $scope.listUrl = $scope.url + "/GetCustomerInvoiceSettlementList";
    $scope.parkUrl = $scope.url + "/ParkCustomerInvoiceSettlement";
    $scope.updateUrl = $scope.url + "/UpdateCustomerInvoiceWriteOff";
    ...
    baseService.init($scope.listUrl, null, null, "DESC", "PostingDate DESC, VoucherNo", "PartyName");
        ...
        angular.forEach($scope.customerreceivableList, function (data, i) {
            if (data.Active === true) {
                ...
                if (getRow.length === 0) {
                    ...
                    data.VoucherDetailId = data.VoucherDetailId;
    ...
    $scope.exchangeGainLossAmount = function (data) {
        var balance = parseFloat(data.Balance), dramount = parseFloat(data.DrAmount);
        if (dramount > balance) {
    ...
    $scope.exchangeGainLossCal = function (rate) {
        for (var i = 0; i < $scope.voucherDetailList.length; i++) {
            if ($scope.voucherDetailList[i].CompanyCurrencyRate < rate) {
                $scope.voucherDetailList[i].ExchangeAmount = $scope.voucherDetailList[i].DrAmount * (rate - $scope.voucherDetailList[i].CompanyCurrencyRate);
                $scope.voucherDetailList[i].ExchangeType = "ExchangeLoss";
            }
            else if ($scope.voucherDetailList[i].CompanyCurrencyRate > rate) {
                $scope.voucherDetailList[i].ExchangeAmount = $scope.voucherDetailList[i].DrAmount * ($scope.voucherDetailList[i].CompanyCurrencyRate - rate);
    ...
    $scope.showCustomerAdvancePopUp = function (partyId, partyPlantId) {
        ...
        $scope.getCustomerAdvanceData = function (pageno) {
            baseService.paginationBase("Banks/BankJournal/GetAvilabeCustomerPaymentList", pageno, $scope.customerAdvanceParameters)
                .then(function (response) {
    ...
    $scope.closeCustomerAdvancePopUpSelected = function (data) {
        $scope.advance.BankJournalId = data.BankJournalId;
        $scope.advance.BankJournalDetailId = data.BankJournalDetailId;
        $scope.advance.VoucherDetailId = data.VoucherDetailId;
