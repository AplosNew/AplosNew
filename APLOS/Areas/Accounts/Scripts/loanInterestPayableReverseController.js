"use strict";
loanInterestPayableReverseController.$inject = ["accountService", "bankService", "cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$http", "$filter", "$controller"];
function loanInterestPayableReverseController(accountService, bankService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller) {
    $rootScope.title = "Loan Interest Payable Reverse";
    $scope.Action = "Save";
    $scope.index = -1;
    $scope.voucherList = [];
    ...
    $scope.deleteUrl = $scope.url + "/DeleteLoanInterestPayableReverse";
    $scope.postUrl = $scope.url + "/PostLoanInterestPayableReverse";
    ...
    $scope.voucher = {
        ...
        SettlementType:"InterestPayableReverse"
    };
    ...
    baseService.init("accounts/Loan/GetLoanInterestPayableReverseList", null, null, "DESC", "PostingDate DESC, VoucherNo", "VoucherNo");
    ...
    $scope.changeReverseType = function (type) {
        $scope.voucher.SettlementType = type;
    };
    ...
    $scope.Save = function () {
        ...
        if ($scope.form0.$valid && !$scope.invalidDocDate && !$scope.invalidPostingDate) {
            if ($scope.Action === "Save") {
                $http({
                    method: "POST",
                    url: "Accounts/Loan/CreateLoanInterestPayableReverse",
                    data: {
    ...
    $scope.Clear = function () {
        ...
        $scope.voucher.SettlementType = "InterestPayableReverse";
        $scope.currencyExchangeRate = [];
        $scope.getCboVoucherTypeLoanList();
