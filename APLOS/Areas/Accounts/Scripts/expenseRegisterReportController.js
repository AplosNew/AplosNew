'use strict';
expenseRegisterReportController.$inject = ['cboService', '$scope', '$rootScope', '$filter', "baseService", "$http", "$window"];
function expenseRegisterReportController(cboService, $scope, $rootScope, $filter, baseService, $http, $window) {
    $rootScope.title = 'Expense Register Report';
    $scope.entityList = {};
    $scope.report = {
    ...
    $scope.getReport = function () {
        ...
        if (baseService.isUndefinedOrNull($scope.report.FromDate)) {
            ...
            var url = 'Accounts/VoucherReport/GetExpenseRegisterReport?reportFormat=' + $scope.report.ReportFormat + '&fromdate=' + $scope.report.FromDate + '&todate=' + $scope.report.ToDate;
            $window.open(url, '_blank');
