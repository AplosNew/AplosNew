'use strict';
roundingGLController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter'];
function roundingGLController(cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter) {
    $rootScope.title = 'Rounding GL';
    $scope.Action = 'Save';
    ...
    $scope.showAll = function (str) {
        if (str === 'all') {
            ...
            $scope.url = 'accounts/FinancingType/GetRoundingGLAllList?coaId=' + $scope.debitNoteTypeGivenGL.COAId;
        }
        if (str === 'notassing') {
            if ($scope.debitNoteTypeGivenGL.COAId === null) {
                return ShowResult('Select COA first', 'failure');
            }
            $scope.url = 'accounts/FinancingType/GetRoundingGLNotAssingList?coaId=' + $scope.debitNoteTypeGivenGL.COAId;
        }
        if (str === 'assing') {
            if ($scope.debitNoteTypeGivenGL.COAId === null) {
                return ShowResult("Select COA first", 'failure');
            }
            $scope.url = 'accounts/FinancingType/GetRoundingGLAssingList?coaId=' + $scope.debitNoteTypeGivenGL.COAId;
        }
        $scope.debitNoteTypeGivenGLWithCombineList = [];
