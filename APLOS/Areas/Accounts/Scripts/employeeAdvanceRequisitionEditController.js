'use strict';
employeeAdvanceRequisitionEditController.$inject = ["cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$filter", "$window", "$http"];
function employeeAdvanceRequisitionEditController(cboService, commonMessage, $scope, $rootScope, baseService, $filter, $window, $http) {
    ...
    $scope.getData = function () {
        $http({
            ...
            url: $scope.path + "GetEmployeeAdvanceRequisitionForCheckList?column=" + $scope.searchCol + "&value=" + $scope.searchVal
    ...
    $scope.getCheckedByCheckedData = function () {
        $http({
            method: 'GET',
            data: { 'parameters': null },
            url: $scope.path + "GetCheckedByEmployeeCheckedDataList?column=" + $scope.searchCol + "&value=" + $scope.searchVal
        }).then(function successCallback(response) {
            $scope.CheckedDataList = response.data;
        });
    };

    $scope.CheckedByCheckedHoldDataList = [];
    $scope.getCheckedByCheckedHoldData = function () {
        $http({
            method: 'GET',
            data: { 'parameters': null },
            url: $scope.path + "GetCheckedByEmployeeCheckedHoldDataList?column=" + $scope.searchCol + "&value=" + $scope.searchVal
        }).then(function successCallback(response) {
            $scope.CheckedByCheckedHoldDataList = response.data;
        });
    };
   
    $scope.CheckedByCheckedRejectDataList = [];
    $scope.getCheckedByCheckedRejectData = function () {
        $http({
            method: 'GET',
            data: { 'parameters': null },
            url: $scope.path + "GetCheckedByEmployeeCheckedRejectDataList?column=" + $scope.searchCol + "&value=" + $scope.searchVal
        }).then(function successCallback(response) {
            $scope.CheckedByCheckedRejectDataList = response.data;
        });
    };
    ...
    function ValidationMaster() {
        try {
            ...
            if ($scope.model.ApprovedBy == $scope.model.CheckedBy) {
                throw "Approved By can not same person";
            }
    ...
    $scope.Save = function () {
        try {
            ...
            $http({
                ...
                url: $scope.path + "EmployeeAdvanceRequisitionCheck"
            }).then(function successCallback(response) {
    ...
    $scope.Reject = function () {
        try {
            ...
            $http({
                method: 'POST',
                data: { EmpAdvanceReqList: $scope.model },
                url: $scope.path + "EmployeeAdvanceRequisitionCheckedRejected"
            }).then(function successCallback(response) {
                if (response.data.Error == false) {
                    ShowResult(response.data.Message, 'success');
    ...
    $scope.Hold = function () {
        try {
            ...
            $http({
                method: 'POST',
                data: { EmpAdvanceReqList: $scope.model },
                url: $scope.path + "EmployeeAdvanceRequisitionCheckHold"
            }).then(function successCallback(response) {
                if (response.data.Error == false) {
                    ShowResult(response.data.Message, 'success');
