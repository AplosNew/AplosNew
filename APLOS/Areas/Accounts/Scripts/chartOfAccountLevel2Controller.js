'use strict';
ChartOfAccountLevel2Controller.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function ChartOfAccountLevel2Controller(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = 'Chart Of Account Level 2';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.chartOfAccountLevel2s = [];
    $scope.path = 'accounts/chartofaccountlevel2/';
    $scope.getListUrl = $scope.path + 'getchartofaccountlevel2list';
    ...
    $scope.Delete = function () {
        if (!baseService.isUndefinedOrNull($scope.chartOfAccountLevel2.Id)) {
            $http({
                method: 'POST',
                url: $scope.deleteUrl + $scope.chartOfAccountLevel2.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error == true) {
                    ShowResult(response.data.Message, 'failure');
