'use strict';
ChartAccountSetupController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function ChartAccountSetupController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    ...
    $scope.ActionLevel1s = 'Save';
    $scope.indexLevel1s = -1;
    $scope.chartOfAccountLevel1s = [];
    $scope.pathLevel1s = 'accounts/chartofaccountlevel1/';
    $scope.getListUrlLevel1s = $scope.pathLevel1s + 'getchartofaccountlevel1list';
    $scope.getUrlLevel1s = $scope.pathLevel1s + 'get';
    $scope.getSeqUrlLevel1s = $scope.pathLevel1s + 'getautosequence';
    $scope.saveUrlLevel1s = $scope.pathLevel1s + 'create';
    $scope.updateUrlLevel1s = $scope.pathLevel1s + 'edit';
    $scope.deleteUrlLevel1s = $scope.pathLevel1s + 'delete/';
    baseService.init($scope.getListUrlLevel1s);

    $scope.getDataLevel1s = function (pageno) {
    ...
    $scope.GetSequenceLevel1s = function () {
        $http.get($scope.getSeqUrlLevel1s)
            .then(function (response) {
                $scope.chartOfAccountLevel1.Sequence = response.data;
            });
    };

    $scope.CheckIdUseLevel1s = function (id) {
    ...
    $scope.GetSequenceLevel1s();

    $scope.GetLevel1s = function (id, index) {
        $scope.index = index;
        $scope.CheckIdUseLevel1s(id);
        ...
        $scope.ActionLevel1s = 'Update';
        if (!$rootScope.isCollapsed) {
            $rootScope.toggle();
        }
    };

    $scope.SaveLevel1s = function () {
        $scope.$broadcast('show-errors-check-validity');
        if ($scope.chartOfAccountLevel1Form.$valid) {
            if ($scope.ActionLevel1s === 'Save') {
                $http({
                    method: 'POST',
                    url: $scope.saveUrlLevel1s,
                    ...
                    if (response.data.Error == true) {
                        ...
                        ClearFieldsLevel1s(response.data.Sequence);
                    }
            ...
            else if ($scope.ActionLevel1s === 'Update') {
                $http({
                    method: 'POST',
                    url: $scope.updateUrlLevel1s,
                    data: $scope.chartOfAccountLevel1,
                    dataType: 'JSON'
                }).then(function successCallback(response) {
                    if (response.data.Error == true) {
    ...
    $scope.DeleteLevel1s = function () {
        if (!baseService.isUndefinedOrNull($scope.chartOfAccountLevel1.Id)) {
            $http({
                method: 'POST',
                url: $scope.deleteUrlLevel1s + $scope.chartOfAccountLevel1.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error == true) {
                    ShowResult(response.data.Message, 'failure');
    ...
    $scope.ClearLevel1s = function () {
        ClearFieldsLevel1s($scope.GetSequenceLevel1s());
        return true;
    };

    function ClearFieldsLevel1s(seq) {
        $scope.Action = 'Save';
        $scope.chartOfAccountLevel1 = {};
