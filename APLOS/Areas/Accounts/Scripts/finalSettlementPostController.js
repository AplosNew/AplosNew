"use strict";
finalSettlementPostController.$inject = ["cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$http", "$filter", "$controller", "$window"];
function finalSettlementPostController(cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller, $window) {
    $rootScope.title = "Final Settlement Posting";
    $scope.Action = "Park";
    $scope.CAction = "Add";
    ...
    $scope.Save = function () {
        if (baseService.isUndefinedOrNull($scope.voucher.EmployeeId)) {
    ...
    $scope.deleteUrl = "Accounts/SalaryDisbursement/DeleteFinalSettlementDisbursementVoucher";
