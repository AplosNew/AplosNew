"use strict";
salaryJournalController.$inject = ["accountService", "cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$http", "$filter", "$controller"];
function salaryJournalController(accountService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller) {
    $rootScope.title = "Salary Journal";
    $scope.Action = "Save";
    $scope.index = -1;
    ...
    $scope.postUrl = "accounts/voucher/PostSalaryJournal";
    $scope.deleteUrl = "accounts/voucher/DeleteSalaryJournal";
    ...
    baseService.init("Accounts/Voucher/GetSalaryJournalList", null, null, "DESC", "PostingDate DESC, VoucherNo", "VoucherNo");
    ...
    $scope.salaryJVTypeList = [];
    $scope.salaryJVType = function () {
        $http({
            method: 'GET',
            url: 'Enum/GetSalaryJVTypeEnumCbo/'
        }).then(function successCallback(response) {
            $scope.salaryJVTypeList = response.data;
    ...
    $scope.changeTransactionType = function () {
        ...
        $scope.voucherDetail.TrnType = row[0].TType;
        $scope.voucherDetail.DocDate = $filter("dateFiltering")($scope.voucher.DocDate);
        $scope.voucherDetail.DocRefNo = $scope.voucher.DocRefNo;
        $scope.voucherDetail.Narration = $scope.voucher.Narration;
        $scope.voucherDetail.EntityId = $scope.voucher.EntityId;
    ...
    $scope.GetemployeeReconGLList = function () {
        $scope.GLUrl2 = "Accounts/glitem/GetEmployeeReconGLBudgetActivity";
    ...
    $scope.closeEmployeeSelectionPopUp = function () {
        if (baseService.isUndefinedOrNull($scope.voucherDetail.EmployeeId)) {
            ShowResult("Please select Employee!", "failure", "employeeSelectionPopUp");
            return;
        }
        if (baseService.isUndefinedOrNull($scope.voucherDetail.ActivityId)) {
            ShowResult("Please select GL!", "failure", "employeeSelectionPopUp");
            return;
        }
        if ($scope.voucherDetail.TrnType==null) {
            ShowResult("Please select Type!", "failure", "employeeSelectionPopUp");
            return;
        }
        else {
            $scope.voucherDetail.CrAmount = null;
    ...
    $scope.Save = function () {
        ...
        if ($scope.form0.$valid) {
            if ($scope.Action === "Save") {
                $http({
                    method: "POST",
                    url: "accounts/voucher/ParkSalaryJournal",
                    data: {
