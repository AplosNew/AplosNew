"use strict";
employeePayableController.$inject = ["cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$http", "$filter", "$controller"];
function employeePayableController(cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller) {
    $rootScope.title = "Employee Payable";
    $scope.Action = "Save";
    $scope.CAction = "Add";
    ...
    $scope.getListUrl = $scope.path + "GetEmployeePayableList";
    $scope.saveUrl = $scope.path + "InsertEmployeePayable";
    ...
    $scope.GetExpensesBookingById = function (id) {
        $http({
            ...
            for (var i = 0; i < $scope.expensesBookingDetailList.length; i++) {
                ...
                $scope.setDrExchangeRate($scope.expensesBookingDetailList[i], $scope.voucher.DocRefNo);
    ...
    $scope.popUp = function () {
        ...
        $scope.GetCOAICodeListData = function (pageno) {
            baseService.paginationBase("Accounts/GLItem/GetExpenseTypeGLBudgetActivityList", pageno, $scope.glListParameters)
                .then(function (result) {
