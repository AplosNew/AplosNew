"use strict";
invoiceChargeWriteOffController.$inject = ["bankService", "cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$http", "$filter", "$controller"];
function invoiceChargeWriteOffController(bankService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller) {
    $rootScope.title = "Invoice Charge Set-off";
    ...
    $scope.showCustomerInvoicePopUp = function () {
        ...
        $scope.getCustomerInvoiceData = function (pageno) {
            baseService.paginationBase("accounts/CustomerInvoice/GetCustomerAvailableInvoiceNewList", pageno, $scope.customerInvoiceParameters)
                .then(function (response) {
