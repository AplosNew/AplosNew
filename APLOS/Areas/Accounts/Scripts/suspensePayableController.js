"use strict";
suspensePayableController.$inject = ["bankService", "cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$http", "$filter", "$controller"];
function suspensePayableController(bankService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller) {
    $rootScope.title = "Suspense Payable";
    $scope.Action = "Save";
    $scope.index = -1;
    $scope.voucherList = [];
    ...
    baseService.init("Accounts/Invoice/GetSuspensesPayableList", null, null, "DESC", "PostingDate DESC, VoucherNo", "VoucherNo");
    $scope.url = "Accounts/Invoice";
    $scope.postUrl = $scope.url + "/PostSuspensesPayable";
    ...
    $scope.getCboVoucherTypeBanksPaymentList = function () {
        cboService.getCboVoucherTypeSuspensePayableList(function (result) {
    ...
    $scope.calbankAmount = function (data) {
        ...
        else if ($scope.voucher.CurrencyId == $scope.BaseCurrencyId && data.BankCurrencyId != $scope.BaseCurrencyId) {
            data.BaseDrAmount = Math.abs(data.Amount * $scope.voucher.CompanyCurrencyRate).toFixed(2);
        }
        else
            data.BankAmount = '';
    ...
    $scope.calbankCurrecyAmount = function (data) {
        if ($scope.voucher.CurrencyId == data.BankCurrencyId == $scope.BaseCurrencyId) {
            ...
            data.BankAmount = data.BankAmount;
        }
        else
            data.BankAmount = '';
        $scope.calBaseAmount();
    ...
    $scope.Save = function () {
        ...
        if ($scope.form1.$valid && !$scope.validation() && !$scope.invalidDocDate && !$scope.invalidPostingDate) {
            if ($scope.Action === "Save") {
                $http({
                    method: "POST",
                    url: "accounts/Invoice/InsertSuspensesPayable",
                    data: {
    ...
    $scope.InvoiceGroupNo = null;
    $scope.confirmPost = function (InvoiceGroupNo) {
        $scope.InvoiceGroupNo = InvoiceGroupNo;
        $scope.message_confirmation = "Are you sure to Post?";
        angular.element(document.querySelector("#confirmPostPopUp")).modal("show");
    };

    $scope.post = function (InvoiceGroupNo) {
        $http({
            method: "POST",
            url: $scope.postUrl,
            data: {
