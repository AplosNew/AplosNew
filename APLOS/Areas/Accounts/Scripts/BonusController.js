...
function BonusController(commonMessage, $scope, $rootScope, baseService, $http, $filter, cboService) {
    ...
    //    $http.get('Accounts/Bonus/GetEmployeeInformation?from=' + $scope.BonusNew.FromDate + '&to=' + $scope.BonusNew.ToDate)
    ...
    $scope.GetEmployeeInformation = function () {
        $http({
            method: 'POST',
            url: 'Accounts/Bonus/GetEmployeeInformation',
            data: { 'from': $scope.BonusNew.FromDate, 'to': $scope.BonusNew.ToDate },
            dataType: 'JSON'
        }).then(function successCallback(response) {
            $scope.EmployeeListTemp = response.data;
        });
