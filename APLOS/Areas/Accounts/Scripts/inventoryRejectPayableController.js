'use strict';
inventoryRejectPayableController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', 'factoryService', '$window'];
function inventoryRejectPayableController(cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, factoryService, $window) {
    $rootScope.title = "Inventory Payable";
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.products = [];
    $scope.path = 'Accounts/InventoryPayable/';
    $scope.getListUrl = 'Accounts/InventoryPayable/GetInventoryReject/';
    $scope.saveUrl = 'Accounts/InvoicePost/CreateRejectPayable';
    ...
    $scope.GetShortageList = function () {
        ...
        $http({
            method: 'GET',
            url: 'Accounts/InventoryPayable/GetInventoryReject'
    ...
    $scope.shortageData = function () {
        $http.get('Products/InventoryReceive/GetListForInvRejectPayable')
            .then(function (response) {
            $scope.rejectList = response.data;
                angular.element(document.querySelector('#rejectpopUp')).modal('show');
        }), function (response) {
    ...
    $scope.closePopUp = function () {
        $scope.valueData = '';
        angular.element(document.querySelector('#rejectpopUp')).modal('hide');
    };

    function getInventoryMaterialList(inveReveiveId, employeeId, isReversCharge) {
        $http.get('Products/InventoryReceive/GetInventoryMaterialRejectPayable?inveReveiveId=' + inveReveiveId + '&employeeId=' + employeeId + '&isReversCharge=' + isReversCharge)
            .then(function (response) {
    ...
    function getRecievedList() {
        $http.get('Products/GoodsReceiveNote/GetInventoryRejectMaterialPayableList?inveReveiveId=' + $scope.modelNew.Id)
            .then(function (response) {
