'use strict';
interCompanyTransactionTakenOpeningBalanceController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', '$controller'];
function interCompanyTransactionTakenOpeningBalanceController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, $controller) {
    $rootScope.title = 'Inter Company Equity Opening Balance';
    ...
    baseService.init('accounts/OpeningBalance/GetInterCompanyTransactionTakenList', null, null, 'DESC', 'PostingDate', 'PostingDate');
        ...
        $http.get('accounts/OpeningBalance/GetOpeningBalanceDetailList?openingBalanceId=' + $scope.openingBalance.Id)
            .then(function (response) {
                ...
                angular.forEach($scope.openingBalanceDetailList, function (item, i) {
                    ...
                    item.CompanyCurrencyId = $scope.companyCurrencyId;
                    item.CompanyCurrencyName = $scope.companyCurrencyName;
                    item.CompanyFromCurrencyId = $scope.companyCurrencyId;
                    item.CompanyToCurrencyId = $scope.companyCurrencyId;
    ...
    $scope.Save = function () {
        ...
        if ($scope.form1.$valid && !$scope.invalidDocDate && !$scope.invalidRow && !invalidEntity) {
            if ($scope.Action === 'Update') {
                $http({
                    method: 'POST',
                    url: 'accounts/OpeningBalance/UpdateInterCompanyTransactionTaken',
                    data: {
