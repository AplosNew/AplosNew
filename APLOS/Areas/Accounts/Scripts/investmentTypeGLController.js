'use strict';
investmentTypeGLController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter'];
function investmentTypeGLController(cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter) {
    $rootScope.title = "Investment Type GL";
    $scope.Action = 'Save';
    $scope.btnActionAll = true;
    $scope.index = -1;
    $scope.investmentTypeGivenGLList = [];
    ...
    $scope.showAll = function (str) {
        if (str === 'all') {
            ...
            $scope.url = 'accounts/FinancingType/GetInvestmentTypeGLAllList?coaId=' + $scope.investmentTypeGivenGL.COAId;
        }
        if (str === 'notassing') {
            if ($scope.investmentTypeGivenGL.COAId === null) {
                return ShowResult('Select COA first', 'failure');
            }
            $scope.url = 'accounts/FinancingType/GetInvestmentTypeGLNotAssingList?coaId=' + $scope.investmentTypeGivenGL.COAId;
        }
        if (str === 'assing') {
            if ($scope.investmentTypeGivenGL.COAId === null) {
                return ShowResult("Select COA first", 'failure');
            }
            $scope.url = 'accounts/FinancingType/GetInvestmentTypeGLAssingList?coaId=' + $scope.investmentTypeGivenGL.COAId;
        }
        $scope.investmentTypeGivenGLWithCombineList = [];
    ...
    $scope.getLiabilityTypeList = function () {
        ...
        $scope.GLUrl1 = 'accounts/glitem/GetEquityLiabilityGLCOAWise?coaId=' + $scope.investmentTypeGivenGL.COAId;
