...
function creditNoteController(accountService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller, $window) {
    ...
    $scope.postUrl = $scope.url + "/PostCreditNote";
    ...
    baseService.init("Accounts/AdjustmentNote/GetCreditNoteList", null, null, "DESC", "PostingDate DESC, VoucherNo", "VoucherNo");
    ...
    cboService.getCboVoucherTypeCreditNoteList(function (result) {
    ...
    $scope.Save = function () {
        ...
        if ($scope.form0.$valid && !$scope.invalidDocDate && !$scope.invalidPostingDate && !$scope.validation()) {
            if ($scope.Action === "Save") {
                $http({
                    method: "POST",
                    url: "accounts/AdjustmentNote/InsertCreditNote",
                    data: {
                ...
                $http({
                    method: "POST",
                    url: "accounts/AdjustmentNote/UpdateCreditNote",
                    data: {
                        "voucherVM": $scope.voucher,
                        "voucherDetailVMList": $scope.invoiceSalesAvailableList,
                        "invoiceTaxVMList": $scope.invoiceTaxDetailList
   ...
   // $scope.changeVoucherType('hh');
    ...
    $scope.selectTDS = function () {
        ...
        if ($scope.TDS.Type == 'FixedPercentage' && !baseService.isUndefinedOrNull($scope.TDS.ValueOfFixed)) {
            $scope.TDS.TaxAmount = parseFloat($filter("sumByKey")($filter("filter")($scope.invoiceSalesAvailableList), "TotalTax") * $scope.TDS.ValueOfFixed / 100).toFixed(4);
        }
    }
    $scope.TDSList = [];
