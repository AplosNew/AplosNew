...
namespace Aplos.Areas.Accounts.Controllers
{
    public class AdvanceController : BaseController
    {
        ...
        [HttpPost]
        public JsonResult PostCustomerSuspense(string advanceId)
        {
            _advanceService.Post(advanceId);
            return Json(new { Message = AplosMessage.Success });
        }
        ...
        [HttpPost]
        public JsonResult PostVendorAdvance(string advanceId)
        {
            _advanceService.Post(advanceId);
            return Json(new { Message = AplosMessage.Updated });
        }
        ...
        [HttpPost]
        public JsonResult PostEmployeeAdvance(string advanceId)
        {
            _advanceService.Post(advanceId);
            return Json(new { Message = AplosMessage.Updated });
        }
        ...
        [HttpPost]
        public JsonResult InsertEmployeeAdvanceWriteOff(VoucherViewModel voucherVM, IEnumerable<VoucherDetailViewModel> voucherDetailList)
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            voucherVM.CompanyGroupId = identity.CompanyGroupId;
            voucherVM.CompanyId = identity.CompanyId;
        ...
        [HttpPost]
        public JsonResult PostInterTransaction(string advanceId)
        {
            _advanceService.Post(advanceId);
            return Json(new { Message = AplosMessage.Success });
        }
        ...
        [HttpPost, Authorize]
        public ActionResult EmployeeAdvanceDueListReportXls(List<Dictionary<string, object>> data, string reportFileName)
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            try
            {
                ...
                //IWorkbook workbook = EmployeeAdvanceDueReport(data, "", reportFileName);

                //string strFileName = "Employee Advance list.xlsx";
                //workbook.SaveAs(strFileName, ExcelSaveType.SaveAsXLS, System.Web.HttpContext.Current.Response, ExcelDownloadType.PromptDialog);
                //workbook.Close();

                string fileName = "";

                fileName = EmployeeAdvanceDueReport(data, "", reportFileName);
        ...
        private string EmployeeAdvanceDueReport(List<Dictionary<string, object>> data, string ReportHeader, string reportFileName)
        {
