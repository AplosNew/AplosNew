'use strict';
ApprovalController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function ApprovalController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = 'Sauda Approval';
    ...
    $http({
        method: 'GET',
        url: 'Farming/Approval/getlocation/',
    ...
    //    $scope.ICSMasterList = [];
        $http({
            method: 'GET',
            url: 'Farming/Approval/geticsmaster/',
        }).then(function successCallback(response) {
 ...
 //   }
    ...
    $http({
        method: 'GET',
        url: 'Farming/Approval/getcustomer/',
    }).then(function successCallback(response) {
    ...
    $scope.GetValidationDate = function () {
        ...
        $http({
            method: 'GET',
            url: 'Farming/Approval/getvalidationenumvalue/',
        }).then(function successCallback(response) {
            $scope.ValidationEnumValue = response.data;
            var VEnumValue = response.data[0].Text;           
        //  var valdate = selecteddate.setDate(selecteddate.getDate() - response.data[0].MaternityLeaveStartDay);
    ...
    $scope.getData = function () {
        $http({
            ...
            for (var i = 0; i < $scope.PurchaseBookingSodaList.length; i++) {
                ...
                if ($scope.PurchaseBookingSodaList[i].IsApproved == 1) {
                    $scope.PurchaseBookingSodaList[i].BookingStatus = "Approved";
 ...
 //       $scope.Action = 'Update';
    ...
    $scope.Save = function () {
        ...
        for (var i = 0; i < $scope.PurchaseBookingSodaList.length; i++) {
            if ($scope.PurchaseBookingSodaList[i].isSelected == true) {
                ...
                $scope.Approval.IsApproved = true;
                $scope.Approval.IsPayment = false;
            }
            //else {
    ...
    //    try {
            ...
            $http({
                method: 'POST',
                data: { HeaderConfirmedChildTabData: HeaderConfirmedData, IsApprovedData: $scope.Approval.IsApproved, IsPaymentData: $scope.Approval.IsPayment },
  ...
  //      BalanceBook: null,
    ...
    $scope.Approval = Object.assign({}, $scope.ApprovalModelTemp);
        ...
        var ConfirmedData = [];
            for (var i = 0; i < $scope.PBSChildList.length; i++) {
                if ($scope.PBSChildList[i].CheckBoxSelect == true) {
                    ...
                    $scope.PBSChildList[i].ApproveQuantity = 0;
                    $scope.PBSChildList[i].ApproveRate = 0;
                    checkedData.push($scope.PBSChildList[i]);
                    $scope.Approval.IsApproved = true;
                    $scope.Approval.IsPayment = false;
                }
            ...
            try {
                if (checkedData.length == 0) {
                    throw 'Please select at least one Approval Data';
                }
                $http({
                    method: 'POST',
                    data: { PBSChildTabData: checkedData, ConfirmedChildTabData: ConfirmedData, data: $scope.purchasebookingsodachild, PurchaseBookingSodaChildMasterId: $scope.PurchaseBookingSoda.Id, IsApprovedData: $scope.Approval.IsApproved, IsPaymentData: $scope.Approval.IsPayment },
