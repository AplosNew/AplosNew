'use strict';
PaymentController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function PaymentController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = 'Sauda Payment';
    ...
    $http({
        method: 'GET',
        url: 'Farming/Payment/getlocation/',
    ...
    //    $scope.ICSMasterList = [];
        $http({
            method: 'GET',
            url: 'Farming/Payment/geticsmaster/',
        }).then(function successCallback(response) {
 ...
 //   }
    ...
    $http({
        method: 'GET',
        url: 'Farming/Payment/getcustomer/',
    }).then(function successCallback(response) {
    ...
    $scope.GetValidationDate = function () {
        ...
        $http({
            method: 'GET',
            url: 'Farming/Payment/getvalidationenumvalue/',
        }).then(function successCallback(response) {
    ...
    $scope.getData = function () {
        $http({
            ...
            for (var i = 0; i < $scope.PurchaseBookingSodaList.length; i++) {
                ...
                if ($scope.PurchaseBookingSodaList[i].IsPayment == 1) {
                    $scope.PurchaseBookingSodaList[i].BookingStatus = "Payment Approved";
    ...
    $scope.Save = function () {
        ...
        for (var i = 0; i < $scope.PurchaseBookingSodaList.length; i++) {
            ...
            if (HeaderConfirmedData.length == 0) {
                throw 'Please select at least one Payment Data';
            }
            $http({
                method: 'POST',
                data: { HeaderConfirmedChildTabData: HeaderConfirmedData, IsPaymentData: $scope.Payment.IsPayment },
...
//      BalanceBook: null,
    ...
    $scope.Payment = Object.assign({}, $scope.PaymentModelTemp);
        ...
        var checkedData = [];
            ...
            for (var i = 0; i < $scope.PBSChildList.length; i++) {
                if ($scope.PBSChildList[i].CheckBoxSelect == true) {
                    ...
                    $scope.PBSChildList[i].PaidQuantity = 0;
                    $scope.PBSChildList[i].PaidRate = 0;
                    checkedData.push($scope.PBSChildList[i]);
                    $scope.Payment.IsPayment = true;
                }
            ...
            try {
                ...
                $http({
                    method: 'POST',
                    data: { PBSChildTabData: checkedData, PurchaseBookingSodaChildMasterId: $scope.PurchaseBookingSoda.Id, IsPaymentData: $scope.Payment.IsPayment },
