...
namespace Aplos.Areas.Farming.Controllers
{
    public class ConfirmationController : BaseController
    {
        ...
        public ConfirmationController(ISqlRepository R)
        {
            _sqlRepository = R;
        }
        ...
        public ActionResult GetList(string column, string value)
        {
            ...
            string sql = @"select top 100 * from (select distinct pbs.*,FORMAT(pbs.Date,'dd-MMM-yyyy') as BookingDate,FORMAT(pbs.ValidationDate,'dd-MMM-yyyy') as ValidateDate,CONVERT(varchar(5),pbs.[Time],108)[BookingTime],crl.UserName as Location,cp.UserName as CropPlanning,p.UserName as Customer,ics.Name as IcsMaster,ics.Id as ICSMasterID,fm.Id as Farmer,fm.FarmerName,fm.Id as FarmerFatherHusbandNameId,fm.FarmerFatherHusbandName,fm.Id as FarmerRegId,fm.FarmerRegistrationID as FarmerRegistration,0 BookDays,kk.BookingStatus                                                              
																...
																left join (
													select BookingStatus='To be Confirmed',PurchaseBookingSodaMasterId FROM TRN.PurchaseBookingSodaChild
													) kk on kk.PurchaseBookingSodaMasterId=pbs.Id
													where kk.PurchaseBookingSodaMasterId=pbs.Id and pbs.IsApproved=0) AS TEMP WHERE " + strkey + " order by Date desc ";

          return Json(_sqlRepository.GetDataCollection(sql, null), JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public JsonResult Create(IEnumerable<PurchaseBookingSodaChild> HeaderConfirmedChildTabData, string IsConfirmedData, string IsApprovedData)
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            try
            {
                ...
                foreach (var item in HeaderConfirmedChildTabData)
                {
                    ...
                    else
                    {
                        ...
                        for(int i=0;i<dsMaster.Tables[0].Rows.Count;i++)
                        {
                            ...
                            dr["ConfirmationQuantity"] = dsMaster.Tables[0].Rows[i]["Quantity"].ToString();
                            dr["ConfirmationRate"] = dsMaster.Tables[0].Rows[i]["Rate"].ToString();
                    ...
                    if (dsMaster1.Tables[0].Rows.Count > 0)
                    {
                        ...
                        dr.BeginEdit();
                        dr["IsConfirmed"] = IsConfirmedData;
        ...
        [HttpPost]
        public JsonResult SavePurchaseBookingSodaChild(Dictionary<string, object> data, IEnumerable<PurchaseBookingSodaChild> PBSChildTabData, IEnumerable<PurchaseBookingSodaChild> ConfirmedChildTabData, string PurchaseBookingSodaChildMasterId, string IsConfirmedData, string IsApprovedData)
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            try
            {
     ...
     //           con.OpenDataSetThroughAdapter("select * from " + TableName1 + " where Id='" + data["Id"] + "'", out dsMaster, false, "1");
                ...
                foreach (var item in PBSChildTabData)
                {
                    ...
                    else
                    {
                        ...
                        dr["ConfirmationQuantity"] = item.ConfirmedQuantity;
                        dr["ConfirmationRate"] = item.ConfirmedRate;
        ...
        //[HttpGet]
        //public JsonResult DeletePurchaseBookingSodaChild(string Id)
        //{
        //    try
        //    {
        //        string sql = @" delete from TRN.PurchaseBookingSodaChild where Id='" + Id + "'";
        ...
        //        {
        //            Error = false,
        //            Message = "Purchase Booking Soda Child deleted successfully"
        ...
        [HttpPost, Authorize]
        public ActionResult LoadAllPBSChildTabForSelection(string PurchaseBookingSodaMasterId)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(PurchaseBookingSodaMasterId))
                {
                    ...
                    if (dsMaster.Tables[0].Rows.Count > 0 && dsMaster.Tables[0].Rows[0]["IsConfirmed"].ToString() == "True")
                    {
                        sql = @"select distinct pbsc.*, cpc.Id as CPCId,cm.UserName as Crop,kk.TotalQuantity, 0 Active,cm.Id as CMId,ct.UserName as CropType,ct.Id as CTId,cpc.PlanQuantity,cpc.CropPlanningMasterId,dcr.Id as DCRId,dcr.TargetRate as TargetRatee,0 Amount,
                                                    ConfirmedQuantity=(pbsc.ConfirmationQuantity),ConfirmedRate=(pbsc.ConfirmationRate)
                                                   ,BalanceBook=cpc.PlanQuantity-kk.TotalQuantity,BalancePurchase=cpc.PlanQuantity-CQ.TotalConfirmedQuantity,pbsc.PurchaseBookingSodaMasterId,BookedAmount=pbsc.Quantity*pbsc.Rate
                    ...
                    {
                        sql = @"select distinct pbsc.*, cpc.Id as CPCId,cm.UserName as Crop,kk.TotalQuantity, 0 Active,cm.Id as CMId,ct.UserName as CropType,ct.Id as CTId,cpc.PlanQuantity,cpc.CropPlanningMasterId,dcr.Id as DCRId,dcr.TargetRate as TargetRatee,0 Amount,
                                                    ConfirmedQuantity=(pbsc.Quantity),ConfirmedRate=(pbsc.Rate)
                                                   ,BalanceBook=cpc.PlanQuantity-kk.TotalQuantity,BalancePurchase=cpc.PlanQuantity-CQ.TotalConfirmedQuantity,pbsc.PurchaseBookingSodaMasterId,BookedAmount=pbsc.Quantity*pbsc.Rate
                                                    from TRN.PurchaseBookingSodaChild pbsc
                                                    full join TRN.CropPlanningChild cpc on cpc.Id=pbsc.CropPlanningChildId
                                                    full join MST.CropMaster cm on cm.Id=cpc.CropId
