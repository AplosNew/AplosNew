...
namespace Aplos.Areas.Farming.Controllers
{
    public class ApprovalController : BaseController
    {
        ...
        public ApprovalController(ISqlRepository R)
        {
            _sqlRepository = R;
        }
        ...
        public ActionResult GetList(string column, string value)
        {
            ...
            string sql = @"select top 100 * from (select distinct pbs.*,FORMAT(pbs.Date,'dd-MMM-yyyy') as BookingDate,FORMAT(pbs.ValidationDate,'dd-MMM-yyyy') as ValidateDate,CONVERT(varchar(5),pbs.[Time],108)[BookingTime],crl.UserName as Location,cp.UserName as CropPlanning,p.UserName as Customer,ics.Name as IcsMaster,ics.Id as ICSMasterID,fm.Id as Farmer,fm.FarmerName,fm.Id as FarmerFatherHusbandNameId,fm.FarmerFatherHusbandName,fm.Id as FarmerRegId,fm.FarmerRegistrationID as FarmerRegistration,0 BookDays,kk.BookingStatus                                                              
																...
																left join (
													select BookingStatus='To be Approve',PurchaseBookingSodaMasterId FROM TRN.PurchaseBookingSodaChild
													) kk on kk.PurchaseBookingSodaMasterId=pbs.Id
													where kk.PurchaseBookingSodaMasterId=pbs.Id and pbs.IsConfirmed=1 and pbs.IsPayment=0) AS TEMP WHERE " + strkey + " order by Date desc ";

          return Json(_sqlRepository.GetDataCollection(sql, null), JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public JsonResult Create(IEnumerable<PurchaseBookingSodaChild> HeaderConfirmedChildTabData, string IsApprovedData, string IsPaymentData)
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            try
            {
                ...
                foreach (var item in HeaderConfirmedChildTabData)
                {
                    ...
                    else
                    {
                        ...
                        for(int i=0;i<dsMaster.Tables[0].Rows.Count;i++)
                        {
                            ...
                            dr["ApprovedQuantity"] = dsMaster.Tables[0].Rows[i]["ConfirmationQuantity"].ToString();
                            dr["ApprovedRate"] = dsMaster.Tables[0].Rows[i]["ConfirmationRate"].ToString();
                    ...
                    if (dsMaster1.Tables[0].Rows.Count > 0)
                    {
                        ...
                        dr.BeginEdit();
                        dr["IsApproved"] = IsApprovedData;
                        dr["ApprovalBy"] = identity.Name;
                        dr["ApprovalDate"] = System.DateTime.Now.ToString();
                        dr["ApprovalIP"] = identity.IPAddress;
        ...
        [HttpPost]
        public JsonResult SavePurchaseBookingSodaChild(Dictionary<string, object> data, IEnumerable<PurchaseBookingSodaChild> PBSChildTabData, IEnumerable<PurchaseBookingSodaChild> ConfirmedChildTabData, string PurchaseBookingSodaChildMasterId, string IsApprovedData, string IsPaymentData)
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            try
            {
                ...
                foreach (var item in PBSChildTabData)
                {
                    ...
                    else
                    {
                        ...
                        dr["ApprovedQuantity"] = item.ApproveQuantity;
                        dr["ApprovedRate"] = item.ApproveRate;
        ...
        public ActionResult LoadAllPBSChildTabForSelection(string PurchaseBookingSodaMasterId, string CropPlanningId)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(PurchaseBookingSodaMasterId))
                {
                    ...
                    if (dsMaster.Tables[0].Rows.Count > 0 && dsMaster.Tables[0].Rows[0]["IsApproved"].ToString() == "True")
                    {
                        sql = @"select distinct pbsc.*, cpc.Id as CPCId,cm.UserName as Crop,kk.TotalQuantity, 0 Active,cm.Id as CMId,ct.UserName as CropType,ct.Id as CTId,cpc.PlanQuantity,cpc.CropPlanningMasterId,dcr.Id as DCRId,dcr.TargetRate as TargetRatee,0 Amount,
                                                    ApproveQuantity=(pbsc.ApprovedQuantity),ApproveRate=(pbsc.ApprovedRate)
                                                   ,BalanceBook=cpc.PlanQuantity-kk.TotalQuantity,BalancePurchase=cpc.PlanQuantity-AQ.TotalApprovedQuantity,AQ.TotalApprovedQuantity,pbsc.PurchaseBookingSodaMasterId,ConfirmedAmount=pbsc.ConfirmationQuantity*pbsc.ConfirmationRate
													...
													left join (
													select SUM(ApprovedQuantity) as TotalApprovedQuantity,CropPlanningChildId FROM TRN.PurchaseBookingSodaChild group by CropPlanningChildId
													) AQ on AQ.CropPlanningChildId=cpc.id
                    ...
                    {
                          sql = @"select distinct pbsc.*, cpc.Id as CPCId,cm.UserName as Crop,kk.TotalQuantity, 0 Active,cm.Id as CMId,ct.UserName as CropType,ct.Id as CTId,cpc.PlanQuantity,cpc.CropPlanningMasterId,dcr.Id as DCRId,dcr.TargetRate as TargetRatee,0 Amount,
                                                    ApproveQuantity=(pbsc.ConfirmationQuantity),ApproveRate=(pbsc.ConfirmationRate)
                                                   ,BalanceBook=cpc.PlanQuantity-kk.TotalQuantity,BalancePurchase=cpc.PlanQuantity-AQ.TotalApprovedQuantity,AQ.TotalApprovedQuantity,pbsc.PurchaseBookingSodaMasterId,ConfirmedAmount=pbsc.ConfirmationQuantity*pbsc.ConfirmationRate
                                                    from TRN.PurchaseBookingSodaChild pbsc
                                                    full join TRN.CropPlanningChild cpc on cpc.Id=pbsc.CropPlanningChildId
                                                    full join MST.CropMaster cm on cm.Id=cpc.CropId
