...
{
    public class PurchaseOrderGroupController : BaseController
    {
        #region Constructor
        ...
        public PurchaseOrderGroupController(
        ...
        #endregion Aplos
        #region  Operations / PurchaseOrderGroupMethods
        ...
        {
            return Json(_purchaseOrderGroupMasterService.GetAutoSequence(), JsonRequestBehavior.AllowGet);
        }
        ...
        //{
        //    _purchaseOrderGroupMasterService.DeleteReqDetails(id);
        ...
        [HttpPost]
        public JsonResult Create(PurchaseOrderGroup entity)
        {
            ...
            try
            {
                ...
                _purchaseOrderGroupMasterService.Insert(entity);
            }
            catch (Exception ex)
            {
                throw new CustomException("PO Group Already Exist");
            }
            return Json(new { entity, Message = AplosMessage.Success + " PO Group No <b>" + entity.Id + "</b>" });
        }
        [HttpPost]
        public JsonResult EditPurchaseOrderGroup(PurchaseOrderGroup entity)
        {
            _purchaseOrderGroupMasterService.Update(entity);
            return Json(new { Message = AplosMessage.Updated });
        }
        [HttpPost]
        ...
        public ActionResult Delete(string id)
        {
            ...
            {
                _purchaseOrderGroupMasterService.DeletePOG(id);
        ...
        [HttpGet, Authorize]
        public ActionResult PurchaseOrderGroupReport(ReportFormat reportFormat, string plantId)
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            plantId = identity.PlantId;
            var reportFileName = "Purchase Order ";
            var workbook = _purchaseOrderGroupMasterService.CreatePurchaseOrderGroupReportSheet(identity.CompanyId, plantId);
            switch (reportFormat)
            {
        ...
        [Authorize, HttpGet]
        public JsonResult GetPurchaseOrderGroupGridData()
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            return Json(_purchaseOrderGroupMasterService.GetPurchaseOrderGroupGridData(), JsonRequestBehavior.AllowGet);
        }
        [Authorize, HttpGet]
        public JsonResult GetMaterialGridData()
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            var jsondata = Json(_purchaseOrderGroupDetailsService.GetMaterialGridData(), JsonRequestBehavior.AllowGet);
