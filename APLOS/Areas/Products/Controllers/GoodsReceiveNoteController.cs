        ...
        public ActionResult SaveDefault(IEnumerable<HttpPostedFileBase> UploadDefault, string UploadDefault_data)
        {
            try
            {
                ...
                foreach (var file in UploadDefault)
                {
                    ...
                    var fileName = Path.GetFileName(UploadDefault_data + new FileInfo(fn).Extension);
                    var destinationPath = Path.Combine(ResourcesPathReader.GetGRNImagePath(), fileName);

                    if (System.IO.Directory.Exists(ResourcesPathReader.GetGRNImagePath()) == false)
                    {
                        try
                        {
                            System.IO.Directory.CreateDirectory(ResourcesPathReader.GetGRNImagePath());
                        }
                        catch (Exception)
                        {
                    ...
                    string sql = "select* from trn.InventoryReceive where id='" + UploadDefault_data + "'";
                    ...
                    if (dsLocal.Tables[0].Rows.Count > 0)
                    {
                        ...
                        dsLocal.Tables[0].Rows[0]["FileName"] = fn;
