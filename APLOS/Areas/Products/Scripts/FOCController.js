'use strict';
FOCController.$inject = ['accountService', 'addressService', '$window', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', '$controller', '$location'];
function FOCController(accountService, addressService, $window, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller, $location) {
    $rootScope.title = "FOC";
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.products = [];
    ...
    $scope.GetSalesTaxData = function (salesId) {
        ...
        $http({
            ...
            for (var i = 0; i < $scope.inventoryMaterialList.length; i++) {
                ...
                //$scope.inventoryMaterialList[i].TransactionRate = $scope.TransactionRate;
                //$scope.inventoryMaterialList[i].TrnAmount = ($scope.TransactionRate * $scope.TransactionRate).toFixed(2);
                // $scope.inventoryMaterialList[i].TotalAmount = list;
                // $scope.inventoryMaterialList[i].TaxList = list;
            }
        });
    };
    ...
    $scope.getReceiveTaxList = function (data, flag, index, Id) {
        ...
        //$scope.BaseTaxAmount = data.BaseTaxAmounts;
        //$scope.BaseAmount = data.BaseAmount;
