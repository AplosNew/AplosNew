'use strict';
MaterialBudgetController.$inject = ['accountService', 'addressService', '$window', 'factoryService', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', '$controller'];
function MaterialBudgetController(accountService, addressService, $window, factoryService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller) {
    $rootScope.title = "Material Budget";
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.products = [];
    $scope.path = 'Products/MaterialBudget/';
    ...
    $scope.closeEmployeePopUp = function () {
        if ($scope.employeeIndex !== -1) {
            ...
            $scope.detailModel.EmployeeId = employee.SystemId;            
        }
    ...
    $scope.BudgetPeriodChange = function () {
        //debugger;
        if ($scope.detailModel.BudgetPeriod === 'Daily') {
            $scope.detailModel.ProcurementsPlanDay = '1';
        }
        if ($scope.detailModel.BudgetPeriod === 'Weekly') {
            $scope.detailModel.ProcurementsPlanDay = '7';
        }
        if ($scope.detailModel.BudgetPeriod === 'Monthly') {
            $scope.detailModel.ProcurementsPlanDay = '30';
        }
        if ($scope.detailModel.BudgetPeriod === 'Bi-Weekly') {
            $scope.detailModel.ProcurementsPlanDay = '14';
        }
        if ($scope.detailModel.BudgetPeriod === 'Quartely') {
            $scope.detailModel.ProcurementsPlanDay = '90';
        }
        if ($scope.detailModel.BudgetPeriod === 'Bi-Annualy') {
            $scope.detailModel.ProcurementsPlanDay = '180';
        }
        if ($scope.detailModel.BudgetPeriod === 'Annually') {
            $scope.detailModel.ProcurementsPlanDay = '365';
        }
        
        if ($scope.detailModel.BudgetPeriod === 'NA') {
            $scope.detailModel.ProcurementsPlanDay = '0';
        }
    }
    ...
    $scope.GetTerms = function (id) {
        $http({
            method: 'GET',
            url: 'Products/MaterialBudget/GetReqMaster?id=' + id
        }).then(function successCallback(response) {
            ...
            $scope.detailModel.Id = $scope.paymentTermList1[0].Id;
            $scope.detailModel.CompanyGroupId = $scope.paymentTermList1[0].CompanyGroupId;
            $scope.detailModel.EntityId = $scope.paymentTermList1[0].EntityId;
            $scope.detailModel.EmployeeId = $scope.paymentTermList1[0].EmployeeId;
            $scope.detailModel.EmployeeName = $scope.paymentTermList1[0].EmployeeName;
            $scope.detailModel.ActivityId = $scope.paymentTermList1[0].ActivityId;
            $scope.detailModel.BudgetMasterId = $scope.paymentTermList1[0].BudgetMasterId;
            $scope.detailModel.GLGeneralInfoId = $scope.paymentTermList1[0].GLGeneralInfoId;

            $scope.detailModel.MaterialMasterId = $scope.paymentTermList1[0].MaterialMasterId;
            $scope.detailModel.ArticleId = $scope.paymentTermList1[0].ArticleId;
            $scope.detailModel.FirstCharacteristicsId = $scope.paymentTermList1[0].FirstCharacteristicsId;
            $scope.detailModel.FirstCharacteristicsValueId = $scope.paymentTermList1[0].FirstCharacteristicsValueId;
            $scope.detailModel.SecondCharacteristicsId = $scope.paymentTermList1[0].SecondCharacteristicsId;
            $scope.detailModel.SecondCharacteristicsValueId = $scope.paymentTermList1[0].SecondCharacteristicsValueId;
            $scope.detailModel.ThirdCharacteristicsId = $scope.paymentTermList1[0].ThirdCharacteristicsId;
            $scope.detailModel.ThirdCharacteristicsValueId = $scope.paymentTermList1[0].ThirdCharacteristicsValueId;
            $scope.detailModel.BudgetPeriod = $scope.paymentTermList1[0].BudgetPeriod;
            $scope.detailModel.BudgetQty = $scope.paymentTermList1[0].BudgetQty;
            $scope.detailModel.ProcurementFrequencyCategory = $scope.paymentTermList1[0].ProcurementFrequencyCategory;
            $scope.detailModel.ProcurementsPlanDay = $scope.paymentTermList1[0].ProcurementsPlanDay;
            $scope.detailModel.Remarks = $scope.paymentTermList1[0].Remarks;

            $scope.detailModel.ActivityName = $scope.paymentTermList1[0].ActivityName
            $scope.detailModel.MaterialMasterName = $scope.paymentTermList1[0].StandardName
    ...
    $scope.GetReq = function () {
        ...
        $http({
            method: 'GET',
            url: 'Products/MaterialBudget/GetAllReqdata'
        }).then(function successCallback(response) {
