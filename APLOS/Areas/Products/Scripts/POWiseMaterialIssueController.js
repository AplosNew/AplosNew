'use strict';
POWiseMaterialIssueController.$inject = ['$window', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', '$controller'];
function POWiseMaterialIssueController($window, cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, $controller) {
	$rootScope.title = "PO Wise Material Issue";
	$scope.Action = 'Save';
	$scope.index = -1;
	$scope.products = [];
	...
	$scope.SaveSlipIssue = function () {
		if ($scope.IsPRChanges == true && baseService.isUndefinedOrNull($scope.productNew.Remarks)) {
			ShowResult("Please Input Remarks", 'failure');
			return false;
		}
		var sumOfmaterialStockList = $filter('sumByKey')($filter('filter')($scope.specificStockList), 'RequisitionQty');
		$scope.selectedRowQty1 = $filter('sumByKey')($filter('filter')($scope.detailList), 'TransactionQty');
	...
	$scope.recorddoubleclick = function ($event) {
		...
		//$scope.productNew.OrderSpecific = x.data.Orderspecific;
		$scope.productNew.OrderSpecific = 'Yes';
		$scope.productNew.ProductionOrderId = x.data.ProductionOrderId;
		$scope.productNew.SlipWisePRNo = x.data.ProductionOrderId;
