'use strict';
ServicePOByRequisitionController.$inject = ['accountService', 'addressService', '$window', 'factoryService', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', '$controller', 'fileReader'];
function ServicePOByRequisitionController(accountService, addressService, $window, factoryService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller, fileReader) {
    $rootScope.title = "Service PO By Requisition";
    $scope.Action = 'Save';
    $scope.Action1 = 'Save';
    $scope.index = -1;
    $scope.products = [];
    ...
    $scope.saveUrl = $scope.path + 'CreateServicePOByReq';
    ...
    $scope.detailSave = function () {
        ...
        try {
            ...
            for (var i = 0; i < $scope.GetServiceRequisitionList.length; i++) {
                if ($scope.GetServiceRequisitionList[i].Active === true && baseService.isUndefinedOrNull($scope.GetServiceRequisitionList[i].TransactionQty)) {
                    ShowResult('Enter the current qty', 'failure', 'ListOfRequisition');
                    return false;
                }
                if ($scope.GetServiceRequisitionList[i].Active === true && $scope.GetServiceRequisitionList[i].TransactionQty === 0) {
                    ShowResult('Enter the current qty', 'failure', 'ListOfRequisition');
                    return false;
                }
                    ...
                    if (((parseFloat($scope.GetServiceRequisitionList[i].OtherPOReceivedQty) + parseFloat($scope.GetServiceRequisitionList[i].TransactionQty)) > parseFloat($scope.GetServiceRequisitionList[i].Qty))) {
                        ...
                        $scope.GetServiceRequisitionList[i].Qty = $scope.GetServiceRequisitionList[i].TransactionQty;
    ...
    $scope.calculateAmount = function (data) {
        ...
        angular.forEach($scope.taxCategoryListNew, function (item) {
            if (item.Id === data.Id) {
                item.TaxAmount = data.Amount * item.Percentage / 100;
                $scope.receiveTaxList.TaxAmount = data.TotalTaxAmount;
            }

        });
        for (var i = 0; i < $scope.taxCategoryListNew.length; i++) {
            if (item.Id === data.Id) {
                data.TotalTaxAmount = $filter('sumByKey')($filter('filter')($scope.taxCategoryListNew), 'TaxAmount');
            }
        }
    ...
    $scope.qtychange = function (data) {
        ...
        if ($scope.taxCategoryListNew.length > 0 && currentRow.Active) {
            for (var i = 0; i < $scope.taxCategoryListNew.length; i++) {
                if ($scope.taxCategoryListNew[i].ServiceMasterId == currentRow.ServiceMasterId && $scope.taxCategoryListNew[i].ServiceRequsitionDetailId == currentRow.ServiceRequsitionDetailId && $scope.taxCategoryListNew[i].HSNCodeId == currentRow.HSNCodeId) {
                    $scope.taxCategoryListNew[i].TaxAmount = ((currentRow.TransactionQty * currentRow.TransactionRate) * $scope.taxCategoryListNew[i].Percentage) / 100;
                }
            }
        }
