...
InOutGatePassController.$inject = ['accountService', 'addressService', '$location', '$window', 'factoryService', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', '$controller'];
function InOutGatePassController(accountService, addressService, $location, $window, factoryService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller) {

	$rootScope.title = "In Out Gate Pass System";
	$scope.Action = 'Save';
	$scope.index = -1;
	$scope.products = [];
	$scope.path = 'Products/GateentryToken/';
	$scope.getListUrl = $scope.path + 'getlist';
	$scope.saveUrl = $scope.path + 'createInOutGatePass';
	$scope.updateUrl = $scope.path + 'createInOutGatePass';
	$scope.deleteUrl = $scope.path + 'DeleteInOutGatePass/';
	...
	$scope.GetTypeWiseData = function () {
		
		if ($scope.productNew.GatePassType === 'PurchaseReturn') {
	...
	$scope.getPurchaseReturnData = function () {
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetPurchaseReturnData'
		}).then(function (response) {
			$scope.TypeWiseDataList = response.data;
		});
		
	}
	$scope.getPurchaseReturnData();
	$scope.getMaterialTransferData = function () {
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetPurchaseReturnData'
		}).then(function (response) {
	...
	$scope.getInventorySalesData = function () {
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetPurchaseReturnData'
		}).then(function (response) {
	...
	$scope.getInventoryScrapData = function () {
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetPurchaseReturnData'
		}).then(function (response) {
	...
	$scope.getFixedAssetSalesData = function () {
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetPurchaseReturnData'
		}).then(function (response) {
	...
	$scope.getFixedAssetScrapData = function () {

	}


	$scope.GetTypeWiseDataDetails = function () {

		if ($scope.productNew.GatePassType === 'PurchaseReturn') {
			$scope.SelectType = 'Select Purchase Return';
			$scope.getPurchaseReturnDataDetails($scope.productNew.ChallanItemTypeId); 
		}
		else if ($scope.productNew.GatePassType === 'MaterialTransfer') {
			$scope.SelectType = 'Select Material Transfer';
			$scope.getMaterialTransferData();
	...
	$scope.getPurchaseReturnDataDetails = function (Id) {
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetPurchaseReturnMaterialDetails?Id='+Id
		}).then(function (response) {
			$scope.inventoryMaterialList = response.data;
		});

	}
	$scope.getMaterialTransferDataDetails = function () {
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetPurchaseReturnData'
		}).then(function (response) {
	...
	$scope.getInventorySalesDataDetails = function () {
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetPurchaseReturnData'
		}).then(function (response) {
	...
	$scope.getInventoryScrapDataDetails = function () {
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetPurchaseReturnData'
		}).then(function (response) {
	...
	$scope.getFixedAssetSalesDataDetails = function () {
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetPurchaseReturnData'
		}).then(function (response) {
	...
	$scope.getFixedAssetScrapDataDetails = function () {
			...
			if ($scope.productNewForm.$valid) {
				...
				if ($scope.Action === "Save") {
					$http({
						...
						url: $scope.saveUrl,
						data: {
							'inOutGatePassMasterModel': $scope.product
	...
	$scope.GetIndexGridDataList = function () {
		...
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetInOutGateIndexGridDataList?PendingApprvedGateOut=' + $scope.PendingApprvedGateOut
		}).then(function successCallback(response) {
			$scope.IndexGridDataList = response.data;
			for (var i = 0; i < $scope.IndexGridDataList.length; i++) {
				response.data[i].CreatedDate = new Date($scope.IndexGridDataList[i].CreatedDate);
			}
		});
	}
	//$scope.GetIndexGridDataList();
	...
	$scope.detailgridpurchaseReturn = function detailGridData(e) {
		...
		debugger;
		var filteredData = e.data["ComId"];
		if (e.data.GatePassFor === 'PurchaseReturn') {			
	...
	$scope.data1 = $scope.PurchaserReturn1st;
	$scope.detailTemppurchaseReturn1 = "#tabGridpurchaseContents1";
	//$scope.detailgrid = "detailGridData(e)";
	$scope.detailgridpurchaseReturn1 = function detailGridData(e) {
		...
		if (e.data.GatePassType === 'InventorySales') {
			var data = ej.DataManager(window.PurchaseReturnDetailsList).executeLocal(ej.Query().where("PurchaseReturnId", "equal", parseInt(filteredData), true).take(500));
			e.detailsElement.find("#detailGrid1st").ejGrid({
				dataSource: data,
				columns: ["MaterialName", "Article", "SKU1", "SKU2", "SKU3", "TransactionQty", "TransactionUoMId", "TransactionUoM", "TransactionRate", "TotalMaterialTranAmount"]
		...
		if (e.data.GatePassType === 'InventoryScrap') {
			var data = ej.DataManager(window.PurchaseReturnDetailsList).executeLocal(ej.Query().where("PurchaseReturnId", "equal", parseInt(filteredData), true).take(500));
			e.detailsElement.find("#detailGrid1st").ejGrid({
				dataSource: data,
				columns: ["MaterialName", "Article", "SKU1", "SKU2", "SKU3", "TransactionQty", "TransactionUoMId", "TransactionUoM", "TransactionRate", "TotalMaterialTranAmount"]
		...
		if (e.data.GatePassType === 'InventoryTransfer') {
			var data = ej.DataManager(window.PurchaseReturnDetailsList).executeLocal(ej.Query().where("PurchaseReturnId", "equal", parseInt(filteredData), true).take(500));
			e.detailsElement.find("#detailGrid1st").ejGrid({
				dataSource: data,
				columns: ["MaterialName", "Article", "SKU1", "SKU2", "SKU3", "TransactionQty", "TransactionUoMId", "TransactionUoM", "TransactionRate", "TotalMaterialTranAmount"]
	...
	$scope.recorddoubleclick1 = function ($event) {
		...
		$scope.productNew.GatePassEntryDate = $filter("dateFiltering")(Date.now())
		$scope.GetSenderName();
