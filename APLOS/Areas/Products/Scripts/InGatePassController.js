...
InGatePassController.$inject = ['accountService', 'addressService', '$location', '$window', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', '$controller'];
function InGatePassController(accountService, addressService, $location, $window, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller) {
	...
	$scope.setTabReqListEntry = function (newTab) {
		$scope.ReqStatus = 1;//'ForChecked';
		$scope.GetIndexGridDataListForInEntry();
		$scope.tab = newTab;
	};
	$scope.isSetReqListEntry = function (tabNum) {
	...
	$scope.GetIndexGridDataList = function () {
		...
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetIndexGridDataList?ReqStatus=' + $scope.ReqStatus + '&GateRegisterType='+'In'
		}).then(function successCallback(response) {
	...
	$scope.IndexGridDataListEntry = [];
	$scope.GetIndexGridDataListForInEntry = function () {
		//debugger;
		$http({
			method: 'GET',
			url: 'Products/GateentryToken/GetIndexGridDataListForInEntry?ReqStatus=' + $scope.ReqStatus + '&GateRegisterType=' + 'In'
		}).then(function successCallback(response) {
			$scope.IndexGridDataListEntry = response.data;
			for (var i = 0; i < $scope.IndexGridDataListEntry.length; i++) {
				response.data[i].GatePassEntryDate = new Date($scope.IndexGridDataListEntry[i].GatePassEntryDate);
			}
		});
	}
	$scope.GetIndexGridDataListForInEntry();
	...
	$scope.setTabGatePassHoldRejectDispatchList = function (newTab) {
		$scope.tab = newTab;
		$scope.tabType = 'HoldRejectDispatchList';
