'use strict';
qualityStdSetController.$inject = ['accountService', 'addressService', '$window', 'factoryService', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', '$controller'];
function qualityStdSetController(accountService, addressService, $window, factoryService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller) {

    $rootScope.title = "QualityStdSet";
    $scope.Action = 'Save';
    $scope.Action1 = 'Save';
    $scope.index = -1;
    $scope.products = [];
    $scope.path = 'Products/QualityStdSet/';
    $scope.getListUrl = $scope.path + 'getlist';
    $scope.saveUrl = $scope.path + 'Create';
    $scope.updateUrl = $scope.path + 'EditQualityStdSet';
    ...
    //$scope.Save = function () {
    //    //debugger;
    //    angular.copy($scope.productNew, $scope.model);
    //    $scope.$broadcast('show-errors-check-validity');
    //    try {
    //        if ($scope.productNewForm.$valid) {
    //            if ($scope.Action === 'Save') {
    ...
    //    //var aa = $scope.Id;
    //    $scope.plantList = [];
    //      $http.get('Products/PurchaseOrderGroup/GetVendorCbo?partyId=' + $scope.detailModel1.PartyId + '&Id=' + $scope.Id).then(function (response) {
    //        angular.forEach(response.data, function (item) {
    //            $scope.plantList.push(item);
    //            if (item.IsDefault) {
    //                $scope.productNew.InvoicingPartyPlantId = item.Value;
    ...
    $scope.GetReq = function () {
        ...
        $http({
            method: 'GET',
            url: 'Products/QualityStdSet/GetQualityStdSetGridData'
        }).then(function successCallback(response) {
            $scope.ReqList = response.data;
        });
    }
    ...
    //$scope.POGVendorList = [];
    //$scope.GetPOGVendor = function () {
    //    //debugger;
    //    $http({
    //        method: 'GET',
    //        url: 'Products/PurchaseOrderGroup/GetAllPOGVendor'
    //    }).then(function successCallback(response) {
    //        $scope.POGVendorList = response.data;
    //    });
    //}
