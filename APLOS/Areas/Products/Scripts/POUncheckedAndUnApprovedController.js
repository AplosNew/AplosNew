...
POUncheckedAndUnApprovedController.$inject = ['accountService', 'addressService', '$window', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', '$controller', '$location'];
function POUncheckedAndUnApprovedController(accountService, addressService, $window, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller, $location) {

	$rootScope.title = "PO Un check/approve";
	$scope.modelList = [];
	...
	$scope.GriddataPOCheckedist = [];
	$scope.getPOCheckedListData = function () {
		$http({
			method: "GET",
			dataType: 'JSON',
			url: 'Products/PurchaseOrder/getPOCheckedListData',
		}).then(function successCallback(response) {
			$scope.GriddataGRNCheckedist = response.data;

		});
	};
	$scope.getPOCheckedListData();

	$scope.GriddataPOApprovedList = [];
	$scope.getPOApprovedListData = function () {
		//debugger;
		$http({
			method: "GET",
			dataType: 'JSON',
			url: 'Products/PurchaseOrder/getPOApprovedListData',
		}).then(function successCallback(response) {
			$scope.GriddataPOApprovedList = response.data;

		});
	};
	$scope.getPOApprovedListData();
	//Muntasir
	...
	$scope.GRNUnChecked = function () {
		
		$http({
			method: 'POST',
			url: 'Products/PurchaseOrder/GRNUncheckUpdate',
			data:
			{
	...
	$scope.onClickGRNApprovedUpdate = function (z) {
		debugger;
		var x = "#" + z;
		var gridObj = $(x).data("ejGrid");
		$scope.GRNUncheckDataForUpdateNew = gridObj.getSelectedRecords()[0];
		$scope.url = $location.absUrl().split('!/')[1]
		$scope.message = 'Are you sure to UnApprove?';
		angular.element(document.querySelector('#poapprovalalert1')).modal('show');
	};

	$scope.GRNUnapprovedUpdate = function () {
		
		$http({
			method: 'POST',
			url: 'Products/PurchaseOrder/GRNUnapprovedUpdate',
			data:
			{
				'InventoryReceiveId': $scope.GRNUncheckDataForUpdateNew.Id,
				'UserSendData': $scope.GRNUncheckDataForUpdateNew
			},
			dataType: 'JSON'
		}).then(function successCallback(response) {
			if (response.data.Error === true) {
