'use strict';
PhysicalStockAdjustmentMasterController.$inject = ['$window', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', '$controller'];
function PhysicalStockAdjustmentMasterController($window, cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, $controller) {
    $rootScope.title = "Physical Stock Adjustment";
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.products = [];
    $scope.path = 'Products/InventoryIssue/';
    $scope.getListUrl = $scope.path + 'GetDataByInventoryIssue';
    $scope.saveUrl = $scope.path + 'InsertPhysicalStockAdjustment';
    ...
    $scope.getdataInventoryIssue = function () {
        ...
        $http({
            ...
            url: 'Products/InventoryIssue/GetDataByPhysicalStockAdjustment',
    ...
    $scope.AllTabPrint = function (z) {
        ...
        location.href = "Products/InventoryIssue/PhysicalStockAdjustmentReport?grnId=" + data.Id;
    ...
    $scope.Save = function () {
        ...
        if ($scope.specificStockList.length===0) {
            ShowResult('Please select the GRN');
            return false;
        }
  //      else {
  //          for (var i = 0; i < $scope.specificStockList.length; i++) {
  //              if ($scope.specificStockList[i].Flag === false) {
  //                  ShowResult('Please select the GRN');
  //                  return false;
  //              }
  //          }
    ...
    $scope.detailAdd = function () {
        ...
        try {
            ...
            if ($scope.detailModel.TransactionQty === 0)
                throw 'Please insert adjustment qty.';
            else {
                if ($scope.detailModel.TransactionUoMId === $scope.detailModel.BaseUOMId) {
                    if ($scope.detailModel.TransactionQty > parseFloat($scope.detailModel.TotalQty))
                        throw 'Adjustment qty must be less than or equal Total Qty.';
                    ...
                    if (tQty > parseFloat($scope.detailModel.TotalQty))
                        throw 'Adjustment qty must be less than or equal Total Qty.';
    ...
    function getMaterialStock() {
        ...
        $http({
            method: 'POST',
            url: $scope.path + 'GetStockForPhysicalStock',
    ...
    function getIssueDetailList() {
        $http.get($scope.path + 'GetAdjustmentDetailByIssueId?issueId=' + $scope.productNew.Id)
            .then(function (response) {
    ...
    $scope.getSpecificMaterialStock = function (data, index) {
        ...
        $http({
            method: 'POST'
            , url: $scope.path + 'GetSpecificMaterialStockForPhysicalStock'
            ...
            for (var i = 0; i < baseService.arrayLength($scope.specificStockList); i++) {
                ...
                for (var t = 0; t < baseService.arrayLength($scope.materialStockList1); t++) {
    ...
    $scope.addMaterialStock = function () {
        ...
        try {
            ...
            for (var n = 0; n < baseService.arrayLength($scope.materialStockList); n++) { // add
                ...
                if (nRow.RequisitionQty > nRow.BalanceStock) {
                    ShowResult('Adjustment Qty can not grater than balance qty', 'failure');
                    return false;
                }
                else {
    ...
    function validationWithTotal(list) {
        ...
        for (var i = 0; i < baseService.arrayLength(list); i++) {
            ...
            if (list[i].Flag) {
                if (parseFloat(list[i].RequisitionQty) === 0)
                    throw 'Please input Adjustment qty';
                else {
        ...
        if (totalQty > qty && qty !== totalQty) throw 'Adjustment qty can\'t over ' + qty + ' .';
        if (totalQty < qty && qty !== totalQty) throw 'Adjustment qty can\'t less ' + qty + ' .';
    ...
    //    $http({
    //        method: 'POST'
    //        , url: $scope.path + 'GetSpecificMaterialStockForAdjustment'
    //        , data: { entity: data, issueDate: $scope.productNew.IssueDate }
    //        , dataType: 'JSON'
    //    }).then(function (response) {
    //        $scope.materialStockList = response.data;
