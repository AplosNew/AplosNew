...
GatePassEmployeeController.$inject = ['accountService', 'addressService', '$location', '$window', 'factoryService', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', '$controller'];
function GatePassEmployeeController(accountService, addressService, $location, $window, factoryService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller) {
    ...
    $scope.GetGatePassdemandPersonDetails = function (args) {
        //debugger;
        $http({
            method: 'GET',
            url: 'Products/GateentryToken/GetDataForGatePass'
        }).then(function (response) {
            $scope.GetSenderNameList = response.data;
            $scope.productNew.EmployeeName = $scope.GetSenderNameList[0].EmployeeName;
            $scope.productNew.FromEmployeeId = $scope.GetSenderNameList[0].FromEmployeeId;
            $scope.productNew.PlantName = $scope.GetSenderNameList[0].PlantName;
            $scope.productNew.ToPlantId = $scope.GetSenderNameList[0].ToPlantId;
            $scope.productNew.UnitName = $scope.GetSenderNameList[0].UnitName;
            $scope.productNew.ToUnitId = $scope.GetSenderNameList[0].UnitId;
            $scope.productNew.DivisionName = $scope.GetSenderNameList[0].DivisionName;
            $scope.productNew.ToDivisionId = $scope.GetSenderNameList[0].DivisionId;
            $scope.productNew.DepartmentName = $scope.GetSenderNameList[0].DepartmentName;
            $scope.productNew.ToDepartmentId = $scope.GetSenderNameList[0].DepartmentId;
            $scope.productNew.SectionId = $scope.GetSenderNameList[0].SectionId;
            $scope.productNew.SebSectionName = $scope.GetSenderNameList[0].SebSectionName;
            $scope.productNew.SubSectionId = $scope.GetSenderNameList[0].SubSectionId;
            $scope.productNew.LineName = $scope.GetSenderNameList[0].LineName;
            $scope.productNew.LineId = $scope.GetSenderNameList[0].LineId;
            $scope.productNew.LegalDesignationName = $scope.GetSenderNameList[0].LegalDesignationName;
            $scope.productNew.LegalDesignationId = $scope.GetSenderNameList[0].LegalDesignationId;
            $scope.productNew.DesignationName = $scope.GetSenderNameList[0].DesignationName;
            $scope.productNew.DesignationId = $scope.GetSenderNameList[0].DesignationId;



        });
    }
    $scope.GetGatePassdemandPersonDetails();
    ...
    //    //$scope.restime = Math.abs($scope.productNew.FromTime.getTime() - $scope.productNew.TOTime.getTime());
    ...
    //    var res = Math.abs($scope.productNew.FromTime - $scope.productNew.TOTime) / 1000;
    ...
    //    alert("<br>Difference (Hours): " + hours + "<br>Difference (Minutes): " + minutes + "<br>Difference (Seconds): " + seconds);
    //}
    //$scope.timediff();
    $scope.Save = function () {
    ...
    $scope.GatePassCheckApproved = function () {
        ...
        if ($scope.url === 'gate-pass-checked') {
            ...
            else if ($scope.gatePassChkedAppData.ApprovedByStatus === "Approval") {
