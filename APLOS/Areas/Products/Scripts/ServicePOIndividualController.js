'use strict';
ServicePOIndividualController.$inject = ['accountService', 'addressService', '$window', 'factoryService', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', '$controller', 'fileReader'];
function ServicePOIndividualController(accountService, addressService, $window, factoryService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller, fileReader) {
	$rootScope.title = "Service PO";
	$scope.Action = 'Save';
	$scope.Action1 = 'Save';
	$scope.index = -1;
	$scope.products = [];
	...
	$scope.ContractWiseData = function (Id) {
		...
		$http({
			...
			$scope.productNew.ContractId = response.data[0].ContractId;
