"use strict";
printNonCashCheckController.$inject = ["commonMessage", "$scope", "$rootScope", "baseService", "$http", "$filter", "$window","bankService"];
function printNonCashCheckController(commonMessage, $scope, $rootScope, baseService, $http, $filter, $window, bankService) {
    $rootScope.title = "Print non cash Cheque";
    $scope.Action = "Save";
    $scope.index = -1;
    ...
    $scope.PrintNonCheckPermission = function () {
        $http({
            ...
            //    url: "Banks/CheckManagement/PrintCheckBeneficiaryPermision",
            //    data: {
            //        "data": $scope.printNonCashCheckNew
            //    },
            //    dataType: "JSON"
            //}).then(function successCallback(response) {
   ...
   // $scope.getPartyBankData();
    ...
    $scope.reportPrint = function () {
        ...
        //}
        //else {
        //    $scope.Party = $scope.printNonCashCheckNew.Party + "  "+  $scope.BankAccountNo;
    ...
    $scope.VoucherPrint = function () {
        try {
            ...
            if ($scope.printNonCashCheckNewForm.$valid) {
                ...
                try {
                    location.href = 'Banks/CheckManagement/GetPrintNonCashCheckVoucherReport?reportFormat=' + 'Pdf' + '&voucherId=' + $scope.printNonCashCheckNew.VoucherId + '&voucherDetailId=' + $scope.printNonCashCheckNew.VoucherDetailId; //voucherDetailId     $scope.printNonCashCheckNew.VoucherDetailId
                } catch (e) {''
                    ShowResult(e, "failure");
                }
            }
