"use strict";
rePrintCashCheckController.$inject = ["commonMessage", "$scope", "$rootScope", "baseService", "$http", "$filter", "$window"];
function rePrintCashCheckController(commonMessage, $scope, $rootScope, baseService, $http, $filter, $window) {
    $rootScope.title = "Re Print Cash Cheque";
    $scope.Action = "Save";
    $scope.index = -1;
    $scope.path = "accounts/voucher/";
    $scope.printpath = "Banks/CheckManagement/";
    $scope.getVoucherListUrl = $scope.path + "getvoucherlistforCashchequeReprinting/";
    ...
    function selectVoucherRow(data) {
        ...
        if (data.GLGeneralInfoId === null) {
            ...
            $scope.printNonCashCheckNew.CheckTamplate = data.CheckTemplate;
    ...
    $scope.reportCashCheckRePrint = function () {
        try {
            location.href = 'Banks/CheckManagement/GetRePrintCashCheckReport?voucherDetailId=' + $scope.printNonCashCheckNew.VoucherDetailId + '&checkDate=' + $scope.printNonCashCheckNew.CheckDate + '&checkTamplate=' + $scope.printNonCashCheckNew.CheckTamplate;
    ...
    //            $http({
    //                method: "GET",
    //                url: $scope.printpath + "NonCashCheckPrint?voucherDetailId=" + $scope.printNonCashCheckNew.VoucherDetailId + "&&checkLotDetailId=" + $scope.printNonCashCheckNew.CheckLotDetailId + "&&amount=" + $scope.printNonCashCheckNew.Amount + "&&bankCurrencyId=" + $scope.printNonCashCheckNew.BankCurrencyId + "&&checkDate=" + $scope.printNonCashCheckNew.CheckDate
    //                //data: {
    ...
    $scope.Print = function () {
        ...
        if ($scope.printNonCashCheckNewForm.$valid && !$scope.invalidDocDate) {
            $http({
                method: "POST",
                url: "Banks/CheckManagement/CashCheckRePrintReport",
                data: {
                    ...
                    , "checkTamplate": $scope.printNonCashCheckNew.CheckTamplate
                    //    //, "bankCurrency": $scope.printNonCashCheckNew.BankCurrency
                    //    //, "partyName": $scope.printNonCashCheckNew.PartyName
    ...
    //    $scope.$broadcast('show-errors-check-validity');
    //    //  ($scope.printNonCashCheckNewForm.$valid && !$scope.invalidDocDate)
    //    if ($scope.printNonCashCheckNewForm.$valid && !$scope.getReport()) {
    //        $http({
    //            method: "POST",
    //            url: "Banks/CheckManagement/NonCashCheckPrint",
    //            data: {
    //                //"voucherVM": $scope.voucher,
    //                //"voucherDetailVMList": $scope.voucherDetailList
    //                "voucherDetailId": $scope.printNonCashCheckNew.VoucherDetailId
    ...
    $scope.VoucherPrint = function () {
        try {
            ...
            if ($scope.printNonCashCheckNewForm.$valid) {
                ...
                try {
                    location.href = 'Banks/CheckManagement/GetRePrintCashCheckVoucherReport?reportFormat=' + 'Pdf' + '&voucherId=' + $scope.printNonCashCheckNew.VoucherId + '&voucherDetailId=' + $scope.printNonCashCheckNew.VoucherDetailId;
                } catch (e) {
                    ''
                    ShowResult(e, "failure");
                }
