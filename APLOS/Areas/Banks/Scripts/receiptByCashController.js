"use strict";
receiptByCashController.$inject = ["bankService", "accountService", "cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$http", "$filter", "$controller"];
function receiptByCashController(bankService, accountService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller) {
    $rootScope.title = "Receipt By Cash";
    $scope.Action = "Save";
    $scope.voucherList = [];
    $scope.index = -1;
    $scope.url = "Banks/CashJournal";
    $scope.listUrl = $scope.url + "/GetReceiptByCashList";
    $scope.parkUrl = $scope.url + "/InsertReceiptByCash";
    $scope.updateUrl = $scope.url + "/UpdateReceiptByCash";
    $scope.postUrl = $scope.url + "/PostReceiptByCash";
    ...
    $scope.voucher = {
        ...
        BankJournalType: "CashToCustomer",
        BankJournalId: null,
        IsReverse: null,
    ...
    $scope.getBankCashPaymentDetailList = function (id) {
        $http({
            method: "GET",
            url: "Banks/CashJournal/GetBankCashReceiptDetailList?bankJournalId=" + id
        }).then(function successCallback(response) {
    ...
    $scope.clear = function () {
        ...
        $scope.voucher.BankJournalType = "CashToCustomer";
    ...
    $scope.getCboVoucherTypeBankJournalList = function () {
        accountService.getCboVoucherTypeReceiptByCashList(function (result) {
    ...
    $scope.onCashChange = function (cashMasterId) {
        var bank = $.grep($scope.cashMasterList, function (item) {
