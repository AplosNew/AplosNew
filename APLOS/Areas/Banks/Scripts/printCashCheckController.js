"use strict";
printCashCheckController.$inject = ["commonMessage", "$scope", "$rootScope", "baseService", "$http", "$filter", "$window"];
function printCashCheckController(commonMessage, $scope, $rootScope, baseService, $http, $filter, $window) {
    $rootScope.title = "Print Cash Cheque";
    $scope.Action = "Save";
    $scope.index = -1;
    $scope.path = "accounts/voucher/";
    $scope.printpath = "Banks/CheckManagement/";
    $scope.getVoucherListUrl = $scope.path + "GetVoucherListForCashCheckPrinting/";//GetVoucherListForCashCheckPrinting
    ...
    function selectVoucherRow(data) {
        ...
        if (data.GLGeneralInfoId === null) {
            ...
            $scope.printNonCashCheckNew.Party ="CASH "+ data.Party;
            $scope.printNonCashCheckNew.PartyNew = data.Party;
            $scope.printNonCashCheckNew.BankMasterId = data.BankMasterId;
            $scope.printNonCashCheckNew.CashMasterId = data.CashMasterId;
    ...
    $scope.reportPrint = function () {
        try {
            location.href = 'Banks/CheckManagement/GetPrintCashCheckReport?voucherDetailId=' + $scope.printNonCashCheckNew.VoucherDetailId + '&checkDate=' + $scope.printNonCashCheckNew.CheckDate + '&checkTamplate=' + $scope.printNonCashCheckNew.CheckTemplate;
    ...
    $scope.Print = function () {
        ...
        if ($scope.printNonCashCheckNewForm.$valid && !$scope.invalidDocDate) {
            $http({
                method: "POST",
                url: "Banks/CheckManagement/CashCheckPrintReport",
                data: {
    ...
    $scope.VoucherPrint = function () {
        try {
            ...
            if ($scope.printNonCashCheckNewForm.$valid) {
                ...
                try {
                    location.href = 'Banks/CheckManagement/GetPrintCashCheckVoucherReport?reportFormat=' + 'Pdf' + '&voucherId=' + $scope.printNonCashCheckNew.VoucherId + '&bankMasterId=' + $scope.printNonCashCheckNew.BankMasterId + '&cashMasterId=' + $scope.printNonCashCheckNew.CashMasterId + '&voucherDetailId=' + $scope.printNonCashCheckNew.VoucherDetailId;
                } catch (e) {
                    ''
                    ShowResult(e, "failure");
                }
