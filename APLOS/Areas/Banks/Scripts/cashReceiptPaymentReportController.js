"use strict";
cashReceiptPaymentReportController.$inject = ["$scope", "$rootScope", "bankService", "$filter", "$window", "baseService"];
function cashReceiptPaymentReportController($scope, $rootScope, bankService, $filter, $window, baseService) {
    $rootScope.title = "Cash Receipt Payment";
    $scope.report = {
        CashMasterId: null,
        ReportFormat: "Pdf",
    ...
    $scope.getReport = function () {
        if (baseService.isUndefinedOrNull($scope.report.CashMasterId)) {
            ...
            var url = "Banks/CashReport/GetCashReceiptPayment?reportFormat=" + $scope.report.ReportFormat + "&fromDate=" + $scope.report.FromDate + "&toDate=" + $scope.report.ToDate + "&cashMasterId=" + $scope.report.CashMasterId;
