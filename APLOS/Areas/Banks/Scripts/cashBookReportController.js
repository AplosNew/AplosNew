"use strict";
cashBookReportController.$inject = ["$scope", "$rootScope", "bankService", "$filter", "$window", "baseService"];
function cashBookReportController($scope, $rootScope, bankService, $filter, $window, baseService) {
    $rootScope.title = "Cash Book";
    $scope.report = {
        CashMasterId: null,
        ReportFormat: "Pdf",
    ...
    $scope.getReport = function () {
        if (baseService.isUndefinedOrNull($scope.report.CashMasterId)) {
            ...
            var url = "Banks/CashReport/GetAdvanceCashBookReport?reportFormat=" + $scope.report.ReportFormat + "&fromDate=" + $scope.report.FromDate + "&toDate=" + $scope.report.ToDate + "&cashMasterId=" + $scope.report.CashMasterId;
