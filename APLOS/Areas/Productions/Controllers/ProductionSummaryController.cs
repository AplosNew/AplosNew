...
,E.UserName Entity,D.UserName DepartmentName,DM.DetentionUserName Detention,FORMAT(MMT.Date,'dd-MMM-yyyy')[Date],P.UserName Process
        ...
        [HttpPost, Authorize]
        public ActionResult GetItemWiseOrderReport(string fromDate, string toDate, string EntityId, string ProcessId)
        {
            try
            {
                string fileName = "";
                fileName = ItemWiseOrderReport(fromDate, toDate, "OrderReport", EntityId, ProcessId);
                return Json(new { FileName = fileName, Error = false }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        [HttpPost, Authorize]
        public ActionResult GetSOWiseOrderReport(string fromDate, string toDate, string EntityId, string ProcessId)
        {
            try
            {
                string fileName = "";
                fileName = SOWiseOrderReport(fromDate, toDate, "OrderReport", EntityId, ProcessId);
                return Json(new { FileName = fileName, Error = false }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
        ...
        public string ItemWiseOrderReport(string fromDate, string toDate, string SheetName, string EntityId, string ProcessId)
        {
            ...
            try
            {
                ...
                sheet = workbook.Worksheets[1];
                DataTable dtOrder;
                _productionSummaryData.ItemReportSQL(fromDate, toDate, Entity, processId, out dtOrder);
                ...
                for (int i = 0; i < dtOrder.Rows.Count; i++)
                {
                    ...
                    sheet[ROW, colWorkCenterMasterId].Text = dtOrder.Rows[i]["WorkCenterMasterId"].ToString();
                    sheet[ROW, colProductionOrderID].Text = dtOrder.Rows[i]["ProductionOrderID"].ToString();
                    sheet[ROW, colItemID].Text = dtOrder.Rows[i]["ItemID"].ToString();
                ...
                ReportUtility reportUtility = new ReportUtility();
                reportUtility.PlantHeader(ref sheet, endCol, "Order Report Item Wise", identity.PlantId);
                reportUtility.PageSetup(ref sheet, 6, ExcelPageOrientation.Landscape);
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignLeft;
                sheet.Range[1, 1, 6, endCol].HorizontalAlignment = ExcelHAlign.HAlignLeft;
                sheet.UsedRange.CellStyle.Font.FontName = "Arial Narrow";
        ...
        public string SOWiseOrderReport(string fromDate, string toDate, string SheetName, string EntityId, string ProcessId)
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            ExcelEngine excelEngine = null;
            IApplication application = null;
            ...
            try
            {
                ...
                sheet = workbook.Worksheets[1];
                DataTable dtOrder;
                _productionSummaryData.SOReportSQL(fromDate, toDate, Entity, processId, out dtOrder);
                ...
                ReportUtility reportUtility = new ReportUtility();
                reportUtility.PlantHeader(ref sheet, endCol, "Order Report SO Wise", identity.PlantId);
                reportUtility.PageSetup(ref sheet, 6, ExcelPageOrientation.Landscape);
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignLeft;
                sheet.Range[1, 1, 6, endCol].HorizontalAlignment = ExcelHAlign.HAlignLeft;
                sheet.UsedRange.CellStyle.Font.FontName = "Arial Narrow";
