...
namespace Aplos.Areas.Productions.Controllers
{
    public class ProductionRelayController : BaseController
    {
            ...
            try
            {
                ...
                for (int i = 0; i < ProductionRelayData.Count; i++)
                {
                    if (clsStaticInfo.nullrecorder(ProductionRelayData[i]["PPRId"]) != "")
                    {
                        if (bplib.clsWebLib.GetBoolData(ProductionRelayData[i]["IsCompleted"]) == false)
                        {
                            ...
                            //{ 
                            //    IncompleteProductionOrder = ProductionRelayData[i]["Id"];
                            //}
                            //else
                            //{
                            //    IncompleteProductionOrder += "," + ProductionRelayData[i]["Id"];
                ...
                for (int i = 0; i < ProductionRelayData.Count; i++)
                {
                    ...
                    if (dsProductionRelay.Tables[0].DefaultView.Count > 0)
                    {
                        ...
                        dr["IsCompleted"] = ProductionRelayData[i]["IsCompleted"];
        ...
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            string sql = @"SELECT convert(bit,0) AS Checked, pss.Id PSSId,ppr.Id PPRId, convert(bit ,isnull(ppr.IsCompleted,0)) AS IsCompleted,P.UserName PreviousProcess,PPR.CompletedBy ClosedBy,Format(PPR.CompletionEntryDate,'dd-MMM-yyyy') ClosedDate 
,  PO.Id,PO.EntityId, PO.Remarks,s.UserName AS ProductionStatus, EN.UserName AS EntityName, PS.UserName AS ProductionStatusName,
                        ISNULL(PO.Qty,0) AS POQuantity,ISNULL(PO.PlannedQty,0) AS SOQuantity,ISNULL(SO.Qty,0) AS SavedQuantity,so.Material, 

                                so.ProductCategory,Format( so.FirstShipmentDate,'dd-MMM-yyyy') FirstShipmentDate,so.Product,
                                Format(so.LastShipmentDate,'dd-MMM-yyyy') LastShipmentDate, so.article,PRODPR.ProductionQtyAtPR ,
							...
							) AS PRODPR ON  PRODPR.ProductionOrderId=po.id AND PRODPR.ProcessId=pss.ProcessId
                            ...
                            WHERE isnull(s.username,'') NOT IN ('CLOSED') AND  PO.entityid='" + EntityId + @"' and  PSS.ProcessId = '" + ProcessId + @"' and isnull(pss.IsCompleted,0)=0";
