...
{
    public class DefineSQCIssueController : BaseController
    {
        ...
        public DefineSQCIssueController(IProductionSummaryService ProductionSummaryService, ISqlRepository sqlRepository, IPKGeneratorService pkGeneratorService)
        {
        ...
        [Authorize, HttpGet]
        public JsonResult GetSQIssueNameList()
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;

            var sql = @"select Id as Value,UserName as Text from [MST].[SQCMaster]";

            return Json(_sqlRepository.GetDataCollection(sql), JsonRequestBehavior.AllowGet);
        }

        [Authorize, HttpGet]
        public JsonResult GetSQEntity(string IssueNameId)
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;

            var sql = @"select E.Id as Value,E.UserName as Text from MST.SQCEntity SQE
left join ORG.Entity E on E.Id=SQE.EntityId
where SQCID='" + IssueNameId + "'";

            return Json(_sqlRepository.GetDataCollection(sql), JsonRequestBehavior.AllowGet);
        }

        [Authorize, HttpGet]
        public JsonResult GetSQAQLLevelList()
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;

            var sql = @"select Id as Value,AQLLevel as Text from [MST].[SQCAQLMaster]";
        ...
        [HttpPost]
        public JsonResult createGeneralIssue(Dictionary<string, object> IssueData)
        {
            try
            {

                ConnectionManager.DAL.ConManager conRack = new ConnectionManager.DAL.ConManager("1");
                conRack.OpenDataSetThroughAdapter("select * from [MST].[SQGeneralIssueDetails] where IssueNameId='" + IssueData["IssueNameId"] + "' and EntityId='" + IssueData["EntityId"] + "'", out DataSet dsItemDetailsIssueNameValidation, false, "1");

                DataSet dsIssueDetails;

                conRack = new ConnectionManager.DAL.ConManager("1");
                conRack.OpenDataSetThroughAdapter("select * from [MST].[SQGeneralIssueDetails] where Id='" + IssueData["Id"] + "'", out dsIssueDetails, false, "1");
                string _Id = "";
                ...
                if (dsIssueDetails.Tables[0].Rows.Count == 0)
                {
                    ...
                    else
                    {
                        ...
                        genid.GenID("SQGeneralIssueDetails", out _Id);
                        _Id = "SQG" + _Id;
                        IssueData["Id"] = _Id;
        ...
        [Authorize, HttpGet]
        public ActionResult LoadGeneralIssueDetails()
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            string sql = @"select GID.*,
(select E.UserName from Org.Entity E where E.Id=GID.EntityId) as Entity,
(select AQL.AQLLevel from MST.SQCAQLMaster AQL where AQL.Id=GID.AQLLevelId) as AQLLevel,
(select SQM.UserName from MST.SQCMaster SQM where SQM.Id=GID.IssueNameId) as GIssueName
from [MST].[SQGeneralIssueDetails] GID";
            return Json(_sqlRepository.GetDataCollection(sql, null), JsonRequestBehavior.AllowGet);
        }

        [Authorize, HttpGet]
        public ActionResult LoadGeneralIssueDetailsEditData(string IssueId)
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
...
(select AQL.AQLLevel from MST.SQCAQLMaster AQL where AQL.Id=GID.AQLLevelId) as AQLLevel,
(select SQM.UserName from MST.SQCMaster SQM where SQM.Id=GID.IssueNameId) as GIssueName
from [MST].[SQGeneralIssueDetails] GID where GID.Id='" + IssueId + @"'";
            return Json(new { Issue = _sqlRepository.GetDataCollection(sql, null) }, JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public ActionResult GeneralIssueDelete(string id)
        {
            try
            {
                ConnectionManager.clsConnection conC = new ConnectionManager.clsConnection();
                conC.BeginTransaction();
                conC.executeQuery("delete from [MST].[SQGeneralIssueDetails] where Id ='" + id + @"'");
        ...
        [HttpPost]
        public JsonResult createGIResponsiblePerson(List<Dictionary<string, object>> GeneralIssueResponsibleData, string GeneralIssueId)
        {
            ConnectionManager.DAL.ConManager objCon;
            DataSet dsProdBooked;
            string TableName = "[MST].[SQGIResponsiblePerson]";
            ...
            try
            {
                ...
                if (GeneralIssueResponsibleData != null)
                {
                    ConnectionManager.clsConnection conC = new ConnectionManager.clsConnection();
                    conC.BeginTransaction();
                    conC.executeQuery("delete from " + TableName + " where GeneralIssueId='" + GeneralIssueId + "'");
                    conC.CommitTransaction();

                    foreach (var item in GeneralIssueResponsibleData)
                    {
        ...
        [HttpPost]
        public JsonResult createGIAuthorizedPerson(List<Dictionary<string, object>> GeneralIssueAuthorizedData, string GeneralIssueId)
        {
            ConnectionManager.DAL.ConManager objCon;
            DataSet dsProdBooked;
            string TableName = "[MST].[SQGIAuthorizedPerson]";
            string contId = string.Empty;
            string _Id, Id = string.Empty;
            try
            {
                ...
                if (GeneralIssueAuthorizedData != null)
                {
                    ConnectionManager.clsConnection conC = new ConnectionManager.clsConnection();
                    conC.BeginTransaction();
                    conC.executeQuery("delete from " + TableName + " where GeneralIssueId='" + GeneralIssueId + "'");
                    ...
                    foreach (var item in GeneralIssueAuthorizedData)
                    {
                        objCon.OpenDataSetThroughAdapter("SELECT * FROM " + TableName + "  where  Id='" + item["Id"] + "'", out dsProdBooked, false, "1");
                        DataView dv = new DataView(dsProdBooked.Tables[0]);
        ...
        public JsonResult GetIssueReasonList()
        {
            ...
            var sql = @"select IssueNameId as Value,(select QMM.UserName from MST.QualityManagementMaster QMM where QMM.Id=IssueNameId) as Text from [MST].[QualityIssueDetails]
        union 
        ...
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            string sql = @"select *,(select QMM.UserName from MST.QualityManagementMaster QMM  where QMM.Id=RD.IssueId) as IssueName from [MST].[QualityReasonDetails] RD";
            return Json(_sqlRepository.GetDataCollection(sql, null), JsonRequestBehavior.AllowGet);
        }
        ...
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            string sql = @"select QPD.*,QMM.UserName as IssueName,P.UserName Process,
        ...
        left join MST.QualityManagementMaster QMM on QMM.Id=QPD.IssueId";
            return Json(_sqlRepository.GetDataCollection(sql, null), JsonRequestBehavior.AllowGet);
        }
        ...
        from [MST].[POQualityPlanDetails] QPD
        left join HKP.Process P on P.Id=QPD.ProcessId
        left join MST.QualityManagementMaster QMM on QMM.Id=QPD.IssueId where QPD.Id='" + PQPId + @"'";
        ...
        public ActionResult LoadReasonDetailsEditData(string ReasonId)
        {
            ...
            string sql = @"select *,(select QMM.UserName from MST.QualityManagementMaster QMM  where QMM.Id=RD.IssueId) as IssueName from [MST].[QualityReasonDetails] RD where RD.Id='" + ReasonId + @"'";
        ...
        {
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            string sql = @"select *,(select QMM.UserName from MST.QualityManagementMaster QMM where QMM.Id=TD.IssueId) as IssueName,format(TD.FromTime,'hh:mm tt') as FTime,format(TD.ToTime,'hh:mm tt') as TTime from [MST].[QualityTimeDetails] TD";
            return Json(_sqlRepository.GetDataCollection(sql, null), JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult LoadTimeDetailsEditData(string TimeId)
        {
            ...
            string sql = @"select *,(select QMM.UserName from MST.QualityManagementMaster QMM where QMM.Id=TD.IssueId) as IssueName,format(TD.FromTime,'hh:mm tt') as FTime,format(TD.ToTime,'hh:mm tt') as TTime from [MST].[QualityTimeDetails] TD where TD.Id='" + TimeId + @"'";
        ...
        where QMM.Id  = '" + IssueId + "' order by QII.SNO";
            return Json(_sqlRepository.GetDataCollection(sql, null), JsonRequestBehavior.AllowGet);
        }
