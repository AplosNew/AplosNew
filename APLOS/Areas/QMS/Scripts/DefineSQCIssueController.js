'use strict';
DefineSQCIssueController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', '$window'];
function DefineSQCIssueController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, $window) {
    $rootScope.title = "Define SQC Issue";
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.productionSummaryes = [];
    $scope.path = 'QMS/DefineSQCIssue/';
    $scope.saveUrlIssue = $scope.path + 'createGeneralIssue';
    $scope.saveUrlGIResponsiblePerson = $scope.path + 'createGIResponsiblePerson';
    $scope.saveUrlGIAuthorizedPerson = $scope.path + 'createGIAuthorizedPerson';
    ...
    $scope.SQIEntityList = [];
    $scope.getSQIEntities = function (Id) {
        $http.get('QMS/DefineSQCIssue/GetSQEntity?IssueNameId=' + Id)
            .then(function (response) {
                if (baseService.arrayLength(response.data) > 0) {
                    $scope.SQIEntityList = response.data;
                    if (baseService.arrayLength(response.data) === 1) {
                        $scope.IssueNew.EntityId = $scope.SQIEntityList[0].Value;
                    }
                }
            });
    }
    ...
    $scope.SQIssueNameList = [];
    $scope.GetSQIssueNameList = function () {
        $http({
            method: 'GET',
            url: 'QMS/DefineSQCIssue/GetSQIssueNameList'
        }).then(function successCallback(response) {
            $scope.SQIssueNameList = response.data;
        });
    }
    $scope.GetSQIssueNameList();

    $scope.SQAQLLevelList = [];
    $scope.GetSQAQLLevelList = function () {
        $http({
            method: 'GET',
            url: 'QMS/DefineSQCIssue/GetSQAQLLevelList'
        }).then(function successCallback(response) {
            $scope.SQAQLLevelList = response.data;
        });
    }
    $scope.GetSQAQLLevelList();


    $scope.GeneralIssueList = [];
    $scope.LoadGeneralIssueDetails = function () {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadGeneralIssueDetails'
        }).then(function successCallback(response) {
            $scope.GeneralIssueList = response.data;
        }
        )
    }
    $scope.LoadGeneralIssueDetails();

    $scope.GeneralIssueSave = function () {
        $scope.$broadcast('show-errors-check-validity');
        if ($scope.GeneralIssueDetailsForm.$valid) {
            $http({
                method: 'POST',
    ...
    $scope.GetGeneralIssueDetails = function (args) {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadGeneralIssueDetailsEditData?IssueId=' + args.data.Id
        }).then(function successCallback(response) {
            $scope.IssueNew = response.data.Issue[0];
            $scope.getSQIEntities($scope.IssueNew.IssueNameId);
    ...
    $scope.removeGeneralIssueModal = function (index, data) {
        try {
            $scope.popUpIndex = index;
            $scope.tempIssueId = data;
            $scope.message_confirmation = "Are you sure you want to delete?";
            angular.element(document.querySelector('#confirmRemoveGeneralIssue')).modal('show');
        }
        catch (e) {
            ShowResult(e, 'Error');
        }
    };
    $scope.removeGeneralIssueRow = function () {
        $http({
            method: 'POST',
            url: 'QMS/DefineSQCIssue/GeneralIssueDelete?id=' + $scope.tempIssueId,
            dataType: 'JSON'
        }).then(function successCallback(response) {
            if (response.data.Error === true) {
                ShowResult(response.data.Message, 'failure');
    ...
    $scope.GeneralIssueClear = function () {
        GeneralIssueClearFields();
    };

    function GeneralIssueClearFields() {
        $scope.Action = "Save";
        $scope.IssueNew = Object.assign({}, $scope.Issue);
    }

    $scope.GeneralIssueId = null;
    $scope.GIResponsiblePersonList = [];
    $scope.getResponsiblePersonPopup = function (data) {
        $scope.NewObject = data.data;
        $scope.GeneralIssueId = $scope.NewObject.Id;
        $http({

            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadGIResponsiblePersonList?GeneralIssueId=' + $scope.GeneralIssueId + '&IssueId=' + $scope.NewObject.IssueNameId
        }).then(function successCallback(response) {
            $scope.GIResponsiblePersonList = response.data;
            var gridObj = $("#GridGIResponsiblePersonPopUp").data("ejGrid"); gridObj.refreshContent(); gridObj.refreshTemplate();
            angular.element(document.querySelector('#GIResponsiblePersonPopup')).modal('show');
        }
        )
    }

    $scope.closeGIResponsiblePersonPopUp = function () {
        angular.element(document.querySelector('#GIResponsiblePersonPopup')).modal('hide');
    }

    $scope.refreshTemplateGIResponsible = function (args) {
        $("#GIRheadchk").ejCheckBox({ "change": CheckBoxSelectAllGIResponsible });
    };
    function CheckBoxSelectAllGIResponsible(e) {
        var ChkOrUnchk = false;
        if (e.model.checkState === "check") {
            ChkOrUnchk = true;
        }

        var filtered = $("#GridGIResponsiblePersonPopUp").data("ejGrid").getFilteredRecords();
        if (angular.isUndefinedOrNull(filtered) || filtered.length == 0) {
            for (var i = 0; i < $scope.GIResponsiblePersonList.length; i++) {
                $scope.GIResponsiblePersonList[i].Flag = ChkOrUnchk;
            }
        }
        else {
    ...
    $scope.SaveGIResponsiblePerson = function () {
        try {
            $scope.SaveList = [];
            for (var i = 0; i < $scope.GIResponsiblePersonList.length; i++) {
                if ($scope.GIResponsiblePersonList[i].Flag == true) {
                    $scope.GIResponsiblePersonList[i].GeneralIssueId = $scope.GeneralIssueId;
                    $scope.SaveList.push($scope.GIResponsiblePersonList[i]);
                }
            }
            $http({
                method: 'POST',
                url: $scope.saveUrlGIResponsiblePerson,
                data: {
                    'GeneralIssueResponsibleData': $scope.SaveList,
                    'GeneralIssueId': $scope.GeneralIssueId
                },
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
    ...
    $scope.GIAuthorizedPersonList = [];
    $scope.getAuthorizedPersonPopup = function (data) {
        $scope.NewObject = data.data;
        $scope.GeneralIssueId = $scope.NewObject.Id;
        $http({

            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadGIAuthorizedPersonList?GeneralIssueId=' + $scope.GeneralIssueId + '&IssueId=' + $scope.NewObject.IssueNameId
        }).then(function successCallback(response) {
            $scope.GIAuthorizedPersonList = response.data;
            var gridObj = $("#GridGIAuthorizedPersonPopUp").data("ejGrid"); gridObj.refreshContent(); gridObj.refreshTemplate();
            angular.element(document.querySelector('#GIAuthorizedPersonPopup')).modal('show');
        }
        )
    }

    $scope.closeGIAuthorizedPersonPopUp = function () {
        angular.element(document.querySelector('#GIAuthorizedPersonPopup')).modal('hide');
    }

    $scope.refreshTemplateGIAuthorized = function (args) {
        $("#GIAheadchk").ejCheckBox({ "change": CheckBoxSelectAllGIAuthorized });
    };
    function CheckBoxSelectAllGIAuthorized(e) {
        var ChkOrUnchk = false;
        if (e.model.checkState === "check") {
            ChkOrUnchk = true;
        }

        var filtered = $("#GridGIAuthorizedPersonPopUp").data("ejGrid").getFilteredRecords();
        if (angular.isUndefinedOrNull(filtered) || filtered.length == 0) {
            for (var i = 0; i < $scope.GIAuthorizedPersonList.length; i++) {
                $scope.GIAuthorizedPersonList[i].Flag = ChkOrUnchk;
            }
        }
        else {
            for (var j = 0; j < filtered.length; j++) {
    ...
    $scope.SaveGIAuthorizedPerson = function () {
        try {
            $scope.SaveList = [];
            for (var i = 0; i < $scope.GIAuthorizedPersonList.length; i++) {
                if ($scope.GIAuthorizedPersonList[i].Flag == true) {
                    $scope.GIAuthorizedPersonList[i].GeneralIssueId = $scope.GeneralIssueId;
                    $scope.SaveList.push($scope.GIAuthorizedPersonList[i]);
                }
            }
            $http({
                method: 'POST',
                url: $scope.saveUrlGIAuthorizedPerson,
                data: {
                    'GeneralIssueAuthorizedData': $scope.SaveList,
                    'GeneralIssueId': $scope.GeneralIssueId
                },
                dataType: 'JSON'
            }).then(function successCallback(response) {
    ...
    $scope.GeneratItemSequenceNo = function () {
        $http({
            method: 'GET',
            url: 'QMS/DefineSQCIssue/GetItemAutoSequence'
        }).then(function successCallback(response) {
    ...
    $scope.GeneratGradeSequenceNo = function () {
        $http({
            method: 'GET',
            url: 'QMS/DefineSQCIssue/GetGradeAutoSequence'
        }).then(function successCallback(response) {
    ...
    $scope.GeneratActionToBeTakenSequenceNo = function () {
        $http({
            method: 'GET',
            url: 'QMS/DefineSQCIssue/GetActionToBeTakenAutoSequence'
        }).then(function successCallback(response) {
    ...
    $scope.GetProcessIssueList = function () {
        $http({
            method: 'GET',
            url: 'QMS/DefineSQCIssue/GetProcessIssueList'
        }).then(function successCallback(response) {
    ...
    $scope.GetIssueType = function (QId) {
        $http({
            method: 'GET',
            url: 'QMS/DefineSQCIssue/GetIssueType?IssueId=' + QId
        }).then(function successCallback(response) {
    ...
    $scope.GetChkInterval = function (QId) {
        $http({
            method: 'GET',
            url: 'QMS/DefineSQCIssue/GetChkInterval?IssueId=' + QId
        }).then(function successCallback(response) {
    ...
    $scope.LoadReasonDetails = function () {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadReasonDetails'
        }).then(function successCallback(response) {
    ...
    $scope.LoadPOQualityDetails = function () {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadPOQualityDetails'
        }).then(function successCallback(response) {
    ...
    $scope.GetGradeGridList = function () {
        $http({
            method: 'GET',
            url: 'QMS/DefineSQCIssue/GetGradeGridList'
        }).then(function successCallback(response) {
    ...
    $scope.LoadTimeDetails = function () {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadTimeDetails'
        }).then(function successCallback(response) {
    ...
    $scope.GetReasonDetails = function (args) {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadReasonDetailsEditData?ReasonId=' + args.data.Id
        }).then(function successCallback(response) {
    ...
    $scope.GetPOQualityDetails = function (args) {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadPOQualityDetailsEditData?PQPId=' + args.data.Id
        }).then(function successCallback(response) {
    ...
    $scope.GetTimeDetails = function (args) {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadTimeDetailsEditData?TimeId=' + args.data.Id
        }).then(function successCallback(response) {
    ...
    $scope.LoadTimeIssueListDetails = function () {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadTimeIssueDetails'
        }).then(function successCallback(response) {
    ...
    $scope.LoadIssueItemIssueListDetails = function () {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadIssueItemIssueDetails'
        }).then(function successCallback(response) {
    ...
    $scope.GetIssueReasonList = function () {
        $http({
            method: 'GET',
            url: 'QMS/DefineSQCIssue/GetIssueReasonList'
        }).then(function successCallback(response) {
    ...
    $scope.GetIssuePOQualityList = function () {
        $http({
            method: 'GET',
            url: 'QMS/DefineSQCIssue/GetIssuePOQualityList'
        }).then(function successCallback(response) {
    ...
    $scope.LoadIssueItemDetails = function (IssueId) {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadIssueItemDetails?IssueId=' + IssueId
        }).then(function successCallback(response) {
    ...
    $scope.GetIssueItemDetails = function (args) {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadIssueItemDetailsEditData?ItemId=' + args.data.Id
        }).then(function successCallback(response) {
    ...
    $scope.LoadGradeDetails = function () {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadGradeDetails'
        }).then(function successCallback(response) {
    ...
    $scope.GetGradeDetails = function (args) {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadGradeDetailsEditData?GradeId=' + args.data.Id
        }).then(function successCallback(response) {
    ...
    $scope.LoadActionToBeTakenDetails = function () {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadActionToBeTakenDetails'
        }).then(function successCallback(response) {
    ...
    $scope.GetActionToBeTakenDetails = function (args) {
        $http({
            method: 'Get',
            url: 'QMS/DefineSQCIssue/LoadActionToBeTakenDetailsEditData?ActionToBeTakenId=' + args.data.Id
        }).then(function successCallback(response) {
    ...
    $scope.removeReasonRow = function () {
        $http({
            method: 'POST',
            url: 'QMS/DefineSQCIssue/ReasonDelete?id=' + $scope.tempReasonId,
            dataType: 'JSON'
        }).then(function successCallback(response) {
            if (response.data.Error === true) {
                ShowResult(response.data.Message, 'failure');
    ...
    $scope.removePOQualityRow = function () {
        $http({
            method: 'POST',
            url: 'QMS/DefineSQCIssue/POQualityDelete?id=' + $scope.tempPOQualityId,
            dataType: 'JSON'
        }).then(function successCallback(response) {
            if (response.data.Error === true) {
                ShowResult(response.data.Message, 'failure');
    ...
    $scope.removeTimeRow = function () {
        $http({
            method: 'POST',
            url: 'QMS/DefineSQCIssue/TimeDelete?id=' + $scope.tempTimeId,
            dataType: 'JSON'
        }).then(function successCallback(response) {
            if (response.data.Error === true) {
                ShowResult(response.data.Message, 'failure');
    ...
    $scope.removeIssueItemRow = function () {
        $http({
            method: 'POST',
            url: 'QMS/DefineSQCIssue/IssueItemDelete?id=' + $scope.tempIssueItemId,
            dataType: 'JSON'
        }).then(function successCallback(response) {
            if (response.data.Error === true) {
                ShowResult(response.data.Message, 'failure');
    ...
    $scope.removeGradeRow = function () {
        $http({
            method: 'POST',
            url: 'QMS/DefineSQCIssue/GradeDelete?id=' + $scope.tempGradeId,
            dataType: 'JSON'
        }).then(function successCallback(response) {
            if (response.data.Error === true) {
                ShowResult(response.data.Message, 'failure');
    ...
    $scope.removeActionToBeTakenRow = function () {
        $http({
            method: 'POST',
            url: 'QMS/DefineSQCIssue/ActionToBeTakenDelete?id=' + $scope.tempActionToBeTakenId,
            dataType: 'JSON'
        }).then(function successCallback(response) {
            if (response.data.Error === true) {
                ShowResult(response.data.Message, 'failure');
    ...
    $scope.getReasonValuePopup = function (data) {
        ...
        $http({
            ...
            method: 'Get',
            url: 'QMS/productionSummary/LoadProcessReasonList?ProcessId=' + $scope.productionSummaryNew.ProcessId + '&ProductionId=' + $scope.ProductionId
        }).then(function successCallback(response) {
    ...
    $scope.getQIProcess = function (Id) {
        $http.get('QMS/DefineSQCIssue/GetProcess?IssueNameId=' + Id)
            .then(function (response) {
                if (baseService.arrayLength(response.data) > 0) {
    ...
    $scope.getLotNumberCbo = function () {
        try {
            $http.get('QMS/Productionsummary/GetLotNumberCbo?SalesOrderId=' + $scope.productionSummaryNew.SalesOrderId + '&ProductionOrderId=' + $scope.productionSummaryNew.ProductionOrderId + '&ProcessId=' + $scope.productionSummaryNew.ProcessId + '&productionLevel=' + $scope.productionSummaryNew.ProductionBookingLevel)
                .then(function (response) {
    ...
    $scope.getProdQty = function () {
        try {
            $scope.ProdQtyCount = 0;
            $http.get('QMS/Productionsummary/GetTotalProductionQty?wcid=' + $scope.productionSummaryNew.WorkCenterMasterId + '&workdate=' + $scope.productionSummaryNew.ProductionDate)
                .then(function (response) {
                    $scope.ProdQtyCount = 0;
    ...
    $scope.GetTotalProductionBookingQty = function () {
        try {
            ...
            if ($scope.NewObject.BookingLevel === 'ProductionOrder') {
                ...
                $http.get('QMS/Productionsummary/GetPOQty?productionOrderId=' + $scope.NewObject.ProductionOrderId + '&processId=' + $scope.productionSummaryNew.ProcessId)
                    .then(function (response) {
                        if (baseService.arrayLength(response.data) > 0) {
            ...
            //    }
            //    $http.get('QMS/Productionsummary/GetTotalSOQty?salesOrderId=' + $scope.productionSummaryNew.SalesOrderId + '&processId=' + $scope.productionSummaryNew.ProcessId)
            //        .then(function (response) {
            //            if (baseService.arrayLength(response.data) > 0) {
            //                $scope.TotalSalesOrderQty = parseFloat(response.data[0].PlannedQty).toFixed(2);
            //                $scope.RemainQty = parseFloat(response.data[0].RemainingQty).toFixed(2);
            ...
            //    }
            //    $http.get('QMS/Productionsummary/GetTotalMOIQty?MasterOrderItemId=' + $scope.NewObject.MasterOrderItemId + '&processId=' + $scope.productionSummaryNew.ProcessId)
            //        .then(function (response) {
            //            if (baseService.arrayLength(response.data) > 0) {
            //                $scope.TotalSalesOrderQty = parseFloat(response.data[0].PlannedQty).toFixed(2);
            //                $scope.RemainQty = parseFloat(response.data[0].RemainingQty).toFixed(2);
    ...
    $scope.GetMasterOrderItemQty = function () {
        try {
            ...
            if ($scope.NewobjectMOI.BookingLevel === 'MasterOrderItem' || $scope.NewobjectMOI.BookingLevel === 'ProductionCode') {
                ...
                $http.get('QMS/Productionsummary/GetTotalMOIQty?POId=' + $scope.NewobjectMOI.ProductionOrderId + '&MasterOrderItemId=' + $scope.NewobjectMOI.MasterOrderItemId + '&processId=' + $scope.productionSummaryNew.ProcessId)
                    .then(function (response) {
                        if (baseService.arrayLength(response.data) > 0) {
                            $scope.TotalSalesOrderQty = parseFloat(response.data[0].PlannedQty).toFixed(2);
                            $scope.RemainQty = parseFloat(response.data[0].RemainingQty).toFixed(2);
    ...
    $scope.GetProductCodeItemQty = function () {
        try {
            ...
            if ($scope.NewobjectPC.BookingLevel === 'ProductCode') {
                ...
                $http.get('QMS/Productionsummary/GetTotalPCQty?POId=' + $scope.NewobjectPC.ProductionOrderId + '&MasterOrderItemId=' + $scope.NewobjectPC.MasterOrderItemId + '&processId=' + $scope.productionSummaryNew.ProcessId)
                    .then(function (response) {
                        if (baseService.arrayLength(response.data) > 0) {
                            $scope.TotalSalesOrderQty = parseFloat(response.data[0].PlannedQty).toFixed(2);
                            $scope.RemainQty = parseFloat(response.data[0].RemainingQty).toFixed(2);
    ...
    $scope.GetSalesOrderItemQty = function () {
        try {
            ...
            if ($scope.NewobjectSO.BookingLevel === 'SalesOrder') {
                ...
                $http.get('QMS/Productionsummary/GetTotalSO?POId=' + $scope.NewobjectSO.ProductionOrderId + '&salesOrderId=' + $scope.NewobjectSO.SalesOrderId + '&processId=' + $scope.productionSummaryNew.ProcessId)
                    .then(function (response) {
                        if (baseService.arrayLength(response.data) > 0) {
                            $scope.TotalSalesOrderQty = parseFloat(response.data[0].PlannedQty).toFixed(2);
                            $scope.RemainQty = parseFloat(response.data[0].RemainingQty).toFixed(2);
    ...
    $scope.getProductionOrderPopUp = function () { 
        $scope.ProductionOrderList = [];
        $http.get('QMS/DefineSQCIssue/GetQualityProductionOrderList?entityid=' + $scope.productionSummaryNew.EntityId  + '&productionLevel=' + $scope.productionSummaryNew.ProductionBookingLevel + '&processId=' + $scope.productionSummaryNew.ProcessId + '&ToCloseAllowed=' + $scope.ToCloseAllowed)
            .then(
                function successCallback(response) {
    ...
    $scope.getChar1Info = function () {
        $scope.ProductionSummaryDetail = [];
        $http.get('QMS/Productionsummary/GetChar1Info?id=' + $scope.productionSummaryNew.Id + '&soid=' + $scope.productionSummaryNew.SalesOrderId)
            .then(function (response) {
                $scope.ProductionSummaryDetail = [];
    ...
    $scope.getChar1 = function (masterid, soid) {
        $scope.ProductionSummaryDetail = [];
        $http.get('QMS/Productionsummary/GetChar1Info?id=' + masterid + '&soid=' + soid)
            .then(function (response) {
                $scope.ProductionSummaryDetail = [];
                $scope.ProductionSummaryDetail = response.data;
            });
    ...
    $scope.getMentorAndRespPersonByWCM = function () {
        $http.get('QMS/productionsummary/getmentorandresppersonbywcm?wcmId=' + $scope.productionSummaryNew.WorkCenterMasterId)
            .then(function (response) {
                if (baseService.arrayLength(response.data) > 0) {
    ...
    $scope.getCharInfo = function () {
        ...
        $http.get('QMS/Productionsummary/GetChar1InfobyPrO?masterid=' + $scope.productionSummaryNew.Id + '&soid=' + $scope.productionSummaryNew.ProductionOrderId)
            .then(function (response) {
                $scope.ProductionSummaryDetail = [];
                $scope.ProductionSummaryDetail = response.data;
            });
    ...
    $scope.getLineGrid = function () {
        try {
            ...
            $scope.LineGridList = [];
            $http.get('QMS/Productionsummary/GetLineItemGrid?entityid=' + entityid + '&processid=' + processid + '&workdate=' + workdate + '&shiftid=' + shiftid + '&wcid=' + wcid + '&productionLevel=' + $scope.productionSummaryNew.ProductionBookingLevel)
                .then(function (response) {
                    $scope.LineGridList = [];
    ...
    $scope.deleteMasterWC = function (master) {
        if (!baseService.isUndefinedOrNull(master.data.Id)) {
            $http({
                method: 'POST',
                url: 'QMS/DefineSQCIssue/DeleteMasterWC?id=' + master.data.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
    ...
    $scope.getProcessParaPopupPoPUp = function (data) {
        ...
        try {
            $scope.ProcessParaList = [];
            $http.get('QMS/ProductionSummary/GetProcessParaData?processId=' + $scope.productionSummaryNew.ProcessId + '&masterId=' + data.data.Id + '&ProductionOrderId=' + data.data.ProductionOrderId)
                .then(
                    function successCallback(response) {
    ...
    $scope.Calculate = function () {
        try {
            ...
            $http({
                method: 'POST',
                url: 'QMS/ProductionSummary/Calculate',
    ...
    $scope.GetShiftList = function () {
        $http.get('QMS/Productionsummary/GetShiftList?processId=' + $scope.productionSummaryNew.ProcessId)
            .then(function (response) {
                if (baseService.arrayLength(response.data) > 0) {
    ...
    $scope.GetIssueList = function (PId) {
        $http.get('QMS/DefineSQCIssue/GetQualityIssueList?processId=' + PId)
            .then(function (response) {
                if (baseService.arrayLength(response.data) > 0) {
    ...
    $scope.GetPeriodList = function (PId) {
        ...
        $http.get('QMS/DefineSQCIssue/GetQualityPeriodList?IssueId=' + PId)
            .then(function (response) {
                if (baseService.arrayLength(response.data) > 0) {
