...
{
    public class ExtraOTController : BaseController
    {
        #region Constructor

        private readonly ISqlRepository _sqlRepository;
        public ExtraOTController(
        ...
        public ActionResult SaveAttendanceProcessDataDateWise(List<AttendanceProcessDataVM> AttendanceProcessData, string WDate)
        {
            ...
            try
            {
                ...
                string sql = "SELECT * FROM [dbo].[AttdnManualData] WHERE EmpSystemID IN (" + EmpSytemId + ") AND WorkDate ='" + WDate + @"'";
                objCon = new ConnectionManager.DAL.ConManager("1");
                objCon.OpenDataSetThroughAdapter(sql, out dsManualAttanData, false, "1");

                string sql1 = "SELECT * FROM [dbo].[HourlyOT] WHERE EmpSystemID IN (" + EmpSytemId + ") AND WorkDate ='" + WDate + @"'";
                objCon = new ConnectionManager.DAL.ConManager("1");
                objCon.OpenDataSetThroughAdapter(sql1, out dsHourlyOTData, false, "1");
                ...
                strSQL1 = @"SELECT * FROM AttdnRawData WHERE LogDownLoadNum IN (" + EmpSytemId + ")  AND PDate='" + WDate + @"'";
                objCon = new ConnectionManager.DAL.ConManager("1");
                objCon.OpenDataSetThroughAdapter(strSQL1, out dsGetdataRef, false, "1");
                ...
                strSQL2 = @"SELECT * FROM AttdnRawDataBackUp WHERE LogDownLoadNum IN (" + EmpSytemId + ") AND  PDate='" + WDate + @"'";
                objCon = new ConnectionManager.DAL.ConManager("1");
                objCon.OpenDataSetThroughAdapter(strSQL2, out dsSaveddataRef, false, "1");
                ...
                for (int i = 0; i < AttendanceProcessData.Count; i++)
                {
                    ...
                    { //Raw Data Delete
                        ...
                        if (dvAttdnRawData.Count > 0)
                        {
                            ...
                            {
                                dvSaveSummary.RowFilter = " Id ='" + dvAttdnRawData[j]["Id"] + "'  AND PDate = '" + WDate + @"'";
                                if (dvSaveSummary.Count == 0)
                                {
                                    string sID = string.Empty;
                                    bplib.clsGenID objGenID = new bplib.clsGenID();
                                    ...
                                    DataRow dr = dsSaveddataRef.Tables[0].NewRow();
                                    dr["Id"] = "AB" + sID;
                                    dr["DeviceID"] = dvAttdnRawData[j]["DeviceID"];
                                    dr["DevSystemID"] = dvAttdnRawData[j]["DevSystemID"];
                                    dr["LogDownLoadNum"] = dvAttdnRawData[j]["LogDownLoadNum"];
                                    dr["PDate"] = dvAttdnRawData[j]["PDate"];
                                    dr["PTime"] = dvAttdnRawData[j]["PTime"];
                                    dr["PType"] = dvAttdnRawData[j]["PType"];
                                    dr["ProcessedFlag"] = dvAttdnRawData[j]["ProcessedFlag"];
                                    dr["GroupID"] = identity.CompanyGroupId;
                                    dr["PlantID"] = identity.PlantId.ToString();
                                    dr["AddedBy"] = identity.Name;
                                    dr["DateAdded"] = System.DateTime.Now.ToString();
                    ...
                    {  //Manual Attendance 
                        DvMaster.RowFilter = "EmpSystemID='" + AttendanceProcessData[i].SystemId + "'";
                        if (DvMaster.Count == 0)
                        {
                    ...
                    DvHourlyOTData.RowFilter = "EmpSystemID='" + AttendanceProcessData[i].SystemId + "'";
                    if (DvHourlyOTData.Count == 0)
                    {
                        string sID = string.Empty;
                        bplib.clsGenID objGenID = new bplib.clsGenID();
    ...
    public class AttendanceProcessDataVM
    {
