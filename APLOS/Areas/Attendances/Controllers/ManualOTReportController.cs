...
{
    public class ManualOTReportController : BaseController
    {
        ...
        public ManualOTReportController(ISqlRepository R)
        {
            _sqlRepository = R;
        }
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(EntityId))
                {
                    sql = @"SELECT distinct convert(bit,0) AS isSelected, Emp.SystemID AS EmployeeSystemId, EMP.EmployeeStatus,
                            EMP.EmployeeName,EMP.EmployeeCode AS Code,cm.Id as CompanyId,cm.UserName as Company,apd.InTime as EMPAPDInTime, CONVERT(varchar(5), apd.[InTime], 108)[APDInTime],apd.OutTime as EMPAPDOutTime, CONVERT(varchar(5), apd.[OutTime], 108)[APDOutTime], apd.OTHr as OverStay, FORMAT(apd.WorkDate, 'dd-MMM-yyyy') as APDEmpWorkDate ,
