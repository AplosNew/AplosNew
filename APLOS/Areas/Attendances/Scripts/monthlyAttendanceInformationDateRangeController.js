'use strict';
monthlyAttendanceInformationDateRangeController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'cboService', '$window'];
function monthlyAttendanceInformationDateRangeController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService, $window) {
    ...
    $scope.GetMonthlyAttendanceSummaryReport = function (reportType) {
        try {
            ...
            if (baseService.isUndefinedOrNull($scope.FromDate)) {
                ...
                $http({
                    method: 'POST',
                    url: 'Attendances/AttendanceProcessUI/XlsDepWiseAttnRptDateRange',
                    data: {
    ...
    $scope.GetEmployeeInformationDateRange = function () {
        ...
        if (baseService.isUndefinedOrNull($scope.FromDate)) {
            ...
            $http({
                method: "POST",
                dataType: 'JSON',
                url: 'Attendances/AttendanceProcessUI/GetEmpInfoDateRang',
                data: parameters
            }).then(function successCallback(response) {
                if (response.data.length > 0) {
                    $scope.empGrid = true;
