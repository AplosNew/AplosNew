'use strict';
weekOffOTReportOriginalController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http'];
function weekOffOTReportOriginalController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http) {
    $rootScope.title = 'Week OFF OT Report';

    $scope.path = 'Attendances/WeekOffHolidayOTReportOriginal/';
    ...
    $scope.GetEmployeeInformation = function () {
        ...
        if (angular.isUndefinedOrNull($scope.year)) {
            ...
            $http({
                method: "POST",
                dataType: 'JSON',
                url: 'Attendances/WeekOffHolidayOTReportOriginal/GetEmpInfo',
                data: parameters
