'use strict';
OTLimitTransactionController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$window', '$filter'];
function OTLimitTransactionController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $window, $filter) {
    $rootScope.title = 'OT Limit Transaction';
    $scope.path = 'Attendances/OTLimitTransaction/';
    ...
    $scope.GetDetailsData = function (arg) {
        ...
        $http({
            ...
            if (response.data.Error === true) {
                ...
                $scope.DetailsDataList = response.data.oOTLimitTransaction;
