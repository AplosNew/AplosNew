'use strict';
ExtraOTDeleteController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$window', '$filter'];
function ExtraOTDeleteController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $window, $filter) {
    $rootScope.title = 'Extra OT';
    $scope.path = 'Attendances/ExtraOTDelete/';
    $scope.GetExtraOTDataDateWiseUrl = $scope.path + 'GetExtraOTDataDateWise';
    $scope.DeleteExtraOTDataDateWiseUrl = $scope.path + 'DeleteExtraOTDataDateWise';
    ...
    $scope.GetExtraOTDataDateWise = function () {
        try {
            //var previousDay = null;
            if (baseService.isUndefinedOrNull($scope.customPara.procdate)) {
               //
            }
            if (baseService.isUndefinedOrNull($scope.customPara.OTType)) {
                throw 'Please select ot type.';
            }
            
            $scope.day = $filter('dateFiltering')($scope.customPara.procdate, 'dd-M-yyyy');
            $http({
                method: 'GET',
                url: $scope.GetExtraOTDataDateWiseUrl + "?WDate=" + $scope.day + '&OTType=' + $scope.customPara.OTType ,
                ...
                if (response.data.Error === true) {
                    ...
                    $scope.ExtraOTDataDateWise = [];
                    $scope.ExtraOTDataDateWise = response.data.data;
                    $scope.ShowSaveButton = true;
                    for (var i = 0; i < $scope.ExtraOTDataDateWise.length; i++) {
                        $scope.ExtraOTDataDateWise[i].DOJ = new Date($scope.ExtraOTDataDateWise[i].DOJ);                       
                    }
                }
    ...
    $scope.SaveData = function () {
        ...
        if ($scope.tabh === 11) {
           //
            ...
            for (var i = 0; i < $scope.ExtraOTDataDateWise.length; i++) {
                ...
                if ($scope.ExtraOTDataDateWise[i].CheckBoxSelect === true) {
                    $scope.employees.push($scope.ExtraOTDataDateWise[i]);
                }
        ...
        $http({
            method: 'POST',
            url: $scope.DeleteExtraOTDataDateWiseUrl,
            data:
            {
                'AttendanceProcessData': $scope.employees,
                'WDate': $scope.day,
                'OTType': $scope.customPara.OTType               
            },
            headers: {
                'Content-Type': 'application/json'
            }
    ...
    function CheckBoxSelectAllEmolyeeWise(e) {
        ...
        if (angular.isUndefinedOrNull(filtered) || filtered.length == 0) {
            for (var i = 0; i < $scope.ExtraOTDataDateWise.length; i++) {
                $scope.ExtraOTDataDateWise[i].CheckBoxSelect = ChkOrUnchk;
            }
        }
        else {
    ...
    $scope.GetAttendanceProcessDetailsDataDateRangWise = function () {
        try {
            ...
            $http({
                method: 'GET',
                url: $scope.GetExtraOTDataDateWiseUrl + "?WDate=" + $scope.day,
                //data: JSON.stringify(data),
                headers: {
                    'Content-Type': 'application/json'
                }
