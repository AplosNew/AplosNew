'use strict';
BuyerjobCardcomplianceReportController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$controller', '$window'];
function BuyerjobCardcomplianceReportController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $controller, $window) {

    $rootScope.title = 'Job Card Compliance ';
    ...
    $scope.GetEmployeeInformation = function () {
        if (baseService.isUndefinedOrNull($scope.JobCardReport.FromDate)) {
            ...
            $http({
                method: "POST",
                dataType: 'JSON',
                url: 'Attendances/ComplianceAttendanceSetting/GetEmployeeInformation',
                data: parameters
    ...
    $scope.BuyerjobcardreportFunc = function () {
        try {
            ...
            if (baseService.isUndefinedOrNull($scope.JobCardReport.FromDate)) {
                ...
                if ($scope.JobCardReport.ReportFormat === 'Excel') {
                    if ($scope.EmployeeListNew.length > 50) {
                        ...
                        var url = 'Attendances/ComplianceAttendanceSetting/GetBuyerComplianceJobCardReport?reportFormat=' + $scope.JobCardReport.ReportFormat + '&fromDate=' + $scope.JobCardReport.FromDate + '&toDate=' + $scope.JobCardReport.ToDate + '&employeeId=' + $scope.EmployeeListNew + '&chkAdditionInfo=' + $scope.JobCardReport.chkAdditionInfo;
