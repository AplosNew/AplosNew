'use strict';
AuditReportSummaryNewController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'cboService', '$window'];
function AuditReportSummaryNewController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService, $window) {
    $rootScope.title = 'Audit Report Summary';
    //$scope.index = -1;

    $scope.downloadgriddataUrlPath = 'GridReports/DownloadUsingFullPath';//DownloadUsingPath
    $scope.path = 'Attendances/AuditReportSummaryNew/';
    ...
    $scope.Report = function () {
        try {
            ...
            $http({
                method: 'POST',
                url: 'Attendances/AuditReportSummaryNew/AuditReportSummary',
                data: {
