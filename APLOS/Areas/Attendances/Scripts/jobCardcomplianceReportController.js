'use strict';
jobCardcomplianceReportController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$controller', '$window'];
function jobCardcomplianceReportController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $controller, $window) {
            ...
            if (baseService.isUndefinedOrNull($scope.JobCardReport.FromDate)) {
                ...
                if ($scope.JobCardReport.ReportFormat === 'Excel') {
                    if ($scope.EmployeeListNew.length > 50) {
                        ...
                        var url = 'Attendances/ComplianceAttendanceSetting/GetComplianceJobCardReport?reportFormat=' + $scope.JobCardReport.ReportFormat + '&fromDate=' + $scope.JobCardReport.FromDate + '&toDate=' + $scope.JobCardReport.ToDate + '&employeeId=' + $scope.EmployeeListNew + '&chkAdditionInfo=' + $scope.JobCardReport.chkAdditionInfo;
