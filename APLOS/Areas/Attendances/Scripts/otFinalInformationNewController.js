...
'use strict';
otFinalInformationNewController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'cboService'];
function otFinalInformationNewController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService) {
    $rootScope.title = 'OT Final Information';
    $scope.path = 'Attendances/otFinalInformationNew';
    ...
    $scope.GetOTFinalReport = function (reportType) {
        try {
            ...
            if (baseService.isUndefinedOrNull($scope.OTFinalInformation.MonthNo)) {
                ...
                $http({
                    method: 'POST',
                    url: $scope.path+ '/XlsOTFinalReport',
                    data: {
