'use strict';
ManualOTReportController.$inject = ['$window',"addressService", 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function ManualOTReportController($window, addressService, cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $scope.OTReportList = [];
    $scope.SearchedOTManualReportsList = [];
    $scope.PlantList = [];
    $scope.EntityList = [];

    $scope.path = 'Attendances/ManualOTReport/';
    ...
    $scope.SearchOTEmpReport = function () {
        ...
        if ($scope.General.$valid) {
            ...
            $http({
                ...
                url: 'Attendances/ManualOTReport/getsearchedotemp/'
            }).then(function successCallback(response) {
