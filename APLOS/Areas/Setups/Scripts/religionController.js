'use strict';
religionController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function religionController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = 'Job Description Category';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.jobDescriptionCategories = [];
    $scope.path = 'Setups/religion/';
    ...
    $scope.religionNew = Object.assign({}, $scope.religion);

    $scope.GetSequence = function () {
        cboService.getSequence($scope.getSeqUrl, function (data) {
            $scope.religionNew.Sequence = data;
        })
    };
    $scope.GetSequence();
    ...
    $scope.Get = function (index) {
        $scope.index = index;
        $scope.religion = $scope.religions[$scope.index];
        $scope.religionNew = Object.assign({}, $scope.religion);
        clearLanguage();
    ...
    $scope.Save = function () {
        ...
        if ($scope.religionForm.$valid) {
            angular.copy($scope.religionNew, $scope.religion);
            if ($scope.Action === 'Save') {
                $http({
                    method: 'POST',
                    url: $scope.saveUrl,
                    data: {
                        'model': $scope.religion
                    ...
                    if (response.data.Error === true) {
                        ...
                        $scope.religions.push(response.data.Religion);
                        $scope.religions = $filter('orderBy')($scope.religions, 'Sequence');
                ...
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        ShowResult(response.data.Message, 'success');
                        if ($scope.index > -1) {
                            $scope.religions[$scope.index] = $scope.religion;
                            $scope.religions = $filter('orderBy')($scope.religions, 'Sequence');
                        }
                        ClearFields(response.data.Sequence);
                    }
    ...
    $scope.Delete = function () {
        if (!baseService.isUndefinedOrNull($scope.religionNew.Id)) {
            $http({
                method: 'POST',
                url: $scope.deleteUrl + $scope.religionNew.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
                    ...
                    ShowResult(response.data.Message, 'success');
                    $scope.religions.splice($scope.index, 1);
    ...
    $scope.languageData = function () {
        $scope.languageDataUrl = 'Setups/LocalLanguage/GetReligionLanguageList?religionId=' + $scope.religionNew.Id;
    ...
    function ClearFields(seq) {
        $scope.Action = 'Save';
        $scope.religion = {};
        $scope.religionNew = { Sequence: seq, Active: true };
        clearLanguage();
        $scope.languageDataList = [];
    }
