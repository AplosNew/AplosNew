'use strict';
professionController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function professionController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = 'Profession';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.continents = [];
    $scope.path = 'setups/profession/';
    $scope.saveUrl = $scope.path + 'create';
    $scope.updateUrl = $scope.path + 'edit';
    $scope.deleteUrl = $scope.path + 'delete/';
    $scope.getListUrl = $scope.path + 'getprofessionlist';
            ...
            if ($scope.Action === 'Save') {
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        $scope.continents.push(response.data.Profession);
    ...
    $scope.languageData = function () {
        $scope.languageDataUrl = 'Setups/LocalLanguage/GetProfessionLanguageList?professionId=' + $scope.continentNew.Id;
