'use strict';
relationshipController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function relationshipController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = 'Relationship';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.continents = [];
    $scope.path = 'Setups/relationship/';
    $scope.saveUrl = $scope.path + 'create';
    $scope.updateUrl = $scope.path + 'edit';
    $scope.deleteUrl = $scope.path + 'delete/';
    $scope.getListUrl = $scope.path + 'getrelationshiplist';
            ...
            if ($scope.Action === 'Save') {
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        $scope.continents.push(response.data.Relationship);
    ...
    $scope.languageData = function () {
        $scope.languageDataUrl = 'Setups/LocalLanguage/GetRelationshipLanguageList?relationshipId=' + $scope.continentNew.Id;
    ...
    function ClearFields(seq) {
        $scope.Action = 'Save';
        $scope.continent = {};
        $scope.continentNew = { Sequence: seq, IsChild: false, Active: true };
        clearLanguage();
        $scope.languageDataList = [];
    }
