...
function BOQApprovalController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, $controller, $window) {
    ...
    $scope.refreshTemplateBOQ = function (args) {
        $("#headchk").ejCheckBox({ "change": CheckBoxSelectAllBOQ });
    };

    function CheckBoxSelectAllBOQ(e) {
        ...
        var filtered = $("#GridMaterialQuantity").data("ejGrid").getFilteredRecords();
        if (angular.isUndefinedOrNull(filtered) || filtered.length == 0) {
            for (var i = 0; i < $scope.MaterialQtyEditList.length; i++) {
                $scope.MaterialQtyEditList[i].RequiredQtyApproved = ChkOrUnchk;
            }
        }
        else {
