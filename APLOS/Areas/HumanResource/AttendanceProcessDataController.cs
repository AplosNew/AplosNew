...
{
    public class AttendanceProcessDataController : BaseController
    {
        ...
        public AttendanceProcessDataController(IUnitOfWork U, ISqlRepository R)
        {
        ...
        public ActionResult getAllEmployees(string fromdate, string todate)
        {
            ...
            string sql = @"
                        ...
                        EMP.EmployeeName,CONVERT (int, EMP.EmployeeCode) EmployeeCode,EMP.EmpPicPath,
        ...
        public ActionResult getAttendanceData(string employeeid, string fromdate, string todate)
        {
            ...
            return Json(new { data = _sqlRepository.GetModelCollection<AttendanceProcessData>(sql), shift = _sqlRepository.GetDataCollection(shiftSQL) }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult SaveSingleEmployee(List<AttendanceProcessData> data)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (inDates != "")
                    {
                        DataTable dtLock = _sqlRepository.GetDataTable("SELECT * FROM PlantWiseAttendanceLock AS pwal WHERE pwal.LockedDate IN (" + inDates + ") AND pwal.PlantId='" + identity.PlantId + "'");
                ...
                foreach (AttendanceProcessData item in DataToBeSaved)
                {
                    ...
                    if (NewShiftStandardTime.DefaultView.Count > 0)
                    {
                        ...
                        {
                            if (item.InTime != item.InTimeOriginal || item.OutTime != item.OutTimeOriginal)
                            {
                        ...
                        if (item.InTime != null)
                        {
                            if (item.InTime != item.InTimeOriginal)
                            {
                        ...
                        {
                            if (item.OutTime != item.OutTimeOriginal)
                            {
            ...
            try
            {
                ...
                for (int i = 0; i < data.Count; i++)
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        throw new Exception("Error occured while processing attendance");
                    }
