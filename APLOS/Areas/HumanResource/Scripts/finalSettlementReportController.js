'use strict';
finalSettlementReportController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$controller','$window'];
function finalSettlementReportController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $controller, $window) {
    ...
    $scope.GetEmployeeInformation = function () {
            ...
            $http({
                method: "POST",
                dataType: 'JSON',
                url: 'HumanResource/AttendanceManagement/GetFinalSattlementInformation',
                //data: parameters
    ...
    $scope.MaternityLeaveReport = function (obj) {
        try {
            ...
            var datum = obj.data;                             
            var url = 'HumanResource/AttendanceManagement/EmployeeSattlementReport?reportFormat=' + $scope.NationlFestival.ReportFormat + '&SystemId=' + datum.SystemId + '&LanguageId=' + $scope.NationlFestival.LanguageId + '&UserName=' + $scope.UserName;
