'use strict';
jobCardReportController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$controller', '$window'];
function jobCardReportController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $controller, $window) {

    $rootScope.title = 'Individual Details';
            ...
            if (baseService.isUndefinedOrNull($scope.JobCardReport.FromDate)) {
                ...
                if ($scope.JobCardReport.ReportFormat === 'Excel') {
                    if ($scope.EmployeeListNew.length > 50) {
                        ...
                        var url = 'HumanResource/AttendanceManagement/GetJobCardReport?reportFormat=' + $scope.JobCardReport.ReportFormat + '&fromDate=' + $scope.JobCardReport.FromDate + '&toDate=' + $scope.JobCardReport.ToDate + '&employeeId=' + $scope.EmployeeListNew + '&chkAdditionInfo=' + $scope.JobCardReport.chkAdditionInfo;
