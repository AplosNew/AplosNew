'use strict';
bonusReportCController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$controller', '$window'];
function bonusReportCController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $controller, $window) {
    ...
    $scope.GetBonusRegister = function (reportType) {
        try {
            ...
            if (baseService.isUndefinedOrNull($scope.fromDate)) {
                ...
                //manualValidation('div_ToDate', true, "To date must be above or equal to From Date.");
                ShowResult("To Date Can not be Greated then fiscal year End Date.", 'failure');
            }
            else {
                $http({
                    method: 'POST',
                    url: 'humanresource/BonusRegisterReports/GetBonusFormC',
                    data: {
                        'yearId': $scope.yearId,
