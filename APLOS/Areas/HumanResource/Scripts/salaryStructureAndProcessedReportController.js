'use strict';
salaryStructureAndProcessedReportController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$window'];
function salaryStructureAndProcessedReportController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $window) {
    ...
    $scope.GetEmployeeSalaryStructureWithProceesd = function () {
        try {
            ...
            $http({
                method: 'POST',
                url: 'humanresource/PayrollReports/GetEmployeeSalaryStructureWithProceesd',
                data: {
                    'month': $scope.month,
                    'year': $scope.year,
