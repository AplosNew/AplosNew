'use strict';
salaryProcessedReportExtraOTCTCOriginalController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$window'];
function salaryProcessedReportExtraOTCTCOriginalController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $window) {
    ...
    $scope.GetEmployeeSalaryProcessedReport = function () {
        try {
            ...
            $http({
                method: 'POST',
                url: 'humanresource/PayrollReports/GetSalarySheetExtraOTCTCReportOriginal',
                data: {
                    'month': $scope.month,
                    'year': $scope.year,
