'use strict';
bonusRegisterReportController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$controller', '$window'];
function bonusRegisterReportController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $controller, $window) {
    ...
    $scope.GetBonusRegister = function (reportType) {
        try {
            ...
            if (baseService.isUndefinedOrNull($scope.fromDate)) {
                ...
                $http({
                    method: 'POST',
                    url: 'humanresource/BonusRegisterReports/GetBonusRegister',
                    data: {
                        'yearId': $scope.yearId,
