'use strict';
professionalTaxReportsController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$controller', '$window'];
function professionalTaxReportsController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $controller, $window) {
    ...
    $scope.GetProfessionalTaxReports = function (reportType) {
        try {
            //var dayStatusarr = [];
            //var dayStatus = null;

            //var DropDownListObj = $("#MonthList").data("ejDropDownList");
            //dayStatusarr = DropDownListObj.getSelectedValue().split(',');
            //if (dayStatusarr[0] != "") {

            //    dayStatus = "(Convert(Date,'" + dayStatusarr.join("')),(Convert(Date,'") + "'))";
            //}
            ...
            if (baseService.isUndefinedOrNull($scope.fromDate)) {
                ...
                $http({
                    method: 'POST',
                    url: 'humanresource/ProfessionalTaxReports/GetProfessionalTaxReports',
                    data: {
                        'yearId': $scope.yearId,               
