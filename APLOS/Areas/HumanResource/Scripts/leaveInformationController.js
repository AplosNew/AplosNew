'use strict';
LeaveInformationController.$inject = ['fileReader', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'cboService', '$window', 'toaster'];
function LeaveInformationController(fileReader, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService, $window, toaster) {


    $rootScope.title = 'Leave Information';
    $scope.index = -1;
    ...
    $scope.GetClanderYear = function () {
        $http({
            method: "GET",
            dataType: 'JSON',
            url: 'HumanResource/MaternityLeaveTransaction/GetClanderYear'
    ...
    $scope.LeaveRegisterReportPrint = function () {
        ...
        location.href = 'HumanResorce/MaternityLeaveTransaction/GetEMPLeaveRegisterReport';
    };

    $scope.ShortLeaveReportPrint = function () {
        try {
            CheckField($scope.shortLeaveDate, "Date");
            var _date = new Date($scope.shortLeaveDate);
            var date = $filter('dateFiltering')(_date, 'dd-MMM-yyyy');
            location.href = 'humanresource/MaternityLeaveTransaction/ShortLeaveReport?date=' + $scope.shortLeaveDate;
        } catch (e) {
            ShowResult(e, 'Error');
        }
    };
    ...
    $scope.LeaveReportDateWise = function () {
        $scope.parameters = 'fromDate=' + $scope.LeaveReport.FromDate + '&toDate=' + $scope.LeaveReport.ToDate + '&employeeCodeString=' + $scope.employeeCodeString;

        try {
            location.href = 'humanresource/MaternityLeaveTransaction/LeaveReport?' + $scope.parameters;
        } catch (e) {
            ShowResult(e,'Error');
        }
    };
    ...
    function getStringArrayJoin() {
        ...
        $scope.employeeCodeString = employeeCodeStringList.join();
    ...
    //--------------------------------------//
    //Divs
    var empWiseLeaveInformation = document.getElementById("empWiseLeaveInformation");
    var shortLeaveInformation = document.getElementById("shortLeaveInformation");
    //var halfDayLeaveInformation = document.getElementById("halfDayLeaveInformation");

    //buttons
    var btnHalfDayLeaveInfo = document.getElementById("btnHalfDayLeaveInfo");
    var btnShortLeaveInfo = document.getElementById("btnShortLeaveInfo");
    var btnLeaveInfo = document.getElementById("btnLeaveInfo");

    shortLeaveInformation.style.display = "none";
    //halfDayLeaveInformation.style.display = "none";
    empWiseLeaveInformation.style.display = "block";
    btnLeaveInfo.style.backgroundColor = "#242841;";
    btnLeaveInfo.style.color = "white";



    $scope.clickShortLeaveInformation = function () {
        if (shortLeaveInformation.style.display === "none") {
            shortLeaveInformation.style.display = "block";
            // halfDayLeaveInformation.style.display = "none";
            empWiseLeaveInformation.style.display = "none";
            btnShortLeaveInfo.style.backgroundColor = "black";
            btnShortLeaveInfo.style.color = "white";
            btnLeaveInfo.style.backgroundColor = "lightgrey";
            btnLeaveInfo.style.color = "white";
            //btnHalfDayLeaveInfo.style.backgroundColor = "lightgrey";
            //btnHalfDayLeaveInfo.style.color = "white";

        }
    };
    //$scope.clickHalfDayLeaveInformation = function () {
    //    if (halfDayLeaveInformation.style.display === "none") {
    //        halfDayLeaveInformation.style.display = "block";
    //        shortLeaveInformation.style.display = "none";
    //        empWiseLeaveInformation.style.display = "none"; 

    //        btnHalfDayLeaveInfo.style.backgroundColor = "black";
    //        btnHalfDayLeaveInfo.style.color = "white";
    //        btnLeaveInfo.style.backgroundColor = "lightgrey";
    //        btnLeaveInfo.style.color = "white";
    //        btnShortLeaveInfo.style.backgroundColor = "lightgrey";
    //        btnShortLeaveInfo.style.color = "white";
    //    }
    //};
    $scope.clickEmpWiseLeaveInformation = function () {
        if (empWiseLeaveInformation.style.display === "none") {
            //halfDayLeaveInformation.style.display = "none";            
            shortLeaveInformation.style.display = "none";
            empWiseLeaveInformation.style.display = "block";

            btnLeaveInfo.style.backgroundColor = "black";
            btnLeaveInfo.style.color = "white";
            //btnHalfDayLeaveInfo.style.backgroundColor = "lightgrey";
            //btnHalfDayLeaveInfo.style.color = "white";
            btnShortLeaveInfo.style.backgroundColor = "lightgrey";
    ...
    $scope.LeaveReportPdf = function (id, reportFormat) {
        debugger;
        var reportFormat = "Pdf";
        //if (baseService.isUndefinedOrNull(id)) return ShowResult('No Id found', 'failure');
        $window.open('HumanResource/MaternityLeaveTransaction/Report?reportFormat=' + reportFormat + '&fromDate=' + $scope.report.FromDate + '&toDate=' + $scope.report.ToDate, '_blank');
    };
    $scope.LeaveReportExcel = function (id, reportFormat) {
        debugger;
        var reportFormat = "Excel";
        //if (baseService.isUndefinedOrNull(id)) return ShowResult('No Id found', 'failure');
        $window.open('HumanResource/MaternityLeaveTransaction/Report?reportFormat=' + reportFormat + '&fromDate=' + $scope.report.FromDate + '&toDate=' + $scope.report.ToDate + '&Qty=' + $scope.productNew.Qty + '&Amount=' + $scope.productNew.Amount, '_blank');
    };
}