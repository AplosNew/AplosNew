'use strict';
NewAttendanceProcessAuditReportController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$controller', '$window'];
function NewAttendanceProcessAuditReportController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $controller, $window) {

    $rootScope.title = 'Attendance Data Audit ';
    ...
    $scope.ManualOutTimeDateWiseData = function () {
        try {
            ...
            if (baseService.isUndefinedOrNull($scope.ManualOutTimeDateWise.FromDate)) {
                ...
                var url = 'HumanResource/NewAttendanceProcessAuditReport/GetManualOutTimeDateWiseReport?reportFormat=Excel' + ' &FromDate=' + $scope.ManualOutTimeDateWise.FromDate + ' &ToDate=' + $scope.ManualOutTimeDateWise.ToDate;
