'use strict';
jobCardReportNewController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$controller', '$window'];
function jobCardReportNewController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $controller, $window) {

    $rootScope.title = 'Job Card';
    $controller('employeeBaseController', { $scope: $scope, $http: $http });
    var date = new Date(), y = date.getFullYear(), m = date.getMonth();
    var firstDay = new Date(y, m, 1);
    $scope.path = 'HumanResource/JobCardReportNew';
    $scope.JobCardReport = {
            ...
            if (baseService.isUndefinedOrNull($scope.JobCardReport.FromDate)) {
                ...
                if ($scope.JobCardReport.ReportFormat === 'Excel') {
                    if ($scope.EmployeeListNew.length > 50) {
                        ...
                        var url = $scope.path +'/GetComplianceJobCardReport?reportFormat=' + $scope.JobCardReport.ReportFormat + '&fromDate=' + $scope.JobCardReport.FromDate + '&toDate=' + $scope.JobCardReport.ToDate + '&employeeId=' + $scope.EmployeeListNew + '&chkAdditionInfo=' + $scope.JobCardReport.chkAdditionInfo;
                ...
                if ($scope.JobCardReport.ReportFormat === 'Pdf') {
                    ...
                    if ($scope.EmployeeListNew.length > 50) {
                        throw "Maximum 50 'Job card' can be downloded at a time in PDF";
                    }
                    else {
                        var url = $scope.path +'/GetComplianceJobCardReport?reportFormat=' + $scope.JobCardReport.ReportFormat + '&fromDate=' + $scope.JobCardReport.FromDate + '&toDate=' + $scope.JobCardReport.ToDate + '&employeeId=' + $scope.EmployeeListNew + '&chkAdditionInfo=' + $scope.JobCardReport.chkAdditionInfo;
                        $rootScope.report(url);
