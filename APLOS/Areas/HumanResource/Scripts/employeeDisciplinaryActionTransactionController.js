'use strict';
employeeDisciplinaryActionTransactionController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', '$window'];
function employeeDisciplinaryActionTransactionController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, $window) {
    $rootScope.title = 'Employee Disciplinary Action Transaction';
    $scope.Action = 'Save';
    $scope.index = -1;
    ...
    $scope.GetDueEmployeeDisciplinaryActionsListUrl = $scope.path + 'GetDueEmployeeDisciplinaryActionsList';
    $scope.GetCompletedEmployeeDisciplinaryActionsListtUrl = $scope.path + 'GetCompletedEmployeeDisciplinaryActionsList';
    ...
    //#region Report
    $scope.PrintDisciplinaryActionLetter = function (obj) {
        try {
            ...
            if ($scope.tabh === 22) {
                var gridObj = $("#GridCompletedEmployeeDisciplinaryActionDetailsList").data("ejGrid");
                EmployeeDisciplinaryActionDetailsId = gridObj.getSelectedRecords()[0].Id;
            }
    ...
    $scope.DueEmployeeDisciplinaryActionsList = [];
    $scope.GetDueEmployeeDisciplinaryActionsList = function () {
        $http({
            method: "GET",
            dataType: 'JSON',
            url: $scope.GetDueEmployeeDisciplinaryActionsListUrl,
            //data: parameters
        }).then(function successCallback(response) {

            $scope.DueEmployeeDisciplinaryActionsList = [];
            if (response.data.length > 0) {
                $scope.DueEmployeeDisciplinaryActionsList = response.data;

                for (var i = 0; i < $scope.DueEmployeeDisciplinaryActionsList.length; i++) {
                    $scope.DueEmployeeDisciplinaryActionsList[i].NextLetterDueDate = new Date($scope.DueEmployeeDisciplinaryActionsList[i].NextLetterDueDate);
                    $scope.DueEmployeeDisciplinaryActionsList[i].EntryDate = new Date($scope.DueEmployeeDisciplinaryActionsList[i].EntryDate);
                    $scope.DueEmployeeDisciplinaryActionsList[i].LetterIssueDate = new Date($scope.DueEmployeeDisciplinaryActionsList[i].LetterIssueDate);
                }
    ...
    $scope.CompletedEmployeeDisciplinaryActionsList = [];
    $scope.GetCompletedEmployeeDisciplinaryActionsList = function () {
        $http({
            method: "GET",
            dataType: 'JSON',
            url: $scope.GetCompletedEmployeeDisciplinaryActionsListtUrl,
            //data: parameters
        }).then(function successCallback(response) {
            $scope.CompletedEmployeeDisciplinaryActionsList = [];
            if (response.data.length > 0) {
                $scope.CompletedEmployeeDisciplinaryActionsList = response.data;
            }
    ...
    $scope.GetCompletedEmployeeDisciplinaryActionsList();
    ...
    $scope.actionCompletedEmployeeDisciplinaryActionsList = function (args) {
        try {
            if (args.requestType === "refresh") {
                var gridObj = $("#GridCompletedEmployeeDisciplinaryActionsList").ejGrid("instance");
                var scrollerwidth = $("#tabpaneldiv").width();//Obtain the width of the container

                $("#GridCompletedEmployeeDisciplinaryActionsList").children('.e-grid.e-headercell').css('height', '100px');
                gridObj.option({ allowScrolling: true, scrollSettings: { width: scrollerwidth - 20, height: 400 } });
                gridObj.windowonresize();
            }
        } catch (e) {
    ...
    $scope.onrowdatabound = function (e) {
        ...
        if (e.data.DueStatus === 'OVERDUE')
            e.row.css("background-color", "red");

        if (e.data.DueStatus === 'DUE')
            e.row.css("background-color", "orange");

        if (e.data.DueStatus === 'TOBEDUE')
    ...
    $scope.AddFunctionCompletedDAList = function () {
        try {
            $scope.ClearModel();
            var eDialog = $("#dialogCompletedDAList").data("ejDialog");
            eDialog.open();
            var gridObj = $("#GridCompletedEmployeeDisciplinaryActionsList").data("ejGrid");
            $scope.LongAbsModel = gridObj.getSelectedRecords()[0];
            $scope.GetActionCategory();
            $scope.ShowcaseLetter = null;
            $scope.UserName = null;
    ...
    $scope.saveChild = function () {
        try {
            ...
            $http({
                method: 'POST',
                url: 'humanresource/employeeDisciplinaryAction/Save',
    ...
    $scope.getLetterDescription = function () {
        $http.get($scope.path + 'GetAllDescription?DisciplinaryActionCategoryId='
