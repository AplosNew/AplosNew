'use strict';
EmployeeLeaveApplicationNewController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'cboService'];
function EmployeeLeaveApplicationNewController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService) {
    $rootScope.title = 'Leave Application New';
    $scope.Action = 'Save';
    $scope.index = -1;
    ...
    $scope.getData = function (pageno) {
        baseService.paginationBase('HumanResource/leaveApplicationNew/GetEmpLeaveList?EmpsystemId=' + $scope.leaveApplicationNew.EmpSystemID + '&yearNo=' + $scope.YearNo, pageno, $scope.leaveParameters)
            .then(function (data) {
    ...
    $scope.popUp = function () {
        try {
            ...
            $http({
                method: 'GET',
                url: 'HumanResource/leaveApplicationNew/getsectionemployeelist?sectionId=' + $scope.leaveApplicationNew.SectionId
    ...
    $scope.LeavePolicyName = function () {
        $http({
            method: 'GET',
            url: 'HumanResource/LeaveApplicationNew/getLeavePolicy'
        }).then(function successCallback(response) {
    ...
    $scope.GetEnterEmployeeOutInfo = function () {
        ...
        $http({
            method: "POST",
            dataType: 'JSON',
            url: 'HumanResource/leaveApplicationNew/GetEmpInfo',
            data: parameters
        }).then(function successCallback(response) {
    ...
    function ValidationLeave() {
        ...
        for (var i = 0; i < $scope.LeaveBalanceList.length; i++) {
            if ($scope.LeaveBalanceList[i].LTSystemID === $scope.leaveApplicationNew.LTSystemID) {
                ...
                if ($scope.LeaveBalanceList[i].LeaveDays - $scope.LeaveBalanceList[i].Applied <= $scope.LeaveBalanceList[i].Balance) {
                    throw 'Leave Duration is greater then balance.';
                }
            }
        }
