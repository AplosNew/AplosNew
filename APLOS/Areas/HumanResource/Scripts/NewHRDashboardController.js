'use strict';
NewHRDashboardController.$inject = ['cboService', '$scope', '$rootScope', '$routeParams', 'baseService', '$http', '$filter'];
function NewHRDashboardController(cboService, $scope, $rootScope, $routeParams, baseService, $http, $filter) {

    $scope.Title = "HR Dashboard - New Attdn";
    $scope.chartList = [];
    $scope.list = [];
    $scope.index = -1;
    $scope.chartLabel = [];
    ...
    $scope.ManPowerBudget = function () {
        ...
        $http({
            method: 'POST',
            url: 'NewHRDashboard/GetGroupWiseCompanyList',
            data: {
                'date': $scope.Date,
                'stat': $scope.Stat,
                'EmpCat': $scope.EmpCat,
    ...
    $scope.GetDetailDrillDownTableJS = function (data) {
        ...
        if ($scope.index + 3 < $scope.ColList.length) {
            $http({
                method: 'POST',
                url: 'NewHRDashboard/GetDetailDrillDownTable/',
                data: {
    ...
    function getDrillDownList(companyId) {
        $http({
            method: 'POST',
            url: 'NewHRDashboard/GetDrillDownListJSON/?CompanyId=' + companyId,
            data: {},
            dataType: 'JSON'
        }).then(function successCallback(response) {
    ...
    function GetCompanyDrillDownList(companyId) {
        $http({
            method: 'GET',
            url: 'NewHRDashboard/GetCompanyDrillDownListJSON/?CompanyId=' + companyId,
            data: {},
            dataType: 'JSON'
        }).then(function successCallback(response) {
            if (baseService.arrayLength(response.data) > 0) {
    ...
    $scope.TableClick = function (data , column) {
        ...
        $http({
            method: 'POST',
            url: 'NewHRDashboard/DetailTableClick/',
            data: {
                'ChartColumnList': $scope.ColList,
                'seq': $scope.index,
                'date': $scope.Date,
