...
function SalaryNotDisbursedController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $window) {
    ...
    $scope.GetEmployeeSalaryNotDisbursedProcessedReportSalaryLogWise = function () {
        try {
            var parameters = [];
            var gridObj = $("#empInfoGrid").ejGrid("instance");
            var filteredRecords = gridObj.getFilteredRecords();
            ...
            $http({
                method: 'POST',
                url: 'humanresource/PayrollReports/GetEmployeeSalaryNotDisbursedProcessedReportSalLogWiseNew',
                data: {
                    'month': $scope.month,
                    'year': $scope.year,
                    'salaryProcessId': $scope.salaryProcessId,
    ...
    $scope.XlsGetEmployeeSalaryNotDisbursedProcessedReportSalary = function () {
        ...
        $http({
            method: 'POST',
            url: 'humanresource/PayrollReports/GetEmployeeSalaryNotDisbursedProcessedReportSalLogWiseNew?year=' + $scope.year,
            dataType: 'JSON',
        })
            .then(function successCallback(response) {
                if (response.data.Error === true) {
