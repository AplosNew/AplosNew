...
{
    public class EmployeePromotionAndIncrementController : BaseController
    {
        #region Constructor
        ...
        public EmployeePromotionAndIncrementController(
            ...
            , IEnumerable<OpenHeadModelNew> EmpSalaryOpenHeadNew)
        {
            ...
            if (para != null)
            {
                ...
                //IsPFOptionalNew = para.IsPFOptional;
            ...
            return Json(new { EmpSalaryInfoDefine
                , newGross
                , newCTC
                , newFormula_Desc
                , IsPFMandatoryNew
        ...
        [HttpGet, Authorize]
        public JsonResult GivenDesignationChange( string GivenDesignationId,string EmpSystemId)
        {
            DataTable ApprovedSalaryRuleId = null;
            clsSalaryStructureAplosNew ob = new clsSalaryStructureAplosNew();
            ob.GetApprovedSalaryRuleIdByEmpId(EmpSystemId, out ApprovedSalaryRuleId);
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            ...
            var ResultApprovedData = ApprovedSalaryRuleId.AsEnumerable().Select(
               dataRow => new
               {
                   SalaryRuleMasterId = dataRow.Field<string>("SalaryRuleMasterSystemID"),

               }).ToList();
            return Json(new { ResultData, ResultApprovedData }, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult LoadEmpSalaryInfoDefineData(string EmpSystemId)
        {
            ...
            return Json(new { EmpSalaryInfoDefine 
                ...
                , IsVPFEntitle
                , IsBonusEntitle
                , VPFPersentage, IsFreshEntry
                , Message = AplosMessage.Updated }, JsonRequestBehavior.AllowGet);
        }
        ...
        // Promotion 
        [HttpGet, Authorize]
        public JsonResult GetUnApprovedEmployeeById(string EmpSystemId)
        {            
            return Json(new { data=_employeePromotionService.GetUnApprovedEmployeeById(EmpSystemId), Message = AplosMessage.Updated }, JsonRequestBehavior.AllowGet);
        }
        ...
        public JsonResult Update(EmployeeInformation employeeInformation, IncrementHistoryModel incrementHistory)
        {
            ...
            //EmployeePromotionNewService obj = new EmployeePromotionNewService();
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            if (incrementHistory.IsPromotion == true)
            {
             var data=   _employeePromotionService.GetApprovedDataByEmployeeId(incrementHistory.EmpSystemID);
                if (data.Count()>0)
                {
                    //if (data[0].ToEffectiveDate <= incrementHistory.ToEffectiveDate)
                    //{
                    //    throw new Exception("");
                    //}
                }
