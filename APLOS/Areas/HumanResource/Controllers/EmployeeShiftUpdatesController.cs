...
namespace Aplos.Areas.HumanResource.Controllers
{
    ...
    public class EmployeeShiftUpdatesController : BaseController
    {
        ...
        EmployeeShiftUpdatesService rs = new EmployeeShiftUpdatesService();
        public EmployeeShiftUpdatesController(ISqlRepository sqlRepository)
        {
        ...
        public ActionResult GetSampleReport(ReportFormat reportFormat)
        {
            ...
            var reportFileName = "EmployeeShift-" + date;
            var workbook = GetEmployeeShiftWorkSheet();
            switch (reportFormat)
            {
        ...
        private IWorkbook GetEmployeeShiftWorkSheet()
        {
            ...
            /// Sheet 1 
            DataTable data = rs.getEmployeeShiftFile();

            sheet.Name = "Employee-Shift";
            ...
            report.SetHeaderText(ref sheet, ROW, COL, "ShiftId", 8, ExcelHAlign.HAlignLeft);
            int ColShiftId = COL;
            COL++;
            ...
            for (int i = 0; i < data.Rows.Count; i++)
            {
                ...
                sheet[ROW, ColShiftId].Text = data.Rows[i]["ShiftId"].ToString();
            ...
            DataTable data2 = rs.getShiftFile();

            sheet2.Name = "ShiftTable";
            ...
            #region Headers
            report.SetHeaderText(ref sheet2, ROW2, COL2, "Shift Id ", 12, ExcelHAlign.HAlignLeft);
            int ColRostersId = COL2;
            COL2++;

            report.SetHeaderText(ref sheet2, ROW2, COL2, "Plant", 8, ExcelHAlign.HAlignLeft);
            int ColPlant = COL2;
            COL2++;

            report.SetHeaderText(ref sheet2, ROW2, COL2, "Shift Defination Name", 8, ExcelHAlign.HAlignLeft);
            int ColSDName = COL2;
            COL2++;

            report.SetHeaderText(ref sheet2, ROW2, COL2, "Shift Defination Description", 8, ExcelHAlign.HAlignLeft);
            int ColSDD = COL2;
            COL2++;

            report.SetHeaderText(ref sheet2, ROW2, COL2, "Shift Type", 8, ExcelHAlign.HAlignLeft);
            int ColST = COL2;
            COL2++;
            endCol2 = COL2;
            #endregion Headers
            ...
            {
                sheet2[ROW2, ColRostersId].Text = data2.Rows[i]["SystemID"].ToString();
                sheet2[ROW2, ColPlant].Text = data2.Rows[i]["Plant"].ToString();
                sheet2[ROW2, ColSDName].Text = data2.Rows[i]["ShiftDefinationName"].ToString();
                sheet2[ROW2, ColSDD].Text = data2.Rows[i]["ShiftDefinationDescription"].ToString();
                sheet2[ROW2, ColST].Text = data2.Rows[i]["ShiftType"].ToString();
        ...
        public List<object> ReadData(string path)
        {
            ...
            try
            {
                List<rosShift> data = new List<rosShift>();
                List<object> ret = new List<object>();
                ReadFile(path, out dsExcel);

                data = dsExcel.Tables[0].ToList<rosShift>();
                ...
                if (data.Count > 0)
                {
                    ...
                    {
                        if (data[i].ShiftId != null)
                        {
                            if (RostersList.Contains(data[i].ShiftId))
                            {
                            ...
                            {
                                throw new Exception("The Shift in Employee Code - " + data[i].EmployeeCode + " is not present!!");
                            }
