"use strict";
salesIncentiveController.$inject = ["cboService", "commonMessage", '$window', "$scope", "$rootScope", "baseService", "$http", "$filter", "$controller", "accountService"];
function salesIncentiveController(cboService, commonMessage, $window, $scope, $rootScope, baseService, $http, $filter, $controller, accountService) {
    $rootScope.title = "Sales Incentive";
    $scope.Action = "Save";
    $scope.index = -1;
    ...
    $scope.getMasterOrderSalesIncentiveList = [];
    $scope.getMasterOrderSalesIncentive = function () {
        $http({
            method: 'GET'
            , url: 'SalesManagements/Sales/GetMasterOrderSalesIncentiveList'
            , dataType: 'JSON'
        }).then(function (response) {
            $scope.getMasterOrderSalesIncentiveList = response.data;
        }), function (response) {
    ...
    $scope.popUp = function () {
        $scope.getMasterOrderSalesIncentive();
        angular.element(document.querySelector('#masterOrderSalesIncentivepopUp')).modal('show');
    };
    $scope.paymentTerm();
    ...
    $scope.closeMOSlesPopUp = function () {
        $scope.valueData = '';
        angular.element(document.querySelector('#masterOrderSalesIncentivepopUp')).modal('hide');
    };
    ...
    $scope.clearEmployee = function () {
        ...
        $scope.modelNew.EmployeeId = null;
    };
    ...
    $scope.GetReconAssetTypeList = function () {
        $scope.GetReconTypeListData = function (pageno) {
            baseService.paginationBase('accounts/glitem/GetAssetGLCOAWiseTaxRecon?coaId=' + "", pageno, $scope.reconTypeListParameters)
                .then(function (data) {
    ...
    $scope.getLiabilityTypeList = function () {
        $scope.GLUrl1 = 'accounts/glitem/GetLiabilityGLCOAWiseTaxRecon?coaId=' + "";
