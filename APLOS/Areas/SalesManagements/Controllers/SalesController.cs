...
namespace Aplos.Areas.SalesManagements.Controllers
{
    public class SalesController : BaseController
    {
        ...
        [HttpPost]
        public ActionResult GetSalesTypelist(string column, string value)
        {
            string strkey = "1=1";
            if (string.IsNullOrEmpty(column) == false && string.IsNullOrEmpty(value) == false)
                strkey = column + " like '%" + value + "%'";
            ...
            var identity = (CustomIdentity)Thread.CurrentPrincipal.Identity;
            string sql = @"select top 100 * from (SELECT * FROM HKP.SalesProcessType) AS TEMP WHERE " + strkey + " order by sequence";
        ...
        [HttpGet, Authorize]
        public JsonResult GetSalesTypeAutoSequence()
        {
            return Json(GetSalesTypeSequence(), JsonRequestBehavior.AllowGet);
        }

        [HttpPost]
        public JsonResult CreateSalesType(Dictionary<string, object> data)
        {
            try
            {
                DataSet dsMaster;
                ConnectionManager.DAL.ConManager con = new ConnectionManager.DAL.ConManager("1");
                con.OpenDataSetThroughAdapter("select * from HKP.SalesProcessType where Code='" + data["Code"] + "' AND  Id<>'" + data["Id"] + "'", out dsMaster, false, "1");
                if (dsMaster.Tables[0].Rows.Count > 0)
                    throw new Exception("Same Code already exists!!!");

                con.OpenDataSetThroughAdapter("select * from HKP.SalesProcessType where UserName='" + data["UserName"] + "' AND  Id<>'" + data["Id"] + "'", out dsMaster, false, "1");
                if (dsMaster.Tables[0].Rows.Count > 0)
                    throw new Exception("Same User Name already exists!!!");


                con.OpenDataSetThroughAdapter("select * from HKP.SalesProcessType where Id='" + data["Id"] + "'", out dsMaster, false, "1");
                ...
                {
                    bplib.clsGenID genid = new bplib.clsGenID();
                    genid.GenID("SalesProcessType", out _Id);
                ...
                return Json(new { Error = false, Data = data, Sequence = GetSalesTypeSequence(), Message = AplosMessage.Updated });
        ...
        public ActionResult DeleteSalesType(string id)
        {
            ...
            try
            {
                ...
                ConnectionManager.clsConnection con = new ConnectionManager.clsConnection();
                con.BeginTransaction();
                con.executeQuery("delete from HKP.SalesProcessType where Id='" + id + "'");
                con.CommitTransaction();

                return Json(new { Error = false, Sequence = GetSalesTypeSequence(), Message = AplosMessage.Deleted }, JsonRequestBehavior.AllowGet);
        ...
        private double GetSalesTypeSequence()
        {
            DataTable dt = _sqlRepository.GetDataTable("SELECT  isnull(Max(Sequence),0) AS Sequence FROM HKP.SalesProcessType");
            if (dt.Rows.Count > 0)
                return clsStaticInfo.dbl(dt.Rows[0]["Sequence"].ToString()) + 1;
