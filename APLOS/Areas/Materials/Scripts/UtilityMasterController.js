...
function UtilityMasterController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    ...
    $scope.refreshTemplateIPS = function (args) {
        $("#headchkips").ejCheckBox({ "change": CheckBoxSelectAll});
    };
    ...
    function CheckBoxSelectAll(e) {
        ...
        var filtered = $("#GridUM").data("ejGrid").getFilteredRecords();
        if (angular.isUndefinedOrNull(filtered) || filtered.length == 0) {
            for (var i = 0; i < $scope.UtilityMasterList.length; i++) {
                $scope.UtilityMasterList[i].Flag = ChkOrUnchk;
            }
        }
        else {
            for (var j = 0; j < filtered.length; j++) {
        ...
        var gridObj = $("#GridUM").data("ejGrid");
        gridObj.refreshContent();
    };

    // #endregion checkbox all

    $scope.closeUtilityMasterPopUp = function () {
        MakeIPSData();
        $scope.SaveIPS();
        angular.element(document.querySelector('#UtilityMasterPopUp')).modal('hide');
    }

    $scope.SelectedIPSDataList = [];
    function MakeIPSData() {
        try {
            for (var i = 0; i < $scope.UtilityMasterList.length; i++) {
                if ($scope.UtilityMasterList[i].Flag == true) {
                    if (checkExistsIPS($scope.SelectedIPSDataList, $scope.UtilityMasterList[i].Id) === false) {
                        var ob = {};
                        ob.Id = Math.floor(Math.random() * 9) - 10;
                        ob.UtilityMasterId = $scope.ModelNew.Id;
                        ob.InPutSourceId = $scope.UtilityMasterList[i].Id;
                        ob.Sequence = $scope.UtilityMasterList[i].Sequence;
                        ob.Code = $scope.UtilityMasterList[i].Code;
                        ob.ShortName = $scope.UtilityMasterList[i].ShortName;
                        ob.StandardName = $scope.UtilityMasterList[i].StandardName;
                        ob.UserName = $scope.UtilityMasterList[i].UserName;
                        ob.Active = $scope.UtilityMasterList[i].Active;

                        $scope.SelectedIPSDataList.push(ob);
                        ob = {};
                    }
                    else {
                        throw "This InPut Source " + $scope.UtilityMasterList[i].UserName + " is already taken.";
                    }
                }
            }
        } catch (e) {
    ...
    function checkExistsIPS(list, id) {
        for (var i = 0; i < list.length; i++) {
            if (list[i].InPutSourceId === id) {
                return true;
            }
        }
        return false;
    ...
    $scope.SaveIPS = function () {
        try {
            if (baseService.arrayLength($scope.SelectedIPSDataList) < 0) {
                throw "Select InPut Source.";
            }

            $http({
                method: 'POST',
                url: 'Materials/UtilityMaster/CreateIPS',
                data: { 'assets': $scope.SelectedIPSDataList },
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
    ...
    $scope.message_IPSconfirmation = null;
    $scope.removeIPS = function (obj) {

        $scope.IPSNew = obj.data;
        if (!baseService.isUndefinedOrNull($scope.IPSNew.Id))
            $scope.message_IPSconfirmation = 'Are you sure want to delete permanently [ ' + $scope.IPSNew.AssetName + ' ]';
        angular.element(document.querySelector('#confirmIPSPopUp')).modal('show');
    }

    $scope.DeleteIPS = function () {
        $http({
            method: 'POST',
            url: 'Materials/UtilityMaster/DeleteIPS?id=' + $scope.IPSNew.Id
        }).then(function successCallback(response) {
            if (response.data.Error === true) {
                ShowResult(response.data.Message, 'failure');
            }
    ...
    function GetIPSData() {
        $http({
            method: 'POST',
            url: 'Materials/UtilityMaster/GetIPSData?UtilityMasterId=' + $scope.ModelNew.Id
        }).then(function successCallback(response) {
            $scope.SelectedIPSDataList = response.data;
        });
    }
