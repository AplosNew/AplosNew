'use strict';
IssueReturnRegisterController.$inject = ['fileReader', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', 'cboService', '$window', '$controller'];
function IssueReturnRegisterController(fileReader, commonMessage, $scope, $rootScope, baseService, $http, $filter, cboService, $window, $controller) {
    $rootScope.title = "Issue Return Register";
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.products = [];
    ...
    $scope.GetIssueRegister = function () {
        ...
        $http({
            method: 'POST',
            //url: $scope.getSearchListUrl,
            url: 'Materials/IssueRegister/GetIssueReturnRegister',
            data: {
                fromDate: $scope.report.FromDate,
                toDate: $scope.report.ToDate,
                Type: $scope.productNew.Type
    ...
    $scope.IssueReturnRegisterReportPdf = function (id, reportFormat) {
        debugger;
        ...
        $window.open('Materials/IssueRegister/IssueReturnRegisterReport?reportFormat=' + reportFormat + '&fromDate=' + $scope.report.FromDate + '&toDate=' + $scope.report.ToDate + '&Type=' + $scope.productNew.Type, '_blank');
    };

  
  //  $scope.IssueReturnRegisterReportExcel = function (id, reportFormat) {
		//	debugger;
		//	if ($scope.report.FromDate === "" || $scope.report.FromDate === null || $scope.report.FromDate === undefined) {
		//		ShowResult('Select From Date', 'failure');
		//		return false;
  ...
  //          $window.open('Materials/IssueRegister/IssueReturnRegisterReport?reportFormat=' + reportFormat + '&fromDate=' + $scope.report.FromDate + '&toDate=' + $scope.report.ToDate + '&Type=' + $scope.productNew.Type, '_blank');
	...
	$scope.IssueReturnRegisterReportExcel = function (reportFormat) {
		//debugger;
		if ($scope.report.FromDate === "" || $scope.report.FromDate === null || $scope.report.FromDate === undefined) {
			ShowResult('Select From Date', 'failure');
			return false;
		...
		try {
			var Excel;
			var file_src = 'Materials/IssueRegister/IssueReturnRegisterReport?reportFormat=' + reportFormat + '&fromDate=' + $scope.report.FromDate + '&toDate=' + $scope.report.ToDate + '&Type=' + $scope.productNew.Type;
