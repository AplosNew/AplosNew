'use strict';
function LineController(cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    $rootScope.title = 'Line';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.lines = [];
    $scope.path = 'Organizations/Line/';
    $scope.getListUrl = $scope.path + 'getlist';
    baseService.init($scope.getListUrl);
    $scope.getData = function (pageno) {
        baseService.pagination(pageno)
            .then(function (result) {
                $scope.lines = result.Rows;
            }, function () {
    ...
    $scope.lineNew = angular.copy($scope.line);
    ...
    $scope.GetSequence = function () {
        $http.get('Organizations/line/getautosequence')
            .then(function (response) {
                $scope.lineNew.Sequence = response.data;
            });
    };
    $scope.GetSequence();
    ...
    $scope.Get = function (id, index) {
        $scope.index = index;
        $scope.line = $scope.lines[$scope.index];
        $scope.lineNew = Object.assign({}, $scope.line);
        clearLanguage();
    ...
    $scope.Save = function () {
        ...
        if ($scope.lineNewForm.$valid) {
            angular.copy($scope.lineNew, $scope.line);
            if ($scope.Action === 'Save') {
                $http({
                    method: 'POST',
                    url: 'Organizations/line/create',
                    data: {
                        'line': $scope.lineNew
                    ...
                    if (response.data.Error === true) {
                        ...
                        $scope.lines.push(response.data.Line);
                        $scope.lines = $filter('orderBy')($scope.lines, 'Sequence');
                ...
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        ShowResult(response.data.Message, 'success');
                        if ($scope.index > -1) {
                            $scope.lines[$scope.index] = $scope.line;
                            $scope.lines = $filter('orderBy')($scope.lines, 'Sequence');
                        }
                        ClearFields(response.data.Sequence);
                    }
    ...
    $scope.Delete = function () {
        if (!baseService.isUndefinedOrNull($scope.lineNew.Id)) {
            $http({
                method: 'POST',
                url: 'Organizations/line/delete/' + $scope.lineNew.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
    ...
    $scope.languageData = function () {
        $scope.languageDataUrl = 'Setups/LocalLanguage/GetLineLanguageList?lineId=' + $scope.lineNew.Id;
    ...
    function ClearFields(seq) {
        ...
        $scope.line = {};
        $scope.lineNew = {};
        $scope.lineNew.Sequence = seq;
        $scope.lineNew.Active = true;
        clearLanguage();
        $scope.languageDataList = [];
        $scope.languageTbl = false;
    }
...
LineController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];