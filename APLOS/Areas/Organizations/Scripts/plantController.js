'use strict';
PlantController.$inject = ["addressService", 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter'];
function PlantController(addressService, cboService, commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter) {
    ...
    $scope.Save = function () {
        ...
        if ($scope.plantForm.$valid && $scope.plantForm1.$valid && $scope.plantForm2.$valid) {
            if ($scope.Action === 'Save') {
                $http({
                    method: 'POST',
                    url: 'Organizations/plant/create',
                    ...
                    if (response.data.Error === true) {
                        ...
                        $scope.plant.AddedDate = $filter('dateFilter')($scope.plant.AddedDate);
    ...
    $scope.Delete = function () {
        try {
            if (!baseService.isUndefinedOrNull($scope.plant.Id)) {
                $http({
                    method: 'POST',
                    url: 'Organizations/plant/delete/' + $scope.plant.Id,
                    dataType: 'JSON'
                }).then(function successCallback(response) {
                    if (response.data.Error === true) {
                        ShowResult(response.data.Message, 'failure');
                        ...
                        ShowResult(response.data.Message, 'success');
                        $scope.plants.splice($scope.index, 1);
