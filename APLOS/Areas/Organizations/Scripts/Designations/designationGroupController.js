'use strict';
DesignationGroupController.$inject = ['cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter'];
function DesignationGroupController(cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter) {
    $rootScope.title = 'Designation Group';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.designationgroups = [];
    $scope.path = 'Organizations/designationgroup/';
    ...
    $scope.Delete = function () {
        if (!baseService.isUndefinedOrNull($scope.designationgroupNew.Id)) {
            $http({
                method: 'POST',
                url: $scope.deleteUrl + $scope.designationgroupNew.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
