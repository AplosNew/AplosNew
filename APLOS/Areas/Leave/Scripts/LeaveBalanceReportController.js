...
function LeaveBalanceReportController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService) {
    ...
    $scope.Report = function () {
        ...
        try {
            ...
            var url = 'Leave/LeaveBalanceReport/GetReport?reportFormat=' + reportFormat + "&Year=" + $scope.YearId + "&ToDate=" + $scope.selectedValues.ToDate;
    ...
    $scope.LeaveTypes = function () {
        $http.get('Leave/LeaveBalanceReport/GetLeaveBalance?YearId=' + $scope.YearNo + "&ToDate=" + $scope.selectedValues.ToDate)
    ...
    $scope.LeaveTypes = function (empId) {
        $http.get('Leave/LeaveBalanceReport/GetLeaveBalance?year=' + $scope.YearId + '&empId=' + empId + "&ToDate=" + $scope.selectedValues.ToDate)
            .then(function (response) {
                $scope.LeaveBalanceList = response.data;
            });
    };
