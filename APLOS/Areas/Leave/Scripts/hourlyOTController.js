'use strict';
hourlyOTController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'cboService'];
function hourlyOTController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService) {
    $rootScope.title = 'Hourly OT';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.maternityLeaveTransactions = [];
    $scope.path = 'Leave/HourlyOT/';
    ...
    $scope.ChangeDuration = function () {
        ...
        //$scope.OffDutyHoursModel.Duration = diffDays
    }
    ...
    $scope.GetHourlyOtReport = function () {
        try {
            ...
            if (baseService.isUndefinedOrNull($scope.ManualOutTimeDateWise.FromDate)) {
                ...
                var url = 'Leave/HourlyOT/GetHourlyOT?reportFormat=Excel' + ' &FromDate=' + $scope.ManualOutTimeDateWise.FromDate + ' &ToDate=' + $scope.ManualOutTimeDateWise.ToDate;
