'use strict';
postOfficeController.$inject = ["addressService", 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService',  '$http', '$filter'];
function postOfficeController(addressService, cboService, commonMessage, $scope, $rootScope, baseService,  $http, $filter) {
    $rootScope.title = 'Post Office';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.postOffices = [];
    $scope.countryList = [];
    $scope.path = 'addresses/postoffice/';
    $scope.saveUrl = $scope.path + 'create';
    $scope.updateUrl = $scope.path + 'edit';
    $scope.deleteUrl = $scope.path + 'delete/';
    $scope.getListUrl = $scope.path + 'getpostofficelist';
    baseService.init($scope.getListUrl);
    $scope.getData = function (pageno) {
        baseService.pagination(pageno)
            .then(function (result) {
                $scope.postOffices = result.Rows;
            }, function () {
    ...
    $scope.postOfficeNew = Object.assign({}, $scope.postOffice);

    $scope.GetSequence = function () {
        $http.get('addresses/postoffice/getautosequence')
            .then(function (response) {
                $scope.postOfficeNew.Sequence = response.data;
            });
    };
    $scope.GetSequence();
    ...
    $scope.getPoliceStationCboByDistrictChange = function (districtId) {
        addressService.getPoliceStationCboByDistrictChange(districtId, function (result) {
            $scope.policeStationList = result;
        });
    };

    $scope.Get = function (index) {
        $scope.index = index;
        $scope.postOffice = $scope.postOffices[$scope.index];
        $scope.postOfficeNew = Object.assign({}, $scope.postOffice);
        clearLanguage();
        $scope.languageDataList = [];
        $scope.languageData();
        $scope.getPoliceStationCboByDistrictChange($scope.postOfficeNew.DistrictId);
        $scope.Action = 'Update';
    ...
    $scope.Save = function () {
        ...
        if ($scope.postOfficeForm.$valid) {
            angular.copy($scope.postOfficeNew, $scope.postOffice);
            if ($scope.Action === 'Save') {
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        $scope.postOffices.push(response.data.PostOffice);
                        $scope.postOffices = $filter('orderBy')($scope.postOffices, 'Sequence');
                ...
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        ShowResult(response.data.Message, 'success');
                        if ($scope.index > -1) {
                            $scope.postOffices[$scope.index] = $scope.postOffice;
                            $scope.postOffices = $filter('orderBy')($scope.postOffices, 'Sequence');
                        }
                        ClearFields(response.data.Sequence);
                    }
    ...
    $scope.Delete = function () {
        if (!baseService.isUndefinedOrNull($scope.postOfficeNew.Id)) {
            $http({
                method: 'POST',
                url: $scope.deleteUrl + $scope.postOfficeNew.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
                    ...
                    ShowResult(response.data.Message, 'success');
                    $scope.postOffices.splice($scope.index, 1);
    ...
    $scope.languageData = function () {
        $scope.languageDataUrl = 'Setups/LocalLanguage/GetPostOfficeLanguageList?postOfficeId=' + $scope.postOfficeNew.Id;
    ...
    function ClearFields(seq) {
        $scope.Action = 'Save';
        $scope.postOffice = {};
        $scope.postOfficeNew = { Sequence: seq, Active: true };
        clearLanguage();
        $scope.languageDataList = [];
    }
