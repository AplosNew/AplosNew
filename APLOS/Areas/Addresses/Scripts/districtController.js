'use strict';
districtController.$inject = ['addressService', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter'];
function districtController(addressService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter) {
    $rootScope.title = 'District';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.districts = [];
    $scope.countryList = [];
    $scope.path = 'addresses/district/';
    $scope.saveUrl = $scope.path + 'create';
    $scope.updateUrl = $scope.path + 'edit';
    $scope.deleteUrl = $scope.path + 'delete/';
    $scope.getListUrl = $scope.path + 'getdistrictlist';
    baseService.init($scope.getListUrl);
    $scope.getData = function (pageno) {
        baseService.pagination(pageno)
            .then(function (result) {
                $scope.districts = result.Rows;
            }, function () {
    ...
    $scope.districtNew = Object.assign({}, $scope.district);

    $scope.GetSequence = function () {
        $http.get('addresses/district/getautosequence')
            .then(function (response) {
                $scope.districtNew.Sequence = response.data;
            });
    };
    $scope.GetSequence();
    ...
    $scope.Get = function (index) {
        $scope.index = index;
        $scope.district = $scope.districts[$scope.index];
        $scope.districtNew = Object.assign({}, $scope.district);
        clearLanguage();
        $scope.languageDataList = [];
        $scope.languageData();
        $scope.onCountryChange($scope.districtNew.CountryId);
        $scope.Action = 'Update';
    ...
    $scope.Save = function () {
        ...
        if ($scope.districtForm.$valid) {
            angular.copy($scope.districtNew, $scope.district);
            if ($scope.Action === 'Save') {
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        $scope.districts.push(response.data.District);
                        $scope.districts = $filter('orderBy')($scope.districts, 'Sequence');
                ...
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        ShowResult(response.data.Message, 'success');
                        if ($scope.index > -1) {
                            $scope.districts[$scope.index] = $scope.district;
                            $scope.districts = $filter('orderBy')($scope.districts, 'Sequence');
                            $scope.getData();
                        }
                        ClearFields(response.data.Sequence);
    ...
    $scope.Delete = function () {
        if (!baseService.isUndefinedOrNull($scope.districtNew.Id)) {
            $http({
                method: 'POST',
                url: $scope.deleteUrl + $scope.districtNew.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
                    ...
                    ShowResult(response.data.Message, 'success');
                    $scope.districts.splice($scope.index, 1);
    ...
    $scope.languageData = function () {
        $scope.languageDataUrl = 'Setups/LocalLanguage/GetDistrictLanguageList?districtd=' + $scope.districtNew.Id;
    ...
    function ClearFields(seq) {
        $scope.Action = 'Save';
        $scope.district = {};
        $scope.districtNew = { Sequence: seq, Active: true };
        clearLanguage();
        $scope.languageDataList = [];
    }
