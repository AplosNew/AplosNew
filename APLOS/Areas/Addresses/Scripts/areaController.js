'use strict';
areaController.$inject = ["addressService", 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService',  '$http', '$filter'];
function areaController(addressService, cboService, commonMessage, $scope, $rootScope, baseService,  $http, $filter) {
    $rootScope.title = 'Area';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.areas = [];
    $scope.cityList = [];
    $rootScope.parameters.url = 'addresses/area/getarealist/';
    $scope.path = 'addresses/area/';
    $scope.saveUrl = $scope.path + 'create';
    $scope.updateUrl = $scope.path + 'edit';
    $scope.deleteUrl = $scope.path + 'delete/';
    $scope.getListUrl = $scope.path + 'getarealist';
    ...
    $scope.areaNew = angular.copy($scope.area);

    $scope.GetSequence = function () {
        $http.get('addresses/area/getautosequence')
            .then(function (response) {
                $scope.areaNew.Sequence = response.data;
            });
    }
    $scope.GetSequence();
    ...
    $scope.Get = function (index) {
        $scope.index = index;
        $scope.area = $scope.areas[$scope.index];
        $scope.areaNew = Object.assign({}, $scope.area);
        clearLanguage();
            ...
            if ($scope.Action === 'Save') {
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        $scope.areas = $filter('orderBy')($scope.areas, 'Sequence');
                ...
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        ShowResult(response.data.Message, 'success');
                        if ($scope.index > -1) {
                            $scope.areas[$scope.index] = $scope.area;
                            $scope.areas = $filter('orderBy')($scope.areas, 'Sequence');
                        }
                        ClearFields(response.data.Sequence);
                    }
    ...
    $scope.Delete = function () {
        if (!baseService.isUndefinedOrNull($scope.areaNew.Id)) {
            $http({
                method: 'POST',
                url: $scope.deleteUrl + $scope.areaNew.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
    ...
    $scope.languageData = function () {
        $scope.languageDataUrl = 'Setups/LocalLanguage/GetAreaLanguageList?areaId=' + $scope.areaNew.Id;
    ...
    function ClearFields(seq) {
        $scope.Action = 'Save';
        $scope.area = {};
        $scope.areaNew = { Sequence: seq, Active: true };
        clearLanguage();
        $scope.languageDataList = [];
    }
