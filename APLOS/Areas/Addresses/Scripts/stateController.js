'use strict';
stateController.$inject = ["addressService", 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter'];
function stateController(addressService, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter) {
    $rootScope.title = 'State';
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.states = [];
    $scope.countryList = [];
    $scope.path = 'addresses/state/';
    $scope.saveUrl = $scope.path + 'create';
    $scope.updateUrl = $scope.path + 'edit';
    $scope.deleteUrl = $scope.path + 'delete/';
    $scope.getListUrl = $scope.path + 'getstatelist';
    baseService.init($scope.getListUrl);
    $scope.getData = function (pageno) {
        baseService.pagination(pageno)
            .then(function (result) {
                $scope.states = result.Rows;
            }, function () {
    ...
    $scope.stateNew = angular.copy($scope.state);

    $scope.GetSequence = function () {
        $http.get('addresses/State/GetAutoSequence')
            .then(function (response) {
                $scope.stateNew.Sequence = response.data;
            });
    };
    $scope.GetSequence();
    ...
    $scope.Get = function (index) {
        ...
        $scope.stateNew = Object.assign({}, $scope.state);
        clearLanguage();
    ...
    $scope.Save = function () {
        ...
        if ($scope.stateForm.$valid) {
            angular.copy($scope.stateNew, $scope.state);
            if ($scope.Action === 'Save') {
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        $scope.states.push(response.data.State);
                        $scope.states = $filter('orderBy')($scope.states, 'Sequence');
                ...
                $http({
                    ...
                    if (response.data.Error === true) {
                        ...
                        ShowResult(response.data.Message, 'success');
                        if ($scope.index > -1) {
                            $scope.states[$scope.index] = $scope.state;
                            $scope.states = $filter('orderBy')($scope.states, 'Sequence');
                        }
                        ClearFields(response.data.Sequence);
                    }
    ...
    $scope.Delete = function () {
        if (!baseService.isUndefinedOrNull($scope.stateNew.Id)) {
            $http({
                method: 'POST',
                url: $scope.deleteUrl + $scope.stateNew.Id,
                dataType: 'JSON'
            }).then(function successCallback(response) {
                if (response.data.Error === true) {
                    ShowResult(response.data.Message, 'failure');
                    ...
                    ShowResult(response.data.Message, 'success');
                    $scope.states.splice($scope.index, 1);
    ...
    $scope.languageData = function () {
        $scope.languageDataUrl = 'Setups/LocalLanguage/GetStateLanguageList?stateId=' + $scope.stateNew.Id;
    ...
    function ClearFields(seq) {
        $scope.Action = 'Save';
        $scope.state = {};
        $scope.stateNew = { Sequence: seq, Active: true };
        clearLanguage();
        $scope.languageDataList = [];
    }
