'use strict';
lineProductionBookingController.$inject = ['fileReader', '$scope', '$rootScope', 'baseService', '$http', '$filter', 'cboService'];
function lineProductionBookingController(fileReader, $scope, $rootScope, baseService, $http, $filter, cboService) {
    $rootScope.title = "Line Production Qty Assign";
    $scope.Action = 'Save';
    ...
    $scope.Save = function () {
        ...
        $http({
            ...
            , data: {
                ...
                , 'prdQty': $scope.modelNew.ProductionQty
                , 'entities': $scope.productionList
            }
            , dataType: 'JSON'
        }).then(function successCallback(response) {
            if (response.data.Error === true)
