'use strict';
masterOrderCheckByController.$inject = ['accountService', '$window', 'cboService', 'commonMessage', '$scope', '$rootScope', 'baseService', '$http', '$filter', '$controller', 'toaster'];
function masterOrderCheckByController(accountService, $window, cboService, commonMessage, $scope, $rootScope, baseService, $http, $filter, $controller, toaster) {
    $rootScope.title = "Master Order CheckBy";
    $scope.Action = 'Save';
    $scope.index = -1;
    $scope.files = [];
    ...
    $scope.getListUrl = $scope.path + 'GetCheckByList';
    ...
    $scope.ApproveByList = [];
    $scope.GetApproveByCboList = function () {
        $http({
            method: 'GET',
            url: $scope.path + 'GetApproveByCboList'
        }).then(function successCallback(response) {
            $scope.ApproveByList = response.data;
        });
    }
    $scope.GetApproveByCboList();
    ...
    function getSalesOrderList() {
        ...
        $http.get('OrderManagements/MasterOrder/GetSOListForCheck?masterItemId=' + $scope.masterItemId)
            .then(function (response) {
    ...
    $scope.soEdit = function (data) {
        try {
            ...
            $http({
                method: 'POST'
                , url: $scope.path + 'CheckSalesOrder'
                , data: {
