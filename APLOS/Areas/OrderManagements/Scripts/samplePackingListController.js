'use strict';
SamplePackingListController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'cboService'];
function SamplePackingListController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, cboService) {
    $rootScope.title = "Sample Packing List";
    $scope.Action = 'Save';
    ...
    $scope.path = 'OrderManagements/samplePackingList/';
    ...
    $scope.getCustomerPopUp = function () {
        ...
        $scope.customerUrl = 'OrderManagements/sampleorder/getcustomerbyspf/';
    ...
    $scope.customerDoubleClick = function (data) {
        ...
        $scope.samplePackingNew.PlantId = data.PlantId;
        $scope.samplePackingNew.EntityId = data.EntityId;
        $scope.samplePackingNew.SalesOrganisationId = data.SalesOrganisationId;
        $scope.samplePackingNew.PartyId = data.PartyId;
        $scope.samplePackingNew.PartyName = data.PartyName;
        ...
        $scope.samplePackingNew.DeliveryByAddress = null;
    ...
    $scope.customerByButton = function () {
        if (baseService.isUndefinedOrNull($scope.valueData)) {
            ShowResult('Please at first select row', 'filure', 'customerPopUp');
        }
    ...
    $scope.clearCustomer = function () {
        $scope.samplePackingNew.PartyId = null;
        $scope.samplePackingNew.PartyName = null;
        $scope.samplePackingNew.InvoicingByAddress = null;
        $scope.samplePackingNew.DeliveryByAddress = null;
        $scope.excluedColumnList = [];
    }
    ...
    function getPartyPlantList(isEdit) {
        ...
        $http.get('Parties/party/GetPartyPlantCbo?partyId=' + $scope.samplePackingNew.PartyId).then(function (response) {
    ...
    //#region Material Master By Customer****
    ...
    $scope.getMaterialMasterPopUp = function () {
        ...
        $scope.materialMasterDataList = [];
        $scope.materialMasterUrl = 'OrderManagements/sampleorder/getmaterialmasterbycustomer?partyId=' + $scope.samplePackingNew.PartyId
            + '&sampleOrderSubMaterialIds=' + baseService.getColumnValueList($scope.materialList, 'SampleOrderSubMaterialId');
        baseService.setCurrentPage('dataList');
            ...
            baseService.paginationBase($scope.materialMasterUrl, pageno, $scope.materialMasterParameters)
                .then(function (result) {
                    for (var a = 0; a < baseService.arrayLength(result.Rows); a++) {
                        result.Rows[a].Flag = ifSelectedInMaterialPopUp(result.Rows[a].SampleOrderSubMaterialId);
                    }
    ...
    function ifSelectedInMaterialPopUp(id) {
        for (var t = 0; t < baseService.arrayLength($scope.materialList); t++) {
            if ($scope.materialList[t].SampleOrderSubMaterialId === id)
                return true;
        }
        return false;
    }
            ...
            cboService.getUomCboByMaterialMaster(JSON.stringify(materialMasterId), function (response) {
                ...
                if (event.currentTarget.checked) {
                    $scope.materialList.push({
                        ...
                        , FirstCharacteristicsId: data.FirstCharacteristicsId
                        , SecondCharacteristicsId: data.SecondCharacteristicsId
                        , ThirdCharacteristicsId: data.ThirdCharacteristicsId

                        , FirstCharacteristicsValueId: data.FirstCharacteristicsValueId
                        , SecondCharacteristicsValueId: data.SecondCharacteristicsValueId
                        , ThirdCharacteristicsValueId: data.ThirdCharacteristicsValueId

                        , Detail: data.Detail

                        , SamplePackingListId: $scope.samplePackingNew.Id
                        , SampleOrderId: data.SampleOrderId
                        , SampleOrderSubMaterialId: data.SampleOrderSubMaterialId
                        , SampleSubMaterial: data.SampleSubMaterial
                    ...
                    materialSplice($scope.materialList, data.SampleOrderSubMaterialId);
    ...
    function materialSplice(list, id) {
        for (var i = 0; i < baseService.arrayLength(list); i++) {
            if (list[i].SampleOrderSubMaterialId === id)
                list.splice(i, 1);
        }
    }
    ...
    //#endregion Material Master By Customer

    //#region PackLessMaterial
    ...
    //#endregion 1st Packing Form

    //#region Second Packing Form
    $scope.secondFormPopup = function (data) {
        try {
            ...
            $scope.secondPacking = {
                ...
                , PackingFormId: null
                , PackingForm: null
                , PackingFormNo: data.PackingFormNo
                , ContainerQty: 0
                , ContentQty: 0
                , UoMId: null
                , UoMName: null
    ...
    // #endregion All Materials

    //#region View Materials
    ...
    $scope.Clear = function () {
        $scope.packingList = [];
        $scope.samplePackingNew = { PackingDate: $filter('dateFiltering')(Date.now()) };
        $scope.samplePacking = {};
        $scope.excluedColumnList = [];
        $scope.billingAddressList = [];
        $scope.shippingAddressList = [];
