'use strict';
salesOrderRevisedateController.$inject = ['commonMessage', '$scope', '$rootScope', 'baseService', '$routeParams', '$location', '$http', '$filter', 'toaster', 'cboService', '$window'];
function salesOrderRevisedateController(commonMessage, $scope, $rootScope, baseService, $routeParams, $location, $http, $filter, toaster, cboService, $window) {
    ...
    $scope.GetInvoiceData = function () {
        ...
        $scope.saveBtnDisable = false;
         if (baseService.isUndefinedOrNull($scope.invoice.FromDate)) {
            ...
            var parameters = { 'fromDate': $scope.invoice.FromDate, 'toDate': $scope.invoice.ToDate,  'DateRange': $scope.invoice.DateRange };
            $http({
                method: "POST",
                dataType: 'JSON',
                url: 'OrderManagements/SalesOrderUpdate/GetSalesOrderReviseDateList',
                data: parameters
            }).then(function successCallback(response) {
