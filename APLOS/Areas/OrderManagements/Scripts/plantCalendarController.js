'use strict';
PlantCalendarController.$inject = ["cboService", "commonMessage", "$scope", "$rootScope", "baseService", "$filter", "$window", "$http"];
function PlantCalendarController(cboService, commonMessage, $scope, $rootScope, baseService, $filter, $window, $http) {
    ...
    $scope.path = 'OrderManagements/PlantCalendar/';
    ...
    $scope.HolidayAssign = function () {
        ...
        try {
            ...
            $http({
                method: 'POST',
                url: $scope.path + 'HolidayAssign',
                data: { 'holidayid': $scope.HolidayCategoryId, 'fromdate': fromdate, 'todate': todate },
                dataType: 'JSON'
            }).then(function successCallback(response) {
                $scope.navigation($scope.navigationargs);
                ShowResult(response.data.Message, 'success');
    ...
    $scope.WorkDayAssign = function () {
        ...
        try {
            ...
            $scope.workigHours = 10;
            $scope.OTHours = 0;

            $http({
                method: 'POST',
                url: $scope.path + 'WorkDayAssign',
                data: { 'fromdate': fromdate, 'todate': todate, 'hours': $scope.workigHours, 'OT': $scope.OTHours },
                dataType: 'JSON'
            }).then(function successCallback(response) {
                $scope.navigation($scope.navigationargs);
                ShowResult(response.data.Message, 'success');
    ...
    $scope.navigation = function (args) {
        ...
        $http({
            method: 'POST',
            url: $scope.path + 'getDayStatusRange',
            data: { wdate: dt },
            dataType: 'JSON'
    ...
    $scope.contextMenu = {
        appointment: [
            ...
            { id: "xedit", text: " Assign as Working Day", },
            { id: "xweekoff", text: "Assign as Week-off", },
            { id: "xholiday", text: "Assign as Holiday", }
        ],
        cells: [
            { id: "yholiday", text: "Assign as Holiday" },
            //{ id: "yweekoff", text: "Assign as Week-off" },
    ...
    $scope.SimulateVisual = function () {
        try {
            $http({
                method: 'POST',
                url: $scope.path + "GetScheduleData"
            }).then(function successCallback(res) {
