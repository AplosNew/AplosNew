...
{
    public class ProductionReportsController : BaseController
    {
        ...
        public ProductionReportsController(IProductionOrderService productionOrderService, IUnitOfWork U, ISqlRepository R)
        {
...
ORDER BY e.PlantId, e.UserName";
        ...
        [HttpPost, Authorize]
        public ActionResult GetProcessForMultipleEntities(string entityIds)
        {
            try
            {
                ...
                var result = orgAuth.GetProcessForMultipleEntitiesCbo(entityIds);
        ...
        public IWorkbook OS2xlsFile(ExcelEngine excelEngine, string entityid, DataTable dt)
        {
            ...
            try
            {
                ...
                COL++;
                sheet[ROW, COL].Text = "Line Start Date (Prod. For Current WC)";
                sheet[ROW, COL].ColumnWidth = 10;
                int colLineStartDateAtWC = COL;
                COL++;
                sheet[ROW, COL].Text = "Line Start Date (Prod. For Current Entity)";
                sheet[ROW, COL].ColumnWidth = 10;
                ...
                COL++;
                sheet[ROW, COL].Text = "Line End Date (Prod. For Current WC)";
                sheet[ROW, COL].ColumnWidth = 10;
                int colLineEndDateAtWC = COL;
                COL++;
                sheet[ROW, COL].Text = "Line End Date (Prod. For Current Entity)";
                sheet[ROW, COL].ColumnWidth = 10;
                ...
                COL++;
                sheet[ROW, COL].Text = "Line Target UOM";
                sheet[ROW, COL].ColumnWidth = 10;
                ...
                COL++;
                sheet[ROW, COL].Text = "Line Plan Hour(Calendar)";
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
                sheet[ROW, COL].ColumnWidth = 10;
                ...
                COL++;
                sheet[ROW, COL].Text = "Actual Line Cost For The Day";
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
                sheet[ROW, COL].ColumnWidth = 10;
                ...
                COL++;
                sheet[ROW, COL].Text = "Line Target Per Day (PR)";
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
                sheet[ROW, COL].ColumnWidth = 10;
                int colLineTargetPerDay = COL;
                COL++;
                ...
                COL++;
                sheet[ROW, COL].Text = "Minimum Line Target (Based on the line cost)";
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
                sheet[ROW, COL].ColumnWidth = 10;
                ...
                COL++;
                sheet[ROW, COL].Text = "Production Loss Due To Line Target";
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
                sheet[ROW, COL].ColumnWidth = 10;
                ...
                COL++;
                sheet[ROW, COL].Text = "Production Loss Due To Line Target Amount";
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
                sheet[ROW, COL].ColumnWidth = 10;
                ...
                workbook.Worksheets[0].Name = "OS2 -Line wise order wise plan";
                ...
                reportUtility.CompanyPlantHeaderNew(ref sheet, 1, "Line Wise Order Wise Day Wise Production Planning. Last Updated(" + Convert.ToDateTime(dt.Rows[0]["TargetDate"].ToString()).ToString("dd-MMM-yyyy") + ")", identity.CompanyId, identity.CompanyName, "");
                ...
                for (int i = 0; i < pivotTable.Fields.Count; i++)
                {
                    if (i == colbuyer - 1 || i == colBuyerOrderNo - 1 || i == colPlant - 1 || i == colEntity - 1)
                        continue;
                    pivotTable.Fields[i].Subtotals = PivotSubtotalTypes.None;
                }
                ...
                field.NumberFormat = clsStaticInfo.NumberFormat();
                pivotTable.DataFields.Add(field, "Actual Line Cost For The Day", PivotSubtotalTypes.Sum);
                ...
                field.NumberFormat = clsStaticInfo.NumberFormat();
                pivotTable.DataFields.Add(field, "Minimum Line Target", PivotSubtotalTypes.Sum);
                ...
                field.NumberFormat = clsStaticInfo.NumberFormat();
                pivotTable.DataFields.Add(field, "Loss Qty (Due To Line Target)", PivotSubtotalTypes.Sum);
                ...
                field.NumberFormat = clsStaticInfo.NumberFormat();
                pivotTable.DataFields.Add(field, "Loss Amount (Due To Line Target)", PivotSubtotalTypes.Sum);
        ...
        public IWorkbook OS5xlsFile(ExcelEngine excelEngine, string entityid, string fromDate, string toDate, DataTable dt)
        {
            ...
            try
            {
                ...
                for (int KK = 0; KK < dtDistinctProcesses.Rows.Count; KK++)
                {
                    ...
                    {
                        if (i == colProduct - 1 || i == colbuyer - 1)
                            continue;
        ...
        private void getOS2Backup(out DataTable dtOS2)
        {
            dtOS2 = new DataTable();
            try
            {
                string sql = @"select 
                           ...
                           --production at WC Level
                            LEFT OUTER JOIN (
				                            SELECT s.EntityId,s.WorkCenterMasterId,s.ProcessId,SUM(d.Qty) AS ProductionQtyAtWC,MIN(s.ProductionDate) AS LineStartDateAtWC,MAX(s.ProductionDate) AS LineENDDateAtWC
                            ...
                            LEFT OUTER JOIN (
				                            SELECT pod.ProductionOrderId,s.ProcessId,SUM(d.Qty) AS ProductionQtyAtPR,MIN(s.ProductionDate) AS LineStartDateAtPR,MAX(s.ProductionDate) AS LineEndDateAtPR
				                            FROM  trn.ProductionOrderDetail POD 
				                            inner join trn.ProductionSummary S ON pod.SalesOrderId=s.SalesOrderId
				                            INNER JOIN trn.ProductionSummaryDetail AS D  ON d.ProductionSummaryId=s.Id
                      ...
                      FORMAT(p1.ProductionDate,'dd-MMM-yyyy') AS TargetDate,p1.ProductionDate AS ProcessDate,po.Id AS ProductionOrderID,
                         ...
                         t1.LSD,t1.CommitmentDate,
						   ...
						   INNER JOIN hkp.Process AS PROS ON PROS.Id=popc.ProcessId
                            ...
                            max(so.DeliveryDate) AS LastDeliveryDate,
                            MAX(so.CommitmentDate) AS ProductionCompletionDate,
                            SUM(so.Rate * so.Qty)/SUM(so.Qty) AS FOB, SUM(so.CM * so.Qty)/SUM(so.Qty) AS CM,
                            sum(SO.Qty) AS OrderQty from trn.ProductionOrderDetail POD 
                            left outer join trn.SalesOrder SO on so.id=pod.SalesOrderId
                            left outer join trn.MasterOrderItem MOI on moi.Id=so.MasterOrderItemId
...
";
        ...
        [HttpGet, Authorize]
        public ActionResult OS4xls_backup(string entityid, string fromDate)
        {
            ExcelEngine excelEngine = null;
            IApplication application = null;
            IWorkbook workbook = null;
            ...
            try
            {
                ...
                getOS4(entityid, fromDate, out dtOrderMaster);
                ...
                workbook.Worksheets[0].Name = "OS4";
                sheet = workbook.Worksheets[0];
                ...
                int colstart = COL;
                sheet[ROW, COL].Text = "Prod. Order ID";
                sheet[ROW, COL].ColumnWidth = 16;
                int colProductionOrderID = COL;
                COL++;
                sheet[ROW, COL].Text = "Work Center";
                ...
                sheet[ROW, COL].Text = "Prod. Order Id";
                sheet[ROW, COL].ColumnWidth = 16;
                int colActualProductionOrderId = COL;
                COL++;
                sheet[ROW, COL].Text = "Work Center";
                sheet[ROW, COL].ColumnWidth = 16;
                int colActualWorkCenter = COL;
                COL++;
                ...
                sheet[ROW, COL].Text = "Buyer";
                sheet[ROW, COL].ColumnWidth = 12;
                int colActualBuyer = COL;
                COL++;
                sheet[ROW, COL].Text = "Style";
                sheet[ROW, COL].ColumnWidth = 12;
                int colActualStyle = COL;
                COL++;
                ...
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].Merge();
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].HorizontalAlignment = ExcelHAlign.HAlignCenter;
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].Text = "Production Data";
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].CellStyle.Font.Bold = true;
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].CellStyle.Font.Size = 10f;
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].BorderInside(ExcelLineStyle.Hair);
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].BorderAround(ExcelLineStyle.Hair);
                ...
                COL++; colstart = COL;
                sheet[ROW, COL].Text = "Difference In Production";
                sheet[ROW, COL].ColumnWidth = 12;
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
                int colDifferenceInProduction = COL;
                COL++;
                sheet[ROW, COL].Text = "Performance In Percentage";
                sheet[ROW, COL].ColumnWidth = 12;
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
                int colPerformanceInPercentage = COL;
                COL++;
                ...
                COL++;
                sheet[ROW, COL].Text = "Plan Efficiency";
                sheet[ROW, COL].ColumnWidth = 12;
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
                int colPlanEfficiency = COL;
                COL++;
                sheet[ROW, COL].Text = "Produce Minutes";
                sheet[ROW, COL].ColumnWidth = 12;
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
                int colProduceMinutes = COL;
                COL++;
                sheet[ROW, COL].Text = "Prod. Efficiency";
                sheet[ROW, COL].ColumnWidth = 12;
                sheet[ROW, COL].HorizontalAlignment = ExcelHAlign.HAlignRight;
                int colProductionEfficiency = COL;

                sheet.Range[ROW - 1, colstart, ROW - 1, COL].Merge();
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].HorizontalAlignment = ExcelHAlign.HAlignCenter;
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].Text = "Performances";
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].CellStyle.Font.Bold = true;
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].CellStyle.Font.Size = 10f;
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].BorderInside(ExcelLineStyle.Hair);
                sheet.Range[ROW - 1, colstart, ROW - 1, COL].BorderAround(ExcelLineStyle.Hair);
                ...
                sheet.Range[ROW, 1, ROW, endCol].BorderInside(ExcelLineStyle.Hair);
                sheet.Range[ROW, 1, ROW, endCol].BorderAround(ExcelLineStyle.Hair);
                sheet.Range[ROW - 1, 1, ROW, endCol].RowHeight = 14;
                ...
                for (int i = 0; i < dtOrderMaster.Rows.Count; i++)
                {
                    ...
                    sheet[ROW, colProductionOrderID].Text = dtOrderMaster.Rows[i]["ProductionOrderID"].ToString();
                    sheet[ROW, colWorkCenter].Text = GetDate(dtOrderMaster.Rows[i]["WorkCenter"].ToString());
                    sheet[ROW, colbuyer].Text = dtOrderMaster.Rows[i]["buyer"].ToString();
                    sheet[ROW, colStyleNo].Text = dtOrderMaster.Rows[i]["StyleNo"].ToString();


                    sheet[ROW, colActualProductionOrderId].Text = dtOrderMaster.Rows[i]["ActualProductionOrderId"].ToString();
                    sheet[ROW, colWorkCenter].Text = dtOrderMaster.Rows[i]["ActualWorkCenter"].ToString();
                    sheet[ROW, colActualBuyer].Number = clsStaticInfo.dbl(dtOrderMaster.Rows[i]["ActualBuyer"].ToString());
                    sheet[ROW, colActualStyle].Text = dtOrderMaster.Rows[i]["ActualStyle"].ToString();


                    //dates
                    SetDate(sheet[ROW, colPlanDate], dtOrderMaster.Rows[i]["PlanDate"].ToString());
                    SetDate(sheet[ROW, colProductionDate], dtOrderMaster.Rows[i]["ProductionDate"].ToString());

                    //numbers
                    sheet[ROW, colPlanQty].Number = clsStaticInfo.dbl(dtOrderMaster.Rows[i]["PlanQty"].ToString());
                    sheet[ROW, colPlanCM].Number = clsStaticInfo.dbl(dtOrderMaster.Rows[i]["PlanCM"].ToString());

                    sheet[ROW, colProductionQty].Number = clsStaticInfo.dbl(dtOrderMaster.Rows[i]["ProductionQty"].ToString());
                    sheet[ROW, colActualCM].Number = clsStaticInfo.dbl(dtOrderMaster.Rows[i]["ActualCM"].ToString());

                    sheet[ROW, colDifferenceInProduction].Number = clsStaticInfo.dbl(dtOrderMaster.Rows[i]["DifferenceInProduction"].ToString());
                    sheet[ROW, colPerformanceInPercentage].Number = clsStaticInfo.dbl(dtOrderMaster.Rows[i]["PerformanceInPercentage"].ToString());
                    sheet[ROW, colPlanMinutes].Number = clsStaticInfo.dbl(dtOrderMaster.Rows[i]["PlanMinutes"].ToString());
                    sheet[ROW, colPlanEfficiency].Number = clsStaticInfo.dbl(dtOrderMaster.Rows[i]["PlanEfficiency"].ToString());
                    sheet[ROW, colProduceMinutes].Number = clsStaticInfo.dbl(dtOrderMaster.Rows[i]["ProduceMinutes"].ToString());
                    sheet[ROW, colProductionEfficiency].Number = clsStaticInfo.dbl(dtOrderMaster.Rows[i]["ProductionEfficiency"].ToString());
                ...
                sheet[startRow, colProductionDate, ROW, colProductionDate].NumberFormat = "dd-MMM-yyyy";
        ...
        public ActionResult OS4xls(string entityid, string fromDate, string toDate)
        {
            ...
            try
            {
                ...
                foreach (KeyValuePair<string, DateTime> item in allDates)
                {
                    ...
                    if (dtOrderMaster.DefaultView.Count > 0)
                    {
                        for (int i = 0; i < dtOrderMaster.DefaultView.Count; i++)
                        {
                            ...
                            sheet[ROW, colPlanEff].Formula = "IF(" + CellAddr(colWorkStation, ROW) + "*" + CellAddr(colPlanHours, ROW) + ">0,(" + CellAddr(colPlanMinutes, ROW) + ")/(" + CellAddr(colWorkStation, ROW) + "*" + CellAddr(colPlanHours, ROW) + "*60),0)";
                ...
                field.NumberFormat = clsStaticInfo.NumberFormat(2, true);
                pivotTable.DataFields.Add(field, "Diff In Eff.", PivotSubtotalTypes.Average);
        ...
        [HttpGet, Authorize]
        public ActionResult ProductionDataXls(string entityid, string fromDate, string toDate,string ProcessId)
        {
            ExcelEngine excelEngine = null;
            IApplication application = null;
            IWorkbook workbook = null;
            ...
            try
            {
                ...
                Dictionary<string, List<DataRow>> dicActualData = null;
                getProductionData(entityid, fromDate, toDate,ProcessId, out dicActualData);
        ...
        public ActionResult OS1xls(string entityid)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < pivotTable.Fields.Count; i++)
                {
                    if (i == colPlant - 1 || i == colEntity - 1 || i == colBuyer - 1 || i == colBuyerOrderNo - 1)
                        continue;
                    pivotTable.Fields[i].Subtotals = PivotSubtotalTypes.None;
                }
                ...
                for (int i = 0; i < pivotTable.Fields.Count; i++)
                {
                    if (i == colPlant - 1 || i == colEntity - 1 || i == colBuyer - 1)
                        continue;
                    pivotTable.Fields[i].Subtotals = PivotSubtotalTypes.None;
                }
        ...
        public ActionResult LineBookingStatus(string entityid)
        {
            ...
            try
            {
                ...
                string strFileName = "Line Booking Status.xlsx";
                workbook.SaveAs(strFileName, ExcelSaveType.SaveAsXLS, System.Web.HttpContext.Current.Response, ExcelDownloadType.PromptDialog);
                workbook.Close();
            }
            catch (Exception ex)
        ...
        private void getOrderMaster(string entityid, out DataTable dtOrderMaster)
        {
            ...
            string sql = @" SELECT trkp.UserName AS Plant,trke.UserName AS Entity,so.Id AS SalesOrderId, b.UserName AS Buyer,ei.EmployeeName AS ResponsiblePerson,mo.MasterOrderNo,mm.UserName AS Material,
            ...
            ORDER BY trkp.UserName,trke.UserName, pod.ProductionOrderId,so.DeliveryDate,SO.ID";
        ...
        private void getOS1(string entityid, out DataTable dtOrderMaster)
        {
            ...
            string sql = @"SELECT so.Id AS SalesOrderId, b.UserName AS Buyer,ei.EmployeeName AS ResponsiblePerson,mo.MasterOrderNo,mm.UserName AS Material,
                           OC.UserName AS OrderCategory,os.UserName AS OrderStatus,OC1.UserName AS SOCategory,os1.UserName AS SOStatus,    MA.StandardName AS Article,                   
                            ...
                            ,So.Rate AS FOB,SO.CM ,SO.Rate*SO.Qty AS OrderAmount,SO.CM*SO.Qty AS CMAmount,FORMAT(SO.LSD,'dd-MMM-yyyy') AS LSD,isnull(DATEDIFF(DAY,so.LSD,so.DeliveryDate),0) AS Diff
                            ,uom.UserName AS UOM,so.[Description] AS SODesc,cur.Code AS Currency,trkp.UserName AS Plant,trke.UserName AS Entity
                            ,PRPD.PRBookedQuantity,sopd.SOBookedQuantity,PLN.PRPlanQty
                              FROM trn.MasterOrder MO
                            left outer join trn.MasterOrderItem MOI on moi.MasterOrderId=mo.Id
            ...
            ORDER BY trkp.UserName,trke.UserName, b.UserName,so.DeliveryDate,SO.ID";
...
WHERE pp.ProductionDate  BETWEEN '" + fromDate + @"' AND '" + toDate + @"'  AND pp.EntityID in (" + entityid + @")
...
";
        ...
        private void getProductionData(string entityid, string fromDate, string toDate,string ProcessId, out Dictionary<string, List<DataRow>> dtOrderMaster)
        {
            string ProcessFilter = "";
            if (ProcessId != "All")
            {
                ProcessFilter = " AND ps.ProcessId='" + ProcessId + "' ";
            }

            string sql = @"SELECT  trkp.UserName AS Plant,trke.UserName AS Entity,pp.EntityID,pp.WorkCenterMasterId, PP.ProductionOrderID,wcm.UserName AS WorkCenter,FORMAT(PP.ProductionDate,'dd-MMM-yyyy') AS ActualDate,pp.Quantity AS ActualQty,ORD.CM*pp.Quantity AS ActualCM,
                            pt1.SPT AS SAM,ord.Material,ord.Article,p.UserName AS Process,
                            ord.Product, ord.ProductCategory,Format(SN.AddedDate,'dd-MMM-yyyy') AS SnapshotDate,
                            sn.Quantity AS PlanQty,ORD.CM*sn.Quantity AS PlanCM,ORD.CM,
                            ...
                            FROM (SELECT  ps.ProcessId,ps.EntityId, ps.ProductionOrderId,ps.ProductionDate,ps.WorkCenterMasterId,COUNT(*) AS ProductionHours,SUM(ps.Quantity) AS Quantity
                                    FROM trn.ProductionSummary AS ps 
      		                            WHERE ps.ProductionDate BETWEEN '" + fromDate + @"' AND '" + toDate + @"' AND ps.EntityID in (" + entityid + @") "+ProcessFilter+@"
      		                            --AND ps.ProcessId=(select XX.ProcessId from trn.ProductionOrderProcessSet AS XX where XX.IsBaseProcess=1 and XX.ProductionOrderID=ps.ProductionOrderId)
                                  GROUP BY   ps.ProcessId,  ps.EntityId,ps.ProductionOrderId,ps.ProductionDate,ps.WorkCenterMasterId
                            ) AS pp
                            LEFT OUTER JOIN ProductionOrderSchedulingParametersType1 AS PT1 ON pt1.ProductionOrderID=pp.ProductionOrderID
...
";
        ...
        private void getOrderMasterMain(string entityid, string status, out DataTable dtOrderMaster)
        {
            ...
            string sql = @"  SELECT trkp.UserName AS Plant,trke.UserName AS Entity, b.UserName AS Buyer,ei.EmployeeName AS ResponsiblePerson,mo.MasterOrderNo,mm.UserName AS Material,
                           OC.UserName AS OrderCategory,os.UserName AS OrderStatus, mo.BuyerReferenceNo,MO.OwnReferenceNo,                         
                            pc.UserName AS ProductCategory,  pm.UserName AS Product,sum(MOI.TotalQty) AS TotalQty,uom.UserName AS UOM,
                            ...
                            GROUP BY  trkp.UserName,trke.UserName, b.UserName,ei.EmployeeName,mo.MasterOrderNo,mm.UserName,
        ...
        private void getOrderMasterWithSalesOrderMain(string entityid, out DataTable dtOrderMaster)
        {
            string sql = @"  SELECT p2.UserName AS Plant,e.UserName AS Entity, b.UserName AS Buyer,ei.EmployeeName AS ResponsiblePerson,mo.MasterOrderNo,mm.UserName AS Material,
                           ...
                           so.Qty, so.MainRawMaterialInhouseDate,
...
ORDER BY p2.UserName,e.UserName, mo.MasterOrderNo";
        ...
        public ActionResult getOrderMasterDummy()
        {
            string sql = @"
                            SELECT so.Id AS SalesOrderId, b.UserName AS Buyer,ei.EmployeeName AS ResponsiblePerson,mo.MasterOrderNo,mm.UserName AS Material,
                            POD.ProductionOrderId,OC.UserName AS OrderCategory,os.UserName AS OrderStatus,ps.UserName  AS productionStatus,
                            so.DeliveryDate,so.CommitmentDate,so.Qty AS SOQty
