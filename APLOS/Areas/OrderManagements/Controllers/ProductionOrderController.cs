...
WHERE " + strkey + " ORDER BY  TEMP.ProductionGrouping,TEMP.ArticleId";
        ...
        [HttpPost, Authorize]
        public ActionResult SaveServiceAcknowledgementDefault(IEnumerable<HttpPostedFileBase> UploadDefault, string UploadDefault_data)
        {
            try
            {
                UploadDefault_data = UploadDefault_data.Replace("\"", "");
                if (string.IsNullOrEmpty(UploadDefault_data))
                    throw new Exception("Save the PO order first");

                foreach (var file in UploadDefault)
                {
                    var fn = file.FileName;
                    var fileName = Path.GetFileName(UploadDefault_data + new FileInfo(fn).Extension);
                    var destinationPath = Path.Combine(ResourcesPathReader.GetServiceAcknoledgementImagePath(), fileName);

                    if (System.IO.Directory.Exists(ResourcesPathReader.GetServiceAcknoledgementImagePath()) == false)
                    {
                        try
                        {
                            System.IO.Directory.CreateDirectory(ResourcesPathReader.GetServiceAcknoledgementImagePath());
                        }
                        catch (Exception)
                        {
                    ...
                    ConnectionManager.clsConnection connection = new ConnectionManager.clsConnection();
                    string sql = "select* from trn.ServiceAcknowledgementMaster where id='" + UploadDefault_data + "'";
                    DataSet dsLocal = null;
                    connection.BeginTransaction();
                    connection.getDataSet(sql, out dsLocal);
                    connection.CommitTransaction();
