...
{
    public class TaskSubCategoryToDoController : BaseController
    {
        private static string Flag = TaskCategoryFlagEnum.ToDo.ToString();
        string TableName = "HKP.TaskSubCategory";
        //authentication for
        ...
        public TaskSubCategoryToDoController(ISqlRepository R)
        {
            _sqlRepository = R;
        }
        ...
        public JsonResult Create(Dictionary<string, object> data)
        {
            try
            {
                ...
                con.OpenDataSetThroughAdapter("select * from " + TableName + " where FLAG='" + Flag + "' AND UserName='" + data["UserName"] + "' AND  Id<>'" + data["Id"] + "'", out dsMaster, false, "1");
                if (dsMaster.Tables[0].Rows.Count > 0)
        ...
        {
            DataTable dt = _sqlRepository.GetDataTable("SELECT  isnull(Max(Sequence),0) AS Sequence FROM " + TableName + " where FLAG='" + Flag + "'");
            if (dt.Rows.Count > 0)
