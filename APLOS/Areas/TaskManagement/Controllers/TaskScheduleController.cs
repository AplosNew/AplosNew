...
namespace Aplos.Areas.TaskManagement.Controllers
{
    public class TaskScheduleController : BaseController
    {
        ...
        public TaskScheduleController(ISqlRepository R)
        {
            _sqlRepository = R;
        }
        #endregion
        ...
        public ActionResult SearchEmployee(string column, string value)
        {
            ...
            string sql = @"
                      select top 100 * from (  SELECT distinct Emp.SystemID AS Id,
                        ...
                        WHERE emp.CompanyId='" + identity.CompanyId + @"' OR (emp.EmpType='GUEST' AND emp.EmployeeStatus='Active' AND emp.GroupID='" + identity.CompanyGroupId + @"')) AS TEMP where " + strkey + " Order By Id";
        ...
        public ActionResult GetSelectedTaskList(string TemplateId)
        {
            ...
            DataTable dt = schedule.GetDataSourceMasterOrderNew(TemplateId, TaskAppliedOnEnum.MasterOrder);
            DateTime GanttStartDate = DateTime.Now.AddYears(1);
            DateTime GanttEndDate = DateTime.Now.AddYears(-10);
            dt.Columns.Add("IdForSort", typeof(int));
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                if (Convert.ToDateTime(dt.Rows[i]["TempStartDate"].ToString()) < GanttStartDate)
                    GanttStartDate = Convert.ToDateTime(dt.Rows[i]["TempStartDate"].ToString());

                if (GanttEndDate < Convert.ToDateTime(dt.Rows[i]["TempEndDate"].ToString()))
                    GanttEndDate = Convert.ToDateTime(dt.Rows[i]["TempEndDate"].ToString());

                int x = clsStaticInfo.dateDiff(dt.Rows[i]["OriginalSequentialStartDate"].ToString(), dt.Rows[i]["OriginalSequentialEndDate"].ToString());
                dt.Rows[i]["Duration"] = x + 1;

                dt.Rows[i]["IdForSort"] = (int)clsStaticInfo.dbl(dt.Rows[i]["Id"].ToString());
            }
            ...
            if (dt.Rows.Count > 0)
            {
                projectstartdate = GanttStartDate.ToString("dd-MMM-yyyy");
                projectenddate = GanttEndDate.ToString("dd-MMM-yyyy");

                duration = Math.Abs(clsStaticInfo.dateDiff(Convert.ToDateTime(projectstartdate).AddDays(-1).ToString("dd-MMM-yyyy"), projectenddate)).ToString() + " day(s)";
            }

            JsonResult json = Json(new { DATA = Helpers.CustomJsonResult.DataTableToJson(dt), Duration = duration, ProjectStartDate = projectstartdate, ProjectEndDate = projectenddate, GanttStartDate = GanttStartDate.AddDays(-3).ToString("dd-MMM-yyyy"), GanttEndDate = GanttEndDate.AddDays(30).ToString("dd-MMM-yyyy") }, JsonRequestBehavior.AllowGet);


            json.MaxJsonLength = int.MaxValue;

            //dt.DefaultView.RowFilter = null;
            //dt.DefaultView.Sort = "IdForSort ASC";
        ...
        {
            string _Task = "SELECT  TT.*,EI.EmployeeCode,EI.EmployeeName FROM TaskTemplate TT left outer join employeeInformation EI on ei.SystemID=TT.EmployeeId where id='" + Id + "'";
