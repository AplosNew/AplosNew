...
namespace Library.MaterialManagement.Inventory
{
	public class InventoryReceiveDetailService : Service<InventoryReceiveDetail>, IInventoryReceiveDetailService
	{
						...
						foreach (var item in taxCategoryList)
						{
							...
							else
							{
								...
								else
								{
									...
									//var res = Convert.ToDecimal(invMaterialDetail) + Convert.ToDecimal(item.TaxAmount);
									//builderSql = @"Update trn.InventoryReceiveDetail set TotalTaxAmount='" + res + "'  where Id='" + item.InventoryReceiveDetailId + "'";
									////builderSql = @"UPDATE [TRN].[InventoryService] SET BaseIssueQty='" + Convert.ToDecimal(issueQty) + "',IssueQty='" + Convert.ToDecimal(issueQty) + "'  WHERE Id='" + item.Id + "'";
									//rdBuilder.Append(builderSql);
									//_sqlRepository.ExecuteSqlCommand(rdBuilder.ToString());
								}
				...
				foreach (var itemDetail in entityMat)
				{
					...
					if (itemDetail.IsNotNull())
					{
							...
							&& (baseUoMFactorList != null && baseUoMFactorList.Count() > 0))
						{
							...
							//AddedDate
							itemDetail.BaseUoMFactor = 1;
							itemDetail.BaseQty = itemDetail.NetQty;//itemDetail.TransactionQty;
							itemDetail.TotalMaterialTranAmount = itemDetail.TrnAmount;
							itemDetail.ChargesTranAmount = itemDetail.ServiceCharge; //itemDetail.TrnAmount * ratio;
							itemDetail.ChargesTaxTranAmount = itemDetail.ServiceTax;//itemDetail.TrnAmount * ratioServiceTax;
							if (itemDetail.TotalTaxAmount == null)
						...
						if (string.IsNullOrEmpty(itemDetail.Id))
						{
							...
							{
								//Id = MakePK(entity.Id, currentId1, 2), //itemDetail.InventoryReceiveId
							...
							try
							{
								...
								//Accaptance Save
								//foreach (var itemDetail1 in entityMat)
								//{
					...
					else
					{
						...
						if (receiveDetailList.IsNotNull())
						{
							...
							foreach (var issue in receiveDetailList)
							{
								...
								_gRNPORequisitionAllocationRepository.Insert(gRNPORequisitionAllocation);
							}
						}
					}
			...
			try
			{
				...
				foreach (var itemDetail in entityMatAndImat)
				{
					...
					if (itemDetail.IsNotNull())
					{
						...
						//var PreviousShortQty = itemDetail.PreviousShortQty;
						//var PreviousRejectionQty = itemDetail.PreviousRejectionQty;
						//var PreviousApprovedQty = itemDetail.PreviousApprovedQty;
						...
						//var sqlres1 = @"Update trn.PurchaseOrderDetail set GRNRcvQty='" + Convert.ToDecimal(((poDetailData.GRNRcvQty - GRNRcvQty) + itemDetail.TransactionQty)) + "' where Id='" + itemDetail.PODetailsID + "'";
						//_sqlRepository.GetDataCollection(sqlres1);
						...
						//builderSql1 = @"update trn.PurchaseOrderDetail set GRNRcvQty='" + Convert.ToDecimal(((poDetailData.GRNRcvQty - GRNRcvQty) + itemDetail.TransactionQty)) + "' where Id='" + itemDetail.PODetailsID + "'";
						//rdBuilder.Append(builderSql);
						//_sqlRepository.ExecuteSqlCommand(rdBuilder.ToString()); 

						//builderSql = @"Update TRn.InventoryMaterial set TotalQty='" + resQty + "',AvgRate='" + resAvg + "',ShortageQty ='" + resShortQty + "', RejectionQty='" + resRejectionQty + "', ApprovedQty='" + resApprovedQty + "' " +
						//	"where MaterialMasterId='" + MaterialQty.MaterialMasterId + "' " +
						//	"AND ArticleId='" + MaterialQty.ArticleId + "' " +
						//	"AND  isnull(FirstCharacteristicsId,'')='" + MaterialQty.FirstCharacteristicsId + "'" +
						//	"AND  isnull(FirstCharacteristicsValueId,'')='" + MaterialQty.FirstCharacteristicsValueId + "'" +
						...
						// poDetail.TotalQty=;
						// _inventoryMaterialService.Update(poDetail);
						...
						if (!string.IsNullOrEmpty(itemDetail.Id))
						{
							...
							try
							{
								...
								//receiveDetail.ChargesTranAmount = Math.Round((receiveDetail.MaterialTranAmount * ratio), 4);//Math.Round(val, 2)
								//receiveDetail.TrnCurrencyBaseRate = itemDetail.IsNonCreditable ? Math.Round((receiveDetail.MaterialTranAmount + receiveDetail.TotalTaxAmount + receiveDetail.ChargesTranAmount), 4) / receiveDetail.TransactionQty
								//						 : Math.Round((receiveDetail.MaterialTranAmount + receiveDetail.ChargesTranAmount), 4) / receiveDetail.TransactionQty;
								//receiveDetail.BooksCurrencyBaseRate = Math.Round((receiveDetail.TrnCurrencyBaseRate), 4);

								//receiveDetail.TotalMaterialTranAmount += itemDetail.IsNonCreditable ? Math.Round(Convert.ToDecimal(itemDetail.TotalTaxAmount + receiveDetail.ChargesTranAmount) * Convert.ToDecimal(itemDetail.ToCurrencyRate), 4) :
								//										 Math.Round(Convert.ToDecimal(receiveDetail.ChargesTranAmount) * Convert.ToDecimal(itemDetail.ToCurrencyRate), 4);

								//itemDetail.TotalQty = Math.Round(Convert.ToDecimal(itemDetail.TotalQty + itemDetail.BaseQty),4);
								//itemDetail.AvgRate = Math.Round(Convert.ToDecimal((totalAmount + receiveDetail.TotalMaterialTranAmount)),4) / itemDetail.TotalQty;
					...
					if (Convert.ToDecimal(itemDetail.POQty) > (Convert.ToDecimal(itemDetail.GRNRcvQty + itemDetail.TransactionQty)))
					{
						var GRNPORequisitionAllocation = _gRNPORequisitionAllocationRepository.Query(t => t.InventoryReceiveDetailId == itemDetail.InventoryReceiveDetailId).Select().ToList();
						...
						if (receiveDetailList.IsNotNull())
						{
							...
							foreach (var issue in receiveDetailList)
							{
								...
								var gRNPORequisitionAllocation = new GRNPORequisitionAllocation
								{
									...
									BaseQty = (decimal)conversion.Convert(itemDetail.MaterialMasterId, itemDetail.TransactionUoMId, itemDetail.BaseUOMId.ToString(), Convert.ToDouble(itemDetail.TransactionQty)),
									BaseUoMId = issue.BaseUOMId,
									POBOQQty = (decimal)conversion.Convert(itemDetail.MaterialMasterId, itemDetail.TransactionUoMId, itemDetail.TransactionUoMId.ToString(), Convert.ToDouble(itemDetail.TransactionQty)),
		...
		private void UpdateInventoryDetail(InventoryReceiveDetail detail, decimal ratio, decimal ratioServiceTax, decimal currencyRate, bool isNonCreditable)
		{
			...
			if (detailList.IsNotNull())
			{
				...
				foreach (var item in detailList)
				{
					...
					//item.ChargesTranAmount = item.MaterialTranAmount * ratio;
					//item.TrnCurrencyBaseRate = isNonCreditable ? (item.MaterialTranAmount + item.TotalTaxAmount + item.ChargesTranAmount) / item.TransactionQty
					//							 : (item.MaterialTranAmount + item.ChargesTranAmount) / item.TransactionQty;
					//item.BooksCurrencyBaseRate = item.TrnCurrencyBaseRate;
					//item.TotalMaterialTranAmount = (item.TotalMaterialTranAmount - (chamnt * currencyRate)) + item.ChargesTranAmount * currencyRate;

					//detail.TotalTaxAmount
					//item.ChargesTaxTranAmount = item.MaterialTranAmount * ratioServiceTax;
					//item.ChargesTranAmount = item.MaterialTranAmount * ratio;	
					...
					else
					{
						...
						//item.ChargesTaxTranAmount = Math.Round(Convert.ToDecimal(Tax1) - Tax, 2);
						//item.ChargesTranAmount = Math.Round(Convert.ToDecimal(serviceCN1) - serviceCN, 2);
						item.ChargesTaxTranAmount = Math.Round(item.MaterialTranAmount * ratioServiceTax, 2);
						item.ChargesTranAmount = Math.Round(item.MaterialTranAmount * ratio, 2);
					}

					//item.ChargesTaxTranAmount = item.ChargesTaxTranAmount * ratio;
					//item.ChargesTranAmount -= item.ChargesTaxTranAmount;
		...
		private void UpdateInventoryDetailAfterDelete(InventoryReceiveDetail detail, decimal ratio, decimal ratioServiceTax, decimal currencyRate, bool isNonCreditable)
		{
			var detailList = Query(t => t.InventoryReceiveId == detail.InventoryReceiveId).Select().ToList();// && t.Id != detail.Id
			if (detailList.IsNotNull())
			{
			...
			try
			{
				...
				if (CheckedBy == "true" && ApprovedBy == "true")
				{
					sql = @"select E.SystemId As Value, E.EmployeeName As Text from dbo.AuthorizationConfig A 
                          ...
                          where  A.ActionStatus='IssueSlipCheckedBy'";//A.PlantId='" + identity.PlantId + "' AND
				}
				else if (CheckedBy == "false" && ApprovedBy == "true")
				{
					sql = @"select E.SystemId As Value, E.EmployeeName As Text from dbo.AuthorizationConfig A 
                          Inner JOin dbo.EmployeeInformation E On E.systemId=A.EmployeeId 
                          where  A.ActionStatus='IssueSlipApproveBy'";//A.PlantId='" + identity.PlantId + "' AND
				}
