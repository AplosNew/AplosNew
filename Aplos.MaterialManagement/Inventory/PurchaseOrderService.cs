...
ORDER BY IR.ID DESC";
        ...
        public IEnumerable<object> GetLCContractListByPartyId(bool isProcurementOnBom, string plantId, string partyId)
        {
            try
            {
                var sql = @"SELECT C.Id ContractId
        ...
        public IEnumerable<object> GetCheckedByAndApprovedBYServicePOAcknowledgement(string CheckedBy, string ApprovedBy)
        {
            ...
            try
            {
                ...
                if (CheckedBy == "true" && ApprovedBy == "true")
                {
                    sql = @"select E.SystemId As Value, E.EmployeeName As Text from dbo.AuthorizationConfig A 
                          Inner JOin dbo.EmployeeInformation E On E.systemId=A.EmployeeId 
                          where  A.ActionStatus='ServiceAcknowledgementCheckedBy'";//A.PlantId='" + identity.PlantId + "' AND
                }
                else if (CheckedBy == "false" && ApprovedBy == "true")
                {
                    sql = @"select E.SystemId As Value, E.EmployeeName As Text from dbo.AuthorizationConfig A 
