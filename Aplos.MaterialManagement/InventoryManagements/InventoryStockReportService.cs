        ...
        public void CreateMaterialStockBalanceSheet(ref IWorksheet sheet1, ref IWorksheet sheet2, ReportUtility report, string sheet1Name, string sheet2Name, string CompanyId, string plantId, string fromDate, string toDate, string Qty, string Amount, string RcptIssue, string Asset, string Inventory, string Country, string materialStorage, bool bale, bool brand)
        {
            ...
            if (fromDate == "" && toDate != "")
            {
                ...
                else if (Amount == "" && Qty != "")
                {
                    ...
                    if (bale)
                    {
                        sheet1.Range[_rowL, sheet1headreColIndex].Text = "Opening Bale Qty";
                        sheet1.Range[_rowL, sheet1headreColIndex].ColumnWidth = 15;
                        sheet1.Range[_rowL, sheet1headreColIndex].HorizontalAlignment = ExcelHAlign.HAlignCenter;
                        sheet1.Range[_rowL, sheet1headreColIndex].VerticalAlignment = ExcelVAlign.VAlignCenter;
                        sheet1.Range[_rowL, sheet1headreColIndex].CellStyle.Font.Bold = true;
        ...
        public void CreateMaterialStockBalanceForThePeriodSheet(ref IWorksheet sheet1, ref IWorksheet sheet2, ReportUtility report, string sheet1Name, string sheet2Name, string CompanyId, string plantId, string fromDate, string toDate, string Qty, string Amount, string RcptIssue, string Asset, string Inventory, string Country, string materialStorage, bool bale, bool brand)
        {
                    ...
                    cmdText = @"SELECT * FROM (
						SELECT Distinct ROW_NUMBER() Over(Order by  IM.Id) As[S.N]             
							...
							,TUoM.UserName UOM,  MM.IsAsset
                             ...
                             --Opening Balance
                            ,ISNULL(opbal.TransactionQty,0) As OpeningBalance ,isnull(opbal.TotalMaterialBooksCurrencyAmount,0) AS OpeningBalanceAmount ,isnull(opbal.OBBaleQty,0) AS OBBaleQty
						    --Receive

							,ISNULL(opbal2.TransactionQty,0) ReceivedForThePeriod ,isnull(opbal2.TotalMaterialBooksCurrencyAmount,0) AS ReceivedForThePeriodAmount ,isnull(opbal2.BaleQty,0) BaleQty
							--Issue
                         
							,ISNULL(IFD1.IssueQty,0) IssueForThePeriod ,isnull(IFD1.PolicyAmount,0) IssueForThePeriodAmount,isnull(IFD1.IssueBaleQty,0) IssueBaleQty		

							--Issue Return
							,ISNULL(IssueReturnData.Qty,0) IssueReturnQtyForThePeriod ,isnull(IssueReturnData.IssueReturnAmount,0) IssueReturnForThePeriodAmount	

							--Purchase Return
							,ISNULL(PurchaseReturnData.Qty,0) PurchaseReturnQtyForThePeriod	 ,isnull(PurchaseReturnData.PurchaseReturnAmount,0) PurchaseReturnForThePeriodAmount	

							--Adjust Return
							,ISNULL(AdjustmentData.Qty,0) AdjustmentQtyForThePeriod ,isnull(AdjustmentData.AdjustmentAmount,0) AdjustmentForThePeriodAmount	

                               --Inventory Sales
							,isnull(InventorySalesData.Qty,0) InventorySalesQtyForThePeriod	 ,isnull(InventorySalesData.InventorySalesAmount,0) InventorySalesForThePeriodAmount	
							--Inventory Scrap
							,isnull(InventoryScrapData.Qty,0) InventoryScrapQtyForThePeriod	 ,isnull(InventoryScrapData.InventoryScrapAmount,0) InventoryScrapForThePeriodAmount	

							--Inventory Transfer Data
							,isnull(InventoryTransferData.CapitalizeQty,0) InventoryTransferQtyForThePeriod	 ,isnull(InventoryTransferData.CapitalizeAmount,0) InventoryTransferForThePeriodAmount

                            --Inventory Sales Return Data
                            ,0 InventorySalesReturnQtyForThePeriod ,0 InventorySalesReturnForThePeriodAmount
                            ...
                            ,isnull(opbal.OBBaleQty,0)+  isnull(opbal2.BaleQty,0)-isnull(IFD1.IssueBaleQty,0) ClosingBaleQty
						FROM TRN.InventoryMaterial AS IM
						LEFT JOIN MST.MaterialMaster AS MM ON IM.MaterialMasterId=MM.Id
                        LEFT join hkp.MaterialCategory as MC on MC.Id = MM.MaterialCategoryId
						LEFT JOIN [HKP].[HSNCode] AS HSNC ON HSNC.ID=MM.HSNCodeId
						...
						LEFT JOIN (  SELECT IRD.InventoryMaterialId
                                ...
                                ,sum(IRD.AlternativeQty)-SUM(ISNULL(II.IssueBaleQty,0)) OBBaleQty
                                    ...
                                    -- InventoryIssue OB
                                    LEFT JOIN (
									    select IID.InventoryMaterialId,IH.InventoryReceiveDetailId, Sum(ISNULL(IH.Qty,0)) IssueQty,SUM(ISNULL(IH.BaleQty,0)) IssueBaleQty , Sum(ISNULL(IH.TotalAmount,0)) PolicyAmount
									    FROM TRN.InventoryIssueDetail IID  
									    LEFT JOIN TRN.InventoryIssue II ON IID.InventoryIssueId=II.Id	 
									    LEFT JOIN TRN.InventoryIssueHistory IH On IH.InventoryIssueDetailId=IID.Id
						...
						LEFT JOIN (select IID.InventoryMaterialId, Sum(IH.Qty) IssueQty , Sum(IH.BaleQty) IssueBaleQty , Sum(ISNULL(IH.TotalAmount,0)) PolicyAmount
									FROM TRN.InventoryIssueDetail IID  
									LEFT JOIN TRN.InventoryIssue II ON IID.InventoryIssueId=II.Id	 
									LEFT JOIN TRN.InventoryIssueHistory IH On IH.InventoryIssueDetailId=IID.Id
                                    LEFT JOIN TRN.inventoryreceiveDetail IRD On IRD.Id=IH.inventoryreceiveDetailId
            ...
            if (fromDate != "" && toDate != "")
            {
                ...
                else if (Amount == "" && Qty != "")
                {
                    ...
                    if (bale)
                    {
                        sheet1.Range[_rowL, sheet1headreColIndex].Text = "Openig Bale Qty";
                        sheet1.Range[_rowL, sheet1headreColIndex].ColumnWidth = 15;
                        sheet1.Range[_rowL, sheet1headreColIndex].HorizontalAlignment = ExcelHAlign.HAlignCenter;
                        sheet1.Range[_rowL, sheet1headreColIndex].VerticalAlignment = ExcelVAlign.VAlignCenter;
                        sheet1.Range[_rowL, sheet1headreColIndex].CellStyle.Font.Bold = true;
                    ...
                    if (bale)
                    {
                        sheet1.Range[_rowL, sheet1headreColIndex].Text = "Receipt Bale Qty";
                        sheet1.Range[_rowL, sheet1headreColIndex].ColumnWidth = 15;
                        sheet1.Range[_rowL, sheet1headreColIndex].HorizontalAlignment = ExcelHAlign.HAlignCenter;
                        sheet1.Range[_rowL, sheet1headreColIndex].VerticalAlignment = ExcelVAlign.VAlignCenter;
                        sheet1.Range[_rowL, sheet1headreColIndex].CellStyle.Font.Bold = true;
                    ...
                    if (bale)
                    {
                        sheet1.Range[_rowL, sheet1headreColIndex].Text = "Issue Bale Qty";
                        sheet1.Range[_rowL, sheet1headreColIndex].ColumnWidth = 15;
                        sheet1.Range[_rowL, sheet1headreColIndex].HorizontalAlignment = ExcelHAlign.HAlignCenter;
                        sheet1.Range[_rowL, sheet1headreColIndex].VerticalAlignment = ExcelVAlign.VAlignCenter;
                        sheet1.Range[_rowL, sheet1headreColIndex].CellStyle.Font.Bold = true;
                    ...
                    if (bale)
                    {
                        sheet1.Range[_rowL, sheet1headreColIndex].Text = "Close Bale Qty";
                        sheet1.Range[_rowL, sheet1headreColIndex].ColumnWidth = 15;
                        sheet1.Range[_rowL, sheet1headreColIndex].HorizontalAlignment = ExcelHAlign.HAlignCenter;
                        sheet1.Range[_rowL, sheet1headreColIndex].VerticalAlignment = ExcelVAlign.VAlignCenter;
                        sheet1.Range[_rowL, sheet1headreColIndex].CellStyle.Font.Bold = true;
            ...
            for (int n = 0; n < inventoryMaterialList.Rows.Count; n++)
            {
                ...
                if (fromDate != "" && toDate != "")
                {
                    if (Amount != "" && Qty != "")
                    {
                        ...
                        if (bale)
                        {
                            report.SetText(ref sheet1, _rowL, col, clsStaticInfo.dbl(inventoryMaterialList.Rows[n]["OBBaleQty"].ToString())); col++;
