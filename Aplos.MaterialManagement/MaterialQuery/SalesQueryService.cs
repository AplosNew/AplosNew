			...
			try
			{
				...
				if (Summary == "Details")
				{
					...
					--	LEFT JOIN [HKP].[HSNCode] AS HSNC ON HSNC.ID=MM.HSNCodeId
						...
						WHERE II.PlantId='" + identity.PlantId + "' AND II.CustomerId<>'' AND convert(Date,II.SalesDate) BETWEEN  '" + fromDate + @"' AND '" + toDate + @"'
						...
						WHERE IR.PlantId='" + identity.PlantId + "' AND IR.CustomerId<>'' AND convert(Date,IR.SalesDate) BETWEEN  '" + fromDate + @"' AND '" + toDate + @"'
                        UNION ALL
						...
						WHERE II.PlantId='" + identity.PlantId + "' AND convert(Date,II.PostingDate) BETWEEN  '" + fromDate + @"' AND '" + toDate + @"' AND II.SourceType='CustomerInvoice'";
					...
					--	LEFT JOIN [HKP].[HSNCode] AS HSNC ON HSNC.ID=MM.HSNCodeId
						...
						WHERE II.PlantId='" + identity.PlantId + "' AND II.CustomerId<>'' AND convert(Date,II.SalesDate) <= '" + toDate + @"'
						...
						WHERE IR.PlantId='" + identity.PlantId + "' AND IR.CustomerId<>'' AND convert(Date,IR.SalesDate) <= '" + toDate + @"'
                        UNION ALL
						SELECT  ROW_NUMBER() Over(Order by   II.Id) As[S.N]
								,'Sale GL' SourceType
								,IID.Id ,II.Id SalesId
						...
						WHERE II.PlantId='" + identity.PlantId + "' AND convert(Date,II.PostingDate) <= '" + toDate + @"' AND II.SourceType='CustomerInvoice' ";
						return _sqlRepository.GetDataTable(sql);
					}
				}
				else
					...
					else
					{
						sql = @"SELECT 
							   ...
							   LEFT JOIN (SELECT A.InvoiceId,Sum(A.TaxAmount) TaxAmount,Sum(A.TaxAmount) BooksCurrencyTransactionAmount
								...
								LEFT JOIN trn.Voucher V On V.Id=II.VoucherId
								WHERE II.PlantId='" + identity.PlantId + @"'   AND convert(Date,II.PostingDate) <= '" + toDate + @"' and II.SourceType ='CustomerInvoice'
								GROUP BY  II.Id ,PPI.UserName,P.UserName ,P.Code,CU.Code,II.IsPark,II.Narration  , II.DocRefNo,II.DocDate  ,v.VoucherNo,V.PostedBy,E.UserName ,II.PostingDate ,II.CompanyCurrencyRate
                                ";
						return _sqlRepository.GetDataTable(sql);
					}
				}
