                       ...
                       -- SET @totalReceiveAmount=(SELECT ISNULL(SUM(ISNULL(TransactionAmount, 0)),1) FROM [TRN].[PurchaseOrderDetail] WHERE " + paramter + @")
                        ...
                        --SET @totalSvcTaxAmount=(SELECT ISNULL(SUM(ISNULL(TaxAmount, 0)),0) FROM [TRN].[PurchaseOrderTax] WHERE " + paramter + @" AND InventoryServiceId<>'')
                           SELECT 
                            ...
                            , ISNULL(GRND.GRNRcvQty,0)-ISNULL(GRND.PurchaseRerutnQty,0) AS GRNRcvQty                           
                            , '' AS TransactionQty
                            , (IRD.TransactionQty+(IRD.TransactionQty*(case when IRD.Tolerance<>0 then IRD.Tolerance else IR.Tolerance end)/100)-ISNULL(GRND.GRNRcvQty,0)+ISNULL(GRND.PurchaseRerutnQty,0)) As Balance
                        ...
                        LEFT JOIN(SELECT gd.PODetailsId,isnull(sum(gd.TransactionQty),0) GRNRcvQty,isnull(sum(gd.PurchaseReturnQty),0) PurchaseRerutnQty FROM  TRN.InventoryReceiveDetail gd 
