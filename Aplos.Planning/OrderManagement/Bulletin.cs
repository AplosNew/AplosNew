        ...
        public void ProductionEfficiencyReport(string PlantId, string entityid, string Date)
        {
            try
            {
                ...
                for (int i = 0; i < dtProductionInfo.Rows.Count; i++)
                {
                    ...
                    sheet[ROW, colCMSpend].Formula = "IF(" + clsStaticInfo.GetxlsCol(colWorkHour) + ROW.ToString() + @">0," + (clsStaticInfo.dbl(dtProductionInfo.Rows[i]["MachineCostPerDay"].ToString()).ToString() + "*" + clsStaticInfo.GetxlsCol(colRUNmc) + ROW.ToString()) + "*" + clsStaticInfo.GetxlsCol(colWorkHour) + ROW.ToString() + @",0)";
